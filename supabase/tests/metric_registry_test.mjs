...
const APPROOT = [_flat, path.join(_flat, 'app'), path.join(_flat, '..', 'app')].find(p => fs.existsSync(path.join(p, 'index.html'))) || _flat;
...
t('Repo-Snapshot existiert und ist identisch mit registry.toJSON()', () => {
  const snapPath = path.join(APPROOT, 'docs', 'gm-ref', 'metric-registry.snapshot.json');
