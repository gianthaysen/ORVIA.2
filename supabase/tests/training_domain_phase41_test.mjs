...
const run = async () => {
  ...
  ok('MOVEMENT_PATTERNS 25 Muster', T.MOVEMENT_PATTERNS.length === 25);
  ...
  // Workout-Satz: gemappt + workout_exercise_id + Dedup-Konfliktkey
  CAP = {}; r = await O.repos.workout.addSet('we1', { setNumber: 2, setType: 'top_set', weight: 100, reps: 5, rir: 1 });
  ...
  ok('addSet: Dedup onConflict workout_exercise_id,set_number', CAP['workout_sets'].opts.onConflict === 'workout_exercise_id,set_number');

  // Workout-Übung: Dedup über (workout_session_id, order_index)
  CAP = {}; r = await O.repos.workout.addExercise('s1', { exerciseId: 'e1', order: 0 });
  ok('addExercise: Dedup onConflict workout_session_id,order_index', shape(r) && r.success && CAP['workout_exercises'].opts.onConflict === 'workout_session_id,order_index');

  // Sportart-Normalisierung (verhindert getrennte Kategorien)
  ok('normSport: Gym/Krafttraining → gym', T.normSport('Gym') === 'gym' && T.normSport('Krafttraining') === 'gym');
  ok('normSport: Laufen/run → running; Rad → cycling', T.normSport('Laufen') === 'running' && T.normSport('run') === 'running' && T.normSport('Rad') === 'cycling');
  ok('normSport: unbekannt → athletics (gültiger Sport), null → null', T.normSport('Yoga') === 'athletics' && T.normSport(null) === null);
  ok('Mobility-Entscheidung: Mobilität/mobility → gym (kein ungültiger Sport-Key)', T.normSport('Mobilität') === 'gym' && T.normSport('mobility') === 'gym');
  ok('valid.sport(normSport(x)) IMMER true', ['Gym', 'Laufen', 'Mobilität', 'Yoga', 'Rad', 'tennis', 'Krafttraining'].every(v => T.valid.sport(T.normSport(v))));
