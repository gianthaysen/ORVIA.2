...
const run = async () => {
  ...
  // readiness_components verschachtelt cross-user.
  const sa = await a.c.from('readiness_scores').upsert({ user_id: a.uid, local_date: TDATE }, { onConflict: 'user_id,local_date,engine_version' }).select();
  const sb = await b.c.from('readiness_scores').upsert({ user_id: b.uid, local_date: TDATE }, { onConflict: 'user_id,local_date,engine_version' }).select();
  const scoreA = sa.data && sa.data[0] && sa.data[0].id;
  const scoreB = sb.data && sb.data[0] && sb.data[0].id;
  ok('Score A & B angelegt', !!scoreA && !!scoreB);
