...
const run = async () => {
  ...
  /* TEST-AKTUALISIERUNG (2026-07-17): Vertrag hat sich mit dem INCIDENT-FIX in migrate-blob.js
     (2026-07-16, Ziel-Duplikat-Loop) bewusst geändert — 'completed_with_warnings' gilt jetzt als
     abgeschlossen und wird ÜBERSPRUNGEN; erneuter Lauf nur noch explizit via force:true. Die alte
     Assertion („wird NICHT übersprungen") prüfte genau das Verhalten, das den Incident verursachte
     (Legacy-Ziele wurden bei jedem Login erneut geseedet). */
  sb = makeEnv({ ls: LS_FULL(), migRows: [{ user_id: 'A', status: 'completed_with_warnings' }] });
  r = await MIG.run();
  ok('INV erneuter Lauf nach completed_with_warnings: wird übersprungen (Incident-Fix 2026-07-16)',
    r.success === true && r.data.skipped === true && r.data.status === 'completed_with_warnings');
  const callsAfterSkip = sb.calls.filter(c => c.op === 'upsert' && c.table !== 'orvia_migrations').length;
  ok('INV Skip schreibt keine Fachdaten (kein erneutes Goal-Seeding)', callsAfterSkip === 0);
  sb = makeEnv({ ls: LS_FULL(), migRows: [{ user_id: 'A', status: 'completed_with_warnings' }] });
  r = await MIG.run({ force: true });
  ok('INV force:true migriert trotz completed_with_warnings erneut', r.success === true && !r.data.skipped);
