...
globalThis.ORVIA.user = { id: 'u1' };
// 20/21 visibleActivityPipeline-Aufschlüsselung + Duplikationswarnung
reset(); globalThis.DB = {};
globalThis.window = { listActivitiesUnified: function () { var a = []; for (var i = 0; i < 12; i++) a.push({ sportId: 'gym', source: 'legacy_local', startedAt: dstr(i) + 'T12:00:00Z' }); a.push({ sportId: 'gym', source: 'orvia_workout', workoutSnapshot: [{ sets: [{ set_type: 'working', completed: true, reps: 10, weight: 70 }] }], startedAt: dstr(1) + 'T10:00:00Z' }); return a; } };
let rVis = await G.buildShadowReport({ days: 28 });
ok('20 visibleActivityPipeline aufgeschlüsselt', rVis.diagnostics.visibleActivityPipeline.gymCanonicalCount === 1 && rVis.diagnostics.visibleActivityPipeline.gymLegacyCount === 12);
ok('21 auffällige Gym-Duplikation → Warnung', rVis.warnings.indexOf('VISIBLE_GYM_DUPLICATION_SUSPECTED') >= 0);
delete globalThis.window;
