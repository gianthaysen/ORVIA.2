...
/* P2-2c + Umbruch-Fix (2026-08-05): die Regel forderte urspruenglich das Literal
   `gap:10px`. Der Umbruch-Fix (langer Ernaehrungstext draengte sich neben das Label)
   braucht row-gap != column-gap, also `gap:6px 10px` — das Literal war damit
   ueberholt, die ABSICHT nicht. Geprueft wird jetzt die Absicht: Spaltenabstand 10px,
   Umbruch erlaubt, Wert rechtsbuendig schrumpfbar, Label bleibt einzeilig. */
ok('4.3 .impact: 10px-Spaltenabstand + Umbruch + min-width:0 + text-align:right (P2-2c)',
   /\.impact\{[^}]*[ ;]gap:(?:\d+px )?10px[;}]/.test(css) && /\.impact\{[^}]*flex-wrap:wrap/.test(css)
   && /\.impact strong\{[^}]*min-width:0;text-align:right/.test(css) && /\.impact>span\{flex:0 0 auto;white-space:nowrap\}/.test(css));
...
ok('4.6 Migration 0029 vorhanden (Spalten + Längen-Guards, Anzeige-Handle ohne Unique-Anspruch)',
   existsSync(join(HERE, '..', 'migrations/0029_profile_handle_bio.sql'))
   && /add column if not exists handle text/.test(RSB('migrations/0029_profile_handle_bio.sql'))
   && /char_length\(bio\) <= 160/.test(RSB('migrations/0029_profile_handle_bio.sql')));
...
/* ============ Avatar-Regression (2026-08-05, Nutzer-Screenshot v8-235) ============
   .ig-avatar war die EINZIGE Avatar-Stelle ohne img-Regel — ein echtes Profilfoto
   rendert dann in Naturgröße über den Profilkopf. Alle Avatar-Container brauchen
   eine begrenzende img-Regel mit object-fit:cover. */
['.ig-avatar img', '.avatar img', '.ob-avatar img', '.tb-avatar img', '.pc-ava img'].forEach(sel => {
  const re = new RegExp(sel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\{[^}]*object-fit:cover');
  ok('Avatar-img-Regel vorhanden: ' + sel, re.test(css));
});

/* ============ Profil-Cover (2026-08-05, Nutzerwunsch „so wie im Beispiel") ============
   Geometrievertrag des Hero-Bildes. Jede dieser Zeilen hat einen konkreten Grund:
   - aspect-ratio statt fixer Hoehe: haelt den Bildausschnitt auf JEDER Breite gleich.
     Mit fixer Hoehe wandert der cover-Beschnitt mit der Viewportbreite und schneidet
     auf schmalen Geraeten die erhobene Hand des Laeufers ab.
   - KEIN negatives margin-top mehr: der alte -55px-Zug hat 55 von 148px Bildhoehe
     ausserhalb des Viewports versteckt.
   - .ig-profile margin-top == -88px == Avatarhoehe: Avatar-Unterkante schliesst mit
     der Bildunterkante ab (Referenzlayout).
   - Scrim ueber rgba(var(--bg-rgb),...): blendet zur SEITENFARBE aus statt zu einem
     harten #08090b — sonst dunkler Balken im Light-Theme.
   - Asset exakt 2:1: nur so ergibt cover null Beschnitt gegen aspect-ratio:2/1. */
const cover = (css.match(/\n\.profile-cover\{[^}]*\}/) || [''])[0];
ok('Cover: aspect-ratio 2/1 statt fixer Hoehe (breitenstabiler Ausschnitt)',
   /aspect-ratio:2\/1/.test(cover) && !/(^|[;{])height:\d/.test(cover), cover.slice(0, 90));
ok('Cover: kein negatives margin-top mehr (kein Bild ausserhalb des Viewports)',
   !/margin-top:-/.test(cover));
ok('Cover: Scrim theme-fest ueber --bg-rgb (kein hartes #08090b / rgba(8,9,11))',
   /rgba\(var\(--bg-rgb\)/.test(cover) && !/rgba\(8,9,11/.test(cover));
ok('Cover: --bg-rgb in Dark- UND Light-Theme definiert',
   /--bg-rgb:7,12,20/.test(css) && /--bg-rgb:238,244,251/.test(css));
ok('Cover: Avatar-Unterkante = Bildunterkante (.ig-profile margin-top = -Avatarhoehe 88px)',
   /\.ig-profile\{padding:0 18px;margin-top:-88px\}/.test(css) && /\.ig-avatar\{width:88px;height:88px/.test(css));
{
  const jpg = readFileSync(join(APP, 'assets/brand/profile-cover.jpg'));
  /* JPEG-SOF-Marker lesen (Breite/Hoehe) — ohne Bildbibliothek. */
  let i = 2, w = 0, h = 0;
  while (i < jpg.length - 9) {
    if (jpg[i] !== 0xFF) { i++; continue; }
    const m = jpg[i + 1];
    if (m >= 0xC0 && m <= 0xCF && m !== 0xC4 && m !== 0xC8 && m !== 0xCC) { h = jpg.readUInt16BE(i + 5); w = jpg.readUInt16BE(i + 7); break; }
    i += 2 + jpg.readUInt16BE(i + 2);
  }
  ok('Cover-Asset: exakt 2:1 (null Beschnitt gegen aspect-ratio:2/1) und >= 1170px breit (3x DPR)',
     w > 0 && h > 0 && w === h * 2 && w >= 1170, w + 'x' + h);
  ok('Cover-Asset: unter 200 KB (precached, mobiler Erstabruf)', jpg.length < 200 * 1024, Math.round(jpg.length / 1024) + ' KB');
}
ok('Cover-Asset im SW-Precache', R('sw.js').indexOf("'./assets/brand/profile-cover.jpg'") >= 0);
