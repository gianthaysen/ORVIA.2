...
ok('appContext: Schwelle aus Resolver, 5-s-Cache (zweiter Aufruf ohne Resolver-Lauf), Rueckfall 10-km-Bestpace', (() => { let calls = 0; const pr = { resolveAll: () => { calls++; return { sports: { running: { ok: true, thresholdPaceSecPerKm: 329 } } }; } }; const c1 = C.appContext({ performanceResolver: pr, today: '2026-09-13', hrMax: 194, now: 1000 }); const c2 = C.appContext({ performanceResolver: pr, today: '2026-09-13', hrMax: 194, now: 3000 }); const c3 = C.appContext({ performanceResolver: null, runBests: { measuredRunBests: () => ({ k10: { sec: 3270 } }) }, activityStore: { listActivities: () => [] }, today: '2026-09-14', now: 9000 }); return c1.thresholdPaceSec === 329 && c1.hrMax === 194 && calls === 1 && c2 === c1 && c3.best10kPaceSec === 327; })());
