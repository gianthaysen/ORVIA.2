...
globalThis.ORVIA.onboardingV2.open({ fresh: false });
await wait();
const visEl = h.byId('vis-cycling'); visEl.checked = false; visEl._ev.change.call(visEl);
...
const occEl = h.byId('occ-cycling'); occEl.checked = true; occEl._ev.change.call(occEl);
...
globalThis.ORVIA.onboardingV2.open({ fresh: false });
await wait();
ok('other: keine Planungs-Checkbox', !h.byId('plan-other'));
ok('other: nicht in Hauptsportart-Auswahl', !h.byId('pr-other'));
ok('other: sichtbar-Checkbox vorhanden', !!h.byId('vis-other'));
