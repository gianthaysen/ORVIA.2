...
ok('Review: Statusansicht „vorgemerkt"', (card(h).innerHTML || '').indexOf('Einrichtung gespeichert') >= 0);
...
globalThis.ORVIA.onboardingV2.open({ fresh: false });
await wait();
ok('alt ready_for_review ohne Profil → Profilformular', (card(h).innerHTML || '').indexOf('Anzeigename') >= 0 && (card(h).innerHTML || '').indexOf('Einrichtung gespeichert') < 0);
...
await wait();
h.reg.get('#ob2-review-ready').onclick();
ok('unvollständiger Review-Draft → KEINE Statusansicht', (card(h).innerHTML || '').indexOf('Einrichtung gespeichert') < 0);
...
ok('Review-Fehler: KEINE Erfolgsansicht', (card(h).innerHTML || '').indexOf('Einrichtung gespeichert') < 0);
