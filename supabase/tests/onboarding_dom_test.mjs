...
ok('fresh:true → Draft NICHT überschrieben', JSON.parse(h.mem[Store.key(null)]).currentStep === 'sports_placeholder');
...
ok('Fresh-Abbruch: Draft unverändert', JSON.parse(h.mem[Store.key(null)]).currentStep === 'sports_placeholder');
...
const seedR = {}; seedR[Store.key(null)] = JSON.stringify(L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'review_placeholder', completedSteps: ['welcome', 'profile', 'sports_placeholder', 'goals_placeholder', 'schedule_placeholder'], draftData: { profile: VPROF } }));
h = await fresh(seedR);
globalThis.ORVIA.onboardingV2.open({ fresh: false });
await wait();
...
const _rrDraft = L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'review_placeholder', completedSteps: ['welcome', 'profile', 'sports_placeholder', 'goals_placeholder', 'schedule_placeholder'], draftData: { profile: VPROF } });
...
ok('Profil valide: weiter zu sports_placeholder', ST().draft.currentStep === 'sports_placeholder');
