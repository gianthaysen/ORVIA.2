...
// Stub spiegelt das REALE normSport inkl. Rate-Fallback 'athletics' (sonst würde der Bug nicht getestet).
globalThis.ORVIA.trainingDomain = { normSport: v => { const s = String(v || '').toLowerCase(); const m = { laufen: 'running', rad: 'cycling', schwimmen: 'swimming', krafttraining: 'gym', 'mobilität': 'gym', fußball: 'football', paddel: 'padel' }; if (m[s]) return m[s]; const SPORTS = ['gym', 'running', 'cycling', 'swimming', 'triathlon', 'football', 'handball', 'padel', 'tennis', 'athletics']; return SPORTS.indexOf(s) >= 0 ? s : 'athletics'; } };
