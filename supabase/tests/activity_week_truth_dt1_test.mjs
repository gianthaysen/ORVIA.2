...
/* 2026-08-06: renderWeekly() ist inzwischen ein reiner Wrapper — die Wochenlogik
   liegt in weeklyReviewHTML(). Die Refaktorierung war sauber (die kanonischen
   Aufrufe sind dort vorhanden), aber dieser Test zeigte weiter auf die alte Stelle
   und war deshalb seitdem rot, OHNE dass ein echter Vertrag verletzt war.
   Geprueft wird jetzt der Koerper, in dem die Logik tatsaechlich steht — die Zusage
   selbst bleibt unveraendert streng, und dass renderWeekly wirklich nur delegiert,
   wird zusaetzlich belegt. */
const rwWrapper = body(proSrc, 'function renderWeekly(){', '\nfunction trainingDays(');
const rwBody = body(proSrc, 'function weeklyReviewHTML(){', '\nfunction renderWeekly(');
ok('[C0] renderWeekly delegiert an weeklyReviewHTML (Logik an genau EINER Stelle)',
   /weeklyReviewHTML\(\)/.test(rwWrapper) && rwBody.length > 0);
