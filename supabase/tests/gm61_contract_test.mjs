...
const uiSrc = R(_APPREL + 'js/ui.js');
...
globalThis.Calc = {
  ...
  hrvScoreOf: () => 60,
  /* v8-317: Die Supplement-Empfehlung liest den HRV-Status jetzt ueber den EINEN
     kanonischen Helfer (Garmin kennt Balanced · Unbalanced · Low · Poor, kein
     'Good' — und 'Poor' fiel bei den frueheren `==='Low'`-Vergleichen ueberall
     durch). Hier als Fixture am selben Vertrag, wie die uebrigen Calc-Stubs
     dieser Datei auch. */
  hrvBelowBaseline: h => { const x = String(h || '').toLowerCase(); return x === 'low' || x === 'poor'; }
};
...
const CHROME = (await import('./_pw-chrome.mjs')).chromeOrSkip(chromium); /* v8-307b */
