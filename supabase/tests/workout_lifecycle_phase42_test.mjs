...
O.repos = {
  workout: {
    ...
    updateSession: async (id, patch) => {
      if (UPDATE_MODE === 'fail' || !SERVER[id]) return failR('no_row_updated', 'Die Workout-Session wurde nicht aktualisiert.');
      if (UPDATE_MODE === 'unverified') { return okR({ ...SERVER[id], ...patch }); } // Rückgabe ok, aber Server bleibt active
      Object.assign(SERVER[id], patch); return okR({ ...SERVER[id] });
    },
...
const run = async () => {
  ...
  r = await WS.finishWorkout({ sessionRpe: 7 });
  ok('finish unverifiziert: kein Erfolg (Server nicht completed)', !r.success && r.error.code === 'transition_unverified');
