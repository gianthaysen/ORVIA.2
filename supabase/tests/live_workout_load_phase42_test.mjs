...
let SAVES = []; let saveMode = 'ok'; let idn = 0;
function repoStub() {
  O.repos = {
    workout: {
...
const run = async () => {
  ...
  ok('Session trotz Last abgeschlossen', r.data.session.status === 'completed');
  ...
  ok('Session trotzdem completed', r.data.session.status === 'completed');

  // 3) save wirft Exception → load_error, Session bleibt completed
  saveMode = 'throw';
  await WS.startFreeWorkout({ sport: 'Gym' });
  r = await WS.finishWorkout({ sessionRpe: 8 });
  ok('Load Exception → loadStatus=load_error', r.success && r.data.loadStatus === 'load_error' && r.data.session.status === 'completed');
  ...
  ok('Dauer = aktiv (≈15 min), Pause abgezogen', r.data.session.duration_min >= 14 && r.data.session.duration_min <= 16, 'duration=' + r.data.session.duration_min);
