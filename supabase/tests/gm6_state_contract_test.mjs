...
const _pw = await (async () => {
  /* Selbstversorgend wie die uebrigen Browser-Tests: normal → Repo-Nachbarn →
     ehrlicher Skip (exit 2), nie ein Crash und nie ein stilles Gruen. */
  const _p = await import('node:path');
  const _u = await import('node:url');
  const _h = _p.dirname(_u.fileURLToPath(import.meta.url));
  const cands = ['playwright',
    _p.join(_h, '..', '..', 'node_modules', 'playwright', 'index.js'),
    _p.join(_h, '..', '..', 'app', 'node_modules', 'playwright', 'index.js'),
    _p.join(_h, '..', '..', '_dev', 'node_modules', 'playwright', 'index.js'),
...
const SPEC = JSON.parse(R(_APPREL + 'docs/gm-ref/gm6/gm6_gm_domspec.json'));
...
/* 2026-08-05 (Nutzerentscheidung): Ernaehrung, Abend-Check-in und Routinen sind neue
   Dashboard-Module — eine BEWUSSTE Erweiterung ueber den Golden Master hinaus, den sie
   nicht kennt. Sie tragen dafuer die Marker-Klasse .gm-ext und werden aus dem
   Sequenzvergleich herausgenommen. Damit bleibt die GM-Grundstruktur weiterhin
   BYTE-GENAU geprueft (ein versehentliches Umsortieren faellt nach wie vor auf),
   ohne dass die gewollte Erweiterung als Regression gilt. B1e prueft die Erweiterung
   selbst: vorhanden, markiert, und ausschliesslich am Ende der Modulliste. */
const isExt = c => /(^|\s|\.)gm-ext(\s|\.|$)/.test(String(c || ''));
const normSeq = arr => arr.filter(c => !isExt(c)).map(normCls)
  .filter(c => c && !/^(statusbar|lvlbadge|demobar)/.test(c));
...
/* ── B1e · Die GM-Erweiterung selbst (2026-08-05) ──────────────────────────
   normSeq() blendet .gm-ext aus dem GM-Sequenzvergleich aus. Damit die drei neuen
   Module dadurch nicht ungeprueft bleiben, wird hier die Erweiterung selbst geprueft:
   sie ist vorhanden, sie ist als Erweiterung markiert, sie steht ausschliesslich am
   ENDE der Modulliste (die GM-Reihenfolge davor bleibt unberuehrt) — und sie fuehrt
   in die bestehenden Formulare statt einen zweiten Zustandsspeicher aufzumachen. */
sec('B1e · Neue Module (Ernährung/Abend/Routinen) sind markierte GM-Erweiterung');
await setState('good', 'f');
{
  const raw = await ev(SEQFN);
  const extIdx = raw.map((c, i) => (/gm-ext/.test(c) ? i : -1)).filter(i => i >= 0);
  const lastNonExt = raw.reduce((acc, c, i) => (/gm-ext/.test(c) || /addmod/.test(c) ? acc : i), -1);
  ok('B1e · alle drei Module gerendert und als .gm-ext markiert', extIdx.length === 3, 'gefunden=' + extIdx.length);
  ok('B1e · Erweiterung steht hinter der GM-Struktur (keine Umsortierung des Golden Master)',
     extIdx.length === 0 || Math.min(...extIdx) > lastNonExt, 'ext=' + JSON.stringify(extIdx) + ' lastGM=' + lastNonExt);
  const wiring = await ev(() => ({
    nutrition: /gmOpenNutritionSheet|openNutritionEditor/.test(gmModNutrition({})),
    evening: /gmGotoEveningCheckin/.test(gmModEvening({})),
    supplements: /gmGotoRoutines/.test(gmModSupplements({})),
    /* Kein zweites Formular: die Module duerfen keine Eingabefelder mitbringen. */
    noOwnInputs: !/<input|<textarea|<select/.test(gmModNutrition({}) + gmModEvening({}) + gmModSupplements({}))
  }));
  ok('B1e · jedes Modul fuehrt in seinen bestehenden Bereich', wiring.nutrition && wiring.evening && wiring.supplements, JSON.stringify(wiring));
  ok('B1e · KEIN zweiter Zustandsspeicher: die Module tragen selbst keine Eingabefelder', wiring.noOwnInputs);
  const hosts = await ev(() => {
    const n = document.getElementById('nutritionBox');
    const e = document.getElementById('eveCard');
    const r = document.getElementById('routinesCard');
    return { nutEmpty: !n || n.innerHTML === '', eveHidden: !e || e.classList.contains('gm-hidden-host'),
      routHidden: !r || r.style.display === 'none' || !r.classList.contains('gm-co-open') };
  });
  ok('B1e · kein Doppelinhalt: die drei Legacy-Hosts bleiben still, solange das Modul traegt',
     hosts.nutEmpty && hosts.eveHidden && hosts.routHidden, JSON.stringify(hosts));
}
