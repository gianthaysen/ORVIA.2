...
function validSports() {
  ...
  // M5b: Fixtures tragen auch den Trainingsstand (training_level-Validität hängt daran).
...
function draftAt(step, completed) {
  ...
  d.draftData = { profile: VALID_PROFILE, sports: validSports(), goals: [{ id: 'goal:t', title: 'HM', category: 'half_marathon', priority: 1, status: 'active' }], availability: { days: { di: { available: true } } }, safety: { hasComplaints: false } };   // M6/M7
  return d;
}
...
ok('VERSION 4 (M6) + v2/v3 werden weiter unterstützt', L.VERSION === 4 && Array.isArray(L.SUPPORTED_VERSIONS) && L.SUPPORTED_VERSIONS.includes(2) && L.SUPPORTED_VERSIONS.includes(3));
...
/* ---------- 1) STEP_CONFIG ---------- */
{
  ...
  ok('SC5 STEP_IDS = aktiver Flow (M7: + safety/body)', JSON.stringify(L.STEP_IDS) === JSON.stringify(['welcome', 'profile', 'sports', 'training_level', 'goals', 'availability', 'safety', 'body', 'review']));
...
/* ---------- 2) Migration v2 → v3 ---------- */
{
  ...
  ok('M1 v2 wird akzeptiert (NICHT als korrupt verworfen) und auf v4 gehoben', !!m1 && m1.version === 4);
...
/* ---------- 3) Skip-Vertrag ---------- */
{
  let d = draftAt('goals', ['welcome', 'profile', 'sports', 'training_level']);
  ...
  d = draftAt('body', ['welcome', 'profile', 'sports', 'training_level', 'goals', 'availability']);
  L.skipStep(d, 'body', NOW);
  ok('S7 Skip des aktuellen Steps bewegt currentStep deterministisch weiter (nächster aktiver)', d.currentStep === 'review');
}

/* ---------- 4) completeStep-Vertrag ---------- */
{
  // M7: alle Essential-Pflichtschritte sind fachvalidiert — CP1 nutzt body (optional, ohne Fachvalidierung).
  const d = draftAt('body', ['welcome', 'profile', 'sports', 'training_level', 'goals', 'availability', 'safety']);
  let r = L.completeStep(d, 'body', NOW);
  ok('CP1 Step ohne Fachvalidierung abschließbar', r.ok === true && d.completedSteps.includes('body'));
...
/* ---------- 5) Navigation ---------- */
{
  ...
  ok('N1 nächster Essential-Step (M5b: training_level)', L.getNextStep(d) === 'training_level');
  ...
  ok('N9 aktive Essential-Kette: availability → safety → body → review', L.getNextStep(draftAt('availability', [])) === 'safety' && L.getNextStep(draftAt('safety', [])) === 'body' && L.getNextStep(draftAt('body', [])) === 'review');
}
...
/* ---------- 6) Progress ---------- */
{
  ...
  ok('P1 0 %: total zählt nur countsTowardProgress+aktiv (M7: 8), welcome ausgeschlossen', p.total === 8 && p.completed === 0 && p.skipped === 0 && p.percentage === 0 && p.current === 1);
  d = draftAt('goals', ['welcome', 'profile', 'sports', 'training_level']);
  p = L.getProgress(d);
  ok('P2 teilweise: completed zählt nur Arbeitsschritte (3/8 = 38 %)', p.completed === 3 && p.percentage === 38 && p.current === 4);
  d = draftAt('review', ['welcome', 'profile', 'sports', 'training_level', 'goals', 'availability', 'safety', 'body', 'review']);
  p = L.getProgress(d);
  ok('P3 vollständig: 100 %, current geclamped auf total', p.percentage === 100 && p.current === 8);
  d = draftAt('goals', ['welcome', 'profile', 'sports', 'training_level']);
  L.skipStep(d, 'body', NOW);
  p = L.getProgress(d, { includeInactive: true });
  ok('P4 übersprungener optionaler Step zählt als erledigt (M7: body aktiv, 4/8 = 50 %)', p.total === 8 && p.skipped === 1 && p.percentage === 50);
  p = L.getProgress(d);
  ok('P5 Standard-Sicht identisch (keine inaktiven Essential-Steps mehr)', p.total === 8 && p.skipped === 1 && p.percentage === 50);
...
{
  const all = ['welcome', 'profile', 'sports', 'training_level', 'goals', 'availability', 'safety', 'review'];
  ok('T1 alle Required erfüllt (+body abgeschlossen) → essential complete', L.isTierComplete(draftAt('review', all.concat(['body'])), 'essential') === true);
  ok('T2 Required fehlt → false', L.isTierComplete(draftAt('review_placeholder', all.filter(s => s !== 'sports')), 'essential') === false);
  const dOpt = draftAt('review', all);
