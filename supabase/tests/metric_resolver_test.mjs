...
import { existsSync as _exA } from 'node:fs';
import { join as _jA, dirname as _dA } from 'node:path';
import { fileURLToPath as _fA } from 'node:url';
const _hA = _jA(_dA(_fA(import.meta.url)), '..', '..');
const APP = [_hA, _jA(_hA, 'app'), _jA(_hA, '..', 'app')].find(p => _exA(_jA(p, 'index.html'))) || _hA;

const require = createRequire(import.meta.url);
const HERE = path.dirname(fileURLToPath(import.meta.url));
const REG = require(path.join(APP, 'js', 'metrics', 'metric-registry.js'));
const RES = require(path.join(APP, 'js', 'metrics', 'metric-resolver.js'));
