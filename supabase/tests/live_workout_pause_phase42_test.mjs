...
const run = async () => {
  ...
  ok('Dauer = 20 min − 10 min Pause ≈ 10 min', r.data.durationMin >= 9 && r.data.durationMin <= 11, 'duration=' + r.data.durationMin);
  ...
  r = await WS.finishWorkout({ sessionRpe: 6 });
  ok('Abschluss während Pause: laufende Pause abgezogen (≈18 min)', r.data.durationMin >= 17 && r.data.durationMin <= 19, 'duration=' + r.data.durationMin);
