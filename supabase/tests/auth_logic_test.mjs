...
const AL = (await import(new URL(_APPREL + 'js/auth-logic.js', import.meta.url))).default;
...
// REGRESSION (Live-Incident 2026-06): die alte Edge-Function-Erfolgsantwort OHNE versionierten
// Vertrag (Direkt-Bestätigung) muss abgelehnt bleiben — exakt dieser Shape verursachte
// "Registrierung serverseitig nicht korrekt bestätigt" trotz angelegtem Auth-User.
ok('ALTE Serverantwort {ok,userId,role,needsConfirmation} → abgelehnt',
  AL.acceptRegistration({ ok: true, userId: 'uuid-x', role: 'tester', needsConfirmation: false }) === false);
// NEUE Edge-Function-Erfolgsantwort (Vertrag §8 inkl. emailSent) muss akzeptiert werden.
ok('NEUE Serverantwort (§8, emailSent true) → akzeptiert',
  AL.acceptRegistration({ ok: true, flowVersion: 2, status: 'confirmation_required', email: 'a@b.de', emailSent: true }) === true);
ok('NEUE Serverantwort (§8, emailSent false) → akzeptiert (Account existiert, Resend-Pfad)',
  AL.acceptRegistration({ ok: true, flowVersion: 2, status: 'confirmation_required', email: 'a@b.de', emailSent: false }) === true);
