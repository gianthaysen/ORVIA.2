...
/* ---------- O: Template ≠ Occurrence (Punkt 3) ---------- */
{
  ...
  /* v8-310a (Gians P0): Heute ist Mi (2026-07-15), die verschobene Einheit
     liegt am Do (2026-07-16) — der Start eines FREMDEN Tages muss verweigert
     werden, sonst entstuende eine Aktivitaet mit falscher Occurrence.
     Haertung: das Datum kommt AUSDRUECKLICH mit; ganz ohne Datum gibt es
     missing_date_context statt einer stillen Rekonstruktion. */
  const rNoDate = sbShift.startPlannedUnit(newDi, 0);
  ok('O5a0 datumsloser Aufruf ⇒ missing_date_context, NICHTS gestartet',
    rNoDate && rNoDate.ok === false && rNoDate.code === 'missing_date_context' && captured === null,
    JSON.stringify(rNoDate));
  const rNotToday = sbShift.startPlannedUnit(newDi, 0, '2026-07-16');
  ok('O5a Start am fremden Tag ⇒ not_today, NICHTS gestartet',
    rNotToday && rNotToday.ok === false && rNotToday.code === 'not_today' && captured === null,
    JSON.stringify(rNotToday));
  /* Am PLANTAG selbst (heute = 2026-07-16) laeuft die Kette wie vertraglich
     zugesagt — Occurrence + Snapshot unveraendert durchgereicht. */
  const sbShift2 = mkPlanSb('2026-07-16');
  sbShift2.PROFILE.weekPlan = saved;
  sbShift2.ORVIA.profileModel.effectiveTrainingConfig = () => ({ availableDayIdx: [3], targetDays: 1, daysSource: 'availability' });
  sbShift2.activeWeekPlan();
  sbShift2.closeSupp = () => {};
  sbShift2.ORVIA.workoutUI = { startSport: (sport, opts) => { captured = opts; } };
  vm.runInContext(startBlock, sbShift2, { filename: 'ui.js#start2' });
  sbShift2.startPlannedUnit(newDi, 0, '2026-07-16');
