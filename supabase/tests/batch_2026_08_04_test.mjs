...
/* NEGATIVKONTROLLE: distance+speed OHNE Zeitachse darf NIE eine Messung liefern —
   ...
   PRAEZISIERT 2026-08-05 (Nutzerwunsch „Abschnitte aus laengeren Einheiten"): Seit
   run-bests.js eine Ersatz-Zeitachse aus der Gesamtdauer bilden kann, ENTSTEHT hier
   ein Wert — die urspruengliche Schutzabsicht war aber nie „kein Wert", sondern
   „kein FALSCHES Mess-Etikett". Genau das wird jetzt geprueft: der Wert existiert,
   traegt aber die eigene Methode 'stream_uniform' und wird nirgends als
   Runden-/Stream-MESSUNG ausgegeben. Zusaetzlich bleibt die Ableitung aus Δd/v
   weiterhin verboten — die Zeit kommt ausschliesslich aus der Aktivitaetsdauer,
   der speed-Stream wird nicht angefasst. */
...
const noAxis = RB.measuredRunBests(actStreams({ distance: dist, speed: dist.map(() => 3) })).k1;
ok('NEGATIVKONTROLLE · ohne Zeitachse KEINE Messung (kein stream_window/lap_window)',
   noAxis == null || (noAxis.method !== 'stream_window' && noAxis.method !== 'lap_window'),
   noAxis && noAxis.method);
ok('NEGATIVKONTROLLE · ein trotzdem entstehender Wert ist als abgeleitet gekennzeichnet',
   noAxis == null || noAxis.method === 'stream_uniform', noAxis && noAxis.method);
ok('NEGATIVKONTROLLE · die Ersatzachse ignoriert den speed-Stream (keine Δd/v-Integration)',
   RB.streamTimeAxis({ speed: dist.map(() => 3), distance: dist }) === null);
ok('NEGATIVKONTROLLE · ohne Gesamtdauer entsteht auch keine Ersatzachse',
   RB.measuredRunBests([{ id: 'a2', sportId: 'running', status: 'completed',
     startedAt: '2026-08-01T06:00:00Z', durationSeconds: null, summary: { distance_m: 1900 },
     metrics: { splits: null, streams: { distance: dist } } }]).k1 == null);
...
ok('KF-015 · Registry-Vertrag repo-intern verankert',
   existsSync(join(APP, 'docs', 'gm-ref', 'metric-registry.snapshot.json'))
