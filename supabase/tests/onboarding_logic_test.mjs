...
await import(new URL(_APPREL + 'js/onboarding/onboarding-profile-logic.js', import.meta.url)); // für profileValid-Guard
await import(new URL(_APPREL + 'js/onboarding/onboarding-sports-logic.js', import.meta.url)); // für sportsValid-Guard
await import(new URL(_APPREL + 'js/profile-model.js', import.meta.url)); // M6: für goalsValid-Guard
const L = (await import(new URL(_APPREL + 'js/onboarding/onboarding-logic.js', import.meta.url))).default;
...
const VALIDS = { sports: [{ sportId: 'running', role: 'primary', enabled: true, visible: true, planningEnabled: true, priority: 1, level: 'intermediate', sessionsPerWeek: 4, typicalDuration: 60 }] };   // M5b/M7: inkl. A3+A5-Angaben
const VALIDG = [{ id: 'goal:test1', title: 'Halbmarathon', category: 'half_marathon', priority: 1, status: 'active' }];   // M6: Essential-Ziel
function withProfile() { var d = L.newDraft(); d.draftData = { profile: JSON.parse(JSON.stringify(VALIDP)), sports: JSON.parse(JSON.stringify(VALIDS)), goals: JSON.parse(JSON.stringify(VALIDG)), availability: { days: { di: { available: true } } }, safety: { hasComplaints: false } }; return d; }
function dd() { return { profile: JSON.parse(JSON.stringify(VALIDP)), sports: JSON.parse(JSON.stringify(VALIDS)), goals: JSON.parse(JSON.stringify(VALIDG)), availability: { days: { di: { available: true } } }, safety: { hasComplaints: false } }; }

// neuer Draft
const d0 = L.newDraft();
ok('newDraft: version 4 (M6-Bump; v2/v3 werden migriert, s. onboarding_state_v3_test)', d0.version === 4);
...
ok('nextStepId(review_placeholder) = null', L.nextStepId('review') === null);

// Fortschritt
ok('progress welcome = Schritt 1 von 9 (M7)', L.progress(L.newDraft()).label === 'Schritt 1 von 9');
ok('progress percent welcome = 0', L.progress(L.newDraft()).percent === 0);

// advance bis zum Review (mit validem Profil). advance schließt Review NIE selbst ab. M7: 8 Vorschritte.
let d = withProfile();
for (let i = 0; i < 8; i++) d = L.advance(d, 1);
ok('letzter Schritt erreicht: currentStep review', d.currentStep === 'review');
d = L.advance(d, 2);
ok('advance auf Review setzt NICHT ready_for_review', d.status !== 'ready_for_review');
ok('advance auf Review ergänzt review_placeholder NICHT', d.completedSteps.indexOf('review') < 0);
ok('completedSteps = 8 Vorschritte (M7: + safety/body)', d.completedSteps.length === 8 && d.completedSteps.indexOf('welcome') >= 0 && d.completedSteps.indexOf('training_level') >= 0 && d.completedSteps.indexOf('safety') >= 0 && d.completedSteps.indexOf('body') >= 0);
...
ok('ready_for_review ohne Profil → in_progress', L.normalizeDraft({ version: 2, status: 'ready_for_review', currentStep: 'review' }).status === 'in_progress');
// ready_for_review mit Lücke (sports fehlt) → zurückgestuft auf ersten fehlenden Schritt
let rGapN = L.normalizeDraft({ version: 2, status: 'ready_for_review', currentStep: 'review', completedSteps: ['welcome', 'profile', 'goals_placeholder'], draftData: dd() });
...
let _full = L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'review', completedSteps: ['welcome', 'profile', 'sports', 'training_level', 'goals_placeholder', 'schedule_placeholder', 'safety', 'body'], draftData: dd() });
...
ok('readyForReview() lehnt inkonsistenten Status ab', L.readyForReview({ status: 'ready_for_review', currentStep: 'review', completedSteps: [] }) === false);
ok('readyForReview() akzeptiert vollständigen Draft', L.readyForReview(_full) === true);
ok('advance(reviewDraft) setzt nicht ready_for_review', (function () { var x = L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'review', completedSteps: ['welcome', 'profile', 'sports', 'training_level', 'goals_placeholder', 'schedule_placeholder', 'safety', 'body'], draftData: dd() }); L.advance(x, 1); return x.status !== 'ready_for_review'; })());
...
function fullReviewDraft() { return L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'review', completedSteps: ['welcome', 'profile', 'sports', 'training_level', 'goals_placeholder', 'schedule_placeholder', 'safety', 'body'], draftData: dd() }); }
...
ok('markReadyForReview merkt review', rdy.completedSteps.indexOf('review') >= 0);
ok('markReadyForReview falscher Schritt → ok:false', L.markReadyForReview(L.newDraft(), 9).ok === false);
ok('markReadyForReview ohne valides Profil → ok:false', L.markReadyForReview(L.normalizeDraft({ version: 2, currentStep: 'review', completedSteps: ['welcome'] }), 9).ok === false);
ok('markReadyForReview lückenhaft → ok:false', L.markReadyForReview(L.normalizeDraft({ version: 2, currentStep: 'review', completedSteps: ['welcome', 'profile'], draftData: { profile: validProfile } }), 9).ok === false);
...
ok('markReadyForReview ohne completedSteps wirft nicht', (function () { try { var x = { currentStep: 'review', draftData: { profile: VALIDP } }; var r = L.markReadyForReview(x); return r && r.ok === false; } catch (e) { return false; } })());
ok('markReadyForReview falscher Typ completedSteps wirft nicht', (function () { try { var r = L.markReadyForReview({ currentStep: 'review', completedSteps: 'kaputt', draftData: { profile: VALIDP } }); return r && r.ok === false; } catch (e) { return false; } })());
...
ok('sports_placeholder → sports (Alias; kohärenter Draft mit erledigtem Profil)', L.normalizeDraft({ version: 2, currentStep: 'sports_placeholder', completedSteps: ['welcome', 'profile'], draftData: { profile: JSON.parse(JSON.stringify(VALIDP)) } }).currentStep === 'sports');
...
ok('advanceSports valide → ok + Wechsel zu training_level (M5b)', (function () { var d = L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'sports', completedSteps: ['welcome', 'profile'], draftData: dd() }); var r = L.advanceSports(d, 1); return r.ok === true && d.currentStep === 'training_level' && d.completedSteps.indexOf('sports') >= 0; })());
...
ok('reviewPrerequisitesComplete verlangt valide Sports', L.reviewPrerequisitesComplete(L.normalizeDraft({ version: 2, status: 'in_progress', currentStep: 'review', completedSteps: ['welcome', 'profile', 'sports', 'training_level', 'goals_placeholder', 'schedule_placeholder', 'safety', 'body'], draftData: { profile: VALIDP } })) === false);
...
let rfInvalid = L.normalizeDraft({ version: 2, status: 'ready_for_review', currentStep: 'review', completedSteps: ['welcome', 'profile', 'sports', 'training_level', 'goals_placeholder', 'schedule_placeholder', 'review'], draftData: { profile: VALIDP } });
ok('reviewFinal: invalide Sports → kein review_placeholder in completedSteps', rfInvalid.completedSteps.indexOf('review') < 0);
ok('reviewFinal: valide (vollständige Vorschritte) → review bleibt', L.normalizeDraft({ version: 2, status: 'ready_for_review', currentStep: 'review', completedSteps: ['welcome', 'profile', 'sports', 'training_level', 'goals_placeholder', 'schedule_placeholder', 'safety', 'body', 'review'], draftData: dd() }).completedSteps.indexOf('review') >= 0);
