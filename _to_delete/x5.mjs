...
const p = 'app/js/onboarding/onboarding-ui.js';
...
const uniq = [...new Set(texts.map(t => t))].filter(t => t.trim().length > 1 && !/^[\s·•–—|:()/.,+×~&]+$/.test(t) && t !== 'ORVIA' && !/^\[ORVIA/.test(t));
...
let n = 0;
for (const t of uniq) {
  let k = 'ob.' + slug(t); if (!k.replace('ob.','')) continue; while (used.has(k)) k += '_'; used.add(k);
...
src = src.replace("  function now() {", "  /* B-13: nutzersichtbare Texte ueber t() (locales/de.js); ohne i18n-Modul bleibt der Key sichtbar. */\n  function T(k, p) { try { if (O.i18n && typeof O.i18n.t === 'function') return O.i18n.t(k, p); } catch (e) {} return String(k); }\n  function now() {");
writeFileSync(p, src);
writeFileSync('_to_delete/ob-catalog.json', JSON.stringify(catalog, null, 1));
