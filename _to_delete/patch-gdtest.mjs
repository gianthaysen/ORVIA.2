...
const f = 'supabase/tests/goal_detail_test.mjs'; let s = fs.readFileSync(f, 'utf8');
const a = s.indexOf("sec('A · Modell');"), b = s.indexOf("sec('C · Verdrahtung');");
if (a < 0 || b < 0) throw new Error('sections');
const mid = `sec('A · Modell (S1.5: Prototyp-v14-Tiefe)');
{
  ...
  ok('A5 Gruende: Umfang plus, 10-km-Bestzeit → 1:48:30 plus, langer Lauf plus (Bedarf 14), Schluessel 1,0/Wo vs 3 minus, Unsicherheit neutral', m.reasons.length === 5 && m.reasons[0].kind === 'plus' && /1:48:30/.test(m.reasons[1].html) && m.reasons[1].kind === 'plus' && m.reasons[2].kind === 'plus' && /21,4 km/.test(m.reasons[2].html) && m.reasons[3].kind === 'minus' && /1,0 Schlüsseleinheiten/.test(m.reasons[3].html) && m.reasons[4].kind === 'neutral' && /ohne Wettkampf/.test(m.reasons[4].html));
  ok('A6 Ausgangswert = naechste gemessene Distanz (10 km), Riegel 1:48:30', m.baseline.key === 'k10' && Math.round(m.baseline.equivMin * 10) / 10 === 108.5);
  ...
  ok('A8 Stellschrauben = nur Minus-Gruende, max. 2, ohne erfundene Sekunden', m.levers.length === 1 && /Schlüsseleinheiten/.test(m.levers[0].html) && /Planer/.test(m.levers[0].effect));
...
sec('B · HTML');
{
  ...
  ok('B3 Zielvertrag + Prognoseverlauf-Karte (ohne Serie: ehrlicher Hinweis) + Plan dieser Woche + Einzahlungen 4 Wochen', /Zielvertrag/.test(h) && /Noch kein Verlauf/.test(h) && /Diese Woche · 1 Einheiten/.test(h) && (h.match(/class="gd-dep/g) || []).length === 4 && /<span class="v">2 \\/ 2</.test(h));
...
s = s.replace("ok('C3 Bearbeiten nur ueber openGoalEditor, kein eigener Schreibpfad', /openGoalEditor\\(goal\\.id\\)/.test(src) && !/goalUpdate\\(|commitGoals\\(|saveProfile\\(/.test(src));",
...
fs.writeFileSync(f, s); console.log('test patched');
