...
var GM_NA='' + T('ui.noch_nicht_verfuegbar') + '';
...
var GM_METRIC_DEFS={sleep_duration_min:{label:'' + T('ui.schlaf') + '',icon:'moon',color:'sleep',unit:' min'},hrv_ms:{label:'Herzfrequenzvariabilität',icon:'pulse',color:'ready',unit:' ms'},resting_hr:{label:'Ruhepuls',icon:'heart',color:'ready',unit:' bpm'},stress_avg:{label:'Stress',icon:'wind',color:'ready',unit:''},body_battery:{label:'Body Battery',icon:'battery',color:'activity',unit:''},steps:{label:'Schritte',icon:'activity',color:'activity',unit:''},active_kcal:{label:'' + T('ui.aktive_energie') + '',icon:'bolt',color:'activity',unit:' kcal'},load:{label:'' + T('ui.trainingsbelastung') + '',icon:'gauge',color:'ready',unit:''},
...
training_readiness:{label:'' + T('ui.training_readiness') + '',icon:'bolt',color:'ready',unit:''},acute_load:{label:'' + T('ui.acute_load') + '',icon:'gauge',color:'activity',unit:''},load_ratio:{label:'' + T('ui.load_ratio_acwr') + '',icon:'gauge',color:'ready',unit:''},recovery_time_h:{label:'' + T('ui.recovery_time') + '',icon:'heart',color:'ready',unit:' h'},
endurance_score:{label:'' + T('ui.endurance_score') + '',icon:'activity',color:'activity',unit:''},running_tolerance:{label:'' + T('ui.running_tolerance') + '',icon:'activity',color:'ready',unit:'' + T('ui.km_wo') + ''},fitness_age:{label:'Fitnessalter',icon:'pulse',color:'ready',unit:'' + T('ui.j') + ''},respiration_avg:{label:'Atemfrequenz',icon:'wind',color:'ready',unit:'/min'},vo2max_running:{label:'' + T('ui.vo_max_laufen') + '',icon:'pulse',color:'ready',unit:''},vo2max_cycling:{label:'' + T('ui.vo_max_radfahren') + '',icon:'pulse',color:'activity',unit:''},
stress_max:{label:'Stress-Maximum',icon:'wind',color:'ready',unit:''},sleep_deep_min:{label:'Tiefschlaf',icon:'moon',color:'sleep',unit:' min'},sleep_light_min:{label:'Leichtschlaf',icon:'moon',color:'sleep',unit:' min'},sleep_rem_min:{label:'REM-' + T('ui.schlaf') + '',icon:'moon',color:'sleep',unit:' min'},sleep_awake_min:{label:'Wachphasen',icon:'moon',color:'sleep',unit:' min'}};
...
var GM_METRIC_INFO={
  sleep_duration_min:{hb:true,
    a:'' + T('ui.so_lange_hast_du_letzte') + '',
    f:'' + T('ui.gemessene_schlafdauer_der_letzten_nacht') + '',
    p:'' + T('ui.schlafdauer_aus_der_geraeteerkennung_bewegung') + '',
    factors:['Zubettgeh-' + T('ui.zeit') + '','' + T('ui.koffein_am_nachmittag') + '','Alkohol','' + T('ui.spaete_schwere_mahlzeiten') + '','' + T('ui.bildschirmzeit_am_abend') + '','Raumtemperatur','' + T('ui.spaete_harte_einheiten') + ''],
    meaning:'' + T('ui.zu_wenig_schlaf_senkt_regeneration') + ''},
  hrv_ms:{hb:true,
    a:'' + T('ui.die_herzfrequenzvariabilitaet_zeigt_wie_erholt') + '',
    f:'' + T('ui.naechtliche_hrv_einzelwerte_schwanken_stark') + '',
    p:'' + T('ui.naechtliches_rmssd_basiertes_signal_interpretation') + '',
    factors:['' + T('ui.trainingsbelastung_der_vortage') + '','Alkohol','Infekt/Krankheit','Schlafqualität','Stress','' + T('ui.hitze_und_dehydration') + ''],
    meaning:'' + T('ui.deutlich_unter_baseline_ueber_mehrere') + ''},
  resting_hr:{hb:false,
    a:'' + T('ui.dein_puls_in_voelliger_ruhe') + '',
    f:'' + T('ui.naechtlicher_ruhepuls_ein_anstieg_von') + '',
    p:'' + T('ui.minimum_nachtwert_des_geraets_als') + '',
    factors:['Erholungszustand','Infekt/Fieber','Alkohol','Hitze','Dehydration','Aufregung/Stress','' + T('ui.ausdauertrainingszustand_langfristig') + ''],
    meaning:'' + T('ui.ein_ueber_tage_erhoehter_ruhepuls') + ''},
  stress_avg:{hb:false,
    a:'' + T('ui.dein_durchschnittlicher_stresswert_heute_0') + '',
    f:'' + T('ui.tages_stresswert_des_geraets_abgeleitet') + '',
    p:'' + T('ui.hrv_basiertes_autonomes_belastungsmass_0') + '',
    factors:['Arbeits-/Alltagsbelastung','Koffein','Schlafdefizit','' + T('ui.training_physiologisch_erwartbar') + '','Infekt','' + T('ui.emotionale_anspannung') + ''],
    meaning:'' + T('ui.hoher_alltagsstress_hartes_training_addieren') + ''},
  body_battery:{hb:true,
    a:'' + T('ui.deine_energiereserve_0_100_nachts') + '',
    f:'' + T('ui.energie_schaetzung_des_geraets_aus') + '',
    p:'' + T('ui.modellwert_0_100_aus_hrv') + '',
    factors:['Schlafqualität','Stresslevel','' + T('ui.trainingsbelastung') + '','Alkohol','Krankheit'],
    meaning:'' + T('ui.ein_niedriger_wert_verbietet_kein') + ''},
  steps:{hb:true,
    a:'' + T('ui.deine_schritte_heute_alltagsbewegung_zaehlt') + '',
    f:'' + T('ui.tagesschritte_als_mass_der_alltagsaktivitaet') + '',
    p:'' + T('ui.schrittzaehlung_des_geraets_an_harten') + '',
    factors:['Alltag/Beruf','Trainingseinheiten','' + T('ui.bewusste_spaziergaenge') + ''],
    meaning:'' + T('ui.sehr_hohe_alltagsaktivitaet_an_harten') + ''},
  active_kcal:{hb:true,
    a:'' + T('ui.kalorien_die_du_heute_durch') + '',
    f:'' + T('ui.aktive_energie_ueber_dem_grundumsatz') + '',
    p:'' + T('ui.geraeteschaetzung_des_aktivitaetsbedingten_verbrauchs_hf') + '',
    factors:['' + T('ui.trainingsumfang_und_intensitaet') + '','Alltagsaktivität','Körpergewicht'],
    meaning:'' + T('ui.anhaltend_hoher_verbrauch_ohne_angepasste') + ''},
  recovery_time_h:{hb:false,
    f:'' + T('ui.vom_geraet_geschaetzte_verbleibende_erholungszeit') + '',
    p:'' + T('ui.garmin_modellwert_aus_belastung_erholungssignalen') + '',
    factors:['' + T('ui.letzte_trainingsbelastung') + '','' + T('ui.schlaf') + '','Stress'],
    meaning:'' + T('ui.innerhalb_der_erholungszeit_sind_lockere') + ''},
  training_readiness:{hb:true,
    f:'' + T('ui.garmin_bereitschaftswert_aus_schlaf_erholungszeit') + '',
    p:'' + T('ui.provider_composite_0_100_parallel') + '',
    factors:['' + T('ui.schlaf') + '','HRV-Status','Erholungszeit','Belastungshistorie'],
    meaning:'' + T('ui.hohe_readiness_stuetzt_qualitaetsreize_niedrige') + ''},
  acute_load:{hb:null,
    f:'' + T('ui.akute_trainingsbelastung_der_letzten_tage') + '',
    p:'' + T('ui.epoc_gewichtete_7_tage_last') + '',
    factors:['Trainingsumfang','' + T('ui.intensitaet') + ''],
    meaning:'' + T('ui.sprunghafte_anstiege_der_akuten_last') + ''},
  load_ratio:{hb:null,
    f:'' + T('ui.verhaeltnis_akuter_zu_chronischer_belastung') + '',
    p:'' + T('ui.provider_acwr_methodisch_umstritten_als') + '',
    factors:['' + T('ui.belastungsaufbau_der_letzten_wochen') + '','Pausen/Ausfälle'],
    meaning:'' + T('ui.deutlich_ueber_1_3_1') + ''},
  respiration_avg:{hb:null,
    f:'' + T('ui.durchschnittliche_atemfrequenz_meist_naechtlich_stabil') + '',
    p:'' + T('ui.atemzuege_min_aus_der_geraeteerkennung') + '',
    factors:['Infekt','Schlafqualität','Stress'],
    meaning:'' + T('ui.auffaellige_anstiege_zusammen_mit_erhoehtem') + ''},
  vo2max_running:{hb:true,
    f:'' + T('ui.geschaetzte_maximale_sauerstoffaufnahme_laufen_der') + '',
    p:'' + T('ui.geraeteschaetzung_aus_pace_hf_relation') + '',
    factors:['' + T('ui.aerobes_training_ueber_monate') + '','Gewichtsänderung','' + T('ui.hitze_verzerrt_schaetzung') + ''],
    meaning:'' + T('ui.langfristig_steigende_vo_max_bestaetigt') + ''},
  vo2max_cycling:{hb:true,
    f:'' + T('ui.geschaetzte_maximale_sauerstoffaufnahme_radfahren') + '',
    p:'' + T('ui.leistungs_hf_basierte_schaetzung_ohne') + '',
    factors:['' + T('ui.aerobes_training') + '','Powermeter-Verfügbarkeit'],
    meaning:'' + T('ui.trend_zaehlt_einzelwerte_nicht_ueberinterpretieren') + ''},
  stress_max:{hb:false,
    f:'' + T('ui.hoechster_gemessener_stresswert_des_tages') + '',
    p:'' + T('ui.spitzenwert_der_intraday_stresskurve_einzelne') + '',
    factors:['' + T('ui.akute_anspannung') + '','' + T('ui.training') + '','Koffein'],
    meaning:'' + T('ui.einzelne_spitzen_sind_unkritisch_lange') + ''},
  /* Phase 4 (2026-08-05, P2-3): Schlafphasen — vorher griff der Generiktext. */
  sleep_deep_min:{hb:true,
    a:'' + T('ui.im_tiefschlaf_erholt_sich_dein') + '',
    f:'' + T('ui.tiefschlaf_dauer_der_letzten_nacht') + '',
    p:'' + T('ui.geraeteschaetzung_aus_bewegung_hf_hrv') + '',
    factors:['' + T('ui.alkohol_stark_negativ') + '','' + T('ui.spaete_intensive_einheiten') + '','' + T('ui.zubettgeh_zeit_erste_nachthaelfte') + '','Raumtemperatur','Schlafregelmäßigkeit'],
    meaning:'' + T('ui.anhaltend_wenig_tiefschlaf_verschlechtert_regeneration') + ''},
  sleep_light_min:{hb:null,
    a:'' + T('ui.leichtschlaf_ist_der_normale_uebergangsschlaf') + '',
    f:'' + T('ui.leichtschlaf_dauer_der_letzten_nacht') + '',
    p:'' + T('ui.sammelkategorie_der_geraeteklassifikation_n1_n2') + '',
    factors:['' + T('ui.schlafdauer_gesamt') + '','Koffein','Stress','' + T('ui.umgebung_laerm_licht') + ''],
    meaning:'' + T('ui.fuer_sich_genommen_wenig_steuerungsrelevant') + ''},
  sleep_rem_min:{hb:true,
    a:'' + T('ui.im_rem_schlaf_verarbeitet_dein') + '',
    f:'' + T('ui.rem_dauer_der_letzten_nacht') + '',
    p:'' + T('ui.rem_haeuft_sich_in_den') + '',
    factors:['' + T('ui.gesamtschlafdauer_zweite_nachthaelfte') + '','Alkohol','' + T('ui.sehr_frueher_wecker') + '','' + T('ui.unregelmaessige_schlafzeiten') + ''],
    meaning:'' + T('ui.wer_chronisch_frueh_raus_muss') + ''},
  sleep_awake_min:{hb:false,
    a:'' + T('ui.wachzeit_in_der_nacht_kurzes') + '',
    f:'' + T('ui.erkannte_wachminuten_innerhalb_des_schlaffensters') + '',
    p:'' + T('ui.geraete_unterschaetzen_ruhiges_wachliegen_systematisch') + '',
    factors:['' + T('ui.alkohol_zweite_nachthaelfte') + '','Stress/Grübeln','' + T('ui.harndrang_spaete_fluessigkeit') + '','Lärm/Licht','Hitze'],
    meaning:'' + T('ui.steigende_naechtliche_wachzeit_ueber_mehrere') + ''}
};
...
var GM_ROUTINE_PRESETS=[['mob','Mobility'],['stretch','Dehnen'],['ss','' + T('ui.spanish_squats_knie') + ''],['ice','Eisbeutel/Kühlen'],['core','Core-Routine'],['walk','Spaziergang'],['med','Meditation/Atmung'],['foam','Faszienrolle']];
function activeRoutines(){
  ...
  if(issues.indexOf('knee')>=0)return [['mob','Mobility'],['ss','' + T('ui.spanish_squats_knie') + ''],['ice','' + T('ui.eisbeutel_knie') + '']];
  return [['mob','Mobility']];
}
function gmOpenRoutinesEditor(){
  ...
  var rows=cur2.map(function(r,i){
    return '<div class="ps-row"><span>'+gmEsc(r[1])+'</span><button class="xbtn" aria-label="' + T('ui.entfernen') + '" onclick="gmRoutineRemove('+i+')">✕</button></div>';}).join('');
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('repeat')+'</div><div><h3>' + T('ui.routinen_anpassen') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.deine_taeglichen_gewohnheiten_individuell_nicht') + '</div></div></div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.aktiv') + '</div>'+(rows||'<p class="muted" style="margin:0">' + T('ui.keine_routinen_aktiv_auch_das') + '</p>')+'</div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.vorschlaege') + '</div><div class="chips" style="display:flex;flex-wrap:wrap;gap:8px">'+(presets||'<span class="muted">' + T('ui.alle_vorschlaege_aktiv') + '</span>')+'</div>'+
    '<div class="calc-field" style="margin-top:10px"><label>' + T('ui.eigene_routine') + '</label><input type="text" id="gmRoutineNewIn" maxlength="40" placeholder="' + T('ui.z_b_10_min_lesen') + '"><button class="btn sec" style="margin-top:8px" onclick="gmRoutineAddCustom()">' + T('ui.hinzufuegen') + '</button></div></div>'+
    '<div class="source">'+icon('info','xs')+' Erfasste Haken vergangener ' + T('ui.tage__') + ' bleiben erhalten, auch wenn eine Routine später entfernt wird.</div>';
...
const SLOTS=['Morgens','Pre-Workout','Post-Workout','' + T('ui.mit_mahlzeit') + '','Abends'];
const DAYNAMES=['' + T('ui.mo_') + '','' + T('ui.di') + '','' + T('ui.mi') + '','' + T('ui.do') + '','' + T('ui.fr') + '','' + T('ui.sa') + '','' + T('ui.so') + ''];
const WEEKPLAN=[
  [{t:'Gym',l:'Ganzkörper',d:'' + T('ui.45') + ' min'}],
  [{t:'Schwimmen',l:'Technik',d:'' + T('ui.6_00_uhr_900_m') + ''},{t:'Gym',l:'Oberkörper',d:'' + T('ui.45') + ' min'}],
  [{t:'Laufen',l:'Intervalle',d:'iv'},{t:'Gym',l:'Ganzkörper',d:'' + T('ui.45') + ' min'}],
  [{t:'Gym',l:'Oberkörper',d:'' + T('ui.45') + ' min'}],
  [{t:'Laufen',l:'' + T('ui.z2_dauerlauf') + '',d:'ez'}],
  [{t:'Schwimmen',l:'' + T('ui.ausdauer') + '',d:'~900 m'},{t:'Rad',l:'' + T('ui.z2_dauerfahrt') + '',d:'60 min · 123–144 bpm'}],
...
function gpG(l){var s=String(l||'').toLowerCase();return {t:'Gym',l:l,d:'' + T('ui.45') + ' min',kind:/bein|ganzk|squat|leg|unterk|kniebeuge/.test(s)?'gym_leg':'gym'};}
...
function generateWeekPlan(){
  ...
  if(triGoal){
    ...
    w[1]=[triBeg?gpR('' + T('ui.z2_dauerlauf') + '','ez'):gpR('Intervalle','iv')];
    w[2]=[gpB('Easy Z2',triBeg?'' + T('ui.45') + ' min':'75 min')];
    w[3]=[gpS('' + T('ui.ausdauer') + '',triBeg?'~900 m':'~1200 m')].concat(gym?[gpG('Oberkörper')]:[]);
    w[4]=gym?[gpG('Ganzkörper')]:[gpM()];
    w[5]=triBeg?[gpB('Easy Z2','60 min')]:[gpB('' + T('ui.long_ride') + '','2–3 h'),gpR('Koppellauf','ez')];
    w[6]=[gpR('Long Run','lr')];
  }else if(runGoal||(run&&!swim)){
    if(lvl==='anfaenger'){w[1]=[gpR('' + T('ui.z2_dauerlauf') + '','ez')];w[3]=[gpR('' + T('ui.z2_dauerlauf') + '','ez')];w[6]=[gpR('Long Run','lr')];}
    else{w[1]=[gpR('Intervalle','iv')];w[3]=[gpR(lvl==='profi'?'Tempo':'' + T('ui.z2_dauerlauf') + '',lvl==='profi'?'tempo':'ez')];w[6]=[gpR('Long Run','lr')];}
    ...
    w[1]=[lvl==='anfaenger'?gpB('Easy Z2','60 min'):gpB('Intervalle','5×5 min')];w[2]=[gym?gpG('Oberkörper'):gpM()];w[3]=[gpB('Easy Z2','75 min')];w[4]=lvl==='anfaenger'?[]:[gym?gpG('Beine'):gpM()];w[5]=[gpB('' + T('ui.long_ride') + '','2–3 h')];w[6]=[gpB('Recovery','' + T('ui.45') + ' min')];
    ...
    w[0]=[gpG('Oberkörper')];w[1]=[gpR('Intervalle','iv')];w[2]=[gpG('Ganzkörper')];w[3]=[gpR('' + T('ui.z2_dauerlauf') + '','ez')];w[4]=[gpG('Oberkörper')];w[6]=[gpR('Long Run','lr')];
  }else if(bike&&gym&&!run){
    w[0]=[gpG('Oberkörper')];w[1]=[gpB('Intervalle','5×5 min')];w[3]=[gpG('Beine')];w[4]=[gpB('Easy Z2','60 min')];w[6]=[gpB('' + T('ui.long_ride') + '','2–3 h')];
  }else if(run&&bike&&!gym){
    w[1]=[gpR('Intervalle','iv')];w[2]=[gpB('Easy Z2','75 min')];w[4]=[gpR('' + T('ui.z2_dauerlauf') + '','ez')];w[5]=[gpB('' + T('ui.long_ride') + '','2–3 h')];w[6]=[gpR('Long Run','lr')];
  }else{
    var d2=planDaysTarget();
    w[0]=[gym?gpG('Ganzkörper'):gpM()];w[2]=[run?gpR('' + T('ui.z2_dauerlauf') + '','ez'):(bike?gpB('Easy Z2','' + T('ui.45') + ' min'):gpM())];w[4]=[gym?gpG('Ganzkörper'):gpM()];
    if(d2>=4)w[5]=[run?gpR('' + T('ui.z2_dauerlauf') + '','ez'):(bike?gpB('Easy Z2','60 min'):gpG('Oberkörper'))];
    w[6]=[gpM()];
  }
  ...
  if(maxDays){
    ...
    while(_activeN<maxDays&&_fi<_fillPool.length){
      ...
      w[_day]=[run?gpR('' + T('ui.z2_dauerlauf') + '','ez'):(bike?gpB('Easy Z2','60 min'):(swim?gpS('Technik','~900 m'):(gym?gpG('Ganzkörper'):gpM())))];
      _activeN++;
    }
  }
  /* ============================================================
  ...
  (function(){
    ...
    var MAKE={
      running:function(){return gpR('' + T('ui.z2_dauerlauf') + '','ez');},
...
/* Batch 2d: Template ≠ konkrete Planinstanz. item.id ('ps:'/'psg:') ist die
   ...
   'po:<lokales ' + T('ui.datum_') + '>:<templateSessionId>': gleiche Woche ⇒ über Reloads
...
function gmPlanWeekHeader(off){
  ...
  return {label:o===0?'' + T('ui.diese_woche') + '':(o===-1?'' + T('ui.letzte_woche') + '':(o===1?'' + T('ui.naechste_woche') + '':
      (o<0?Math.abs(o)+'' + T('ui.wochen_zurueck') + '':o+'' + T('ui.wochen_voraus') + ''))),
...
var PLAN_PRESETS=[
  {t:'Laufen',l:'Intervalle',d:'iv'},{t:'Laufen',l:'' + T('ui.z2_dauerlauf') + '',d:'ez'},{t:'Laufen',l:'Tempo',d:'tempo'},{t:'Laufen',l:'Long Run',d:'lr'},
  {t:'Rad',l:'Easy Z2',d:'60 min'},{t:'Rad',l:'' + T('ui.long_ride') + '',d:'90 min'},{t:'Rad',l:'Commute',d:'Pendeln'},
  {t:'Schwimmen',l:'Technik',d:'~900 m'},{t:'Schwimmen',l:'' + T('ui.ausdauer') + '',d:'~1000 m'},
  {t:'Gym',l:'Ganzkörper',d:'' + T('ui.45') + ' min'},{t:'Gym',l:'Oberkörper',d:'' + T('ui.45') + ' min'},{t:'Gym',l:'Push',d:'' + T('ui.45') + ' min'},{t:'Gym',l:'Pull',d:'' + T('ui.45') + ' min'},{t:'Gym',l:'Core',d:'30 min'},{t:'Gym',l:'Beine',d:'' + T('ui.45') + ' min'},
...
function planActualToday(){
  var UNBEST={key:'unbestimmt',label:'' + T('ui.nicht_bestimmbar') + '',assessable:false};
  var today=(typeof todayStr==='function')?todayStr():null;
  ...
  if(!day){
    if(hasUnmatched)return{key:'ungeplant',label:'' + T('ui.ungeplante_einheit') + '',assessable:true};
    return{key:'keins',label:'',assessable:true};
  }
  if(day.status==='ambiguous')return UNBEST;   // mehrdeutig ⇒ ehrlich nicht bestimmbar
  if(day.status==='unknown'&&hasUnmatched)return{key:'ungeplant',label:'' + T('ui.ungeplante_einheit') + '',assessable:true};
  var map={completed:{key:'erfuellt',label:'' + T('ui.plan_erfuellt') + '',assessable:true},
           partial:{key:'teilweise',label:'' + T('ui.teilweise_erfuellt') + '',assessable:(day.assessable!==false)},
           missed:{key:'ausgefallen',label:'' + T('ui.einheit_ausgefallen') + '',assessable:true},
...
function orviaScore(){
  ...
  var subs=[['' + T('ui.erholung_') + '',d.subscores.recovery.value,d.subscores.recovery.weight],
...
function setUiDetailMode(m){if(m!=='anfaenger'&&m!=='fortgeschritten'&&m!=='profi')return;try{localStorage.setItem('orvia_ui_mode',m);}catch(e){}
  ...
  if(typeof applyLevelClass==='function')applyLevelClass();if(typeof renderLevelBox==='function')renderLevelBox();if(typeof renderCommand==='function')renderCommand();if(typeof toast==='function')toast('' + T('ui.ansicht') + ''+({anfaenger:'Anfaenger',fortgeschritten:'' + T('ui.fortgeschritten_') + '',profi:'Profi'}[m]||m));}
...
function renderLevelBox(){var el=document.getElementById('levelBox');if(!el)return;var c=uiDetailMode();
  var opts=[['anfaenger','' + T('ui.anfaenger') + '','' + T('ui.klar_reduziert') + ''],['fortgeschritten','' + T('ui.fortgeschritten_') + '','' + T('ui.klarheit_analyse') + ''],['profi','Profi','' + T('ui.maximale_detailtiefe') + '']];
  el.innerHTML='<div class="lvl-opts">'+opts.map(function(o){return '<button class="lvl-opt'+(c===o[0]?' on':'')+'" onclick="setUserLevel(\''+o[0]+'\')"><b>'+o[1]+'</b><span>'+o[2]+'</span></button>';}).join('')+'</div>'+
    '<p class="note" style="text-align:left;margin-top:10px">' + T('ui.aendert_nur_informationsdichte_amp_fachtiefe') + '</p>';}
function simpleIntensity(a,score){
  if(a.c==='r')return {w:'' + T('ui.ruhetag_') + '',c:'r'};
...
function dataConfidence(){
  ...
  var msg=lvl.l==='hoch'?'' + T('ui.genug_daten_fuer_verlaessliche_empfehlungen') + '':lvl.l==='mittel'?'' + T('ui.solide_basis_mehr_check_ins') + '':'' + T('ui.orvia_braucht_mehr_check_ins') + '';
...
function renderConfidence(){
  ...
  try{ if(window.ORVIA&&window.ORVIA.readinessStore){
    ...
    var lbl=st==='active'?['' + T('ui.persoenliche_baseline_aktiv_') + '','g']:st==='building'?['' + T('ui.persoenliche_baseline_wird_aufgebaut') + '','y']:['' + T('ui.persoenliche_baseline_noch_zu_wenig') + '','r'];
...
var FIXED_EVENT_TYPES=[['club_training','Vereinstraining'],['match','Spiel'],['race','Wettkampf'],['work','Arbeit'],['school','Schule'],['travel','Reise'],['course','Kurs'],['other','' + T('ui.sonstiges') + '']];
...
function openFixedEventEditor(){
  ...
  wrap.innerHTML='<div class="orvia-modal goal-modal"><h3>' + T('ui.fester_termin') + '</h3>'+
    '<div class="gm-field"><label>' + T('ui.titel') + '</label><input type="text" id="feTitle" placeholder="' + T('ui.z_b_spiel_vereinstraining') + '"></div>'+
    '<div class="gm-field"><label>' + T('ui.typ') + '</label><div class="gm-chips" id="feType">'+FIXED_EVENT_TYPES.map(function(t,i){return '<button type="button" class="gm-chip'+(i===0?' on':'')+'" data-v="'+t[0]+'" onclick="gmPick(this,\'feType\')">'+esc(t[1])+'</button>';}).join('')+'</div></div>'+
    '<div class="gm-field"><label>' + T('ui.datum_') + '</label><input type="date" id="feDate" value="'+escH(todayStr())+'"></div>'+
    '<div class="gm-field"><label>' + T('ui.prioritaet') + '</label><div class="gm-chips" id="fePrio"><button type="button" class="gm-chip on" data-v="normal" onclick="gmPick(this,\'fePrio\')">Normal</button><button type="button" class="gm-chip" data-v="high" onclick="gmPick(this,\'fePrio\')">Wichtig</button></div></div>'+
    '<button class="btn" onclick="saveFixedEventFromForm()">' + T('ui.termin_speichern') + '</button>'+
    '<button class="btn sec" style="margin-top:10px" onclick="closeFixedEventEditor()">' + T('ui.abbrechen') + '</button></div>';
...
function saveFixedEventFromForm(){
  ...
  if(!isDay(date)){if(typeof toast==='function')toast('' + T('ui.bitte_gueltiges_datum') + '');return;}
  ...
  closeFixedEventEditor();renderTrainingSetup();if(typeof renderAdaptCard==='function')renderAdaptCard();if(typeof toast==='function')toast('' + T('ui.termin_gespeichert') + '');
}
function renderFixedEventsBox(){
  var evs=fixedEventsList().slice().sort(function(a,b){return (a.date||'').localeCompare(b.date||'');});
  var rows=evs.length?evs.map(function(ev){return '<div class="fe-row"><span class="fe-main"><b>'+esc(ev.title||fixedEventLabel(ev.type))+'</b><span class="fe-sub">'+esc(fixedEventLabel(ev.type))+' · '+esc(ev.date)+(ev.priority==='high'?' · wichtig':'')+'</span></span><button class="fe-del" onclick="removeFixedEvent(\''+ev.id+'\')" aria-label="' + T('ui.entfernen') + '">✕</button></div>';}).join(''):'<div class="ob-empty">' + T('ui.noch_keine_festen_termine') + '</div>';
  return '<div class="ts-row" style="flex-direction:column;align-items:stretch"><div class="ts-lab" style="margin-bottom:6px">' + T('ui.feste_termine_fix_nicht_verschiebbar') + '</div>'+
    '<div class="fe-list">'+rows+'</div>'+
    '<button class="btn sec" style="margin-top:8px" onclick="openFixedEventEditor()">' + T('ui.termin_hinzufuegen') + '</button>'+
    '<p class="note" style="text-align:left;margin-top:6px">' + T('ui.feste_termine_spiel_vereinstraining_wettkampf') + '</p></div>';
}
...
function renderTrainingSetup(){
  ...
  var srcDE={availability:'' + T('ui.aus_deiner_verfuegbarkeit') + '',legacy:'' + T('ui.alt_einstellung_verfuegbarkeit_im_profil') + '',none:'' + T('ui.standard_nach_trainingsniveau') + ''};
  var modeDE={manual:'' + T('ui.nur_hinweis') + '',assisted:'Vorschlag',automatic:'Automatisch'};
  var riskDE={conservative:'Konservativ',balanced:'Ausgewogen',ambitious:'Ambitioniert'};
  var dayNames=['' + T('ui.mo_') + '','' + T('ui.di') + '','' + T('ui.mi') + '','' + T('ui.do') + '','' + T('ui.fr') + '','' + T('ui.sa') + '','' + T('ui.so') + ''];
  var availStr=(cfg.availableDayIdx&&cfg.availableDayIdx.length)?cfg.availableDayIdx.map(function(i){return dayNames[i];}).join(', '):'' + T('ui.noch_nicht_festgelegt') + '';
  function ro(label,val){return '<div class="ts-row"><div class="ts-lab">'+label+'</div><div class="ts-opts">'+val+'</div></div>';}
  el.innerHTML=
    '<p class="note" style="text-align:left;margin:0 0 10px">' + T('ui.dein_profil_ist_die_einzige') + '</p>'+
    ro('' + T('ui.trainingstage_woche') + '','<b>'+days+'</b> <span class="fe-sub">('+esc(srcDE[cfg.daysSource]||srcDE.none)+')</span>')+
    ro('' + T('ui.verfuegbare_tage') + '',esc(availStr))+
    (cfg.gymDays!=null?ro('Gym-' + T('ui.einheiten_') + '',cfg.gymDays+'' + T('ui.woche') + ''):'')+
    ro('Anpassungs-Modus',esc(modeDE[cfg.adaptationMode]||'Vorschlag'))+
    ro('Risikobereitschaft',esc(riskDE[cfg.riskTolerance]||'Ausgewogen'))+
    '<div class="gmc-acts" style="margin:10px 0"><button class="gmc-b" onclick="openAvailabilityEditor()">' + T('ui.verfuegbarkeit_bearbeiten') + '</button>'+
    '<button class="gmc-b" onclick="openPreferencesEditor()">' + T('ui.modus_amp_risiko_praeferenzen') + '</button></div>'+
...
function setTrainingDays(n){if(typeof PROFILE!=='undefined'&&PROFILE){PROFILE.trainingDays=n;if(typeof saveProfile==='function')saveProfile();}renderTrainingSetup();if(typeof renderWeekPlan==='function')renderWeekPlan();if(typeof toast==='function')toast('' + T('ui.trainingstage_') + ''+n+'' + T('ui.woche') + '');}
function setAdaptMode(m){if(typeof PROFILE!=='undefined'&&PROFILE){PROFILE.adaptationMode=m;if(typeof saveProfile==='function')saveProfile();}renderTrainingSetup();if(typeof renderAdaptCard==='function')renderAdaptCard();if(typeof toast==='function')toast('' + T('ui.modus') + ''+({manual:'' + T('ui.nur_hinweis') + '',assisted:'Vorschlag',automatic:'Automatisch'}[m]||m));}
function setRiskTol(r){if(typeof PROFILE!=='undefined'&&PROFILE){PROFILE.riskTolerance=r;if(typeof saveProfile==='function')saveProfile();}renderTrainingSetup();if(typeof toast==='function')toast('' + T('ui.risiko') + ''+({conservative:'Konservativ',balanced:'Ausgewogen',ambitious:'Ambitioniert'}[r]||r));}
function setGymDays(n){if(typeof PROFILE!=='undefined'&&PROFILE){PROFILE.gymDays=n;if(typeof saveProfile==='function')saveProfile();}renderTrainingSetup();if(typeof renderWeekPlan==='function')renderWeekPlan();if(typeof toast==='function')toast('' + T('ui.gym_tage') + ''+n+'' + T('ui.woche') + '');}
...
function applyWeekAdjustments(confirmed){
  ...
  var DN=['' + T('ui.mo_') + '','' + T('ui.di') + '','' + T('ui.mi') + '','' + T('ui.do') + '','' + T('ui.fr') + '','' + T('ui.sa') + '','' + T('ui.so') + ''];
  ...
  if(typeof toast==='function')toast(confirmed?'' + T('ui.wochenplan_angepasst') + '':'' + T('ui.wochenplan_automatisch_angepasst') + '');
}
function revertWeekAdjustments(){
  ...
  if(typeof saveProfile==='function')saveProfile();if(typeof save==='function')save();
  if(typeof renderPlan==='function')renderPlan();if(typeof renderAdaptCard==='function')renderAdaptCard();
  if(typeof toast==='function')toast('' + T('ui.anpassung_rueckgaengig_gemacht') + '');
}
...
function renderAdaptCard(){
  ...
  var avoid=d.avoidedSession?'<div class="adp-block"><span class="adp-lab">' + T('ui.vermeiden') + '</span><div class="adp-val">'+esc(d.avoidedSession.label)+'</div></div>':'';
  ...
  var week='';
  if(applied){
    week='<div class="adp-week"><b>' + T('ui.wochenplan_angepasst_') + '</b>' + T('ui.aenderungen_gespeichert_siehe_plan') + '<button class="linklike" onclick="revertWeekAdjustments()">' + T('ui.rueckgaengig') + '</button></div>';
  }else if(moves.length){
    week='<div class="adp-week"><b>' + T('ui.wochenplan_vorschlag') + '</b> '+moves.map(function(c){return c.action==='MOVE_SESSION'?('' + T('ui.harte_einheit') + ''+DN[c.day]+'' + T('ui.48_h_abstand_kein_konflikt') + ''):esc(c.reason);}).join(' ')+
      '<div class="adp-btns" style="margin-top:8px"><button class="btn sec" onclick="applyWeekAdjustments(true)">' + T('ui.wochenplan_anpassen') + '</button></div></div>';
  }
  var actions='';
  if(hasChange){
    if(ch==='accepted')actions='<div class="adp-accepted">✓ '+(mode==='automatic'?'' + T('ui.automatisch_angepasst') + '':'Übernommen')+'. <button class="linklike" onclick="adaptReopen()">'+(mode==='automatic'?'' + T('ui.rueckgaengig') + '':'ändern')+'</button></div>';
    else if(ch==='original')actions='<div class="adp-folded">' + T('ui.nur_als_hinweis_angezeigt') + '<button class="linklike" onclick="adaptReopen()">doch anpassen</button></div>';
  ...
  var feel='<button class="linklike adp-feel" onclick="adaptFeelDifferent()">' + T('ui.ich_fuehle_mich_anders_check') + '</button>';
  el.innerHTML='<div class="adapt-card adp-'+state.toLowerCase()+'">'+head+safety+rec+avoid+why+week+actions+feel+'</div>';
}
function adaptChoose(choice){var d=currentDecision();if(!d)return;var e=entry(todayStr());e.adaptChoice={action:d.todayAction,choice:choice};if(typeof save==='function')save();renderAdaptCard();if(typeof toast==='function')toast(choice==='original'?'' + T('ui.als_hinweis_angezeigt') + '':'' + T('ui.anpassung_uebernommen') + '');}
...
function adaptFeelDifferent(){var mf=document.getElementById('morningForm');if(mf)mf.scrollIntoView({behavior:'smooth',block:'start'});if(typeof toast==='function')toast('' + T('ui.passe_deinen_check_in_an') + '');}
/* ============ MORGEN ============ */
function checkinContextHint(){
  var hints=[];
  try{var y=todayStr(new Date(Date.now()-864e5));var ye=DB[y];if(ye&&ye.morning&&ye.morning.knee>=3)hints.push('' + T('ui.gestern_knie') + ''+ye.morning.knee+'' + T('ui.10_heute_gezielt_pruefen') + '');}catch(e){}
  var u=(typeof todayPrimaryUnit==='function')?todayPrimaryUnit():null;
  if(u&&typeof isHardUnit==='function'&&isHardUnit(u))hints.push('' + T('ui.heute_harte_einheit_geplant_achte') + '');
  if(typeof pauseFor==='function'&&pauseFor(todayStr()))hints.push('' + T('ui.pause_aktiv_kurz_check_reicht') + '');
...
function _ciFieldHTML(f,m,mode){
  ...
  switch(f.kind){
    ...
    case 'chipsBool':return chips(f.label,f.el,f.opts,[m[f.key]?'Ja':'' + T('ui.nein') + '']);
    ...
    case 'issues':return (typeof checkinIssuesHTML==='function')?checkinIssuesHTML(m):slider('m_knee',(mode==='quick'?'' + T('ui.beschwerden_jetzt') + '':'' + T('ui.knie_schmerz_jetzt') + ''),0,10,(m.knee!=null?m.knee:0),'kein','max');
...
function _ciFormHTML(fields,m,mode){
  ...
  if(mode==='quick')parts.push('<p class="note" style="margin-top:8px">' + T('ui.schnell_check_fuer_pace_hf') + '</p>');
  return parts.join('');}
...
function renderMorning(){
  ...
  var body=REG?_ciFormHTML(REG,m,mode):gmStateError({icon:'alert',title:'' + T('ui.check_in_modul_nicht_geladen') + '',desc:'' + T('ui.lade_die_app_neu_sobald') + ''});
...
function canEditCur(silent){
  ...
  if(!silent&&typeof toast==='function')toast('' + T('ui.tag_abgeschlossen_korrektur_erfassen_nutzen') + '');
  return false;
}
function startCorrection(){window._correctionMode=true;renderDay();if(typeof toast==='function')toast('' + T('ui.korrektur_modus_aktiv') + '');}
...
function saveMorning(){if(!canEditCur())return;entry(cur).morning=gatherMorning();if(typeof gatherCheckinIssues==='function')gatherCheckinIssues();if(window._correctionMode&&cur!==todayStr())logCorrection('morning');save();renderDecision();try{if(window.ORVIA&&window.ORVIA.checkinStore){window.ORVIA.checkinStore.persistMorning(cur).then(function(r){if(!r||typeof toast!=='function')return;
  if(r.success===false){
    ...
    toast('' + T('ui.check_in_lokal_gespeichert_cloud') + ''+((r.error&&r.error.code)||'unbekannt')+')');
  }
  else if(r.sync_status==='pending')toast('' + T('ui.offline_gespeichert_wird_synchronisiert') + '');});}}catch(e){}
  ...
  if(typeof collapseCheckinCard==='function')collapseCheckinCard();if(typeof renderCheckinCompact==='function')renderCheckinCompact();toast(window._correctionMode&&cur!==todayStr()?'' + T('ui.korrektur_gespeichert') + '':'' + T('ui.gespeichert') + '');window.scrollTo({top:0,behavior:'smooth'});}
...
function renderReadiness(){const e=entry(cur);const out=document.getElementById('readyOut');
  ...
  const txt=r.band==='g'?'Bereit':r.band==='y'?'Moderat':'' + T('ui.erholung_noetig') + '';
...
function toggleType(t){
  ...
  if(activeTypes.has(t)){
    ...
    if(hasContent(data)){_trash={date:cur,type:t,data};
      ...
      toastAction(t+' entfernt','' + T('ui.rueckgaengig') + '',undoTrash);return;}
...
function blockRun(d){return `<div class="sescard"><div class="seshead">${ic('run')}Laufen</div>
  ${chips('' + T('ui.typ') + '','l_sub',['Walk-Run','Easy Z2','Tempo','Intervalle','Long Run','Wettkampf'],d.sub?[d.sub]:[])}
  ...
  ${chips('' + T('ui.einheit_war') + '','l_felt',['zu leicht','passend','zu hart'],d.felt?[d.felt]:[])}
  ${slider('l_knee','' + T('ui.knie_post') + '',0,10,d.knee??0,'kein','max')}
...
var EXERCISES=[
 {n:'Spanish Squat',g:'' + T('ui.reha_knie') + '',m:'' + T('ui.quad_exzentrisch') + ''},
 {n:'VMO Step-down',g:'' + T('ui.reha_knie') + '',m:'' + T('ui.vastus_medialis') + ''},
 {n:'Terminal Knee Extension',g:'' + T('ui.reha_knie') + '',m:'' + T('ui.vmo_knie') + ''},
 {n:'Step-up',g:'' + T('ui.reha_knie') + '',m:'' + T('ui.quad_glute') + ''},
 {n:'Wall Sit',g:'' + T('ui.reha_knie') + '',m:'' + T('ui.quad_isometrisch') + ''},
 {n:'Wadenheben',g:'' + T('ui.reha_knie') + '',m:'' + T('ui.waden_achilles') + ''},
 {n:'Glute Bridge',g:'' + T('ui.glutes_posterior') + '',m:'Gesäß'},
 {n:'Single-Leg Bridge',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.gesaess_einbeinig') + ''},
 {n:'Hip Thrust',g:'' + T('ui.glutes_posterior') + '',m:'Gesäß'},
 {n:'Clamshells',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.gluteus_medius') + ''},
 {n:'Monster Walks',g:'' + T('ui.glutes_posterior') + '',m:'Abduktoren'},
 {n:'Romanian Deadlift',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.hamstrings_gesaess') + ''},
 {n:'Kreuzheben',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.gesamte_kette') + ''},
 {n:'Good Morning',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.hamstrings_ruecken') + ''},
 {n:'Nordic Curl',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.hamstrings_exzentrisch') + ''},
 {n:'Kabel-Kickback',g:'' + T('ui.glutes_posterior') + '',m:'' + T('ui.gesaess_isoliert') + ''},
 {n:'Bankdrücken',g:'' + T('ui.push_oberkoerper') + '',m:'' + T('ui.brust_trizeps') + ''},
 {n:'Schrägbankdrücken',g:'' + T('ui.push_oberkoerper') + '',m:'' + T('ui.obere_brust') + ''},
 {n:'Kurzhantel-Bankdrücken',g:'' + T('ui.push_oberkoerper') + '',m:'Brust'},
 {n:'Schulterdrücken',g:'' + T('ui.push_oberkoerper') + '',m:'Schultern'},
 {n:'Arnold Press',g:'' + T('ui.push_oberkoerper') + '',m:'Schultern'},
 {n:'Seitheben',g:'' + T('ui.push_oberkoerper') + '',m:'' + T('ui.seitliche_schulter') + ''},
 {n:'Dips',g:'' + T('ui.push_oberkoerper') + '',m:'' + T('ui.brust_trizeps') + ''},
 {n:'Liegestütze',g:'' + T('ui.push_oberkoerper') + '',m:'' + T('ui.brust_core') + ''},
 {n:'Trizeps-Pushdown',g:'' + T('ui.push_oberkoerper') + '',m:'Trizeps'},
 {n:'Klimmzug',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.lat_bizeps') + ''},
 {n:'Klimmzug breit',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.lat_breit') + ''},
 {n:'Latzug',g:'' + T('ui.pull_oberkoerper') + '',m:'Lat'},
 {n:'Rudern (Langhantel)',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.oberer_ruecken') + ''},
 {n:'Kabelrudern eng',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.mittlerer_ruecken') + ''},
 {n:'Face Pull',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.hintere_schulter') + ''},
 {n:'Reverse Fly',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.hintere_schulter') + ''},
 {n:'Bizeps-Curl',g:'' + T('ui.pull_oberkoerper') + '',m:'Bizeps'},
 {n:'Hammer-Curl',g:'' + T('ui.pull_oberkoerper') + '',m:'' + T('ui.bizeps_unterarm') + ''},
 {n:'Plank',g:'Core',m:'Rumpf'},
 {n:'Side Plank',g:'Core',m:'' + T('ui.seitlicher_rumpf') + ''},
 {n:'Pallof Press',g:'Core',m:'Anti-Rotation'},
 {n:'Bird Dog',g:'Core',m:'' + T('ui.rumpf_stabilitaet') + ''},
 {n:'Dead Bug',g:'Core',m:'' + T('ui.tiefe_bauchmuskeln') + ''},
 {n:'Bicycle Crunch',g:'Core',m:'' + T('ui.schraege_bauchmuskeln') + ''},
 {n:'Russian Twist',g:'Core',m:'Rotation'},
 {n:'Hängendes Beinheben',g:'Core',m:'' + T('ui.untere_bauchmuskeln') + ''},
 {n:'Ab Wheel',g:'Core',m:'' + T('ui.gesamter_rumpf') + ''},
 {n:'Mountain Climbers',g:'Core',m:'' + T('ui.rumpf_kondition') + ''}
];
...
function gymProgressHint(name){
  ...
  return '<div class="gymrow-prog gymrow-new">' + T('ui.erste_erfassung_ab_jetzt_mit') + '</div>';
}
function gymRowsHTML(list){
  if(!list||!list.length)return gmStateEmpty({icon:'dumbbell',title:'' + T('ui.noch_keine_uebung_gewaehlt') + '',desc:'' + T('ui.tippe_uebungen_waehlen_um_saetze') + ''});
  return list.map(function(x){
    return '<div class="gymrow" data-n="'+esc(x.n)+'"><div class="gymrow-top"><span class="gymrow-n">'+esc(x.n)+'</span>'+
      '<button type="button" class="gymrow-x" onclick="removeGymRow(this)" aria-label="' + T('ui.entfernen') + '">✕</button></div>'+
      '<div class="gymrow-in"><label>' + T('ui.saetze__') + '<input type="number" inputmode="numeric" class="gx-sets" value="'+(x.sets!=null?x.sets:'')+'" placeholder="3" oninput="autoPost()"></label>'+
      '<label>' + T('ui.wdh_') + '<input type="number" inputmode="numeric" class="gx-reps" value="'+(x.reps!=null?x.reps:'')+'" placeholder="8" oninput="autoPost()"></label>'+
      '<label>kg<input type="number" inputmode="decimal" class="gx-kg" value="'+(x.kg!=null?x.kg:'')+'" placeholder="' + T('ui.45') + '" oninput="autoPost()"></label></div>'+
...
function openGymPicker(){
  ...
  var wrap=document.createElement('div');wrap.className='orvia-modal-bg';
  wrap.innerHTML='<div class="orvia-modal goal-modal gym-pick"><h3>' + T('ui.uebungen_waehlen') + '</h3>'+
    '<div class="gx-add"><input type="text" id="gx_new" placeholder="' + T('ui.eigene_uebung') + '"><button type="button" class="btn sec" onclick="addCustomExercise()">+</button></div>'+
    '<div class="gx-scroll" id="gx_scroll">'+gymPickerBody(curNames)+'</div>'+
    '<button class="btn" onclick="applyGymPicker()">' + T('ui.uebernehmen') + '</button>'+
    '<button class="btn sec" style="margin-top:10px" onclick="closeGymPicker()">' + T('ui.abbrechen') + '</button></div>';
...
function _mvStatusFor(d,group){
  var e=d.per[group];if(!e)return {weekly:0,status:{color:'muted',label:'' + T('ui.keine_daten') + '',key:'no_data'},ctx:null};
...
function renderBodyMap(statusByGroup,neutral){
  function fig(data,tx){
    ...
    Object.keys(data).forEach(function(slug){
      ...
      if(de){
        ...
        var st=statusByGroup[de]||{color:'muted',label:'' + T('ui.keine_daten') + ''};
  ...
  var legend=neutral
    ?'<div class="bm-leg"><span>' + T('ui.noch_nicht_ausreichend_bewertet_muskelbereiche') + '</span></div>'
    :'<div class="bm-leg"><span><i style="background:'+bmFill('low')+'"></i>' + T('ui.unter_ziel') + '</span><span><i style="background:'+bmFill('good')+'"></i>' + T('ui.im_ziel') + '</span><span><i style="background:'+bmFill('high')+'"></i>' + T('ui.ueber_ziel') + '</span><span><i style="background:'+bmFill('warn')+'"></i>' + T('ui.warnung') + '</span><span><i style="background:'+bmFill('info')+'"></i>' + T('ui.wenig_historie') + '</span><span><i style="background:#5f6a42"></i>' + T('ui.gedaempft_noch_keine_daten') + '</span></div>';
  return '<div class="bodymap"><svg viewBox="0 0 232 216" width="100%" role="img" aria-label="' + T('ui.koerperkarte_muskelvolumen') + '">'+
    fig(BODY_ANT,8)+fig(BODY_POST,128)+
    '<text x="58" y="214" text-anchor="middle" font-size="8" fill="var(--mut,#94a3b8)">' + T('ui.vorderseite') + '</text>'+
    '<text x="178" y="214" text-anchor="middle" font-size="8" fill="var(--mut,#94a3b8)">' + T('ui.rueckseite') + '</text>'+
    '</svg>'+legend+'</div>';
}
...
var CONF_REASON_DE={
  legacy_synthetic_data:'' + T('ui.aeltere_trainingsdaten_enthalten_nur_zusammengefasste') + '',
  too_few_workouts:'' + T('ui.noch_zu_wenige_verwertbare_workouts') + '',
  too_short_period:'' + T('ui.beobachtungszeitraum_noch_zu_kurz') + '',
  unclassified_exercises:'' + T('ui.einige_uebungen_konnten_noch_nicht') + '',
  partial_data:'' + T('ui.trainingsdaten_sind_nur_teilweise_verfuegbar') + ''
};
...
var MV_STATUS_META={
  in:{label:'' + T('ui.im_ziel_') + '',sym:'✓',color:'#34d399'},
  below:{label:'' + T('ui.unter_ziel_') + '',sym:'▽',color:'#eab308'},
  above:{label:'' + T('ui.ueber_ziel_') + '',sym:'▲',color:'#3b82f6'},
  low_history:{label:'' + T('ui.wenig_historie_') + '',sym:'~',color:'#a78bfa'},
  no_data:{label:'' + T('ui.keine_daten') + '',sym:'–',color:'#64748b'}
};
...
function gmKorridorAbgrenzungHTML(){
  try{
    ...
    if(!v)return '';
    var klasse=(v.herkunft&&v.herkunft.evidenceClass)?('' + T('ui.klasse') + ''+v.herkunft.evidenceClass):null;
    return '<div class="md-corr-cmp">'+
      '<b>' + T('ui.dazu_aus_einer_quelle') + '</b> '+escH(v.wert.min+'–'+v.wert.max)+' '+
      escH(v.einheit||'' + T('ui.saetze_je_muskelgruppe_und_trainingseinheit') + '')+
      (klasse?' <span class="muted">['+escH(klasse+' · '+(v.regelId||''))+']</span>':'')+
      '<br><span class="muted">Andere Bezugsgröße als der Richtwert oben — '+
      'je <b>' + T('ui.einheit') + '</b> statt je Woche. Umgerechnet wird nicht: die Quelle '+
...
function mvNextStep(statusKey){
  return {
    below:'' + T('ui.1_2_gezielte_saetze_pro') + '',
    in:'' + T('ui.kurs_halten_volumen_liegt_im') + '',
    above:'' + T('ui.volumen_halten_oder_leicht_reduzieren') + '',
    low_history:'' + T('ui.weiter_erfassen_nach_2_wochen') + '',
    no_data:'' + T('ui.fuer_diese_gruppe_liegen_im_') + ''
  }[statusKey]||'' + T('ui.weiter_beobachten') + '';
}
...
function renderMuscleMap(model){
  ...
  var toggle='<div class="mvx-toggle" role="tablist"><button role="tab" aria-selected="'+(side==='front')+'" class="'+(side==='front'?'on':'')+'" onclick="setMvSide(\'front\')">' + T('ui.vorderseite') + '</button>'+
    '<button role="tab" aria-selected="'+(side==='back')+'" class="'+(side==='back'?'on':'')+'" onclick="setMvSide(\'back\')">' + T('ui.rueckseite') + '</button></div>';
  ...
  var note='<p class="note mvx-note" style="text-align:left">' + T('ui.farbe') + '<b>und</b>' + T('ui.symbol_zeigen_den_status_keine') + '<b>nicht</b>' + T('ui.zu_wenig_training_sondern_keine') + '</p>';
  return '<div class="mvx-map">'+toggle+'<div class="mvx-figure"><svg viewBox="0 0 200 348" class="mvx-svg" role="img" aria-label="Muskelkarte '+(side==='front'?'' + T('ui.vorderseite') + '':'' + T('ui.rueckseite') + '')+'">'+sil+regions+'</svg></div>'+legend+note+'</div>';
}
function showMuscleDetail(idOrGroup){
  ...
  if(!m){ if(typeof oModal==='function')oModal(mvLabelDe(idOrGroup)||'Muskel','<p class="muted" style="margin:0">' + T('ui.fuer_diese_gruppe_liegen_im') + '</p>'); return; }
  ...
  function line(c){return '<div class="md-row"><span>'+escH(c.exerciseName)+'</span><b>'+c.completedWorkingSets+' '+(c.relationship==='direct'?'direkte':'indirekte')+'' + T('ui.saetze') + ''+fmtDe(c.coefficient)+' = '+fmtDe(c.contribution)+'</b></div>';}
  var corridor=(num.targetDisplay==='insufficient_data'||num.targetMin==null)
    ?'' + T('ui.noch_nicht_genug_daten_fuer') + ''
    :'' + T('ui.richtwert_') + ''+fmtDe(num.targetMin)+'–'+fmtDe(num.targetMax)+'' + T('ui.effektive_satzaequivalente_woche') + '';
  ...
  if(mode==='anfaenger'){
    body=head+
      '<p class="md-core">'+escH(mvLabelDe(m.muscleId))+': <b>'+escH(st.label)+'</b>. '+fmtDe(num.effectiveSetEquivalents)+'' + T('ui.effektive_satzaequivalente_in_den_letzten') + ''+days+'' + T('ui.tagen_') + '.</p>'+
      '<div class="md-corr">'+escH(corridor)+
        (korridorHerkunft?'<span class="md-corr-src">'+escH(korridorHerkunft)+'</span>':'')+'</div>'+
      '<p class="md-next"><b>' + T('ui.naechster_schritt') + '</b> '+escH(mvNextStep(st.key))+'</p>';
  } else {
    // Fortgeschritten: direkte/indirekte Sätze, Korridor, Entwicklung.
    body=head+
      '<div class="md-stat">'+
        '<div class="md-row"><span>' + T('ui.direkte_arbeitssaetze') + '</span><b>'+fmtDe(num.directSets)+'</b></div>'+
        '<div class="md-row"><span>' + T('ui.indirekte_satzaequivalente') + '</span><b>'+fmtDe(num.indirectSetEquivalents)+'</b></div>'+
        '<div class="md-row"><span><b>' + T('ui.effektive_satzaequivalente') + '</b></span><b>'+fmtDe(num.effectiveSetEquivalents)+'</b></div>'+
      '</div>'+
      '<div class="md-corr">'+escH(corridor)+
        (korridorHerkunft?'<span class="md-corr-src">'+escH(korridorHerkunft)+'</span>':'')+
        gmKorridorAbgrenzungHTML()+'</div>'+
      (direct.length?'<div class="modlbl">' + T('ui.direkte_beitraege') + '</div>'+direct.map(line).join(''):'')+
      (indirect.length?'<div class="modlbl">' + T('ui.indirekte_beitraege') + '</div>'+indirect.map(line).join(''):'')+
      (!contribs.length?'<p class="muted" style="margin:6px 0 0">' + T('ui.keine_beitraege_im_zeitraum') + '</p>':'')+
      '<p class="md-next"><b>' + T('ui.naechster_schritt') + '</b> '+escH(mvNextStep(st.key))+'</p>';
    ...
    if(mode==='profi'){
      ...
      body+='<div class="md-pro">'+
        '<div class="md-row"><span>' + T('ui.reale_arbeitssaetze') + '</span><b>'+fmtDe(num.realWorkingSets)+'</b></div>'+
        '<div class="md-row"><span>' + T('ui.beitrag_reale_saetze_koeffizient') + '</span><b>'+fmtDe(num.effectiveSetEquivalents)+'</b></div>'+
        '<div class="md-row"><span>' + T('ui.zeitraum_') + '</span><b>'+days+'' + T('ui.tage') + ''+fmtDe(weeks)+'' + T('ui.wo') + '.)</b></div>'+
        '<div class="md-row"><span>' + T('ui.konfidenz__') + '</span><b>'+(CONF[num.confidence]||num.confidence||'–')+(num.confidenceReason?' ('+escH(num.confidenceReason)+')':'')+'</b></div>'+
        '<div class="md-row"><span>' + T('ui.korridor_basis') + '</span><b>'+escH(num.targetBasis==='produktwert'?'' + T('ui.produktwert_keine_quelle') + '':(num.targetBasis||'–'))+'</b></div>'+
        '<div class="md-row"><span>' + T('ui.korridor_kennung') + '</span><b>'+escH(num.targetSource||'–')+'</b></div>'+
        '<div class="md-row"><span>' + T('ui.ausgeschlossene_saetze') + '</span><b>'+num.exclusionCount+'</b></div>'+
      '</div>';
    }
  }
  if(ex&&ex.exclusions&&ex.exclusions.length&&mode!=='anfaenger'){ body+='<p class="note" style="text-align:left;margin-top:8px">Ausgeschlossen: '+ex.exclusions.length+' Satz/' + T('ui.saetze__') + ' (z. B. Aufwärm-/unvollständige ' + T('ui.saetze__') + ') — nachvollziehbar, nicht gewertet.</p>'; }
  if(m.confidenceReason==='legacy_synthetic_data'){ body+='<p class="note" style="text-align:left;margin-top:6px">' + T('ui.satzdetails_aus_aelteren_zusammengefassten_trainingsdaten') + '</p>'; }
  body+='<p class="note" style="text-align:left;margin-top:6px">' + T('ui.evidenzinformierte_schaetzung_keine_exakte_biologische') + '</p>';
...
function renderMuscleVolume(){
  ...
  if(!(window.ORVIA&&ORVIA.gymVolume&&ORVIA.gymVolume.getProductiveVolumeModel)){out(gmStateError({icon:'alert',title:'' + T('ui.muskelvolumen_modul_nicht_geladen') + '',desc:'' + T('ui.die_auswertung_steht_erst_nach') + ''}));return;}
  ...
  ORVIA.gymVolume.getProductiveVolumeModel({days:days,refresh:true,experience:mvExperience()}).then(function(model){
    ...
    if(model.status==='data_unavailable'||model.status==='load_error'||model.fallbackUsed){
      out(gmStateError({title:'' + T('ui.eingeschraenkte_datenbasis') + '',desc:'' + T('ui.muskelvolumen_konnte_momentan_nicht_vollstaendig') + '',retry:'renderMuscleVolume()',label:'' + T('ui.erneut_versuchen') + ''}));return;
    }
    if(model.status==='no_gym_workouts'||!model.muscles.length){
      out(gmStateEmpty({icon:'dumbbell',title:'' + T('ui.noch_keine_krafttrainingsdaten') + '',desc:'' + T('ui.starte_ein_gym_training_mit') + ''}));return;
    }
    ...
    var partial=model.status==='partial_data'?gmStateError({icon:'info',title:'' + T('ui.teilweise_ausgewertet') + '',desc:'' + T('ui.ein_teil_deiner_trainingsdaten_konnte') + ''}):'';
    var CONF={low:'niedrig',medium:'mittel',high:'hoch'};
    var cards=model.muscles.map(function(m){
      ...
      var corr=(m.targetRange&&m.targetRange.displayStatus==='insufficient_data')
        ?'' + T('ui.noch_kein_individueller_zielkorridor') + ''
        :'' + T('ui.richtwert_') + ''+fmtDe(m.targetRange.min)+'–'+fmtDe(m.targetRange.max)+'/Woche';
      var lines='';
      if(mode==='anfaenger'){
        lines='<div class="mvc-eff">'+fmtDe(m.effectiveSetEquivalents)+'' + T('ui.effektive_saetze_letzte') + ''+days+' T</div>'+
        ...
        lines='<div class="mvc-sub">'+fmtDe(m.directSets)+' direkt · '+fmtDe(m.indirectSetEquivalents)+' indirekt</div>'+
              '<div class="mvc-eff">Letzte '+days+'' + T('ui.t') + ''+fmtDe(m.effectiveSetEquivalents)+' eff. · Ø '+fmtDe(m.weeklyEquivalent)+'/Wo</div>'+
    ...
    var info='<details class="mv-info"><summary>' + T('ui.was_bedeuten_diese_werte') + '</summary>'+
      '<p><b>' + T('ui.direkte_saetze') + '</b>' + T('ui.saetze_mit_diesem_muskel_als') + '</p>'+
      '<p><b>' + T('ui.indirekte_satzaequivalente') + '</b>' + T('ui.gewichtete_sekundaere_belastung_aus_zusammengesetzten') + '</p>'+
      '<p><b>' + T('ui.effektive_satzaequivalente') + '</b>' + T('ui.direkte_plus_gewichtete_indirekte_beitraege') + '</p>'+
      '<p class="muted">' + T('ui.werte_amp_zielkorridor_stammen_aus') + '</p></details>';
    out(renderMuscleMap(model)+partial+'<div class="mvcards mvx-cards">'+cards+'</div>'+info+
      '<p class="note" style="text-align:left;margin-top:8px">' + T('ui.antippen_fuer_details_beitraege_je') + '</p>');
  }).catch(function(e){ _mvP.mark('renderMuscleVolume: getProductiveVolumeModel (error)',_mvT0); if(reqId!==_mvReq)return; try{console.error('[muscleVolume]',e);}catch(_){ } out(gmStateError({title:'Muskelvolumen konnte momentan nicht berechnet werden.',retry:'renderMuscleVolume()',label:'' + T('ui.erneut_versuchen') + ''})); });
}
...
function gymInjuryHint(){
  ...
  var tips=[];
  if(knee>=2)tips.push('' + T('ui.knie') + ''+knee+'' + T('ui.10_meide_tiefe_kniebeugen_step') + '');
  if(shoulder>=2||pIss.indexOf('shoulder')>=0)tips.push('' + T('ui.schulter_kein_schweres_overhead_druecken') + '');
  if(back>=2||pIss.indexOf('back')>=0)tips.push('' + T('ui.ruecken_weniger_axiale_last_maschinen') + '');
...
function blockGym(d){return `<div class="sescard"><div class="seshead">${ic('dumbbell')}Gym</div>${(typeof gymInjuryHint==='function')?gymInjuryHint():''}
  ${chips('' + T('ui.fokus_mehrfach') + '','g_sub',['Ganzkörper','Oberkörper','Push','Pull','Core','Beine','Glute-Aktivierung','VMO/Rehab'],d.sub,true)}
  ...
  ${slider('g_rpe','' + T('ui.rpe_anstrengung') + '',1,10,d.rpe??6,'leicht','max')}
  ${slider('g_perf','Leistung',1,10,d.perf??6)}
  ${slider('g_knee','' + T('ui.knie_post') + '',0,10,d.knee??0,'kein','max')}
...
function blockRad(d){return `<div class="sescard"><div class="seshead">${ic('bike')}Rad</div>
  ${chips('' + T('ui.typ') + '','r_sub',['Commute','Easy Z2','' + T('ui.tempo_z3') + '','Intervalle','' + T('ui.long_ride') + ''],d.sub?[d.sub]:[])}
  ...
  ${slider('r_knee','' + T('ui.knie_post') + '',0,10,d.knee??0,'kein','max')}
...
function blockMob(d){return `<div class="sescard"><div class="seshead">${ic('stretch')}Mobilität</div>
  ${chips('' + T('ui.bereich_mehrfach') + '','mo_sub',['Sprunggelenk','' + T('ui.spanish_squats') + '','Glute-Aktivierung','Stretching','' + T('ui.full_routine') + ''],d.sub,true)}
  <div class="field"><label>Dauer (min)</label><input type="number" inputmode="numeric" id="mo_dur" value="${d.dur??''}" placeholder="15"></div>
  ${slider('mo_knee','' + T('ui.knie_post') + '',0,10,d.knee??0,'kein','max')}
...
function savePost(silent){if(!canEditCur(silent))return;const e=entry(cur);e.sessions=e.sessions||{};
  ...
  if(!silent){
    ...
    if(pbs&&pbs.length&&L&&L._pbSig!==sig&&typeof celebratePB==='function'){L._pbSig=sig;save();celebratePB(pbs);}
      ...
      if(fb&&typeof oModal==='function')oModal('' + T('ui.training') + '-Feedback','<div class="coachbubble">'+esc(fb)+'</div>');
      else toast(activeTypes.size?activeTypes.size+'' + T('ui.einheit_en_gespeichert') + '':'' + T('ui.keine_einheit_gewaehlt') + '');}
  }}
...
function trainingFeedback(){
  ...
  if(L.sub==='Wettkampf')p.push('' + T('ui.wettkampf_im_kasten_stark_dass') + ''+fmtDe(L.dist)+' km in '+fmtDurInput(L.dur)+' ('+fmtPace(pace)+'/km).');
  else p.push('' + T('ui.einheit_gespeichert') + ''+fmtDe(L.dist)+' km in '+fmtDurInput(L.dur)+'' + T('ui.schnitt') + ''+fmtPace(pace)+'/km.');
  var days=Object.keys(DB).filter(isDay).filter(function(k){return k<cur;}).sort().reverse();
  for(var i=0;i<days.length;i++){var pe=DB[days[i]];if(pe&&pe.sessions&&pe.sessions.Laufen&&_validRun(pe.sessions.Laufen)){var P=pe.sessions.Laufen;
    if(P.dist&&P.dur&&Math.abs(P.dist-L.dist)/L.dist<=0.3){var diff=Math.round(P.dur*60/P.dist-pace);
      if(diff>=8)p.push('' + T('ui.das_sind') + ''+diff+'' + T('ui.s_km_schneller_als_bei') + '');
      else if(diff>=3)p.push('' + T('ui.etwas_schneller_als_zuletzt_bei') + '');
      else if(diff<=-8)p.push('' + T('ui.bewusst_lockerer_als_letztes_mal') + '');
      else p.push('' + T('ui.tempo_auf_dem_niveau_deiner') + '');
      break;}}}
  ...
  if(z){var easy=z.find(function(x){return x.k==='Easy';});
    if(L.sub==='Easy Z2'&&easy&&pace<easy.lo)p.push('' + T('ui.fuer_einen_easy_run_war') + ''+fmtPace(easy.lo)+'–'+fmtPace(easy.hi)+'' + T('ui.km_waere_der_bereich_naechstes') + '');}
  if(L.knee!=null&&L.knee>=3)p.push('' + T('ui.knie_nach_dem_lauf') + ''+L.knee+'' + T('ui.10_heute_kuehlen_morgen_lockerer') + '');
  else if(L.knee!=null&&L.knee<=1)p.push('' + T('ui.knie_blieb_ruhig') + ''+L.knee+'' + T('ui.10_gutes_zeichen') + '');
  if(L.felt)p.push('' + T('ui.du_hast_die_einheit_als') + ''+L.felt+'" empfunden.'+(L.felt==='zu hart'?'' + T('ui.wenn_das_oefter_vorkommt_pace') + '':(L.felt==='zu leicht'?'' + T('ui.beim_naechsten_mal_ruhig_etwas') + '':'')));
  p.push('' + T('ui.nimm_mit_was_gut_lief') + '');
  return p.join(' ');
}
...
function suppRecs(){
  ...
  out.push({n:'Vitamin D3',why:'' + T('ui.basis_im_norden_1000_4000') + ''});
  out.push({n:'Omega-3 (EPA/DHA)',why:'' + T('ui.entzuendungsmodulation_herz_1_2g_taeglich') + ''});
  out.push({n:'Kreatin',why:'' + T('ui.3_5g_taeglich_timing_egal') + ''});
  const run=plan.find(p=>p.t==='Laufen');
  if(run&&/Intervalle|Tempo/.test(run.l))out.push({n:'Koffein',why:run.l+' heute — 3–6mg/kg, ' + T('ui.45') + '–60min vorher'});
  if(run&&/Long/.test(run.l))out.push({n:'Elektrolyte/Natrium',why:'' + T('ui.long_run_heute_natrium_ersetzen') + ''});
  if(run||plan.find(p=>p.t==='Rad'))out.push({n:'Kollagen + Vit C',why:'' + T('ui.15g_vit_c_1h_vor') + ''});
  if(m.sleepQ!=null&&m.sleepQ<=5)out.push({n:'Magnesium-Glycinat',why:'' + T('ui.schlafqualitaet') + ''+m.sleepQ+'' + T('ui.10_heute_abend_300_400mg') + ''});
  else out.push({n:'Magnesium-Glycinat',why:'' + T('ui.abends_schlaf_muskelfunktion') + ''});
  if((activeWeekPlan()[(wd+1)%7]||[]).find(p=>p.t==='Schwimmen'))out.push({n:'Melatonin',why:'' + T('ui.morgen_frueher_schwimmtag_0_5') + ''});
  if(ev.prot!=null&&ev.prot<150)out.push({n:'Whey/Protein',why:'' + T('ui.erst') + ''+ev.prot+'' + T('ui.g_luecke_zum_150g_ziel') + ''});
  else out.push({n:'Whey/Protein',why:'' + T('ui.baustein_fuers_150_165g_ziel') + ''});
  /* v8-317: Garmins echte Kategorien (Low UND Poor) plus Schwelle statt Gleichheit. */
  if(Calc.hrvBelowBaseline(m.hrv)||(function(){var _s=Calc.hrvScoreOf(m,recoveryCtx(cur));return _s!=null&&_s<=40;})())out.push({n:'L-Theanin',why:'' + T('ui.hrv_gedrueckt_beruhigend_koffein_heute') + ''});
...
function renderSupps(){
  ...
  let html='<div class="supphd">' + T('ui.dein_stack_schnellauswahl') + '<span>' + T('ui.antippen_heute_genommen_stack_bearbeiten') + '</span></div>';
  if(stack.length){
    ...
    html+=gmStateEmpty({icon:'db',title:'' + T('ui.noch_kein_stack_angelegt') + '',
      desc:'' + T('ui.lege_feste_supplements_an_auch') + '',
      action:stackEdit?null:'openStackEditor()',actionIcon:'plus',label:'' + T('ui.stack_bearbeiten') + ''});
  }
  html+=`<div style="margin-top:10px;display:flex;gap:10px">
    <button class="chip" onclick="stackEdit=!stackEdit;renderSupps()">${stackEdit?'' + T('ui.fertig') + '':'' + T('ui.stack_bearbeiten') + ''}</button>
    <button class="chip" onclick="browseOpen=!browseOpen;renderBrowse()" id="browseBtn">${browseOpen?'' + T('ui.schliessen') + '':'' + T('ui.einmalig_genommen') + ''}</button></div>`;
  if(stackEdit){
...
function renderBrowse(){const subs=(entry(cur).subs)||[];const el=document.getElementById('subBrowse');
  const bb=document.getElementById('browseBtn');if(bb)bb.textContent=browseOpen?'' + T('ui.schliessen') + '':'' + T('ui.einmalig_genommen') + '';
...
function renderSuppBody(q){q=(q||'').toLowerCase();let html='';
  ...
  document.getElementById('suppListBody').innerHTML=html||'<p class="muted">' + T('ui.nichts_gefunden') + '</p>';}
...
function renderEve(){const e=(entry(cur).eve)||{};
  ...
  var done=!!(e&&e.ts);
  var head=done
    ?'<div class="mini-note" style="margin:0 0 10px">'+icon('check','xs')+'<div><b>' + T('ui.heute_erledigt') + '</b>' + T('ui.du_kannst_werte_bis_tagesende') + '</div></div>'
    :'<div class="mini-note" style="margin:0 0 10px">'+icon('moon','xs')+'<div><b>' + T('ui.tagesabschluss') + '</b>' + T('ui.1_minute_verbessert_die_empfehlung') + '</div></div>';
  document.getElementById('eveForm').innerHTML=head+(fields?_ciFormHTML(fields,e,'full'):gmStateError({icon:'alert',title:'' + T('ui.check_in_modul_nicht_geladen') + '',desc:'' + T('ui.lade_die_app_neu_sobald') + ''}));
  initRanges();}
...
function saveEve(){if(!canEditCur())return;entry(cur).eve=gatherEve();if(window._correctionMode&&cur!==todayStr())logCorrection('eve');save();_persistEve();toast(window._correctionMode&&cur!==todayStr()?'' + T('ui.korrektur_gespeichert') + '':'' + T('ui.abend_gespeichert') + '');}
...
function renderBanners(){const out=document.getElementById('banners');let html='';
  ...
  if(nDays>=5&&(!lb||Date.now()-lb>7*864e5))
    html+=`<div class="banner warn2">${ic('save')}<span>${lb?'' + T('ui.backup_aelter_als_7_tage') + '':'' + T('ui.noch_kein_backup') + ''} — Daten liegen nur im Browser.</span><button onclick="exportData();renderBanners()">Sichern</button></div>`;
  out.innerHTML=html;}
...
function renderDay(){if(typeof invalidateDecision==='function')invalidateDecision();const hr=new Date().getHours();const today=cur===todayStr();
  ...
  (function(){try{
    ...
    if(mb&&typeof uiDetailMode==='function'){var _m=uiDetailMode();mb.textContent=({anfaenger:'' + T('ui.einfache_ansicht') + '',fortgeschritten:'' + T('ui.fortgeschritten_') + '',profi:'Profi-Ansicht'})[_m]||'';}
  ...
  document.getElementById('greet').textContent=today?(hr<11?'' + T('ui.guten_morgen') + '':hr<18?'Hi':'' + T('ui.guten_abend') + ''):'' + T('ui.ausgewaehlter_tag') + '';
  ...
  if(isPast){
    ...
    if(anchor&&anchor.parentNode){
      ...
      b.innerHTML=window._correctionMode
        ?'<span>' + T('ui.korrektur_modus_aenderungen_an_diesem') + '<button class="lexlink" onclick="endCorrection()">' + T('ui.fertig_') + '</button></span>'
        :'<span>' + T('ui.dieser_tag_ist_abgeschlossen_und') + '<button class="lexlink" onclick="startCorrection()">' + T('ui.korrektur_erfassen') + '</button></span>';
...
function renderPhases(){
  ...
  if(!phases.length){
    box.innerHTML='<div class="empty-card"><div class="empty-h">' + T('ui.phasen_') + '</div><p class="empty-p">' + T('ui.trainingsphasen_erscheinen_sobald_ein_aktives') + '</p></div>';return;}
  ...
  var segs=phases.map(function(p){var st=past?(_phState(p,t)==='now'?'done':_phState(p,t)):_phState(p,t);
    return '<button type="button" class="phv5-seg is-'+st+'" data-ph="'+escH(p.n+'·'+(p.from||'')+'·'+(p.to||''))+'" data-state="'+st+'" onclick="openPhaseSheet(\''+escH(p.n)+'\')" aria-label="'+escH('' + T('ui.phase__') + ''+p.n+(st==='now'?', aktuell':st==='done'?', abgeschlossen':', kommend'))+'">'+
      '<b>'+escH(p.n)+'</b>'+(mode==='anfaenger'?'':'<span>'+escH((p.from?_phFmt(p.from)+'–':'bis ')+_phFmt(p.to))+'</span>')+'</button>';}).join('');
  var head='';
  if(past){head='<p class="note phv5-note" style="text-align:left">Das Zieldatum ('+escH(_phFmt(rd))+') liegt '+Math.abs(d)+'' + T('ui.tage_') + ' zurück — kein aktiver Trainingsblock. Aktualisiere ' + T('ui.dein_ziel') + ' im Profil.</p>';}
  else if(d!=null){var now=null;for(var x=0;x<phases.length;x++)if(phases[x].on)now=phases[x];
    head='<p class="phv5-head">'+(now?('Aktuell: <b>'+escH(now.n)+'</b> · '):'')+escH(String(d))+'' + T('ui.tage_bis_zum_ziel') + ''+escH(_phFmt(rd))+')</p>';}
  var foot='';
  if(mode==='anfaenger')foot='<p class="note phv5-note" style="text-align:left">' + T('ui.dein_training_laeuft_in_phasen') + '</p>';
  if(mode==='profi')foot='<p class="note phv5-note" style="text-align:left">' + T('ui.quelle_calc_racephases_zieldatum_aus') + '</p>';
...
function openPhaseSheet(name){
  ...
  if(!p){if(typeof oModal==='function')oModal(name,'<p class="muted" style="margin:0">' + T('ui.fuer_diese_phase_liegen_keine') + '</p>');return;}
  var mode=(typeof uiDetailMode==='function')?uiDetailMode():'fortgeschritten';
  var t=todayStr();var st=_phState(p,t);
  var stTxt=st==='now'?'' + T('ui.aktuelle_phase') + '':st==='done'?'' + T('ui.abgeschlossen') + '':'Kommend';
  var body='<div class="rcv-sh-v">'+escH(p.n)+'</div>'+
    '<p style="margin:8px 0 0">'+escH(p.d||'')+'</p>'+
    '<div class="rcv-sh-meta">' + T('ui.zeitraum') + '<b>'+escH((p.from?_phFmt(p.from)+' – ':'offen bis ')+_phFmt(p.to))+'</b>' + T('ui.status') + '<b>'+escH(stTxt)+'</b></div>';
  if(mode==='profi'){body+='<div class="rcv-sh-meta">Quelle: Calc.racePhases · Zieldatum '+escH(rd||'–')+' ('+escH((g&&g.type)||'–')+')</div>';}
  body+='<p class="note" style="text-align:left;margin-top:10px">' + T('ui.phasenstruktur_aus_dem_wettkampfdatum_keine') + '</p>';
  try{if(typeof _rcvLastFocus!=='undefined')_rcvLastFocus=document.activeElement;}catch(_){ }
  if(typeof oModal==='function')oModal('' + T('ui.phase') + ''+p.n,body);
...
function renderRamp(){
  ...
  var mode=(typeof uiDetailMode==='function')?uiDetailMode():'fortgeschritten';
  if(W.state==='unknown'){
    box.innerHTML='<div class="empty-card"><div class="empty-h">' + T('ui.laufumfang') + '</div><p class="empty-p">' + T('ui.wochen_km_aktuell_nicht_bestimmbar') + '</p></div>';return;}
  if(W.state==='no_target'){
    box.innerHTML='<div class="empty-card"><div class="empty-h">' + T('ui.laufumfang') + '</div><p class="empty-p">' + T('ui.kein_wochenziel_u2014_das_renndatum') + '</p>'+
      (W.act!=null?'<p class="empty-p">' + T('ui.diese_woche_gelaufen') + '<b>'+fmtDe(W.act)+' km</b></p>':'')+'</div>';return;}
  var barPct=Math.min(100,W.pct);
  var big=fmtDe(W.act)+' / '+W.eff+' km';
  var lead=mode==='anfaenger'
    ?'Du bist bei <b>'+fmtDe(W.act)+'</b> von <b>'+W.eff+'</b>' + T('ui.lauf_kilometern') + ' diese Woche.'
  ...
  var head='<div class="wkv5-head"><span>' + T('ui.diese_woche_u00b7_laufen') + '</span><b class="wkv5-big" data-wk="'+_wkEsc(fmtDe(W.act)+'|'+W.eff+'|'+W.cal)+'">'+_wkEsc(big)+'</b></div>';
  var bar='<div class="wkv5-bar'+(W.over?' wkv5-over':'')+'" role="img" aria-label="'+_wkEsc(fmtDe(W.act)+' von '+W.eff+'' + T('ui.lauf_kilometern') + '')+'"><i style="width:'+barPct.toFixed(0)+'%"></i></div>';
  var notes='';
  if(mode!=='anfaenger'){
    ...
    var next='';for(var i2=1;i2<=3;i2++){var t2=Calc.weekKmTarget(W.d,i2);if(t2<=0)break;
      next+='<div class="wkv5-next"><span>In '+i2+'' + T('ui.woche_') + ''+(i2>1?'n':'')+'</span><b>~'+t2+' km'+(Calc.weekKmTarget(W.d,i2)<Calc.weekKmTarget(W.d,i2-1)&&i2<3?'' + T('ui.u00b7_entlastung') + '':'')+'</b></div>';}
    if(next)notes+='<div class="wkv5-nextwrap">'+next+'</div>';
  }
  if(mode==='profi')notes+='<p class="note wkv5-note">' + T('ui.zeitraum') + ''+_wkEsc(_wkVolRange())+' (lokale Zeitzone) \u00b7 Quelle: kanonische Wochenaggregation (dedupliziert, nur abgeschlossene Lauf-' + T('ui.einheiten_') + ') \u00b7 Ziel: weekKmTarget mit Ist-Kopplung.</p>';
  box.innerHTML='<button type="button" class="wkv5-tap" onclick="openWeekVolumeSheet()" aria-label="' + T('ui.laufumfang') + '-' + T('ui.details_') + ' \u00f6ffnen">'+head+bar+'<p class="wkv5-lead">'+lead+'</p></button>'+notes;
}
function openWeekVolumeSheet(){
  var W=_wkVolData();
  if(W.state!=='ok'){if(typeof oModal==='function')oModal('' + T('ui.laufumfang') + '','<p class="muted" style="margin:0">'+(W.state==='unknown'?'' + T('ui.wochen_km_aktuell_nicht_bestimmbar_') + '':'' + T('ui.kein_aktives_wochenziel') + '')+'</p>');return;}
  var mode=(typeof uiDetailMode==='function')?uiDetailMode():'fortgeschritten';
  var body='<div class="rcv-sh-v" data-wk="'+_wkEsc(fmtDe(W.act)+'|'+W.eff)+'">'+_wkEsc(fmtDe(W.act)+' / '+W.eff+' km')+'</div>'+
    '<p style="margin:8px 0 0">' + T('ui.lauf_kilometer_dieser_woche_mo') + '</p>'+
    '<div class="rcv-sh-meta">'+(W.over?'\u00dcber dem Ziel: <b>+'+fmtDe(Math.round((W.act-W.eff)*10)/10)+' km</b>':'' + T('ui.offen') + ': <b>'+fmtDe(W.rest)+' km</b>')+
    (W.deload?'' + T('ui.u00b7_kalenderziel') + ''+W.cal+'' + T('ui.km_gedeckelt_ist_kopplung') + '':'')+'</div>';
  if(mode==='profi')body+='<div class="rcv-sh-meta">' + T('ui.zeitraum') + ''+_wkEsc(_wkVolRange())+' \u00b7 Nur Laufen \u00b7 kanonische Aggregation (dedupliziert, Nutzer-Zeitzone).</div>';
  body+='<p class="note" style="text-align:left;margin-top:10px">' + T('ui.richtwerte_mit_entlastungswochen_u2014_bei') + '</p>';
  try{if(typeof _rcvLastFocus!=='undefined')_rcvLastFocus=document.activeElement;}catch(_){ }
  if(typeof oModal==='function')oModal('' + T('ui.laufumfang_u00b7_woche') + '',body);
  try{var sh=document.getElementById('suppSheet');if(sh){sh.setAttribute('tabindex','-1');sh.focus();}}catch(_){ }
}
...
function _wkVolRange(){
  ...
  return '' + T('ui.mo') + ''+f(mon)+'' + T('ui.u2013_so') + ''+f(sun);
}
...
function orviaRebuildPlan(){if(typeof PROFILE==='undefined'||!PROFILE)return;
  ...
  if(typeof toast==='function')toast('' + T('ui.plan_neu_aufgebaut') + '');}
...
function renderWeekPlan(){
  ...
  try{var _pb=document.getElementById('planRebuildBanner');
    ...
    if(_pb){var _pend=(typeof PROFILE!=='undefined'&&PROFILE&&PROFILE.weekPlan&&PROFILE.planImpact&&PROFILE.planImpact.pending);
      _pb.innerHTML=_pend?'<div class="banner warn"><span><b>' + T('ui.deine_konfiguration_hat_sich_geaendert') + '</b>' + T('ui.der_angezeigte_plan_basiert_noch') + '</span><button onclick="orviaRebuildPlan()">' + T('ui.plan_neu_aufbauen') + '</button></div>':'';}}catch(e){}
  ...
  const _rs=document.getElementById('runnaSub');if(_rs)_rs.textContent=isRunna
    ?('' + T('ui.orvia_laufplan_woche') + ''+wk+'/25 — wird automatisch abgehakt, sobald du loggst.')
    :'' + T('ui.dein_wochenplan_passt_sich_an') + '';
  ...
  if(_pref!=null&&isFinite(_pref)&&_pref>0){
    ...
    pd={iv:'' + T('ui.zuegig_kontrolliert') + '',ez:'' + T('ui.locker_z2_gespraech_moeglich') + '',lr:'' + T('ui.gleichmaessig_locker') + ''};
  }
  const lk=lrKm(wk);
  ...
  let html='';for(let i=0;i<7;i++){const d=new Date(mon);d.setDate(mon.getDate()+i);const k=todayStr(d);const e=DB[k];const isToday=k===todayStr();
    ...
    const items=renderItems.length?renderItems.map((it,idx)=>{const det=pd[it.d]||it.d;var _occId=(it&&it.id)?('po:'+k+':'+it.id):null;var _paR=(_occId&&_paByOcc[_occId])?_paByOcc[_occId]:null;var _isDone=(_paR&&_paR.state==='completed'); // I3b.1 fail-closed: kein Rückfall auf Tag+Sport-done
      let lbl=it.l; if(it.l==='Long Run'&&lk)lbl='' + T('ui.long_run') + ''+lk+' km';
      ...
      const adaptBadge=isAdapt?'<span class="pl-adapt">angepasst</span> ':(it.race?'<span class="pl-adapt pl-race">' + T('ui.renntag') + '</span> ':(it.absenceAdjusted?'<span class="pl-adapt">' + T('ui.angepasst_ausfall') + '</span> ':(it.phaseAdjusted?'<span class="pl-adapt">' + T('ui.angepasst_phase') + '</span> ':'')));
    ...
    :(function(){var _s5=(typeof gmDayStateFor==='function')?gmDayStateFor(i,(function(){try{return (window.ORVIA&&ORVIA.profileModel&&ORVIA.profileModel.effectiveTrainingConfig)?ORVIA.profileModel.effectiveTrainingConfig(typeof PROFILE!=='undefined'?PROFILE:null):null;}catch(_){return null;}})()):'rest';
      return _s5==='rest'?'<div class="sess5-rest"><span aria-hidden="true">☾</span>' + T('ui.ruhetag') + '</div>'
        :_s5==='unavailable'?'<div class="sess5-rest"><span aria-hidden="true">–</span>' + T('ui.nicht_verfuegbar') + '</div>'
        :'<div class="sess5-rest"><span aria-hidden="true">·</span>' + T('ui.frei') + '</div>';})();
    const pz=(typeof pauseFor==='function')?pauseFor(k):null;
    html+=`<div class="pday${isToday?' today':''}${pz?' paused':''}"><div class="pd">${DAYNAMES[i]} ${d.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'})}${isToday?'' + T('ui.heute') + '':''}${pz?' <span class="pd-pause">'+esc(pz.reason||'Pause')+'</span>':''}</div>${items}</div>`;}
  const fmt=function(dt){return dt.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'});};
  const nav='<div class="pweek-nav"><button class="pwk-arw" onclick="shiftPlanWeek(-1)"'+(isRunna&&wk<=1?' disabled':'')+' aria-label="' + T('ui.vorige_woche') + '">‹</button>'+
    '<div class="pwk-mid"><span class="pwk-w">'+(isRunna?('' + T('ui.woche__') + ''+wk+' / 25'):'Woche')+'</span><span class="pwk-r">'+fmt(mon)+'–'+fmt(sun)+'</span></div>'+
    '<button class="pwk-arw" onclick="shiftPlanWeek(1)"'+(isRunna&&wk>=25?' disabled':'')+' aria-label="' + T('ui.naechste_woche_') + '">›</button></div>'+
    (off!==0?'<button class="pwk-today" onclick="planWeekToday()">' + T('ui.zur_aktuellen_woche') + '</button>':'');
  const meta=(mode==='profi')?'<p class="sess5-meta">' + T('ui.quelle_wochenplan_profil_generator_erledigt') + '</p>':'';
...
function renderPlanVariants(){
  ...
  var v=planVariants();var dn=['' + T('ui.mo_') + '','' + T('ui.di') + '','' + T('ui.mi') + '','' + T('ui.do') + '','' + T('ui.fr') + '','' + T('ui.sa') + '','' + T('ui.so') + ''];
  var aList=v.A.map(function(x){return dn[x.di]+' '+x.it.l;}).join(', ')||'—';
  el.innerHTML='<div class="pv-grid">'+
    '<div class="pv"><span class="pv-h">' + T('ui.ideal') + '</span><b>'+v.all.length+'</b><span class="pv-s">' + T('ui.alle_einheiten') + '</span></div>'+
    '<div class="pv"><span class="pv-h">' + T('ui.realistisch') + '</span><b>'+v.AB.length+'</b><span class="pv-s">' + T('ui.a_b') + '</span></div>'+
    '<div class="pv"><span class="pv-h">' + T('ui.notfall') + '</span><b>'+v.A.length+'</b><span class="pv-s">' + T('ui.nur_a') + '</span></div></div>'+
    '<div class="pv-must"><b>' + T('ui.pflicht_a') + '</b> '+esc(aList)+'</div>'+
    '<p class="note" style="text-align:left;margin-top:8px">' + T('ui.wenig_zeit_oder_recovery_zuerst') + '</p>';
}
function planQualityChecks(){
  ...
  if(restDays===0)warns.push(['' + T('ui.kein_fester_ruhetag') + '','' + T('ui.plane_mindestens_1_ruhetag_woche') + '']);
  if(lvl==='anfaenger'&&sessDays>=6)warns.push(['' + T('ui.viele_trainingstage_fuer_anfaenger') + '','' + T('ui.reduziere_auf_3_4_tage') + '']);
  var bb=false;for(var i=0;i<7;i++){if((w[i]||[]).some(isHardUnit)&&(w[(i+1)%7]||[]).some(isHardUnit))bb=true;}
  if(bb)warns.push(['' + T('ui.harte_tage_direkt_hintereinander') + '','' + T('ui.zwischen_zwei_harte_einheiten_einen') + '']);
  var dRace=g.raceDate?daysTo(g.raceDate):null;
  if(g.type==='marathon'&&lvl==='anfaenger'&&dRace!=null&&dRace<84)warns.push(['' + T('ui.marathon_ziel_sehr_ambitioniert') + '','' + T('ui.als_anfaenger_braucht_ein_marathon') + '']);
  if(gcat(g.type)==='half_marathon'&&lvl==='anfaenger'&&dRace!=null&&dRace<42)warns.push(['' + T('ui.hm_ziel_knapp_fuer_anfaenger') + '','' + T('ui.unter_6_wochen_ist_riskant') + '']);
  var e=DB[todayStr()];var knee=(e&&e.morning&&e.morning.knee!=null)?e.morning.knee:0;
  var u=(typeof todayPrimaryUnit==='function')?todayPrimaryUnit():null;
  if(knee>=4&&u&&isHardUnit(u))warns.push(['' + T('ui.knie') + ''+knee+'' + T('ui.10_und_harte_einheit_geplant') + '','' + T('ui.heute_ersetzen_easy_bike_siehe') + '']);
...
function renderPlanQuality(){
  ...
  var head='<button type="button" class="pqv5-head" data-pq-rating="'+_pqEsc(q.rating.c)+'" data-pq-warns="'+n+'" onclick="openPlanQualitySheet()" aria-haspopup="dialog" aria-label="Planqualität-' + T('ui.details_') + ' öffnen">'+_pqChip(q.rating)+'<span class="pqv5-more">' + T('ui.details') + '</span></button>';
  var lead=n===0
    ?'<p class="pqv5-lead">'+(mode==='anfaenger'?'' + T('ui.keine_auffaelligkeiten_dein_wochenplan_ist') + '':'' + T('ui.keine_auffaelligkeiten_in_den_planpruefungen') + '')+'</p>'
    :'<p class="pqv5-lead">'+n+(n===1?'' + T('ui.auffaelligkeit') + '':'' + T('ui.auffaelligkeiten') + '')+(mode==='anfaenger'?'' + T('ui.die_hinweise_zeigen_dir_direkt') + '':'' + T('ui.in_den_planpruefungen') + '')+'</p>';
  var meta=mode==='profi'?'<p class="pqv5-meta">' + T('ui.datenbasis_wochenplanstruktur_zielkonfiguration_level_heutiges') + '</p>':'';
...
function openPlanQualitySheet(){
  ...
  var body='<div class="rcv-sh-v" data-pq-rating="'+_pqEsc(q.rating.c)+'" data-pq-warns="'+q.warns.length+'">'+_pqChip(q.rating)+'</div>'+
    (q.warns.length?_pqWarnList(q):'<p style="margin:8px 0 0">' + T('ui.keine_auffaelligkeiten_in_den_planpruefungen') + '</p>')+
    '<div class="rcv-sh-meta">' + T('ui.bewertung_und_hinweise_stammen_unveraendert') + '</div>';
  if(mode==='profi')body+='<div class="rcv-sh-meta">' + T('ui.datenbasis_wochenplanstruktur_zielkonfiguration_level_heutiges_') + '</div>';
  try{_pqLastFocus=document.activeElement;if(typeof _rcvLastFocus!=='undefined')_rcvLastFocus=document.activeElement;}catch(_){ }
  if(typeof oModal==='function')oModal('' + T('ui.planqualitaet_sicherheit') + '',body);
  try{var sh=document.getElementById('suppSheet');if(sh){sh.setAttribute('tabindex','-1');sh.focus();}}catch(_){ }
...
function gmPlannedLinesHTML(item){
  ...
  for(var i=0;i<lines.length;i++){
    html+='<li'+(lines[i].resolved?'':' class="sc-plex-unknown" title="Diese Übung steht ' + T('ui.nicht_in_der_bibliothek') + ' — die Kennung wird unverändert angezeigt."')+'>'+
...
function gmMuskelHinweisHTML(item){
  try{
    ...
    if(offen>0){
      h[0].nichtGezaehlt=(h[0].nichtGezaehlt||[]).concat([{name:offen+'' + T('ui.uebung_en_ohne_bibliothekseintrag') + ''}]);
    }
...
function openPlanEditor(){
  ...
  var wrap=document.createElement('div');wrap.className='orvia-modal-bg';
  wrap.innerHTML='<div class="orvia-modal goal-modal plan-edit"><h3>' + T('ui.wochenplan_bearbeiten') + '</h3><div class="pe-scroll" id="pe_scroll"></div>'+
    '<button class="btn" onclick="savePlanEdit()">' + T('ui.speichern') + '</button>'+
    '<button class="btn sec" style="margin-top:10px" onclick="resetPlan()">' + T('ui.auf_standard_zuruecksetzen') + '</button>'+
    '<button class="btn sec" style="margin-top:10px" onclick="closePlanEditor()">' + T('ui.abbrechen') + '</button></div>';
...
var PE_ERR_TEXT={
  missing:'' + T('ui.pflichtangabe_fehlt') + '', not_integer:'' + T('ui.nur_ganze_zahlen') + '', not_finite:'' + T('ui.keine_zahl_') + '',
  out_of_range:'' + T('ui.ausserhalb_des_zulaessigen_bereichs') + '', reversed_range:'' + T('ui.von_wert_groesser_als_bis') + '',
  too_many:'' + T('ui.obergrenze_erreicht') + '', not_object:'' + T('ui.unbrauchbare_eingabe') + ''
};
var PE_FIELD_DE={sets:'' + T('ui.saetze__') + '',minReps:'' + T('ui.wdh_von') + '',maxReps:'' + T('ui.wdh_bis') + '',targetWeightKg:'Zielgewicht',targetRir:'RIR',restSeconds:'Pause',exerciseId:'Übung'};
function _peErrText(errs){
  if(!errs||!errs.length)return '' + T('ui.eingabe_abgelehnt') + '';
  var e=errs[0];
...
function peAddEx(di,ii){
  ...
  if(!id){_peErr={di:di,ii:ii,msg:'' + T('ui.bitte_zuerst_eine_uebung_auswaehlen') + ''};return renderPlanEditor();}
  var sets=_peNum(setsEl);
  if(sets===null||(typeof sets==='number'&&isNaN(sets))){
    _peErr={di:di,ii:ii,msg:'' + T('ui.saetze_pflichtangabe_fehlt') + ''};return renderPlanEditor();
  }
...
function peUpdateEx(di,ii,idx,field,el){
  ...
  if(typeof v==='number'&&isNaN(v)){
    _peErr={di:di,ii:ii,msg:(PE_FIELD_DE[field]||field)+'' + T('ui.keine_zahl') + ''};return renderPlanEditor();
  }
  ...
  for(var i=0;i<list.length;i++){
    ...
    rows+='<div class="pe-exrow">'+
      '<div class="pe-exname'+(nm?'':' pe-exname-unknown')+'">'+(nm?'':'⚠ ')+esc(nm||e.exerciseId)+
        (nm?'':'<span class="pe-exhint">' + T('ui.nicht_in_der_bibliothek') + '</span>')+'</div>'+
      '<div class="pe-exfields">'+
        '<label>' + T('ui.saetze__') + '<input type="number" inputmode="numeric" min="1" max="20" id="pe_f_sets_'+idp+'" value="'+e.sets+'" onchange="peUpdateEx('+di+','+ii+','+i+',\'sets\',this)"></label>'+
        '<label>' + T('ui.wdh_von') + '<input type="number" inputmode="numeric" min="1" max="100" value="'+(e.minReps==null?'':e.minReps)+'" onchange="peUpdateEx('+di+','+ii+','+i+',\'minReps\',this)"></label>'+
        ...
        '<label>' + T('ui.pause_s') + '<input type="number" inputmode="numeric" min="0" max="900" value="'+(e.restSeconds==null?'':e.restSeconds)+'" onchange="peUpdateEx('+di+','+ii+','+i+',\'restSeconds\',this)"></label>'+
      '</div>'+
      '<div class="pe-exact">'+
        ...
        '<button type="button" onclick="peRemoveEx('+di+','+ii+','+i+')" aria-label="' + T('ui.uebung_entfernen') + '">✕</button>'+
      '</div></div>';
  }
  if(!rows)rows='<p class="pe-empty">' + T('ui.noch_keine_uebungen_geplant') + '</p>';
  var opts=peExOptions();
  var add;
  if(!opts){
    add='<p class="pe-exwarn">' + T('ui.die_uebungsbibliothek_ist_gerade_nicht') + '</p>';
  }else{
    var os='<option value="">' + T('ui.uebung_waehlen') + '</option>';
    ...
    add='<div class="pe-exadd"><select id="pe_ex_sel_'+di+'_'+ii+'">'+os+'</select>'+
      '<input type="number" inputmode="numeric" min="1" max="20" id="pe_ex_sets_'+di+'_'+ii+'" value="3" aria-label="' + T('ui.saetze__') + '">'+
      '<button type="button" class="btn sec" onclick="peAddEx('+di+','+ii+')">' + T('ui.hinzufuegen') + '</button></div>';
  }
  var err=(_peErr&&_peErr.di===di&&_peErr.ii===ii)?'<p class="pe-exerr" role="alert">'+esc(_peErr.msg)+'</p>':'';
  var est=SP.estimateDurationMin(list);
  var meta=list.length?'<p class="pe-exmeta">'+list.length+'' + T('ui.uebung') + ''+(list.length===1?'':'en')+(est?'' + T('ui.geschaetzt') + ''+est+' min':'')+'</p>':'';
  ...
  sc.innerHTML=_planEdit.map(function(day,di){
    var items=day.length?day.map(function(it,ii){
      ...
      if(SP&&SP.isStrengthItem(it)){
        ...
        chip+='<button type="button" class="pe-exbtn'+(open?' on':'')+'" onclick="peToggleEx('+di+','+ii+')" aria-expanded="'+(open?'true':'false')+'' + T('ui.uebungen') + ''+n+')</button>';
      }
      chip+='<button type="button" onclick="removePlanItem('+di+','+ii+')" aria-label="' + T('ui.entfernen') + '">✕</button></span>';
      if(SP&&SP.isStrengthItem(it)&&_peOpen&&_peOpen.di===di&&_peOpen.ii===ii)chip+=peExPanelHTML(di,ii);
      return chip;
    }).join(''):'<span class="pe-empty">' + T('ui.ruhetag_') + '</span>';
...
function savePlanEdit(){
  ...
  if(typeof gmCanonPlanOn==='function'&&gmCanonPlanOn()&&_gmCanonPlan.plan&&typeof gmCanonPlanSaveEdit==='function'&&gmCanonPlanSaveEdit(JSON.parse(JSON.stringify(_planEdit)))){
    ...
    if(typeof toast==='function')toast('' + T('ui.wochenplan_gespeichert_kanonisch') + '');return;
  }
  if(typeof PROFILE!=='undefined'&&PROFILE){PROFILE.weekPlan=JSON.parse(JSON.stringify(_planEdit));_planMeta('manual_edit');if(typeof saveProfile==='function')saveProfile();}closePlanEditor();renderWeekPlan();try{if(typeof renderPlan==='function')renderPlan();}catch(_){ }if(typeof toast==='function')toast('' + T('ui.wochenplan_gespeichert') + '');}
function resetPlan(){if(typeof PROFILE!=='undefined'&&PROFILE){PROFILE.weekPlan=null;PROFILE._planUndo=null;_planMeta('reset');if(PROFILE.planImpact)PROFILE.planImpact.pending=false;if(typeof saveProfile==='function')saveProfile();}closePlanEditor();renderWeekPlan();try{if(typeof renderPlan==='function')renderPlan();}catch(_){ }if(typeof toast==='function')toast('' + T('ui.plan_neu_aufgebaut_aus_deiner') + '');}
...
function renderPauseBanner(){
  ...
  el.style.display='';
  el.innerHTML='<div class="pause-banner"><div><b>' + T('ui.pause_aktiv') + ' · '+esc(p.reason||'Pause')+'</b><span>bis '+(typeof fmtDate==='function'?fmtDate(p.to):p.to)+' — kein Trainingsdruck, ' + T('ui.erholung_') + ' zählt. Logge nur, was du wirklich machst.</span></div></div>';
}
...
var GM_PROV_NOTE={
  planned_week:null,
  recurring_preview:'' + T('ui.vorschau_aus_deiner_wiederkehrenden_wochenstruktur') + '',
  loading:'' + T('ui.plan_dieser_woche_wird_geladen') + ''
};
...
function gmCanonPlanPersist(plan,after){
  ...
  repo.save(plan).then(function(r){
    if(!r.success&&r.sync_status!=='pending'&&typeof toast==='function')toast('' + T('ui.plan_sync_fehlgeschlagen_lokal_gespeichert') + '');
    if(after)after(r);
  });
...
function gmCanonPlanEngineRebase(adjustedDays,batchId){
  ...
  if(r.conflicts.length&&typeof toast==='function')toast(r.conflicts.length+'' + T('ui.plan_konflikt_e_siehe_plan') + '');
  return true;
}
...
function gmEngineActivateWeek(){
  ...
  gmCanonPlanPersist(r.plan);
  if(r.conflicts&&r.conflicts.length&&typeof toast==='function')toast(r.conflicts.length+'' + T('ui.plan_konflikt_e_siehe_plan') + '');
  return r;
}
...
function gmOpenPlanConflictsSheet(){
  ...
  var body;
  if(!cs.length){body='<p class="muted">' + T('ui.keine_offenen_plan_konflikte') + '</p>';}
  else{
    body=cs.map(function(c){
      return '<div class="card"><div class="ctitle"><div class="l">'+icon('alert')+' Änderung nicht übertragbar</div></div>'+
        '<p style="font-size:12px;color:var(--muted);margin:0 0 8px">Deine Änderung („'+gmEsc(c.type)+'' + T('ui.hing_an_einer_einheit_die') + ''+gmEsc(c.reason)+'). Sie wird nicht geraten neu zugeordnet (E-16).</p>'+
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--attention-t,rgba(237,180,78,.16));color:var(--attention)">'+icon('alert')+'</div><div><h3>' + T('ui.plan_konflikte') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.engine_update_vs_deine_aenderungen') + '</div></div></div>'+
...
function renderPlanPauses(){
  ...
  el.innerHTML='<div class="pause-list">'+ps.map(function(o){return '<div class="pause-item"><span>'+esc(o.p.reason||'Pause')+' · '+(typeof fmtDate==='function'?fmtDate(o.p.from)+'–'+fmtDate(o.p.to):o.p.from+'–'+o.p.to)+'</span><button onclick="delPause('+o.i+')" aria-label="' + T('ui.entfernen') + '">✕</button></div>';}).join('')+'</div>';
}
function openPauseEditor(){
  var t=(typeof todayStr==='function'?todayStr():'');
  var wrap=document.createElement('div');wrap.className='orvia-modal-bg';
  wrap.innerHTML='<div class="orvia-modal goal-modal"><h3>' + T('ui.pause_eintragen') + '</h3>'+
    '<div class="gm-field"><label>Grund</label><div class="gm-chips" id="pause_r"><button type="button" class="gm-chip on" data-v="Urlaub" onclick="gmPick(this,\'pause_r\')">Urlaub</button><button type="button" class="gm-chip" data-v="Krank" onclick="gmPick(this,\'pause_r\')">Krank</button><button type="button" class="gm-chip" data-v="Pause" onclick="gmPick(this,\'pause_r\')">' + T('ui.sonstiges') + '</button></div></div>'+
    '<div class="gm-field"><label>' + T('ui.von') + '</label><input type="date" id="pause_f" value="'+t+'"></div>'+
    '<div class="gm-field"><label>' + T('ui.bis') + '</label><input type="date" id="pause_t" value="'+t+'"></div>'+
    '<button class="btn" onclick="savePause()">' + T('ui.pause_speichern') + '</button>'+
    '<button class="btn sec" style="margin-top:10px" onclick="closePause()">' + T('ui.abbrechen') + '</button></div>';
...
function savePause(){
  if(typeof PROFILE!=='undefined'&&PROFILE){
    ...
    if(!f||!tt){if(typeof toast==='function')toast('' + T('ui.von_bis_fehlt') + '');return;}
    if(tt<f){var x=f;f=tt;tt=x;}
    PROFILE.pauses=PROFILE.pauses||[];PROFILE.pauses.push({from:f,to:tt,reason:r});
    if(typeof saveProfile==='function')saveProfile();
  }
  closePause();if(typeof renderPlanPauses==='function')renderPlanPauses();renderWeekPlan();if(typeof renderPauseBanner==='function')renderPauseBanner();if(typeof toast==='function')toast('' + T('ui.pause_gespeichert') + '');
}
...
function unitPace(kind){
  ...
  var key={interval:'' + T('ui.intervall_vo2') + '',easy:'Easy',long:'Long Run',tempo:'' + T('ui.tempo_schwelle') + ''}[kind];if(!key)return null;
...
function unitStruct(item,kind){
  var S={
    interval:{goal:'' + T('ui.vo2max_schnelligkeit_reize_ueber_der') + '',warmup:'' + T('ui.10_15_min_locker_3') + '',main:'' + T('ui.konkret_6_800_m_im') + '',cooldown:'10 min auslaufen',rpe:'' + T('ui.rpe_8_9') + '',alt:'' + T('ui.bei_muedigkeit_knie_easy_z2') + ''},
    easy:{goal:'' + T('ui.grundlagenausdauer_locker_aerob_foerdert_erholung') + '',warmup:'5 min eintraben',main:'' + T('ui.gleichmaessig_im_easy_bereich_unterhaltungstempo') + '',cooldown:'kurz auslaufen',rpe:'' + T('ui.rpe_3_4') + '',alt:'' + T('ui.bei_knie_bike_schwimmen_z2') + ''},
    long:{goal:'' + T('ui.ausdauer_long_run_oekonomie') + '',warmup:'locker starten',main:'' + T('ui.konkret_gleichmaessig_im_long_run') + '',cooldown:'' + T('ui.letzte_minuten_locker') + '',rpe:'' + T('ui.rpe_4_5') + '',alt:'' + T('ui.bei_knie_kuerzen_oder_durch') + ''},
    tempo:{goal:'' + T('ui.laktatschwelle_kontrolliert_hart') + '',warmup:'10 min locker',main:'' + T('ui.konkret_nach_dem_warm_up') + '',cooldown:'10 min locker',rpe:'' + T('ui.rpe_7') + '',alt:'' + T('ui.bei_muedigkeit_in_easy_umwandeln') + ''},
    gym:{goal:'' + T('ui.kraft_stabilitaet_verletzungsschutz_und_laufoekonomie') + '',warmup:'' + T('ui.mobilitaet_aktivierung') + '',main:item.l+'' + T('ui.nach_plan_grunduebungen_sauber_technik') + '',cooldown:'' + T('ui.kurze_mobility') + '',rpe:'Technik-Fokus',alt:'' + T('ui.bei_knie_keine_schweren_beine') + ''},
    swim:{goal:'' + T('ui.gelenkschonende_ausdauer_technik') + '',warmup:'4 × 25 m locker',main:item.l,cooldown:'2 × 25 m locker',rpe:'kontrolliert',alt:''},
    bike:{goal:'' + T('ui.aerobe_basis_gelenkschonend') + '',warmup:'5 min locker',main:'' + T('ui.gleichmaessige_z2_dauerfahrt') + '',cooldown:'5 min ausrollen',rpe:'' + T('ui.rpe_3_4') + '',alt:''},
    mob:{goal:'' + T('ui.beweglichkeit_praevention') + '',warmup:'',main:item.l,cooldown:'',rpe:'',alt:''}
  };
...
function unitGuidance(item,kind){
  ...
  if(!m)return '' + T('ui.mach_zuerst_den_morgen_check') + '';
  ...
  var knee=m.knee!=null?m.knee:0;var alt=unitStruct(item,kind).alt||'' + T('ui.bike_schwimmen_z2') + '';
  if(item.t==='Laufen'){
    if(knee>=4)return '' + T('ui.knie') + ''+knee+'' + T('ui.10_lauf_heute_nicht_empfohlen') + ''+alt+'.';
    if(knee>=3)return '' + T('ui.knie') + ''+knee+'' + T('ui.10_wenn_ueberhaupt_nur_locker') + '';
    if(a.c==='r'||a.c==='o')return '' + T('ui.tagesform_') + ''+(a.c==='r'?'rot':'orange')+'' + T('ui.heute_keine_intensitaet') + ''+((kind==='interval'||kind==='tempo')?'' + T('ui.easy_z2_statt_der_harten') + '':'' + T('ui.wenn_dann_sehr_locker_und') + '');
    if(a.c==='y')return '' + T('ui.tagesform_gelb') + ''+((kind==='interval'||kind==='tempo')?'' + T('ui.reduzieren_weniger_wiederholungen_oder_easy') + '':'' + T('ui.nach_plan_aber_pace_am') + '');
    var p=unitPace(kind);return '' + T('ui.gruenes_licht_einheit_nach_plan') + ''+(p?'' + T('ui.pace_') + ''+p+' halten.':'' + T('ui.im_zielbereich_bleiben') + '');
  }
  if(item.t==='Gym'){if(knee>=3)return '' + T('ui.knie') + ''+knee+'' + T('ui.10_keine_schweren_beine_oberkoerper') + '';return '' + T('ui.technik_vor_last_vor_intervall') + '';}
  return '' + T('ui.locker_und_gleichmaessig_gelenkschonende_ausdauer') + '';
}
...
function runPrescription(item,kind){
  ...
  if(kind==='easy'){p.dist=8;p.pace=ez;p.dur=8*easyP/60;p.splits=[['1 km','locker einlaufen'],['6 km','stabil locker @ '+pz(ez)],['1 km','auslaufen']];p.goal='' + T('ui.grundlagenausdauer_locker_aerob_foerdert_erholung') + '';p.cues='Unterhaltungstempo.';}
  else if(kind==='long'){p.dist=lk;p.pace=lz;p.dur=lk*longP/60;p.splits=[['0–'+Math.round(lk*0.3)+' km','sehr locker'],[Math.round(lk*0.3)+'–'+Math.round(lk*0.8)+' km','stabil @ '+pz(lz)],[Math.round(lk*0.8)+'–'+lk+' km','' + T('ui.optional_leicht_zuegiger_wenn_score') + '']];p.goal='' + T('ui.ausdauer_long_run_oekonomie') + '';p.cues='' + T('ui.bewusst_langsam_starten') + '';}
  else if(kind==='tempo'){p.dist=10;p.pace=tz;p.dur=(2*easyP+5*tempoP+3*easyP)/60;p.splits=[['2 km','' + T('ui.warm_up_easy') + ''],['5 km','@ '+pz(tz)+' (komfortabel hart)'],['3 km','' + T('ui.cool_down_easy') + '']];p.goal='' + T('ui.laktatschwelle_kontrolliert_hart') + '';p.cues='' + T('ui.gleichmaessig_noch_kontrolliert') + '';}
  else{p.dist=9;p.pace=iz;p.dur=(2*easyP+6*0.8*intP+6*0.4*easyP+2*easyP)/60;p.splits=[['2 km','' + T('ui.warm_up_easy_3_4') + ''],['6 × 800 m','@ '+pz(iz)],['je 400 m / 2 min','Trab-Pause'],['2 km','' + T('ui.cool_down_easy') + '']];p.goal='' + T('ui.vo2max_schnelligkeit_reize_ueber_der') + '';p.cues='' + T('ui.letzte_wiederholung_nicht_langsamer') + '';}
  p.hf=unitHF(kind);p.rpe={easy:'' + T('ui.rpe_3_4') + '',long:'' + T('ui.rpe_4_5') + '',tempo:'' + T('ui.rpe_7') + '',interval:'' + T('ui.rpe_8_9') + ''}[kind];p.alt=unitStruct(item,kind).alt;return p;
}
...
function bikePrescription(item){
  ...
  if(sub==='long'){p.dist=70;p.durStr='2:30–3:00 h';p.zone='' + T('ui.hf_zone_2') + '';p.splits=[['30 min','locker einrollen'],['~2 h','' + T('ui.konstant_zone_2') + ''],['20 min','' + T('ui.nur_zuegiger_wenn_tagesform_gut') + '']];p.goal='' + T('ui.lange_aerobe_ausdauer') + '';p.rpe='' + T('ui.rpe_3_5') + '';p.alt='' + T('ui.bei_schlechter_form_40_km') + '';}
  else if(sub==='recovery'){p.dist=25;p.durStr='' + T('ui.45') + '–60 min';p.zone='' + T('ui.hf_zone_1_2') + '';p.splits=[['gesamt','' + T('ui.locker_z1_2_keine_antritte') + '']];p.goal='' + T('ui.durchblutung_erholung') + '';p.rpe='' + T('ui.rpe_2_3') + '';p.alt='' + T('ui.kann_entfallen_wenn_sehr_muede') + '';}
  else if(sub==='interval'){p.dist=45;p.durStr='90 min';p.zone='' + T('ui.z2_intervalle') + '';p.splits=[['15 min','locker einrollen'],['5 × 5 min','hart / 5 min locker'],['15 min','ausrollen']];p.goal='' + T('ui.schwelle_vo2_auf_dem_rad') + '';p.rpe='' + T('ui.rpe_8_in_den_bloecken') + '';p.alt='' + T('ui.bei_schlechter_form_nur_z2') + '';}
  else{p.dist=35;p.durStr='75–90 min';p.zone='' + T('ui.hf_zone_2') + '';p.splits=[['10 min','locker einrollen'],['60 min','' + T('ui.konstant_zone_2') + ''],['10 min','ausrollen']];p.goal='' + T('ui.aerobe_basis_gelenkschonend_') + '';p.rpe='' + T('ui.rpe_3_4') + '';p.alt='' + T('ui.commute_zaehlt_als_regeneration') + '';}
  return p;
}
...
function unitSplits(splits,profi){if(!splits||!splits.length)return '';
  if(profi)return '<div class="unit-splits"><div class="us-h">' + T('ui.splits') + '</div>'+splits.map(function(s){return '<div class="usp"><span class="usp-k">'+escH(s[0])+'</span><span class="usp-v">'+escH(s[1])+'</span></div>';}).join('')+'</div>';
...
function unitBodyRun(item,kind,lvl){
  ...
  var cells=[[p.dist+' km','' + T('ui.distanz_') + ''],['~'+fmtDur(p.dur),'Dauer']];
  ...
  var guid='<div class="unit-rec"><b>' + T('ui.heute_fuer_dich') + '</b><br>'+escH(unitGuidance(item,kind))+'</div>';
  var alt=p.alt?'<div class="unit-alt"><b>' + T('ui.alternative') + '</b> '+escH(p.alt)+'</div>':'';
  if(lvl==='anfaenger'){
    var word={easy:'locker',long:'' + T('ui.ruhig_und_gleichmaessig') + '',tempo:'' + T('ui.zuegig_aber_kontrolliert') + '',interval:'' + T('ui.mit_ein_paar_schnellen_abschnitten') + ''}[kind]||'locker';
    var cue={easy:'' + T('ui.du_solltest_dich_dabei_noch') + '',long:'' + T('ui.starte_langsam_und_halte_ein') + '',tempo:'' + T('ui.komfortabel_hart_du_kannst_nur') + '',interval:'' + T('ui.zwischen_den_schnellen_stuecken_locker') + ''}[kind]||'';
    var simple='' + T('ui.laufe_ca') + ''+Math.round(p.dur)+'' + T('ui.minuten') + ''+word+'. '+cue+'' + T('ui.fuehlst_du_dich_schwer_kuerze') + ''+Math.round(p.dur*0.7)+'' + T('ui.minuten_') + '';
    return unitHero(cells)+goal+'<div class="unit-simple">'+escH(simple)+'</div>'+guid+alt;
  }
  var stats=[];if(p.hf)stats.push(['HF-Zone',p.hf]);if(p.rpe)stats.push(['' + T('ui.intensitaet') + '',p.rpe]);
  ...
  var cues=(lvl==='profi'&&p.cues)?'<div class="unit-cues">'+escH(p.cues)+((kind==='easy'||kind==='long')?'' + T('ui.bei_hr_drift_5_pace') + '':'')+'</div>':'';
...
function unitBodyBike(item,lvl){
  ...
  var cells=[[p.dist+' km','' + T('ui.distanz_') + ''],[p.durStr,'Dauer']];
  if(lvl!=='anfaenger')cells.push([p.zone,'Zone']);
  var goal='<div class="unit-goal">'+escH(p.goal)+'</div>';
  var guid='<div class="unit-rec"><b>' + T('ui.heute_fuer_dich') + '</b><br>'+escH(unitGuidance(item,'bike'))+'</div>';
  var alt=p.alt?'<div class="unit-alt"><b>' + T('ui.alternative') + '</b> '+escH(p.alt)+'</div>':'';
  if(lvl==='anfaenger'){
    var simple='' + T('ui.fahre') + ''+p.durStr+' '+(p.kind==='recovery'?'ganz locker':(p.kind==='long'?'' + T('ui.gleichmaessig_und_ruhig') + '':'locker'))+'. Kein hartes Antreten — du sollst dich danach gut fühlen.';
    return unitHero(cells)+goal+'<div class="unit-simple">'+escH(simple)+'</div>'+guid+alt;
  }
  var stats=[];if(p.hf)stats.push(['HF-Zone',p.hf]);if(p.rpe)stats.push(['' + T('ui.intensitaet') + '',p.rpe]);
  var statHTML='<div class="unit-stats">'+stats.map(function(x){return '<div class="unit-stat"><span class="us-k">'+escH(x[0])+'</span><span class="us-v">'+escH(x[1])+'</span></div>';}).join('')+'</div>';
  var cues=(lvl==='profi')?'<div class="unit-cues">' + T('ui.ohne_powermeter_hf_zone_rpe') + '</div>':'';
  return unitHero(cells)+goal+statHTML+unitSplits(p.splits,lvl==='profi')+cues+guid+alt;
}
function unitBodyOther(item,kind,lvl){
  ...
  var guid='<div class="unit-rec"><b>' + T('ui.heute_fuer_dich') + '</b><br>'+escH(unitGuidance(item,kind))+'</div>';
  var alt=s.alt?'<div class="unit-alt"><b>' + T('ui.alternative') + '</b> '+escH(s.alt)+'</div>':'';
  if(lvl==='anfaenger')return goal+'<div class="unit-simple">'+escH(s.main)+'</div>'+guid+alt;
  var steps='';
  if(s.warmup)steps+='<div class="ustep"><span class="ust-l">' + T('ui.warm_up') + '</span><span>'+escH(s.warmup)+'</span></div>';
  if(s.main)steps+='<div class="ustep"><span class="ust-l">' + T('ui.hauptteil') + '</span><span>'+escH(s.main)+'</span></div>';
  if(s.cooldown)steps+='<div class="ustep"><span class="ust-l">' + T('ui.cool_down') + '</span><span>'+escH(s.cooldown)+'</span></div>';
  var statHTML=(lvl==='profi'&&s.rpe)?'<div class="unit-stats"><div class="unit-stat"><span class="us-k">' + T('ui.intensitaet') + '</span><span class="us-v">'+escH(s.rpe)+'</span></div></div>':'';
...
function planEntryClick(di,ii,dateIso){
  ...
  try{
    ...
    if(res&&res.status==='ambiguous'&&typeof toast==='function'){ toast('' + T('ui.mehrere_aktivitaeten_fuer_diese_einheit') + ''); }
    openUnit(di,ii,dIso);
...
function openUnit(di,ii,dateIso){
  ...
  var foot='';
  if(isToday){
    foot+='<button class="btn" onclick="startPlannedUnit('+di+','+ii+',\''+dIso+'\')">' + T('ui.training_starten') + '</button>'+
    ...
    foot+='<div class="mini-note">'+icon('info','xs')+'<div>Nur lesbar — diese ' + T('ui.einheit') + ' ist für '+(_dd3?('den '+_dd3):'' + T('ui.einen_anderen_tag') + '')+' geplant. Starten und Erledigen sind nur am Tag selbst möglich.</div></div>';
  }
  foot+='<button class="btn sec" style="margin-top:10px" onclick="closeSupp();openPlanEditor()">' + T('ui.plan_bearbeiten_verschieben') + '</button>';
...
function planNoteFor(item){
  try{
    ...
    if(typeof unitStruct==='function'){
      ...
      if(s){if(s.warmup)parts.push('' + T('ui.auf') + ''+s.warmup);if(s.main)parts.push(s.main);if(s.cooldown)parts.push('' + T('ui.ab') + ''+s.cooldown);}
...
function startPlannedUnit(di,ii,dateIso){
  ...
  if(!dateIso){
    if(typeof toast==='function')toast('' + T('ui.nicht_gestartet_fehlender_datumskontext') + '');
    return {ok:false,code:'missing_date_context'};
  }
  var dIso=dateIso;
  if(dIso!==todayStr()){
    if(typeof toast==='function')toast('' + T('ui.nicht_gestartet_diese_einheit_ist') + '');
...
function markPlannedDone(type,di,ii,dateIso){
  ...
  try{
    ...
    if(!dateIso){
      ...
      if(typeof toast==='function')toast('' + T('ui.nicht_markiert_fehlender_datumskontext') + '');
      return result;
    }
    var dIso=dateIso;
    if(dIso!==todayStr()){
      result.code='not_today';
      if(typeof toast==='function')toast('' + T('ui.nicht_markiert_erledigen_ist_nur') + '');
      return result;
    }
    ...
    if(!occ){
      ...
      if(typeof toast==='function')toast('' + T('ui.nicht_markiert_keine_eindeutige_plan') + '');
    }else{
      var e=entry(dIso);
      ...
      if(ex&&ex.source==='plan_done'&&ex.plannedSessionId===occ){
        ...
        if(typeof toast==='function')toast('' + T('ui.nicht_markiert_fuer_heute_existiert') + ''+type+'' + T('ui.eintrag_nichts_ueberschrieben') + '');
      }else{
        ...
        var rec={note:'' + T('ui.als_erledigt_markiert_ohne_messwerte') + '',source:'plan_done',
        ...
        if(typeof save!=='function'){
          ...
          if(typeof toast==='function')toast('' + T('ui.speichern_nicht_moeglich_nicht_markiert') + '');
        }else{
          ...
          if(persisted){
            ...
            if(typeof toast==='function')toast('' + T('ui.speichern_fehlgeschlagen_nicht_markiert') + '');
          }
        }
      }
    }
...
function undoPlanDone(type,dateIso,occurrenceId){
  ...
  if(typeof toast==='function')toast('' + T('ui.erledigt_markierung_zurueckgenommen') + '');
...
function confirmUndoPlanDone(type,dateIso,occurrenceId){
  ...
  if(typeof orviaConfirm==='function'){
    orviaConfirm({title:'' + T('ui.erledigt_markierung_zuruecknehmen') + '',text:'' + T('ui.es_wird_nur_die_manuelle') + '',okLabel:'' + T('ui.markierung_entfernen') + '',onOk:run});
...
function renderGoals(){
  ...
  if(!keys.length){box.innerHTML='<p class="muted" style="margin:0">' + T('ui.wochenziele_erscheinen_sobald_dein_trainingsplan') + '</p>';return;}
...
function setHmTarget(){const t=numIn('hmTarget',60,240);if(t){
  try{var g=goalOf();if(g&&g._canonicalId&&typeof goalUpdate==='function'){
    goalUpdate(g._canonicalId,{metricType:'time',unit:'s',targetValue:Math.round(t*60)},'' + T('ui.zielzeit_geaendert_pace_seite') + '');}}catch(e){}
...
function renderPace(){
  ...
  if(t==null&&(goal.state==='nodata'||goal.tPred==null)){
    ...
    if(_pb)_pb.innerHTML='<p class="muted" style="margin:0">Keine ' + T('ui.zielzeit') + ' und noch keine belastbare ' + T('ui.prognose__') + '— trage oben eine ' + T('ui.zielzeit') + ' ein, dann berechnet ORVIA deine Pace-Zonen. '+(goal.need?'<span class="muted">('+escH(goal.need)+')</span>':'')+'</p>';
    return;
  }
  ...
  html+=zone('' + T('ui.easy_z2') + '',rp*1.18,rp*1.30)+zone('Long Run',rp*1.10,rp*1.18)
    +zone('Tempo',rp*0.97,rp*1.02)+zone('' + T('ui.intervalle_1km') + '',rp*0.90,rp*0.94);
  html+=`<p class="note" style="text-align:left">${goal.state!=='nodata'
    ?'Trainings-Zonen sind an deiner <b>' + T('ui.aktuellen_fitness') + '</b> verankert (nicht am Wunschziel) — das schützt vor systematischem Zu-schnell-Laufen.'
    :'' + T('ui.noch_keine_fitness_prognose_zonen') + ''+ (goal.need||'')}</p>`;
...
function renderRaceHeader(){
  ...
  if(!mg){
    el.innerHTML='<div class="racehead"><div class="rh-top"><span class="rh-name">' + T('ui.noch_kein_ziel') + '</span>'+
      '<button class="rh-edit" onclick="openGoalEditor()">' + T('ui.ziel_hinzufuegen') + '</button></div>'+
      '<div class="rh-date">' + T('ui.lege_ein_ziel_fest_dann') + '</div></div>';
    return;
  }
  ...
  var dateTxt=mg.targetDate?new Date(mg.targetDate+'T12:00').toLocaleDateString('de-DE',{day:'2-digit',month:'long',year:'numeric'}):'' + T('ui.kein_datum') + '';
  var editBtn='<button class="rh-edit" onclick="openGoalEditor(\''+esc(mg.id)+'\')">' + T('ui.ziel_bearbeiten') + '</button>';
  if(isRun){
    var tm=null;
    ...
    el.innerHTML='<div class="racehead">'+
      ...
      '<div class="rh-grid">'+
        '<div class="rh-cell"><span class="rh-num">'+(d!=null?(d>=0?d:'—'):'–')+'</span><span class="rh-lab">' + T('ui.tage__') + '</span></div>'+
        '<div class="rh-cell"><span class="rh-num">'+escH(tgtTime)+'</span><span class="rh-lab">' + T('ui.zielzeit') + '</span></div>'+
        '<div class="rh-cell"><span class="rh-num">'+escH(tgtPace)+'</span><span class="rh-lab">Zielpace</span></div>'+
      '</div>'+
      '<div class="rh-phase">' + T('ui.phase_') + '<b>'+escH(phase)+'</b></div>'+_feasibilityLineHTML()+'</div>';
    return;
  }
  ...
  var tgt=(typeof mg.targetValue==='number')?('<div class="rh-cell"><span class="rh-num">'+escH(''+mg.targetValue)+(mg.unit?' '+escH(mg.unit):'')+'</span><span class="rh-lab">' + T('ui.zielwert') + '</span></div>'):'';
  el.innerHTML='<div class="racehead">'+
    '<div class="rh-top"><button type="button" class="rh-name rh-name-btn" onclick="openGoalDetail(\''+esc(mg.id)+'\')" aria-label="Zieldetails öffnen">'+escH(title)+'</button>'+editBtn+'</div>'+
...
function _feasibilityLineHTML(){
  try{
    ...
    var txt=f.status==='within_modeled_corridor'?'' + T('ui.zielzeit_liegt_im_modellierten_korridor') + ''
      :f.status==='outside_modeled_corridor'?'' + T('ui.zielzeit_liegt_ausserhalb_des_modellierten') + ''
      :f.status==='insufficient_data'?'' + T('ui.machbarkeit_noch_nicht_bewertbar_zu') + '':'';
    if(!txt)return '';
...
function renderPaceZones(){
  ...
  var g=goalOf();
  if(!g.targetMin){var _peId=g&&g._canonicalId?("openGoalEditor('"+esc(g._canonicalId)+"')"):"openGoalEditor()";el.innerHTML='<p class="muted" style="margin:0">' + T('ui.lege_eine_zielzeit_fest_dann') + '<button class="lexlink" onclick="'+_peId+'">' + T('ui.ziel_bearbeiten') + '</button></p>';return;}
  ...
  '<p class="note" style="text-align:left;margin-top:12px">' + T('ui.automatisch_aus_zielzeit_amp_distanz') + '</p>';
}
...
function _legacyRaceGoalEditor(){
  closeGoalEditor();
  var g=goalOf();
  var types=[['run_5k','5 km'],['run_10k','10 km'],['half_marathon','Halbmarathon'],['marathon','Marathon'],['custom','' + T('ui.freie_distanz') + '']]; // R1.2: nur kanonische IDs
  var prios=[['finish','Finish'],['solide','' + T('ui.solide_leistung') + ''],['ambitioniert','' + T('ui.ambitionierte_zeit') + '']];
  var tval=g.targetMin?Math.floor(g.targetMin/60)+':'+String(Math.round(g.targetMin%60)).padStart(2,'0'):'';
  var wrap=document.createElement('div');wrap.className='orvia-modal-bg';
  wrap.innerHTML='<div class="orvia-modal goal-modal"><h3>' + T('ui.ziel_festlegen') + '</h3>'+
    '<div class="gm-field"><label>' + T('ui.distanz_') + '</label><div class="gm-chips" id="gmType">'+types.map(function(t){return '<button type="button" class="gm-chip'+(g.type===t[0]?' on':'')+'" data-v="'+t[0]+'" onclick="gmPick(this,\'gmType\')">'+t[1]+'</button>';}).join('')+'</div></div>'+
    '<div class="gm-field"><label>' + T('ui.freie_distanz_km_nur_bei') + '</label><input type="number" inputmode="decimal" id="gmDist" placeholder="' + T('ui.z_b_15') + '" value="'+escH(g.type==='custom'&&g.distanceKm?g.distanceKm:'')+'"></div>'+
    '<div class="gm-field"><label>' + T('ui.wettkampfdatum') + '</label><input type="date" id="gmDate" value="'+escH(g.raceDate||'')+'"></div>'+
    '<div class="gm-field"><label>' + T('ui.zielzeit_std_min_z_b') + '</label><input type="text" id="gmTime" inputmode="numeric" placeholder="' + T('ui.1_50') + '" value="'+escH(tval)+'"></div>'+
    '<div class="gm-field"><label>' + T('ui.prioritaet') + '</label><div class="gm-chips" id="gmPrio">'+prios.map(function(t){return '<button type="button" class="gm-chip'+((g.priority||'solide')===t[0]?' on':'')+'" data-v="'+t[0]+'" onclick="gmPick(this,\'gmPrio\')">'+t[1]+'</button>';}).join('')+'</div></div>'+
    '<button class="btn" onclick="saveGoal()">' + T('ui.ziel_speichern') + '</button>'+
    '<button class="btn sec" style="margin-top:10px" onclick="closeGoalEditor()">' + T('ui.abbrechen') + '</button></div>';
...
function saveGoal(){
  ...
  var label=RACE_LABELS_P[type]||(type==='custom'?(dist?dist+' km':'' + T('ui.freie_distanz') + ''):type);
  ...
  try{
    if(typeof goalAdd==='function'&&typeof goalUpdate==='function'&&RACE_DIST[type]){
      ...
      if(_cur&&_cur._canonicalId)goalUpdate(_cur._canonicalId,_patch,'' + T('ui.zieldatum_geaendert') + '');
      else goalAdd(Object.assign({priority:1},_patch),'' + T('ui.hauptziel_geaendert') + '');
    }
  ...
  if(typeof toast==='function')toast('' + T('ui.ziel_gespeichert') + '');
}
function renderPlan(){flushAuto();renderRaceHeader();if(typeof renderRaceMode==='function')renderRaceMode();renderPaceZones();
  ...
  if(isRun){
    ...
    var lbl=(typeof RACE_LABELS_P!=='undefined'&&RACE_LABELS_P[g.type])||g.type||'' + T('ui.dein_ziel') + '';
    _planEmpty('rampBox','Wochen-Umfang','' + T('ui.die_lauf_umfangsrampe_erscheint_fuer') + '');
    _planEmpty('paceBox','Renn-Pace','' + T('ui.konkrete_renn_paces_gibt_es') + ''+escH(lbl)+'' + T('ui.zaehlen_trainingszonen_und_belastungssteuerung') + '');
  }
}
...
var GM_BT_SRC={lap_window:'' + T('ui.gemessen_runden_aus_der_uhr') + '',activity_total:'' + T('ui.gemessen_aktivitaet') + '',
  stream_window:'' + T('ui.gemessen_messreihe_der_uhr') + '',
  ...
  stream_uniform:'' + T('ui.aus_messreihe_abgeleitet_abschnitt_obergrenze') + '',
  manual_best:'' + T('ui.eingetragene_bestleistung') + '',estimate:'' + T('ui.geschaetzt_riegel_modell_keine_messung') + ''};
...
function renderBestTimes(){
  ...
  if(!b){el.innerHTML=gmStateEmpty({icon:'run',title:'' + T('ui.noch_keine_laeufe') + '',desc:'' + T('ui.bestzeiten_erscheinen_sobald_du_laeufe') + ''});return;}
  ...
  el.innerHTML='<div class="bt-grid">'+cell('1 km',b.t1,'k1')+cell('5 km',b.t5,'k5')+cell('10 km',b.t10,'k10')+'</div>'+
    ...
    (anyReal?'' + T('ui.gemessen_echte_leistung_aus_deinen') + '':'')+
    (anyDerived?'' + T('ui.abgeleitet_schnellster_abschnitt_aus_einer') + '':'')+
    (b.estPace?'' + T('ui.geschaetzt_aus_deinem_schnellsten_lauf') + ''+fmtDe(b.estDist)+' km @ '+fmtPace(b.estPace)+'' + T('ui.km_riegel_modell_nur_dort') + '':'')+'</p>'+
    (_scan?('<p class="note" style="text-align:left;margin-top:6px;opacity:.75">Datenlage: '+_scan.scanned+'' + T('ui.laeufe_ausgewertet') + ''+_scan.withSplits+'' + T('ui.mit_runden') + ''+(_scan.withStreams||0)+'' + T('ui.mit_messreihe') + ''+((_scan.withDerivedTime||0)?' (davon '+_scan.withDerivedTime+'' + T('ui.ohne_zeitstempel_zeit_aus_der') + '':'')+'.</p>'):'');
}
function renderDash(){
  flushAuto();
  ...
  document.getElementById('dashSegs').innerHTML=[['ueber','Überblick'],['ausdauer','' + T('ui.ausdauer') + ''],['erholung','' + T('ui.erholung_') + ''],['koerper','Körper']]
  ...
  document.getElementById('chartWarn').innerHTML=chartOK()?'':'<div class="banner warn2">' + T('ui.charts_brauchen_einmalig_internet_die') + '</div>';
...
function renderSegUeber(){
  ...
  document.getElementById('kpiBox').innerHTML=
    ...
    kpi(r7!=null?Math.round(r7)+'%':'–','' + T('ui.ready_7t') + '')+
    kpi(last?last.e.morning.knee:'–','' + T('ui.knie_heute') + '',last?(last.e.morning.knee<=2?'var(--green)':last.e.morning.knee>=6?'var(--red)':'var(--yellow)'):'')+
    kpi((function(){var w=weekRunKm(0);return w==null?'–':w.toFixed(0);})(),'' + T('ui.km_diese_wo') + '')+
    (isRunDistanceGoal()
      ...
      :kpi('–','Prognose'))+
    kpi((()=>{const p=[];for(let i=0;i<7;i++){const e=DB[dkey(-i)];if(e&&e.eve&&e.eve.prot!=null)p.push(e.eve.prot);}const a=avg(p);return a!=null?Math.round(a)+'g':'–';})(),'' + T('ui.protein_7t') + '');
  ...
  drawBarLine('cLoad',labels,{label:'Trainingslast',data:S.map(s=>Calc.sessionLoad(s.e)||null),color:'#c9ae7c'},{label:'' + T('ui.tagesform') + '',data:ready,color:'#16a34a'},{maxY2:100});
}
function renderGoalCard(elId){
  ...
  if(!isRunDistanceGoal()){
    var gg=goalOf();var glabel=(typeof GOAL_LABELS!=='undefined'&&GOAL_LABELS[gg.type])||gg.type||'' + T('ui.allgemeine_gesundheit') + '';
  ...
  if(g.state==='no_target'){var _ntId=(goalOf()._canonicalId)?("openGoalEditor('"+esc(goalOf()._canonicalId)+"')"):"openGoalEditor()";
    ...
    <div style="font-size:14px;line-height:1.6">Prognose aus deiner aktuellen Form: <b>${Calc.fmtTime(g.tPred)}</b> (Riegel ${Calc.fmtTime(g.tRiegel)}${g.tEF?'' + T('ui.ef_check') + ''+Calc.fmtTime(g.tEF):''}).<br>
  ...
  const lab=g.state==='ontrack'?'' + T('ui.on_track') + '':g.state==='border'?'GRENZWERTIG':'GEFÄHRDET';
  el.innerHTML=`<div class="rtr" style="background:${bg}">
    <h2 style="color:#fff;margin-bottom:8px">${ic('target')}Goal Engine · ${lab}</h2>
    <div style="font-size:14px;line-height:1.6">Prognose: <b>${Calc.fmtTime(g.tPred)}</b> (Riegel ${Calc.fmtTime(g.tRiegel)}${g.tEF?'' + T('ui.ef_check') + ''+Calc.fmtTime(g.tEF):''}) · Ziel ${Calc.fmtTime(g.target)} · Puffer ${g.delta>0?'+':''}${g.delta}%<br>
    ${g.vetos.length?'<b>' + T('ui.engpaesse') + '</b> '+g.vetos.map(esc).join(' · '):'' + T('ui.alle_bewertbaren_volumen_gates_erfuellt') + ''}<br>
    ${(g.notAssessable&&g.notAssessable.length)?'<b>' + T('ui.eingeschraenkt_bewertbar') + '</b> '+g.notAssessable.map(esc).join(', ')+' — Prognose mit reduzierter Sicherheit.<br>':''}
...
function renderACWRCard(){
  ...
  let bg,txt,desc;
  if(!a.enough){bg='linear-gradient(135deg,#2a3342,#1a2330)';txt='–';desc='' + T('ui.lastsprung_indikator_erscheint_nach_21') + '';}
  else if(_lcc.suppressNumbers){bg='linear-gradient(135deg,#2a3342,#1a2330)';txt='nicht belastbar';desc='' + T('ui.lastserie_unvollstaendig_acwr_aktuell_nicht') + '';}
  else if(a.ratio<0.8){bg='linear-gradient(135deg,#0ea5e9,#0369a1)';txt=a.ratio;desc='' + T('ui.akute_last_unter_kapazitaet_spielraum') + '';}
  else if(a.ratio<=1.3){bg='linear-gradient(135deg,#0e9f6e,#056649)';txt=a.ratio;desc='' + T('ui.optimaler_korridor_0_8_1') + '';}
  else if(a.ratio<=1.5){bg='linear-gradient(135deg,#d97706,#92500a)';txt=a.ratio;desc='' + T('ui.erhoeht_du_steigerst_schneller_als') + '';}
  else{bg='linear-gradient(135deg,#e8345c,#9f1239)';txt=a.ratio;desc='' + T('ui.deutlicher_lastsprung_1_5_genau') + '';}
  document.getElementById('acwrBox').innerHTML=`<div class="acwr" style="background:${bg}"><div class="al">Lastsprung-Indikator · ACWR (EWMA)</div><div class="ar">${txt}</div><div class="ad">${desc}${(a.enough&&!_lcc.suppressNumbers)?`<br><span style="opacity:.8;font-size:12px">Akut: ${a.acute} AU · Chronisch: ${a.chronic} AU/Wo</span>`:''}${(_lcc.ctlAtlNote||_lcc.acwrTsbNote)?`<br><span style="opacity:.85;font-size:12px">${[_lcc.acwrTsbNote,_lcc.ctlAtlNote,(_ctlLB!=null?('' + T('ui.bekannte_teilsumme_nur_gemessen_ctl') + ''+_ctlLB+'' + T('ui.au') + ''):null)].filter(Boolean).join(' ')}</span>`:''}</div></div>`;}
function renderInsights(){const days=Object.keys(DB).filter(isDay).sort();let out=[];
  ...
  document.getElementById('insightBox').innerHTML=out.length?out.map(x=>`<div class="insight">${x}</div>`).join(''):'<p class="muted">' + T('ui.mehr_daten_noetig_insights_erscheinen') + '</p>';}
...
function renderBadges(){const days=Object.keys(DB).filter(isDay);
  ...
  const B=[
    {ic:ic('swim'),t:'' + T('ui.erste_200m_am_stueck') + '',d:'Schwimmen',on:days.some(k=>DB[k].sessions&&DB[k].sessions.Schwimmen&&(DB[k].sessions.Schwimmen.long||0)>=200)},
    {ic:ic('stretch'),t:'' + T('ui.14_tage_mobility') + '',d:'' + T('ui.streak_14') + '',on:streak(e=>e&&e.routines&&e.routines.mob)>=14},
    {ic:ic('pulse'),t:'Knie 7T <2',d:'' + T('ui.7_tage_stabil') + '',on:last7.length>=7&&Math.max(...last7)<2},
    {ic:ic('calendar'),t:'' + T('ui.30_check_ins') + '',d:'Konsistenz',on:days.filter(k=>DB[k].morning).length>=30},
    {ic:ic('bike'),t:'' + T('ui.30_km_ride') + '',d:'' + T('ui.distanz_') + '',on:days.some(k=>DB[k].sessions&&DB[k].sessions.Rad&&(DB[k].sessions.Rad.dist||0)>=30)},
    {ic:ic('dumbbell'),t:'' + T('ui.14_tage_squats') + '',d:'Reha-Disziplin',on:streak(e=>e&&e.routines&&e.routines.ss)>=14},
    {ic:ic('run'),t:'Comeback-Lauf',d:'' + T('ui.erster_lauf_geloggt') + '',on:days.some(k=>DB[k].sessions&&DB[k].sessions.Laufen)},
    {ic:ic('flag'),t:'' + T('ui.10_km_am_stueck') + '',d:'Lauf-' + T('ui.distanz_') + '',on:days.some(k=>DB[k].sessions&&DB[k].sessions.Laufen&&(DB[k].sessions.Laufen.dist||0)>=10)}];
...
function renderSegAusdauer(){
  ...
  let nrTxt='' + T('ui.morgen_check_in_noetig_fuer') + '';
  ...
  let ezHtml;
  if(es==null)ezHtml='<p class="muted">' + T('ui.erscheint_ab_6_laeufen_in') + '</p>';
    ...
    ezHtml=`<div class="goal"><div class="goalhead"><span>Easy-Anteil (Ziel ≥80%)</span><span>${pct}%</span></div>
      ...
      <p class="note" style="text-align:left">${ok?'' + T('ui.polarisierung_stimmt_harte_einheiten_bleiben') + '':'' + T('ui.zu_viel_intensitaet_easy_laeufe') + ''}${tooHard?' · '+tooHard+'' + T('ui.easy_zu_hart_hf_78') + '':''}</p>`;}
  ...
  if(!efs.length){killChart('cEF');wrap.innerHTML='<p class="muted" style="padding-top:50px;text-align:center">' + T('ui.braucht_easy_z2_laeufe_mit') + '</p>';}
    ...
    drawLine('cEF',efs.map(p=>new Date(p.date+'T12:00').toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'})),
      [{label:'' + T('ui.ef_easy_z2_m_min') + '',data:efs.map(p=>p.ef),color:'#fb7185'}],{minAuto:true});}
  // Schwimmen
  renderSwimChart();
  // Interferenz heute
...
function renderSwimChart(){
  ...
  if(!pts.length){killChart('cSwim');wrap.innerHTML=gmStateEmpty({icon:'drop',title:'' + T('ui.noch_keine_schwimm_einheiten') + '',desc:'' + T('ui.sobald_du_eine_schwimmeinheit_loggst') + ''});return;}
  ...
  drawBarLine('cSwim',pts.map(p=>new Date(p.k+'T12:00').toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'})),
    {label:'' + T('ui.laengste_am_stueck_m') + '',data:pts.map(p=>p.long),color:'#c9ae7c'},
    {label:'' + T('ui.pace_s_100m') + '',data:pts.map(p=>p.pace),color:'#16a34a'},{goalY:400,goalLabel:'' + T('ui.ziel_400m') + ''});
}
/* --- Erholung --- */
function renderSegErholung(){
  ...
  drawLine('cReady',labels,[{label:'' + T('ui.readiness') + '',data:ready,color:'#b89a60'},{label:'Knie',data:g('knee'),color:'#e11d48',y2:true}],{max:100});
  const hrvMs=g('hrvMs');
  ...
  drawLine('cHRV',labels,[{label:'' + T('ui.hrv_ms') + '',data:hrvMs,color:'#dcc79a'},{label:'' + T('ui.7t') + '',data:hrvAvg,color:'#b89a60'}],{minAuto:true});
  ...
  drawLine('cBB',labels,[{label:'' + T('ui.body_batt') + '',data:g('bb'),color:'#16a34a'},{label:'Ruhepuls',data:g('rhr'),color:'#f59e0b',y2:true}],{max:100});
  const ctx=recoveryCtx(todayStr());
  document.getElementById('recovNote').innerHTML=
    `<div class="insight">Baselines (28T): Ruhepuls ${ctx.rhrBase!=null?Math.round(ctx.rhrBase)+' bpm':'' + T('ui.braucht_7_werte') + ''} · HRV-Datenpunkte ${ctx.hrvN}/14 nötig für Baseline-Score · Schlaf-Konto 7T: ${ctx.sleepDebtH!=null?'−'+ctx.sleepDebtH.toFixed(1)+'h':'–'}</div>`;
}
/* --- Körper --- */
function renderSegKoerper(){
  ...
  drawLine('cWeight',labels,[{label:'kg',data:w,color:'#b89a60'},{label:'' + T('ui.7t') + '',data:wAvg,color:'#16a34a'}],{minAuto:true});
  drawLine('cProt',labels,[{label:'' + T('ui.protein_g') + '',data:S.map(s=>s.e&&s.e.eve?s.e.eve.prot:null),color:'#16a34a'}],{max:200,goal:150});
  ...
  document.getElementById('weightHint').innerHTML=hint
    ...
    :'<p class="muted">' + T('ui.gewichts_trend_erscheint_nach_5') + '</p>';
}
...
function renderHist(){
  flushAuto();
  document.getElementById('histChips').innerHTML=[['alle','Alle'],['lauf','Läufe'],['train','' + T('ui.training') + ''],['notiz','Notizen']]
  ...
  if(!keys.length){document.getElementById('histList').innerHTML='<p class="muted">' + T('ui.keine_passenden_eintraege') + '</p>';return;}
  ...
  document.getElementById('histList').innerHTML=shown.map(k=>{const e=DB[k];const m=e.morning;
    ...
    let bits=[];if(s&&s.score!=='')bits.push(s.score+'%');if(m)bits.push('' + T('ui.knie') + ''+m.knee);
...
function buildAIReview(){
  ...
  const _acwrReason2=_lcValid2?_lcc2.acwrTsbNote:'' + T('ui.acwr_tsb_nicht_belastbar_last') + '';
  ...
  if(es!=null&&es<0.75)warnungen.push('' + T('ui.easy_anteil_nur') + ''+Math.round(es*100)+'% (' + T('ui.ziel__') + '≥75–80%)');
  (goal.vetos||[]).forEach(x=>warnungen.push('' + T('ui.ziel_veto') + ''+x));
  var _g=(typeof goalOf==='function')?goalOf():{};var _zl=((typeof RACE_LABELS_P!=='undefined'&&RACE_LABELS_P[_g.type])||_g.type||'' + T('ui.allgemeine_fitness') + '')+(_g.raceDate?(' · '+_g.raceDate):'');
...
function weekSummaryText(){
  ...
  const lines=[
    '' + T('ui.woche_bis') + ''+r.erstellt+': '+(r.wochenKm.aktuell!=null?(r.wochenKm.aktuell+' km gelaufen'):'' + T('ui.wochen_km_nicht_bestimmbar') + '')+' ('+runs+'' + T('ui.laeufe_soll') + ''+r.wochenKm.soll+'' + T('ui.km_readiness') + ''+(ready.length?Math.round(Calc.avg(ready))+'%':'–')+'.',
    g.state==='nodata'?'' + T('ui.hm_prognose_noch_nicht_belastbar') + ''+g.nQuality+'' + T('ui.quality_laeufe') + '':g.state==='no_target'?'' + T('ui.prognose_') + ''+Calc.fmtTime(g.tPred)+'' + T('ui.keine_zielzeit_hinterlegt') + '':'' + T('ui.hm_prognose') + ''+Calc.fmtTime(g.tPred)+' ('+(g.state==='ontrack'?'on track':g.state==='border'?'grenzwertig':'gefährdet')+'' + T('ui.bei_ziel') + ''+Calc.fmtTime(g.target)+'.',
    r.warnungen.length?'' + T('ui.warnungen') + ''+r.warnungen.join(' | '):'' + T('ui.keine_aktiven_warnungen') + ''];
  return lines.join('\n');
}
function copyAIReview(){
  const j=JSON.stringify(buildAIReview(),null,1);
  var _g=(typeof goalOf==='function')?goalOf():{};var _gl=(typeof RACE_LABELS_P!=='undefined'&&RACE_LABELS_P[_g.type])||_g.type||'' + T('ui.allgemeine_fitness_') + '';
  const prompt='' + T('ui.du_bist_mein_trainings_coach') + ''+_gl+(_g.raceDate?('' + T('ui.datum') + ''+_g.raceDate+')'):'')+'. Analysiere die Woche: größter Engpass, konkrete Anpassung für ' + T('ui.naechste_woche_') + ', Risiken. ' + T('ui.daten') + '\n'+j;
  navigator.clipboard.writeText(prompt).then(()=>toast('' + T('ui.coach_briefing_kopiert') + '')).catch(()=>toast('' + T('ui.kopieren_fehlgeschlagen') + ''));
}
function copySummary(){navigator.clipboard.writeText(weekSummaryText()).then(()=>toast('' + T('ui.zusammenfassung_kopiert') + '')).catch(()=>toast('' + T('ui.kopieren_fehlgeschlagen') + ''));}
...
function renderFormFitnessV5(){
  ...
  try{
    if(!(window.ORVIA&&window.ORVIA.charts&&window.ORVIA.charts.richChart)){
      host.innerHTML=gmStateError({icon:'alert',title:'' + T('ui.diagramm_modul_nicht_geladen') + '',desc:'' + T('ui.die_kurve_erscheint_nach_einem') + ''});return;}
    var ld=allLoads();
    ...
    if(lcc.suppressNumbers){
      ...
      host.innerHTML=gmStateEmpty({icon:'chart',title:'' + T('ui.form_kurve_noch_nicht_belastbar') + '',desc:lcc.ctlAtlNote||'' + T('ui.ctl_atl_nicht_belastbar_die') + ''});return;}
    var S=Calc.loadSeries(ld.loads||[]);var n=(S.ctl||[]).length;
    if(n<14){
      host.innerHTML=gmStateEmpty({icon:'chart',title:'' + T('ui.noch_zu_wenig_lasthistorie') + '',desc:'' + T('ui.erst') + ''+n+'' + T('ui.tage_erfasst_die_form_kurve') + ''});return;}
    var k=Math.min(28,n);
    ...
    var legend='<div class="ffv-legend">'+
      '<span><i style="background:var(--accent,#C9AE7C)"></i>' + T('ui.fitness') + '<b>'+cT+'</b></span>'+
      '<span><i style="background:var(--crit,#F0637A)"></i>' + T('ui.ermuedung') + '<b>'+aT+'</b></span>'+
      '<span><i style="background:var(--ready,#43D693)"></i>' + T('ui.form') + '<b>'+(tT>0?'+':'')+tT+'</b></span></div>';
    var expl=mode==='anfaenger'
      ?'' + T('ui.gold_wie_fit_du_bist') + ''
      :mode==='profi'
        ?'' + T('ui.ctl_ewma_42_der_kanonischen') + ''+k+'' + T('ui.tage_scrubbing_auf_der_fitness') + ''
        :'' + T('ui.fitness_baut_sich_ueber_wochen') + '';
    ...
    window.ORVIA.charts.richChart(document.getElementById('ffvChart'),{
      label:'' + T('ui.fitness_ctl') + '',series:ctl,times:labels,unit:'',color:'gold',
    ...
    host.innerHTML=gmStateError({title:'' + T('ui.form_fitness_konnte_gerade_nicht') + '',retry:'renderFormFitnessV5()',label:'' + T('ui.erneut_versuchen') + ''});}
}
...
var _RCV_TILES=[
  {id:'sleep_duration_min',label:'' + T('ui.schlaf') + '',icon:'zzz'},
...
function _rcvSrc(r){return ({automatic:'' + T('ui.automatisch_geraet') + '',manual:'Manuell',override:'' + T('ui.manuell_korrigiert') + '',estimate:'Schätzung',historical:'Historisch'})[r.source]||'' + T('ui.quelle_unbekannt') + '';}
...
function renderRecoveryTilesV5(){
  ...
  if(!P||typeof P.collect!=='function'){host.innerHTML=gmStateError({icon:'alert',title:'' + T('ui.metrik_modul_nicht_geladen') + '',desc:'' + T('ui.die_erholungswerte_erscheinen_nach_einem') + ''});return;}
...
function _rcvError(host){host.innerHTML=gmStateError({title:'' + T('ui.erholungswerte_konnten_nicht_geladen_werden') + '',desc:'' + T('ui.offline_oder_server_nicht_erreichbar') + '',retry:'renderRecoveryTilesV5()',label:'' + T('ui.erneut_versuchen') + ''});}
function _rcvRender(host,resolved){
  ...
  if(!tiles.length){host.innerHTML=gmStateEmpty({icon:'moon',title:'' + T('ui.noch_keine_erholungswerte_fuer_heute') + '',desc:'' + T('ui.sobald_dein_geraet_werte_liefert') + '',action:'expandCheckinCard()',actionIcon:'activity',label:'' + T('ui.manuell_erfassen') + ''});return;}
  host.innerHTML='<div class="rcv-grid">'+tiles.join('')+'</div>'+
    (mode==='anfaenger'?'<p class="note" style="text-align:left;margin-top:8px">' + T('ui.tippe_eine_kachel_fuer_details') + '</p>':'');}
function openRecoveryMetricSheet(metricId){
  ...
  if(!r||!tdef){if(typeof oModal==='function')oModal(tdef?tdef.label:'Metrik','<p class="muted" style="margin:0">' + T('ui.fuer_diese_metrik_liegt_heute') + '</p>');return;}
  var mode=(typeof uiDetailMode==='function')?uiDetailMode():'fortgeschritten';
  var EXPL={sleep_duration_min:'' + T('ui.gemessene_schlafdauer_der_letzten_nacht_') + '',hrv_ms:'' + T('ui.naechtliche_herzfrequenzvariabilitaet_einzelwerte_schwanken_aussagekraeftig') + '',resting_hr:'' + T('ui.ruhepuls_der_nacht') + '',stress_avg:'' + T('ui.tages_stresswert_deines_geraets_0') + '',body_battery:'' + T('ui.energie_schaetzung_deines_geraets_0') + ''};
  ...
  if(mode!=='anfaenger'){body+='<div class="rcv-sh-meta">' + T('ui.quelle') + '<b>'+escH(_rcvSrc(r))+'</b>' + T('ui.stand') + '<b>'+escH(_rcvWhen(r))+'</b>'+(r.stale?' · <b>veraltet</b> (kein frischer Wert im Gültigkeitsfenster)':'')+'</div>';}
  if(mode==='profi'){body+='<div class="rcv-sh-meta">Roh-' + T('ui.quelle') + ''+escH(r.sourceType||'–')+'' + T('ui.metrik_datum') + ''+escH(r.metricDate||'–')+(r.isOverride?'' + T('ui.manueller_override_aktiv') + '':'')+'</div>';}
  body+='<p class="note" style="text-align:left;margin-top:10px">' + T('ui.anzeige_aus_dem_kanonischen_metrik') + '</p>';
...
function gmStandLbl(r){
  ...
  try{var d=new Date(r.metricDate+'T12:00');return '' + T('ui.stand__') + ''+d.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'});}catch(_){return '' + T('ui.stand__') + ''+gmEsc(String(r.metricDate));}
}
...
function gmStressDistribution(pts){
  try{
    ...
    return '<div style="margin-top:12px"><div class="bh">' + T('ui.stressanalyse_gemessene_anteile') + '</div>'+buckets.map(function(bk,b){
...
function gmBbBalance(pts){
  try{
    ...
    return '<div style="display:flex;gap:10px;margin:0 0 10px">'+
      '<div style="flex:1;background:var(--ready-t);border:1px solid rgba(67,214,147,.28);border-radius:14px;padding:13px"><div style="font-size:11px;color:var(--muted);font-weight:700">' + T('ui.aufgeladen') + '</div><div style="font-size:22px;font-weight:800;color:var(--ready);margin-top:4px">+'+Math.round(up)+'</div><div style="font-size:10.5px;color:var(--muted)">' + T('ui.summe_der_anstiege_gespeicherte_kurve') + '</div></div>'+
      '<div style="flex:1;background:var(--crit-t);border:1px solid rgba(240,99,122,.28);border-radius:14px;padding:13px"><div style="font-size:11px;color:var(--muted);font-weight:700">' + T('ui.verbraucht') + '</div><div style="font-size:22px;font-weight:800;color:var(--crit);margin-top:4px">−'+Math.round(down)+'</div><div style="font-size:10.5px;color:var(--muted)">' + T('ui.summe_der_rueckgaenge_gespeicherte_kurve') + '</div></div></div>';
  }catch(_){return '';}
}
...
function gmReadinessDeltas(todayScore){
  ...
  var ysLabel=ysAgo<=1?'vs. gestern':('vs. vor '+ysAgo+'' + T('ui.tagen_') + '');
  ...
  return [mk(ys!=null?todayScore-ys:null,ysLabel),mk(avg!=null?todayScore-avg:null,'' + T('ui.vs_14_t') + '')];
}
...
function gmReadinessBreakdown(os){
  ...
  return [['Ausgangswert',null,'neutral','—'],['' + T('ui.schlaf') + '',null,'sleep','—'],['HRV',null,'ready','—'],['Ruhepuls',null,'ready','—'],['' + T('ui.energie_subj') + '',null,'cyan','—'],['Belastung',null,'activity','—'],['Schmerzen',null,'crit','—']];
}
...
function gmConfVM(){
  try{var c=dataConfidence();
    ...
    return {levelLabel:c.level.l,levelColor:c.level.c==='g'?'ready':c.level.c==='y'?'attention':'crit',
      complete:c.ci+'' + T('ui.check_ins') + ''+c.acts+'' + T('ui.aktivitaeten') + ''+c.n+'' + T('ui.tage_') + '',
      sd:sd,note:c.msg+(bs==='active'?'' + T('ui.persoenliche_baseline_aktiv') + '':bs==='building'?'' + T('ui.baseline_wird_aufgebaut') + '':''),pct:null};
...
function gmLoadContrib(){
  ...
  try{var L=(typeof allLoads==='function')?allLoads():null;if(!L||!L.loads||!L.loads.length)return null;
    ...
    for(var i=len-1;i>=0&&rows.length<3;i--){
      ...
      rows.push([types.length?types.join(' + '):'' + T('ui.training') + '',wd+(types.length?'':'' + T('ui.tageslast') + ''),fmtDe(Math.round(v)),'activity']);
    }
...
function gmDevProviderName(){
  return ({garmin:'Garmin',strava:'Strava',apple_health:'' + T('ui.apple_health') + '',applehealth:'' + T('ui.apple_health') + ''})[String(_gmDevSync.provider||'').toLowerCase()]||_gmDevSync.provider;
}
...
function gmDeviceSyncText(){
  ...
  if(_gmDevSync.state==='ready'&&_gmDevSync.provider){
    ...
    if(_gmDevSync.reauth)return provName+'' + T('ui.neuanmeldung_erforderlich') + '';
    var rel=F&&F.fmtRelTime?F.fmtRelTime(_gmDevSync.lastSyncAt):null;
    return rel?(provName+' · '+rel+' synchronisiert'):(provName+' · verbunden');
  }
  if(_gmDevSync.state==='none')return '' + T('ui.kein_geraet_verbunden') + '';
...
function gmApplySyncLine(){
  try{var stx=document.getElementById('syncTxt'),sl=document.getElementById('syncLine');
    ...
    if(stx&&typeof window.orviaSyncState==='function'){var st=window.orviaSyncState();
      stx.textContent=({local:'' + T('ui.lokaler_modus') + '',synced:'' + T('ui.cloud_synchronisiert') + '',pending:'' + T('ui.cloud_sync_laeuft') + '',error:'Sync-Fehler',offline:'' + T('ui.offline_lokal') + ''})[st]||'' + T('ui.lokaler_modus') + '';
    ...
    if(wk&&wk.bySport){
      ...
      out.sport=E.create({metricId:'training_load_by_sport',
        ...
        provenance:{method:'duration_share_by_sport',version:'1.0.0',sources:['garmin','manual'],
          assumptions:[]},
        reason:tot>0?null:'' + T('ui.keine_einheiten_mit_bekannter_dauer') + ''});
    }
  }catch(_){ }
  try{
    ...
    out.hard=E.create({metricId:'hard_sessions_share',
      ...
      reason:withRpe.length?null:'' + T('ui.keine_einheiten_mit_rpe_in') + ''});
    ...
    out.trimp=E.create({metricId:'trimp_avg',
      ...
      provenance:{method:'banister_trimp',version:'1.0.0',sources:['garmin','manual'],
        inputs:{formula:'' + T('ui.min_hrr_0_64_e') + '',k:k,restingHr:rhr,maxHr:hrMax,
        ...
        assumptions:assum},
      reason:(hrMax==null||rhr==null)?'' + T('ui.ohne_gemessenen_ruhepuls_bzw_hfmax') + '':(trs.length?null:'' + T('ui.keine_einheiten_mit_hf') + '')});
    ...
    try{
      ...
      var interfTxt=null;
      if(eli){interfTxt=eli.notes&&eli.notes.length?eli.notes.join(' + ')+(eli.spikePct!=null?' (+'+eli.spikePct+' % ' + T('ui.ueber') + ' 7-' + T('ui.tage__') + '-Schnitt)':''):'' + T('ui.keine_auffaelligkeit_lastsprung_bein_interferenz') + '';}
      out.interf=E.create({metricId:'load_interference',
        ...
        reason:(l3==null||l7==null)?'' + T('ui.tageslast_serie_fuer_3_7') + '':null});
    }catch(_){ }
    ...
    try{
      ...
      if(esd)out.easy=E.create({metricId:'easy_share',
        ...
        reason:esd.share==null?'' + T('ui.unter_6_klassifizierbare_laeufe_in') + '':null});
    }catch(_){ }
  }catch(_){ }
  return out;
}
...
function gmDashVM(){
  ...
  var lead=os?(os.dayState==='GREEN'?'' + T('ui.trainieren_kontrolliert_bleiben') + '':os.dayState==='YELLOW'?'' + T('ui.reduzieren_empfohlen') + '':os.dayState==='ORANGE'?'' + T('ui.anpassen_ersatztraining') + '':'' + T('ui.regeneration_priorisieren') + ''):'' + T('ui.check_in_ausstehend') + '';
  ...
  var pillWord=os?((os.statusText==='Peak')?'Peak':({GREEN:'Bereit',YELLOW:'Moderat',ORANGE:'' + T('ui.anpassen') + '',RED:'' + T('ui.erholung_') + ''})[os.dayState]||os.status.l):'Check-in';
  ...
  if(!why.length)why=[os?'' + T('ui.stabile_werte_nichts_auffaelliges') + '':'' + T('ui.2_minuten_morgen_check_in') + ''];
  ...
  var wk=null;try{if(typeof isRunDistanceGoal==='function'&&isRunDistanceGoal()&&g&&g.raceDate)wk='' + T('ui.woche__') + ''+Math.max(1,Math.min(25,Calc.runnaWeek(daysTo(RACE.date))))+' / 25';}catch(_){ }
  var goalName=null;try{goalName=gmGoalLabel(g);}catch(_){ }
  var ciVals=[];
  if(ciDone){if(m.sleepMin!=null)ciVals.push(['' + T('ui.schlaf') + '',fmtDe(m.sleepMin/60)+' h']);
  ...
  var pain=(m&&m.knee!=null&&m.knee>=1)?{region:'Knie',level:fmtDe(m.knee)+'/10 gemeldet',note:'' + T('ui.aus_deinem_morgen_check_in') + ''}:null;
  ...
  try{var pz=(typeof pauseFor==='function')?pauseFor(todayStr()):null;if(pz)warnings.push(['moon','' + T('ui.pause_aktiv') + '',(pz.reason||'Pause')+'' + T('ui.der_plan_pausiert_bis') + ''+(pz.to||'' + T('ui.auf_weiteres') + '')+'.']);}catch(_){ }
  var hrv=gmMetric('hrv_ms'),rhr=gmMetric('resting_hr'),sl=gmMetric('sleep_duration_min'),str=gmMetric('stress_avg');
  var _vm={
    hasScore:!!os,score:os?os.score:null,status:os?os.status.l:'' + T('ui.check_in_ausstehend') + '',statusColor:stC,pillWord:pillWord,
    simpleStatus:os?os.status.l:'' + T('ui.check_in_ausstehend') + '',lead:lead,simpleLead:lead,
    ...
    reco:{cls:stC==='neutral'?'attention':stC,ic:stC==='ready'?'bolt':stC==='crit'?'shield':'gauge',
      t:rec?rec.label:(os?lead:'' + T('ui.check_in_ausfuellen') + ''),d:rec?(rec.detail||''):'' + T('ui.danach_steht_deine_konkrete_empfehlung') + ''},
    simpleReco:null,
    ...
    session:{name:rec?rec.label:(u?u.l:'' + T('ui.training') + ''),detail:rec?(rec.detail||''):(u?u.t:'')},
    ...
    goal:(function(){
      ...
      try{var bt=(typeof bestTimes==='function')?bestTimes():null;
        if(bt&&bt.t10!=null&&g&&/half|hm/i.test(String(g.type||''))&&Calc.riegelHM){
          ...
          if(hmMin!=null)pred='' + T('ui.prognose__') + ''+Calc.fmtTime(hmMin);}}catch(_){ }
...
function gmHero(d){
  ...
  var scoreRing='<div class="ring-wrap" role="button" tabindex="0" aria-label="' + T('ui.orvia_score') + ', ' + T('ui.details_') + ' öffnen" onclick="openScore()" onkeydown="if(event.key===\'Enter\')openScore()">'+ringHtml+
    '<div class="ring-c"><div class="big">'+(d.hasScore?gmEsc(String(d.score)):'—')+'</div><div class="u">'+(lvl==='a'?'' + T('ui.dein_score') + '':'ORVIA-SCORE')+' '+icon('chev','xs')+'</div>'+
  ...
  if(lvl==='p'){
    reco='<div class="reco flow '+d.reco.cls+'"><div class="rc-ic">'+icon(d.reco.ic)+'</div><div><div class="rt">'+gmEsc(d.reco.t)+'</div><div class="prescription">'+d.pro+'</div></div></div>'+
      '<div class="adjust"><div class="adjust-h">' + T('ui.empfehlung_anpassen') + '</div><div class="adjust-row">'+
      ['gauge|' + T('ui.intensitaet') + '','calendar|Dauer','chart|Volumen','run|Sportart','target|' + T('ui.prioritaet') + ''].map(function(x){var p=x.split('|');
      ...
      (d.changelog?'<div class="changelog">'+icon('pen','sm')+'<div><b>' + T('ui.zuletzt_geaendert') + '</b> '+gmEsc(d.changelog.from)+' → '+gmEsc(d.changelog.to)+'' + T('ui.grund') + ''+gmEsc(d.changelog.reason)+'</div></div>':'')+'</div>';
  }else{
...
function renderCheckinCompact(){try{
  ...
  if(lvl==='a'&&!d.ciDone){
    ...
    var MOODS=[['😃','Top','top'],['🙂','' + T('ui.geht_so') + '','ok'],['😴','Müde','tired']];
    box.innerHTML='<div class="ci-simple"><div class="q">' + T('ui.wie_fuehlst_du_dich_heute') + '</div><div class="moods">'+
  ...
  if(!d.ciDone){
    box.innerHTML='<div class="checkin"><div class="ci-ic" style="background:var(--attention-t);color:var(--attention)">'+icon('pen')+'</div>'+
      '<div class="ci-b"><div class="ci-t">' + T('ui.morgen_check_in') + '</div><div class="ci-s">'+(lvl==='a'?'' + T('ui.kurz_ausfuellen_dann_steht_deine') + '':'' + T('ui.noch_nicht_ausgefuellt_verbessert_die') + '')+'</div></div>'+
      '<span class="ci-pill ci-open">' + T('ui.offen') + '</span></div>';
  }else{
    box.innerHTML='<div class="checkin"><div class="ci-ic" style="background:var(--ready-t);color:var(--ready)">'+icon('check')+'</div>'+
      '<div class="ci-b"><div class="ci-t">' + T('ui.morgen_check_in') + '</div><div class="ci-s">' + T('ui.heute_erledigt_fliesst_in_den') + '</div></div>'+
      '<span class="ci-pill ci-done">' + T('ui.erledigt_') + '</span></div>'+
...
function gmModRecovery(d){var M=d.metrics;
  return '<div '+gmTap('sleep_duration_min')+'><span class="taphint">'+icon('chev','xs')+'</span><div class="kh">'+icon('moon')+' ' + T('ui.schlaf') + '</div><div class="kv">'+(M.sleepScore!=null?gmEsc(fmtDe(M.sleepScore))+'<small>/100</small>':'—')+'</div><div class="kd flat">'+(M.sleepMin!=null?gmEsc(fmtDe(M.sleepMin/60))+' h geschlafen':GM_NA)+'</div><div class="bar-mini"><i style="width:'+(M.sleepScore!=null?M.sleepScore:0)+'%;background:var(--sleep)"></i></div><div style="font-size:10.5px;color:var(--muted);font-weight:650;margin-top:7px">'+(M.sleepMin!=null?gmEsc(fmtDe(M.sleepMin/60))+' h'+(function(){var _dp=gmMetric('sleep_deep_min');return (_dp&&_dp.value!=null)?'' + T('ui.tief') + ''+gmEsc(fmtDe(Math.round(_dp.value)))+' min':'' + T('ui.phasen') + '';})():'—')+'</div></div>'+
  '<div class="kcard tap" role="button" tabindex="0" onclick="openRecoverySheet()" onkeydown="if(event.key===\'Enter\')openRecoverySheet()"><span class="taphint">'+icon('chev','xs')+'</span><div class="kh">'+icon('heart')+' ' + T('ui.erholung_') + '</div><div class="kv">'+(M.recovery!=null?gmEsc(fmtDe(M.recovery))+'<small>%</small>':'—')+'</div><div class="kd flat">'+(M.hrv!=null?'' + T('ui.hrv') + ''+gmEsc(fmtDe(M.hrv))+' ms':GM_NA)+'</div><div class="bar-mini"><i style="width:'+(M.recovery!=null?M.recovery:0)+'%;background:var(--ready)"></i></div><div style="font-size:10.5px;color:var(--muted);font-weight:650;margin-top:7px">'+(M.rhr!=null?'' + T('ui.ruhepuls') + ''+gmEsc(fmtDe(M.rhr))+' bpm':GM_NA)+'</div></div>';}
function gmModVitals(d){var M=d.metrics;
  var chart=M.hrvSeries?sparkline(M.hrvSeries.slice(-8),SC.ready):'<div class="spark" style="display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--faint)">' + T('ui.verlauf') + ''+GM_NA.toLowerCase()+'</div>';
  return '<div class="kcard tap" role="button" tabindex="0" style="grid-column:span 2" onclick="openMetric(\'hrv_ms\')" onkeydown="if(event.key===\'Enter\')openMetric(\'hrv_ms\')"><span class="taphint">'+icon('chev','xs')+'</span><div class="ctitle" style="margin-bottom:8px"><div class="l">'+icon('pulse')+' ' + T('ui.hrv') + '&amp; Ruhepuls</div><span class="kd flat">'+(M.hrv!=null?'heute':'—')+'</span></div>'+
    '<div style="display:flex;gap:14px;align-items:flex-end"><div style="flex:1">'+chart+'</div><div style="text-align:right"><div style="font-size:22px;font-weight:800;font-variant-numeric:tabular-nums">'+(M.hrv!=null?gmEsc(fmtDe(M.hrv))+' ms':'—')+'</div><div style="font-size:10.5px;color:var(--muted);font-weight:700">' + T('ui.hrv_heute') + '</div></div></div>'+
    '<div style="display:flex;justify-content:space-between;font-size:10.5px;color:var(--muted);font-weight:650;margin-top:8px"><span>'+(function(){
      ...
      try{var _c=recoveryCtx(todayStr());if(_c&&_c.hrvBase7!=null)return '' + T('ui.baseline') + ''+Math.round(Math.exp(_c.hrvBase7))+' ms';}catch(_){ }
      return '' + T('ui.baseline_') + '';})()+'</span><span>' + T('ui.ruhepuls') + ''+(M.rhr!=null?gmEsc(fmtDe(M.rhr))+' bpm':'—')+'</span></div></div>';}
function gmModVitalsFull(d){var M=d.metrics;
  var chart=M.hrvSeries?sparkline(M.hrvSeries,SC.ready):'<div class="spark" style="display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--faint)">' + T('ui.verlauf') + ''+GM_NA.toLowerCase()+'</div>';
  ...
  return '<div class="card tap" role="button" tabindex="0" onclick="openMetric(\'hrv_ms\')" onkeydown="if(event.key===\'Enter\')openMetric(\'hrv_ms\')"><div class="ctitle"><div class="l">'+icon('pulse')+' Vitalwerte</div><span class="more">' + T('ui.details_') + ' '+icon('chev','xs')+'</span></div>'+
    '<div style="display:flex;gap:14px;align-items:flex-end"><div style="flex:1">'+chart+'</div><div style="text-align:right"><div style="font-size:24px;font-weight:800;font-variant-numeric:tabular-nums">'+(M.hrv!=null?gmEsc(fmtDe(M.hrv))+' ms':'—')+'</div><div style="font-size:10.5px;color:var(--muted);font-weight:700">' + T('ui.hrv') + ''+(M.hrv!=null?gmEsc(M.hrvLbl||'—'):'—')+'</div></div></div>'+
    '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:13px;padding-top:13px;border-top:1px solid var(--hair)">'+cell(M.rhr,'Ruhepuls')+cell(null,'HRR')+cell(M.vo2max,M.vo2maxCycling!=null?'' + T('ui.vo_max_lauf') + '':'VO₂max')+cell(M.respiration,'Atmung')+(M.vo2maxCycling!=null?cell(M.vo2maxCycling,'' + T('ui.vo_max_rad') + ''):'')+'</div></div>';}
function gmModStress(d){var M=d.metrics;
  var chart=M.stressSeries?sparkline(M.stressSeries,SC.ready):'<div class="spark" style="display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--faint)">' + T('ui.verlauf') + ''+GM_NA.toLowerCase()+'</div>';
...
function gmModLoadPro(d){var L=d.load;
  ...
  return '<div class="card mod-wide tap" role="button" tabindex="0" onclick="openMetric(\'load\')" onkeydown="if(event.key===\'Enter\')openMetric(\'load\')"><div class="ctitle"><div class="l">'+icon('gauge')+' Belastungssteuerung</div><span class="more">' + T('ui.details_') + ' '+icon('chev','xs')+'</span></div>'+
    '<div class="body"><div class="ring-wrap" style="width:74px;height:74px">'+ring(L.atl!=null?Math.min(L.atl/85*100,100):0,'var(--activity)',74,8)+'<div class="ring-c"><div style="font-size:19px;font-weight:800;font-variant-numeric:tabular-nums">'+(L.atl!=null?gmEsc(fmtDe(L.atl)):'—')+'</div><div style="font-size:9px;color:var(--muted);font-weight:700">' + T('ui.atl_') + '</div></div></div>'+
    '<div class="load-stats"><div class="ls"><div class="n">'+(L.ctl!=null?gmEsc(fmtDe(L.ctl)):'—')+'</div><div class="l">' + T('ui.ctl__') + '</div></div><div class="ls"><div class="n">'+(L.tsb!=null?gmEsc(L.tsb):'—')+'</div><div class="l">' + T('ui.tsb__') + '</div></div><div class="ls"><div class="n">'+(L.acwr!=null?gmEsc(String(L.acwr)):'—')+'</div><div class="l">' + T('ui.acwr__') + '</div></div></div></div>'+
    '<div class="zoneband">'+(L.band!=null?'<div class="mk" style="left:'+L.band+'%"></div>':'')+'</div><div class="range-lbl"><span>' + T('ui.erhaltung') + '</span><span>'+(L.status?gmEsc(L.status):'—')+'</span><span>Überlastet</span></div>'+
    ...
    '<div class="datarow">'+dc(L.mono,'Monotonie')+dc(L.strain,'' + T('ui.strain_wo') + '')+dc(L.trimp,'' + T('ui.trimp') + '')+dc(L.hi,'' + T('ui.harte_einheiten') + '')+'</div>'+
    ...
    (function(){var env=L.env;if(!env)return '';
      var parts=[];
      [['hard','' + T('ui.harte_einheiten') + '','' + T('ui.einheiten_') + ''],['trimp','TRIMP','' + T('ui.einheiten_') + '']].forEach(function(x){
        var e=env[x[0]];if(e&&e.status==='partial')parts.push(x[1]+' aus '+e.coverage.available+' von '+e.coverage.eligible+' '+x[2]);});
      if(!parts.length)return '';
      return '<div class="interf">'+icon('alert','xs')+' <b>' + T('ui.teilabdeckung') + '</b> '+gmEsc(parts.join(' · '))+' · letzte 7 ' + T('ui.tage__') + ' — ' + T('ui.details_') + ' im Last-Sheet.</div>';})()+
    '<div style="font-size:10.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--faint);font-weight:800;margin-top:14px">' + T('ui.belastung_nach_sportart') + '</div>'+
    ...
    '<div class="interf">'+icon('info','xs')+' <b>' + T('ui.interferenz') + '</b> '+(L.interf?gmEsc(L.interf):GM_NA)+'</div></div>';}
function gmModReadinessPro(d){
  ...
  return '<div class="card tap" role="button" tabindex="0" onclick="openScore()" onkeydown="if(event.key===\'Enter\')openScore()"><div class="ctitle"><div class="l">'+icon('target')+' ' + T('ui.readiness_') + '&amp; ' + T('ui.konfidenz__') + '</div><span class="more">'+(d.conf.levelLabel?gmEsc(d.conf.levelLabel)+' ':'')+icon('chev','xs')+'</span></div>'+
    '<div class="breakdown">'+rows+'</div>'+
    '<div class="datarow"><div class="datacell"><div class="dl">' + T('ui.datenkonfidenz') + '</div><div class="dn" style="color:'+(SC[d.conf.levelColor]||'var(--muted)')+'">'+(d.conf.levelLabel?gmEsc(d.conf.levelLabel):'—')+'</div></div><div class="datacell"><div class="dl">' + T('ui.baseline_abw') + '</div><div class="dn">'+(d.conf.sd!=null?gmEsc(d.conf.sd):'—')+'</div></div></div>'+
    '<div class="interf" style="margin-top:8px">'+icon('db','xs')+' <b>' + T('ui.daten') + '</b> '+gmEsc(d.conf.complete!=null?d.conf.complete:GM_NA)+'. '+gmEsc(d.conf.note||'')+'</div></div>';}
function gmModNext(d){
  return '<div class="card"><div class="ctitle"><div class="l">'+icon('calendar')+' Bevorstehendes ' + T('ui.training') + '</div></div><div class="next-row"><div class="next-ic">'+icon('run')+'</div><div class="next-b"><div class="next-t">'+(d.next?gmEsc(d.next.t):'—')+'</div><div class="next-s">'+(d.next?gmEsc(d.next.s):'' + T('ui.kein_eintrag_im_wochenplan') + '')+'</div></div><span class="next-tag">'+(d.next?gmEsc(d.next.tag):'—')+'</span></div></div>';}
function gmModNextSimple(d){
  return '<div class="card"><div class="simplecard"><div class="sc-ic" style="background:var(--activity-t);color:var(--activity)">'+icon('run')+'</div><div><div class="sc-t">' + T('ui.naechstes_training') + '</div><div class="sc-v">'+(d.next?gmEsc(d.next.t):'' + T('ui.kein_eintrag_im_wochenplan') + '')+'</div></div><span class="next-tag" style="margin-left:auto">'+(d.next?gmEsc(d.next.tag):'—')+'</span></div></div>';}
function gmModGoal(d){
  return '<div class="card"><div class="ctitle"><div class="l">'+icon('target')+' Ziel-Fortschritt</div><span class="more">'+gmEsc(d.goal.wk)+'</span></div><div style="font-size:15px;font-weight:800">'+gmEsc(d.goal.name)+'</div><div class="goalbar"><i style="width:'+(d.goal.pct!=null?d.goal.pct:0)+'%"></i></div><div class="goalmeta"><span>'+(d.goal.pct!=null?gmEsc(fmtDe(d.goal.pct))+'% des Aufbaus':'' + T('ui.fortschritt') + '')+'</span><span>'+arrow(d.goal.predD)+' '+gmEsc(d.goal.pred)+'</span></div></div>';}
...
function gmModPain(d){
  if(!d.pain)return '<div class="card"><div class="simplecard"><div class="sc-ic" style="background:var(--ready-t);color:var(--ready)">'+icon('check')+'</div><div><div class="sc-t">' + T('ui.keine_beschwerden') + '</div><div class="sc-v">' + T('ui.nichts_gemeldet_weiter_so') + '</div></div></div></div>';
...
function gmModNutrition(d){
  ...
  if(!t){
    return '<div class="card tap gm-ext" role="button" tabindex="0" onclick="openNutritionEditor()" onkeydown="if(event.key===\'Enter\')openNutritionEditor()"><span class="taphint">'+icon('chev','xs')+'</span>'+
      ...
      '<div style="flex:1"><div class="sc-t">' + T('ui.energie_amp_ernaehrung') + '</div><div class="sc-v">' + T('ui.koerperdaten_fehlen_dann_rechnet_orvia') + '</div></div></div></div>';
  }
  var pk=t.protein*4,ck=t.carbs*4,fk=t.fat*9,tot=pk+ck+fk;
  var pct=function(v){return tot>0?Math.round(v/tot*100):0;};
  var dtL={rest:'' + T('ui.ruhetag_') + '',easy:'' + T('ui.lockerer_tag') + '',quality:'' + T('ui.intensiver_tag') + '',long:'Long-Run-Tag',strength:'Krafttag'}[t.dayType]||'';
  return '<div class="card tap gm-ext" role="button" tabindex="0" onclick="gmOpenNutritionSheet()" onkeydown="if(event.key===\'Enter\')gmOpenNutritionSheet()"><span class="taphint">'+icon('chev','xs')+'</span>'+
    '<div class="ctitle"><div class="l">'+icon('battery')+' ' + T('ui.energie_amp_ernaehrung') + '</div><span class="more">'+gmEsc(dtL)+'</span></div>'+
    '<div class="nu-hero"><b>'+gmEsc(fmtDe(t.kcal))+'</b><span>kcal Tagesziel'+(t.burn?' · inkl. ~'+gmEsc(fmtDe(t.burn))+'' + T('ui.kcal_training') + '':'')+'</span></div>'+
    ...
    '<div class="nu-legend">'+
      '<span><i class="nu-p"></i>' + T('ui.protein') + '<b>'+gmEsc(fmtDe(t.protein))+' g</b></span>'+
      '<span><i class="nu-c"></i>' + T('ui.carbs') + '<b>'+gmEsc(fmtDe(t.carbs))+' g</b></span>'+
      '<span><i class="nu-f"></i>' + T('ui.fett') + '<b>'+gmEsc(fmtDe(t.fat))+' g</b></span></div>'+
    (wk?'<div class="nu-foot"><span>' + T('ui.protein_ziel_diese_woche') + '</span><b>'+wk.proteinDays+'/7 ' + T('ui.tage__') + '</b></div>':'')+
    '</div>';
}
...
function gmOpenNutritionSheet(){
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--activity-t);color:var(--activity)">'+icon('battery')+'</div><div><h3>' + T('ui.energie_amp_ernaehrung') + '</h3><div class="sh-sub" style="margin:2px 0 0">'+(t?fmtDe(t.kcal)+'' + T('ui.kcal_tagesziel') + '':GM_NA)+'</div></div></div>'+
    (t?('<div class="sh-block"><div class="nu-legend nu-legend-lg">'+
      '<span><i class="nu-p"></i>' + T('ui.protein') + '<b>'+gmEsc(fmtDe(t.protein))+' g</b></span>'+
      '<span><i class="nu-c"></i>' + T('ui.carbs') + '<b>'+gmEsc(fmtDe(t.carbs))+' g</b></span>'+
      '<span><i class="nu-f"></i>' + T('ui.fett') + '<b>'+gmEsc(fmtDe(t.fat))+' g</b></span></div></div>'):'')+
    '<div class="sh-block"><h4 style="margin:0 0 8px;font-size:13px">' + T('ui.protein_letzte_14_tage') + '</h4>'+
      (got?('<div class="nu-bars">'+bars+'</div><p class="note" style="text-align:left;margin-top:8px">'+got+'' + T('ui.von_14_tagen_erfasst') + ''+(pT?'' + T('ui.ziel') + ''+fmtDe(pT)+'' + T('ui.g_gruen_mindestens_90_erreicht') + '':'')+'. Nur eingetragene Abendwerte — nichts hochgerechnet.</p>')
        :'<p class="note" style="text-align:left">' + T('ui.noch_keine_protein_eintraege_der') + '</p>')+'</div>'+
    (wk&&wk.weightTrend?'<div class="sh-block"><div class="nu-foot"><span>' + T('ui.gewicht_7_tage') + '</span><b>'+gmEsc(wk.weightTrend)+'</b></div></div>':'')+
    '<div class="sheet-cta"><button class="sec" onclick="gmCloseSheets()">' + T('ui.schliessen') + '</button><button class="prim" onclick="gmCloseSheets();openNutritionEditor()">' + T('ui.ernaehrung_anpassen') + '</button></div>';
  gmOpenSheet('detailSheet');
}
...
function gmModEvening(d){
  ...
  var done=filled>0;
  var bits=[];
  try{if(ev){if(ev.prot!=null)bits.push(fmtDe(ev.prot)+'' + T('ui.g_protein') + '');if(ev.knee!=null)bits.push('' + T('ui.knie') + ''+ev.knee);if(ev.note)bits.push('Notiz');}}catch(_){ }
  return '<div class="card tap gm-ext" role="button" tabindex="0" onclick="gmGotoEveningCheckin()" onkeydown="if(event.key===\'Enter\')gmGotoEveningCheckin()"><span class="taphint">'+icon('chev','xs')+'</span>'+
    ...
    '<div style="flex:1"><div class="sc-t">' + T('ui.abend_check_in') + '</div><div class="sc-v">'+(done?gmEsc(bits.length?bits.join(' · '):'erfasst'):'' + T('ui.noch_offen_protein_beschwerden_tagesnotiz') + '')+'</div></div></div></div>';
}
...
function gmModSupplements(d){
  ...
  var sub;
  if(!act&&!subs)sub='' + T('ui.noch_nichts_eingerichtet_routinen_und') + '';
  else if(open>0)sub=doneN+' von '+act+'' + T('ui.routinen_erledigt') + ''+(subs?' · '+subs+'' + T('ui.supplements') + '':'');
  else sub=(act?'' + T('ui.alle') + ''+act+'' + T('ui.routinen_erledigt') + '':'' + T('ui.keine_routinen') + '')+(subs?' · '+subs+'' + T('ui.supplements') + '':'');
  ...
  return '<div class="card tap gm-ext" role="button" tabindex="0" onclick="gmGotoRoutines()" onkeydown="if(event.key===\'Enter\')gmGotoRoutines()"><span class="taphint">'+icon('chev','xs')+'</span>'+
    ...
    '<div style="flex:1"><div class="sc-t">' + T('ui.routinen_amp_supplements') + '</div><div class="sc-v">'+gmEsc(sub)+'</div>'+
...
function gmLoadingMods(){var k='<div class="kcard"><div class="sk" style="height:12px;width:60%;margin-bottom:12px"></div><div class="sk" style="height:26px;width:50%;margin-bottom:9px"></div><div class="sk" style="height:30px"></div></div>';
  ...
  return '<div class="card"><div class="sk" style="height:14px;width:' + T('ui.45') + '%;margin-bottom:14px"></div><div class="sk" style="height:44px"></div></div><div class="kgrid">'+g+'</div>';}
function gmErrorBar(){return '<div class="errbar">'+icon('wifi','sm')+'<div><b>' + T('ui.offline_zwischengespeicherter_stand') + '</b>' + T('ui.werte_werden_aktualisiert_sobald_die') + '</div></div>';}
function gmEmptyHero(){return '<div class="hero gap"><div class="empty" style="padding:24px 14px"><div class="e-ic" style="width:64px;height:64px;border-radius:20px">'+icon('db')+'</div><div class="et" style="font-size:15px">' + T('ui.noch_keine_bereitschaft_berechnet') + '</div><div class="ed"><b style="color:var(--activity)">' + T('ui.das_ist_kein_schlechter_wert') + '</b>' + T('ui.es_fehlen_nur_daten_orvia') + '</div><div class="eb" style="margin-top:16px" onclick="gotoCheckinForm()">'+icon('bolt','sm')+' Check-in starten</div></div></div>'+
  '<div class="gapnote">'+icon('info','sm')+'<div><b>' + T('ui.datenluecke_schlechter_score') + '</b>' + T('ui.ein_niedriger_score_heisst_schlecht') + '</div></div>';}
function gmEmptyMods(){return '<div class="card"><div class="empty"><div class="e-ic">'+icon('moon')+'</div><div class="et">' + T('ui.schlaf_amp_erholung') + '</div><div class="ed">' + T('ui.verbinde_ein_geraet_oder_erfasse') + '</div><div class="eb" onclick="expandCheckinCard()">'+icon('activity','sm')+' ' + T('ui.manuell_erfassen') + '</div></div></div>';}
...
function gmStateLoading(o){
  ...
  var body=kc
    ...
    : '<div class="sk" style="height:14px;width:' + T('ui.45') + '%;margin-bottom:14px"></div><div class="sk" style="height:44px"></div>';
...
function gmStateEmpty(o){
  o=o||{};
  return '<div class="card"><div class="empty"><div class="e-ic">'+icon(o.icon||'db')+'</div><div class="et">'+gmEsc(o.title||'' + T('ui.noch_keine_daten') + '')+'</div><div class="ed">'+gmEsc(o.desc||'')+'</div>'+
    (o.action?'<div class="eb" onclick="'+o.action+'">'+icon(o.actionIcon||'bolt','sm')+' '+gmEsc(o.label||'')+'</div>':'')+'</div></div>';
}
function gmStateError(o){
  o=o||{};
  return '<div class="errbar">'+icon(o.icon||'wifi','sm')+'<div><b>'+gmEsc(o.title||'' + T('ui.momentan_nicht_verfuegbar') + '')+'</b>'+(o.desc?' '+gmEsc(o.desc):'')+'</div></div>'+
    ...
    (o.retry?'<button class="cta wide-ghost" style="margin-top:14px" onclick="'+o.retry+'">'+icon('wifi','sm')+' '+gmEsc(o.label||'' + T('ui.erneut_versuchen') + '')+'</button>':'');
}
...
function gmErrorHero(){var d=gmDashVM();var sc=d.hasScore?d.score:null;
  return '<div class="hero"><div class="hero-top"><div class="ring-wrap" style="width:150px;height:150px;opacity:.55">'+ring(sc!=null?sc:0,'var(--neutral)',150,12)+'<div class="ring-c"><div class="big" style="color:var(--muted)">'+(sc!=null?sc:'—')+'</div><div class="u">' + T('ui.zuletzt_') + '</div></div></div><div class="hero-right"><div class="lead" style="color:var(--muted)">' + T('ui.zwischengespeicherter_stand') + '</div><div class="why">' + T('ui.werte_koennten_veraltet_sein_pruefe') + '</div></div></div>'+
  '<button id="gmRetryBtn" class="cta wide-ghost" style="margin-top:14px;width:100%" onclick="renderDay&&renderDay()">'+icon('wifi','sm')+' ' + T('ui.erneut_versuchen') + '</button></div>';}
...
function renderCommand(){
  ...
  try{var mb=document.getElementById('modeBadge');if(mb){var l=gmLevel();mb.innerHTML=l==='p'?icon('bolt','xs')+'' + T('ui.profi_ansicht') + '':l==='a'?'' + T('ui.einfache_ansicht') + '':'';}}catch(_){ }
...
function renderModules(){
  ...
  var edu=(lvl==='a'?'<div class="eduhint">'+icon('info','sm')+'<div><b>' + T('ui.neu_bei_orvia') + '</b>' + T('ui.dein_score_fasst_schlaf_erholung') + '</div></div>':'');
  var sect='<div class="sectlabel" data-gm-slot="dashboard-modules">'+(lvl==='a'?'' + T('ui.das_wichtigste') + '':'' + T('ui.deine_module') + '')+' <span class="edit" onclick="gmOpenMM()">'+icon('gear','xs')+' ' + T('ui.anpassen') + '</span></div>';
  ...
  flush();
  html+='<div class="addmod" onclick="gmOpenMM()">'+icon('bolt','sm')+' '+(lvl==='a'?'' + T('ui.mehr_anzeigen') + '':'' + T('ui.modul_hinzufuegen_oder_anordnen') + '')+'</div>';
  host.innerHTML=html;
}
...
function gmOpenDaySheet(){
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('calendar')+'</div><div><h3>' + T('ui.tag_waehlen') + '</h3><div class="sh-sub" style="margin:2px 0 0">'+gmEsc((typeof fmtDate==='function'&&typeof cur!=='undefined')?fmtDate(cur):'')+'</div></div></div>'+
    '<div class="sheet-cta"><button class="sec" onclick="shiftDay(-1);gmOpenDaySheet()">' + T('ui.vortag') + '</button>'+(isToday?'':'<button class="sec" onclick="shiftDay(1);gmOpenDaySheet()">' + T('ui.naechster') + '</button>')+(isToday?'':'<button class="prim" onclick="goToday();gmCloseSheets()">Heute</button>')+'</div>';
  gmOpenSheet('detailSheet');
}
...
function openScore(){
  ...
  if(_dec&&_dec.scoreParts&&_dec.scoreParts.length){
    ...
    if(_dec.scoreCapped)_calcLine+='<p style="margin:6px 0 0;color:var(--attention);font-size:11.5px">'+
      gmEsc('' + T('ui.sicherheitsgrenze_aktiv_auf') + ''+_dec.score+' begrenzt — '+((_dec.readinessReasons||[])[0]||'' + T('ui.zustand_des_tages') + ''))+'</p>';
  }
  ...
  var detail=brk.map(function(b,i){
    ...
    return '<div class="fcard" id="gmFc'+i+'"><div class="fhead" role="button" tabindex="0" onclick="gmToggleFactor('+i+')" onkeydown="if(event.key===\'Enter\')gmToggleFactor('+i+')">'+
      ...
      '<div class="fbody"><div class="fbody-in">'+
        ...
        '<div class="fchips">'+((b.length>5&&b[5]!=null)?'<span class="fchipv">' + T('ui.gemessen') + '<b>'+gmEsc(String(b[5]))+'</b></span>':'')+'<span class="fchipv">' + T('ui.bewertet') + '<b>'+(norm!=null?norm:'—')+'</b>/100</span><span class="fchipv">' + T('ui.beitrag') + '<b>+'+gmEsc(String(b[1]))+' Pkt</b></span></div>'+
        (b[3]?'<div class="fex">'+gmEsc(b[3])+'</div>':'')+
        (link?'<div class="deeplink" role="button" tabindex="0" onclick="openMetric(\''+link+'\')" onkeydown="if(event.key===\'Enter\')openMetric(\''+link+'\')">'+gmEsc(b[0])+'' + T('ui.oeffnen') + ''+icon('chev','xs')+'</div>':'')+
      '</div></div></div>';}).join('');
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="ring-wrap" style="width:96px;height:96px">'+
    ...
    '<div><h3>' + T('ui.orvia_score') + '</h3><div class="sh-sub" style="margin:3px 0 0"><span class="statuspill sp-'+d.statusColor+'" style="background:'+(TINT[d.statusColor]||'var(--surface)')+';color:'+(SC[d.statusColor]||'var(--muted)')+'">'+gmEsc(d.status)+'</span></div></div></div>'+
    (chips?'<div class="sh-block" style="padding-top:6px"><div class="ci-vals" style="flex-wrap:wrap;gap:6px">'+chips+'</div></div>':'')+
    (rows?'<div class="sh-block"><div class="bh">' + T('ui.so_entsteht_dein_score') + '</div><div class="breakdown">'+rows+'</div>'+_calcLine+'</div>':'<div class="sh-block"><div class="bh">' + T('ui.so_entsteht_dein_score') + '</div><p>'+GM_NA+' — sobald dein Check-in vorliegt, erscheinen hier die Teilwerte.</p></div>')+
    ...
    (detail?'<div class="sh-block"><div class="bh">' + T('ui.aufschluesselung_der_erholung') + '</div><p style="margin:0 0 10px;color:var(--muted);font-size:11.5px">' + T('ui.diese_faktoren_ergeben_den_erholungswert') + '</p><div class="breakdown">'+detail+'</div></div>':'')+
...
function openMetric(key){
  ...
  if(key==='load'){return openLoadSheet();}   /* GM7.2: dediziertes Last-Sheet (gleiche Serie wie Übersicht) */
    ...
    if(r){curVal=(r.value!=null)?r.value:null;
      ...
      src=({device_measurement:'Gerätemessung',provider_calculation:'Provider-' + T('ui.berechnung') + '',manual_entry:'' + T('ui.manuell_erfasst') + '',manual_override:'' + T('ui.manuell_korrigiert') + '',lab_test:'Labormessung',orvia_estimate:'ORVIA-Schätzung',historical:'Historisch'})[r.sourceType]||r.sourceType||'Metrik-Speicher';
      ...
      var _md='';try{if(r.metricDate){var _d=new Date(r.metricDate+'T12:00');_md='' + T('ui.stand_') + ''+_d.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit'});}}catch(_){ }
  ...
  if(series&&series.values.length>=3){
    ...
    chart='<div class="ochart"><div class="spark" style="height:60px;display:flex;align-items:center;justify-content:center;font-size:11px;color:var(--faint);background:var(--surface);border:1px solid var(--border);border-radius:12px;text-align:center;padding:0 12px">'+
      (series?'' + T('ui.verlauf') + ''+GM_NA.toLowerCase()+' — erst '+series.values.length+'' + T('ui.messwert_e_gespeichert') + '':'' + T('ui.verlauf') + ''+GM_NA.toLowerCase()+'' + T('ui.fuer_diese_metrik_ist_keine') + '')+'</div></div>';
  }
  ...
  var statsRow='<div class="statgrid3">'+
    ...
    '<div><div class="n">'+(baseVal!=null?gmEsc(_fmtStat(baseVal))+'<small>'+gmEsc(def.unit)+'</small>':'—')+'</div><div class="l">' + T('ui.baseline__') + '</div></div>'+
  ...
  if(key==='sleep_duration_min'){
    ...
    if(_anyPh&&_tot>0){
      ...
      _phaseHtml='<div style="display:flex;height:10px;border-radius:6px;overflow:hidden;margin:8px 0 4px">'+_pv.map(function(x){var w=(x.v!=null)?(x.v/_tot*100):0;return x.v!=null?'<i style="width:'+w+'%;background:'+x.c+'"></i>':'';}).join('')+'</div>'+
        ...
        '<p style="margin-top:8px;color:var(--muted);font-size:11px">' + T('ui.phasen_dauern_aus_dem_kanonischen') + '</p>';
    }else{
      _phaseHtml='<p style="margin-top:6px;color:var(--muted);font-size:11.5px">Schlafphasen (Tief/Leicht/REM/Wach): '+GM_NA+' — für diesen Tag sind keine ' + T('ui.phasen_') + '-Dauern gespeichert. Kein erfundenes ' + T('ui.hypnogramm') + '.</p>';
    }
        ...
        try{var _sd=gmMetricSeries('sleep_deep_min',14),_sr=gmMetricSeries('sleep_rem_min',14),_sl=gmMetricSeries('sleep_light_min',14);
          if(_sd&&_sr&&_sl&&_sd.values.length>=3){
            ...
            if(_ps.length>=3){var _pavg=_ps.reduce(function(a,b){return a+b;},0)/_ps.length;var _pd2=Math.round(_restPct-_pavg);
              _vs14='<p style="font-size:12px;color:var(--muted);margin-top:9px;line-height:1.5">' + T('ui.tief_und_rem_schlaf_sind') + '<b style="color:'+(_pd2>=0?'var(--ready)':'var(--attention)')+'">'+(_pd2>=0?'+':'')+_pd2+' %P</b> vs. deinem '+_ps.length+'-Nächte-Durchschnitt.</p>';}}
        }catch(_){ }
        _restHtml='<div class="sh-block"><div class="bh">' + T('ui.erholsamer_schlaf') + '</div>'+
          '<div class="ih" style="display:flex;align-items:center;justify-content:space-between"><div style="font-size:20px;font-weight:800">'+_restPct+'<small>%</small> · '+gmEsc(_fmtHm(_restMin))+'</div><span class="ppct" style="background:'+((_restPct>=35&&_restPct<=50)?'var(--ready-t)':'rgba(255,255,255,.06)')+';color:'+((_restPct>=35&&_restPct<=50)?'var(--ready)':'var(--muted)')+'">'+((_restPct>=35&&_restPct<=50)?'' + T('ui.im_idealband') + '':'' + T('ui.referenz_35_50') + '')+'</span></div>'+
          ...
          '<div style="display:flex;justify-content:space-between;font-size:10px;color:var(--faint);font-weight:650;margin-top:4px"><span>0%</span><span style="color:var(--ready)">' + T('ui.ideal_35_50_referenzwert') + '</span><span>100%</span></div>'+_vs14+'</div>';
      }
    }catch(_){ }
    extra='<div class="sh-block"><div class="bh">' + T('ui.schlaf_score') + '</div><p>'+(ssc&&ssc.value!=null?('<b style="font-size:18px">'+gmEsc(fmtDe(ssc.value))+'</b>/100 — aus dem kanonischen Speicher (Provider-Score).'):(GM_NA+'' + T('ui.kein_schlaf_score_im_kanonischen') + ''))+'</p></div>'+
      ...
      '<div class="sh-block"><div class="bh">' + T('ui.schlaf_coach_heute_nacht') + '</div><p style="color:var(--muted);font-size:12px">'+GM_NA+' — eine persönliche Schlafenszeit-' + T('ui.empfehlung_') + ' braucht den Schlafziel-Vertrag (' + T('ui.zielkorridor') + '). ORVIA erfindet keine ' + T('ui.empfehlung_') + '.</p></div>'+
      ...
      '<div class="sh-block"><div class="bh">' + T('ui.hypnogramm') + '</div><div id="gmHypnoSlot"><div style="font-size:11px;color:var(--muted)">'+GM_NA+' — lädt …</div></div></div>'+
      ...
      '<div class="sh-block"><div class="bh">' + T('ui.nachtverlauf') + '</div>'+
      ['gmSleepHrSlot:' + T('ui.herzfrequenz__') + '','gmSleepStressSlot:Stress','' + T('ui.gmsleepbbslot_body_battery') + '','gmSleepRespSlot:Atmung'].map(function(s){
    ...
    extra='<div class="sh-block"><div class="bh">' + T('ui.tages_maximum') + '</div><p>'+(_smx&&_smx.value!=null?('<b style="font-size:18px">'+gmEsc(fmtDe(_smx.value))+'</b> — höchster gemessener Stresswert des Tages (Garmin).'):(GM_NA+'' + T('ui.kein_tages_maximum_gespeichert') + ''))+'</p></div>'+
      ...
      '<div class="sh-block"><div class="bh">' + T('ui.intraday_verlauf') + '</div><div id="gmStressIntraSlot"><div style="font-size:11px;color:var(--muted)">'+GM_NA+' — lädt …</div></div></div>'+
      /* GM7.6: Skalen-Einordnung (GM stressSheet, statische Skala 0-100) — nur F/P. */
      (((typeof gmLevel==='function'?gmLevel():'f')!=='a')?'<div class="sh-block"><div class="bh">' + T('ui.die_skala') + '</div>'+[['0–25','Ruhe','var(--sleep)'],['26–50','Niedrig','var(--ready)'],['51–75','Mittel','var(--attention)'],['76–100','Hoch','var(--crit)']].map(function(r){return '<div class="scalerow"><span class="sr" style="color:'+r[2]+'">'+r[0]+'</span><span class="sd2" style="background:'+r[2]+'"></span><span class="sl">'+r[1]+'</span></div>';}).join('')+'</div>':'');
  }
  else if(key==='hrv_ms'){
    ...
    extra='<div class="sh-block"><div class="bh">' + T('ui.naechtlicher_hrv_verlauf') + '</div><div id="gmHrvNightSlot"><div style="font-size:11px;color:var(--muted)">'+GM_NA+' — lädt …</div></div></div>';
  }
  else if(key==='body_battery'){
    /* GM7.4.1: echte Intraday-Body-Battery-Kurve (body_battery_intraday, user_metric_series). */
    extra='<div class="sh-block"><div class="bh">' + T('ui.intraday_verlauf') + '</div><div id="gmBbIntraSlot"><div style="font-size:11px;color:var(--muted)">'+GM_NA+' — lädt …</div></div></div>';
  }
  ...
  if(key==='sleep_duration_min'){try{var _ss2=gmMetric('sleep_score');
    ...
    subTxt='' + T('ui.letzte_nacht') + ''+_durTxt+((_ss2&&_ss2.value!=null)?'' + T('ui.score') + ''+Math.round(_ss2.value):'');}catch(_){ }}
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:'+(TINT[def.color]||'var(--surface-2)')+';color:'+(SC[def.color]||'var(--muted)')+'">'+icon(def.icon)+'</div><div><h3>'+gmEsc(def.label)+'</h3><div class="sh-sub" style="margin:2px 0 0">'+gmEsc(subTxt)+'</div></div></div>'+
    ...
    (function(){
      ...
      if(!info)return '<div class="sh-block"><div class="bh">' + T('ui.was_das_heisst') + '</div><p>' + T('ui.anzeige_aus_dem_kanonischen_speicher') + '</p></div>';
      ...
      if(lvl2!=='a'){
        ...
        try{var _cx2=recoveryCtx(todayStr());
          if(key==='hrv_ms'&&curVal!=null&&_cx2&&_cx2.hrvBase7!=null&&_cx2.hrvSd28){var _z=(Math.log(curVal)-_cx2.hrvBase7)/_cx2.hrvSd28;devChip=(_z>=0?'+':'')+fmtDe(Math.round(_z*10)/10)+'' + T('ui.sd') + '';}
        ...
        confBlock='<div class="sh-block"><div class="bh">' + T('ui.berechnung_amp_konfidenz') + '</div><div class="confidence">'+
          '<span class="confchip">'+icon('check','xs')+' ' + T('ui.datenqualitaet') + ' <b>'+(dqLbl?gmEsc(dqLbl):'—')+'</b></span>'+
          '<span class="confchip">'+icon('pulse','xs')+' Abweichung <b>'+gmEsc(devChip)+'</b></span>'+
          '<span class="confchip">'+icon('db','xs')+' ' + T('ui.baseline__') + ' <b>'+(baseVal!=null?gmEsc(fmtDe(baseVal))+gmEsc(def.unit):'—')+'</b></span></div>'+
          (lvl2==='p'?'<p style="margin-top:10px;font-size:11.5px;color:var(--muted)">'+(key==='hrv_ms'?'' + T('ui.abweichung_in_sd_der_persoenlichen') + '':'')+'Ein ' + T('ui.konfidenz__') + '-Prozentwert je Metrik existiert nicht als kanonischer Vertrag — ORVIA zeigt die echte Datenqualitätsstufe statt einer erfundenen Zahl.</p>':'')+'</div>';
      }
      return '<div class="sh-block"><div class="bh">' + T('ui.was_das_heisst') + '</div><p>'+gmEsc(interp)+'</p></div>'+
        (facts?'<div class="sh-block"><div class="bh">' + T('ui.moegliche_einfluesse') + '</div><div class="factchips">'+facts.map(function(f){return '<span class="factchip">'+gmEsc(f)+'</span>';}).join('')+'</div></div>':'')+
        (info.meaning?'<div class="sh-block"><div class="bh">' + T('ui.bedeutung_fuer_training_amp_erholung') + '</div><p>'+gmEsc(info.meaning)+'</p></div>':'')+
        confBlock;
    })()+
...
function openLoadSheet(){
  ...
  if(hasSeries){
    ...
    chart='<div class="ochart"><div class="spark" style="height:70px;display:flex;align-items:center;justify-content:center;font-size:11px;color:var(--faint);text-align:center;padding:0 12px;background:var(--surface);border:1px solid var(--border);border-radius:12px">'+
      ((lcc&&lcc.ctlAtlNote)?gmEsc(lcc.ctlAtlNote):(GM_NA+'' + T('ui.ctl_atl_erscheinen_ab_belastbarer') + ''))+'</div></div>';
  }
  ...
  if(atl!=null){
    ...
    headCard='<div class="card" style="margin:14px 0 0;padding:16px"><div style="display:flex;gap:14px;align-items:center">'+
      '<div class="ring-wrap" style="width:82px;height:82px;flex-shrink:0">'+ring(_apct,SC.activity,82,7)+'<div class="ring-c"><div style="font-size:22px;font-weight:800">'+atl+'</div><div style="font-size:9px;color:var(--muted);font-weight:700">' + T('ui.atl_') + '</div></div></div>'+
      '<div style="flex:1;min-width:0"><div style="font-size:10px;letter-spacing:.08em;color:var(--faint);font-weight:800">' + T('ui.was_das_bedeutet') + '</div>'+
      '<div style="font-size:14px;font-weight:750;margin-top:3px">'+(statusWord?gmEsc(statusWord):'—')+'</div>'+
      '<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap"><span class="fchipv" style="background:var(--activity-t);color:var(--activity);border-color:transparent">' + T('ui.ctl_') + '<b>'+(ctl!=null?ctl:'—')+'</b></span><span class="fchipv">' + T('ui.tsb_') + '<b>'+(tsb!=null?((tsb>=0?'+':'')+tsb):'—')+'</b></span><span class="fchipv">' + T('ui.acwr_') + '<b>'+(acwr!=null?gmEsc(acwr):'—')+'</b></span></div>'+
      '<div style="margin-top:8px;font-size:11.5px;color:var(--muted)">' + T('ui.gesunder_ctl_bereich') + '<b>—</b>' + T('ui.kein_kanonischer_vertrag') + '</div></div></div></div>';
  }
  ...
  var guide=(function(){
    var rows=[['Abnehmend','chart','sleep'],['' + T('ui.erhaltung') + '','db','neutral'],['Aufbau','bolt','ready'],['Höchstform','target','activity'],['Überlastet','alert','attention'],['Übertrainiert','shield','crit']];
    ...
    return '<div class="sh-block"><div class="bh">' + T('ui.statusleitfaden_30_tage') + '</div><div class="sguide">'+rows.map(function(r){
      ...
      return '<div class="srow"'+(on?' style="border-color:'+col+'"':'')+'><div class="st">'+
        ...
        '<span class="sd"><b>—</b>' + T('ui.tage_') + '</span><span class="spct" style="background:'+tint+';color:'+col+'">—</span></div>'+
        '<div class="sbar"><i style="width:0%;background:'+col+'"></i></div></div>';
    }).join('')+'</div><p style="margin-top:4px;font-size:11px;color:var(--muted)">Tagesanteile je ' + T('ui.status_') + ''+GM_NA+' — es gibt keinen kanonischen Fitness-Status-' + T('ui.verlauf') + '(Garmin-Konzept, keine Engine-Quelle). Markiert ist nur der heutige, aus dem echten ' + T('ui.acwr__') + ' abgeleitete Zustand.</p></div>';
  })();
  var band=headCard;
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--activity-t);color:var(--activity)">'+icon('gauge')+'</div>'+
    '<div><h3>' + T('ui.trainingsbelastung') + '</h3><div class="sh-sub" style="margin:2px 0 0">heute '+(atl!=null?'' + T('ui.atl') + ''+atl+'' + T('ui.ctl') + ''+(ctl!=null?ctl:'—')+'' + T('ui.tsb') + ''+(tsb!=null?((tsb>=0?'+':'')+tsb):'—'):'—')+'</div></div></div>'+
    chart+
    '<div class="sh-stats">'+stat(ctl,'' + T('ui.ctl__') + '')+stat(atl,'' + T('ui.atl_') + '')+stat(tsb!=null?((tsb>=0?'+':'')+tsb):null,'' + T('ui.tsb_form') + '')+stat(acwr,'' + T('ui.acwr__') + '')+'</div>'+
    band+guide+
    ...
    (function(){
      ...
      if(!E||!env)return '';
      var DEFS=[['hard','' + T('ui.harte_einheiten') + '','' + T('ui.einheiten_') + ''],['trimp','' + T('ui.trimp') + '','' + T('ui.einheiten_') + ''],
        ['easy','' + T('ui.easy_share_laufen') + '','Läufen'],['sport','' + T('ui.belastung_nach_sportart') + '','' + T('ui.einheiten_') + ''],['interf','Interferenz','Signalen']];
      var rows=DEFS.map(function(x){
        ...
        var warn=e.status==='partial'?' <span style="color:var(--attention);font-weight:700">' + T('ui.teilabdeckung_') + '</span>':'';
      ...
      return '<div class="sh-block"><div class="bh">' + T('ui.berechnungsgrundlage') + '</div>'+rows+
        '<p style="margin-top:6px;font-size:11px;color:var(--muted)">Kein Wert ohne ' + T('ui.zeitraum_') + ', Abdeckung und Methode (Metrik-Envelope v'+gmEsc(E.VERSION)+'). Echte HF-Zonen folgen erst mit einem Zonen-Datenpfad — bis dahin keine Zonen-Behauptung.</p></div>';
    })()+
    '<div class="sh-block"><div class="bh">' + T('ui.was_das_heisst') + '</div><p>' + T('ui.ctl_42_tage_ewma_der') + '</p></div>'+
    '<div class="source">'+icon('db','xs')+'' + T('ui.orvia_kanonisches_lastmodell_srpe') + ''+((L&&L.provenance==='legacy_fallback')?'' + T('ui.reduzierte_datenlage') + '':'')+'</div>';
  gmOpenSheet('detailSheet');
  if(hasSeries){try{var el=document.getElementById('loadSheetChart');
    if(el&&window.ORVIA&&ORVIA.charts&&ORVIA.charts.richChart){var k2=Math.min(rng,S.ctl.length);
      ORVIA.charts.richChart(el,{label:'' + T('ui.ctl_fitness') + '',series:S.ctl.slice(-k2).map(function(v){return Math.round(v);}),
...
function openRecoverySheet(){
  ...
  var _rdefs=[['hrv_ms','HRV','ready',function(v){return fmtDe(Math.round(v))+' ms';}],
    ...
    ['sleep_duration_min','' + T('ui.schlaf') + '','sleep',function(v){return fmtDe(Math.round(v/6)/10)+' h';}],
  ...
  _rdefs.forEach(function(rd){
    ...
    if(s2&&s2.values.length>=3){_rany=true;
      ...
      _rrows+='<div style="display:flex;align-items:center;gap:12px;padding:7px 0;border-bottom:1px solid var(--hair)"><div style="width:86px;font-size:11.5px;font-weight:700;color:var(--muted)">'+rd[1]+'</div><div style="flex:1;font-size:10.5px;color:var(--faint)">'+(s2?'erst '+s2.values.length+'' + T('ui.messwert_e') + '':'' + T('ui.keine_serie_gespeichert') + '')+'</div><div style="width:64px;text-align:right;font-weight:800;color:var(--muted)">—</div></div>';
    }});
  var chart='<div class="sh-block" style="padding-top:8px"><div class="bh">' + T('ui.erholungstrend_14_tage') + '</div>'+_rrows+
    '<p style="margin-top:8px;color:var(--muted);font-size:11px">' + T('ui.ein_zusammengefasster_erholungs_verlauf_wird') + '</p></div>';
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--ready-t);color:var(--ready)">'+icon('heart')+'</div>'+
    '<div><h3>' + T('ui.erholung_') + '</h3><div class="sh-sub" style="margin:2px 0 0">heute '+(rec!=null?gmEsc(fmtDe(rec))+' %':'—')+'</div></div></div>'+
    chart+
    '<div class="sh-stats">'+stat(M.hrv,'HRV',' ms')+stat(M.rhr,'Ruhepuls',' bpm')+stat(M.sleepMin!=null?M.sleepMin/60:null,'' + T('ui.schlaf') + '',' h')+'</div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.was_das_heisst') + '</div><p>'+(rec!=null?('' + T('ui.erholung') + ''+fmtDe(rec)+' % — der kanonische ORVIA-Composite kombiniert HRV, Ruhepuls und ' + T('ui.schlaf') + ' (plus subjektive Marker). Er entscheidet mit ' + T('ui.ueber') + ' die Trainingsfreigabe.'):(GM_NA+'' + T('ui.der_composite_erscheint_nach_dem') + ''))+'</p></div>'+
...
function gmOpenQA(){
  ...
  var grid=list.length?list.map(function(a,i){var c=tint[a.id]||'activity';
    ...
    var d=a.description||'';if(ctxIds[a.id])d=d?('' + T('ui.jetzt_sinnvoll') + ''+d):'' + T('ui.jetzt_sinnvoll_') + '';
  ...
  if(list.length&&canEdit){
    grid+='<div class="qa" role="button" tabindex="0" onclick="gmOpenQAFavs()" onkeydown="if(event.key===\'Enter\')gmOpenQAFavs()">'+
      ...
      '<div><div class="q-t">' + T('ui.anpassen') + '</div><div class="q-d">' + T('ui.auswahl_und_reihenfolge') + '</div></div></div>';
  }
  sh.innerHTML='<div class="grab"></div><h3>' + T('ui.schnell_hinzufuegen') + '</h3><div class="sh-sub">Häufig genutzt zuerst'+(canEdit?' · anpassbar':'')+'</div><div class="qa-grid">'+grid+'</div>';
  gmOpenSheet('qaSheet');
}
...
function gmStartTraining(){
  ...
  try{if(typeof toast==='function')toast('' + T('ui.training_laesst_sich_gerade_nicht') + '');}catch(_){ }
}
...
function gmRenderMM(){
  ...
  if(inactive.length){
    rows+='<div class="sectlabel" style="margin:14px 0 6px">' + T('ui.ausgeblendet') + '<span class="ana-count">'+inactive.length+'</span></div>'+
          inactive.map(function(id){return row(id,false,-1,0);}).join('');
  }
  sh.innerHTML='<div class="grab"></div><h3>' + T('ui.module_anpassen') + '</h3><div class="sh-sub">' + T('ui.ein_ausblenden_und_reihenfolge_sichtbarkeitsstufe') + '</div><div id="mmList">'+rows+'</div>'+
    '<div class="sheet-cta"><button class="sec" onclick="gmMMReset()">' + T('ui.standard') + '</button><button class="prim" onclick="gmMMDone()">' + T('ui.fertig_') + '</button></div>';
}
...
var GM_PLAN_VARIANTS=(function(){
  ...
  return {A:{name:'Vollständig',desc:'' + T('ui.der_plan_wie_gebaut_alle') + ''},
    B:{name:'Reduziert',desc:'' + T('ui.gleiche_wochenstruktur_ohne_doppeleinheiten_zeitsparend') + ''},
    C:{name:'Minimal',desc:'' + T('ui.nur_die_einheiten_die_das') + ''}};})();
...
function gmOpenVariantSheet(){
  ...
  var blocks=['A','B','C'].map(function(v){
    ...
    var stat=(d&&d.count!=null)?(d.count+' von '+vm.total+'' + T('ui.einheiten') + ''+d.days+'' + T('ui.trainingstage') + ''+d.rest+'' + T('ui.ruhetage') + ''):GM_NA;
    var list='';
    ...
    return '<div class="sh-block"><div class="bh">' + T('ui.variante') + ''+v+' · '+meta.name+(vm&&vm.sel===v?'' + T('ui.ausgewaehlt') + '':'')+'</div><p>'+meta.desc+'</p>'+
      '<p style="margin:6px 0 0;font-size:12.5px;font-weight:700">'+gmEsc(stat)+'</p>'+list+
      '<div class="sheet-cta" style="margin-top:8px"><button class="'+(vm&&vm.sel===v?'':'sec ')+'" onclick="gmSetPlanVariant(\''+v+'\')">'+(vm&&vm.sel===v?'Ausgewählt':'' + T('ui.variante') + ''+v+'' + T('ui.waehlen') + '')+'</button></div></div>';
  }).join('');
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--activity-t);color:var(--activity)">'+icon('calendar')+'</div><div><h3>' + T('ui.planvarianten') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.a_b_c_teilmengen_deines') + '</div></div></div>'+
    blocks+
    '<div class="source">'+icon('info','xs')+' Einstufung nach Einheitstyp (Kern / Aufbau / Ergänzung). Dein gespeicherter Plan bleibt unverändert — entfallende ' + T('ui.einheiten_') + ' werden nur ausgeblendet markiert. Die individuelle Varianten-Planung übernimmt die Trainingsengine.</div>';
  gmOpenSheet('detailSheet');
}
function gmOpenPlanSettingsSheet(){
  ...
  var prov='';try{var pm=(typeof PROFILE!=='undefined'&&PROFILE&&PROFILE.weekPlanMeta)||null;
    if(pm&&pm.source){var srcDE={manual_edit:'manuell bearbeitet',engine_adjustment:'' + T('ui.durch_die_engine_angepasst') + '',reset:'neu aufgebaut'}[pm.source]||pm.source;
      ...
      prov='<div class="source">'+icon('info','xs')+'' + T('ui.zuletzt') + ''+gmEsc(srcDE)+gmEsc(when)+'.</div>';}}catch(_){ }
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('gear')+'</div><div><h3>' + T('ui.plan_amp_wochenstruktur') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.bestehende_werkzeuge') + '</div></div></div>'+
    '<div class="sheet-cta"><button class="sec" onclick="gmCloseSheets();openPlanEditor()">' + T('ui.plan_bearbeiten') + '</button><button class="sec" onclick="gmCloseSheets();openPauseEditor()">' + T('ui.pause_urlaub') + '</button>'+
    /* Phase 3 · Block 2: Wochenreview + Coach Briefing — kontextueller Einstieg am Plan. */
    ((typeof gmFeatureFlag!=='function'||gmFeatureFlag('weekReview'))?'<button class="sec" onclick="gmOpenWeekReviewSheet()">' + T('ui.wochenreview') + '</button>':'')+'</div>'+prov+
    ...
    '<div class="sh-block" style="margin-top:10px"><div class="bh">' + T('ui.kanonisches_planmodell_beta') + '</div>'+
    '<p style="font-size:12px;color:var(--muted);margin:0 0 8px">' + T('ui.engine_anpassungen_und_deine_manuellen') + '</p>'+
    '<div class="sheet-cta"><button class="sec" onclick="gmSetFeatureFlag(\'canonPlan\',!gmCanonPlanOn());gmCloseSheets();try{renderGMPlan()}catch(e){}">'+((typeof gmCanonPlanOn==='function'&&gmCanonPlanOn())?'' + T('ui.deaktivieren_zurueck_zum_legacy_plan') + '':'Aktivieren')+'</button></div></div>';
  gmOpenSheet('detailSheet');
}
...
function gmOpenWeekReviewSheet(){
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--activity-t);color:var(--activity)">'+icon('chart')+'</div><div><h3>' + T('ui.wochenreview') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.kanonischer_wochenvertrag_mo_so') + '</div></div></div>'+
    '<div class="sh-block">'+body+'</div>'+
    ...
    '<div class="source">'+icon('info','xs')+' Briefing = vollständiges Wochen-Datenpaket (Prognose, ' + T('ui.acwr__') + '-Status, Warnungen, letzte 7 ' + T('ui.tage__') + ') als Prompt für deinen Coach oder eine KI — nur in die Zwischenablage, es verlässt das Gerät nicht automatisch.</div>';
  gmOpenSheet('detailSheet');
}
function gmDailyGoalsBlock(){
  var slots=[['Schritte','foot'],['' + T('ui.aktive_kcal') + '','bolt'],['Wasser','drop'],['' + T('ui.schlaf') + '','moon']];
...
var GM_FEAS_TEXT={within_modeled_corridor:'' + T('ui.im_rahmen_dessen_was_das') + '',
  outside_modeled_corridor:'' + T('ui.ausserhalb_des_modellkorridors') + '',insufficient_data:'' + T('ui.datenlage_reicht_nicht') + ''};
var GM_MISSING_TEXT={current_performance:'' + T('ui.ein_gemessener_leistungswert') + '',
  current_performance_not_decision_eligible:'' + T('ui.ein_leistungswert_mit_datum_undatiert') + '',
  goal:'' + T('ui.eine_bezifferte_zielzeit') + ''};
function gmGoalForecastCard(lvl,perfBySport){
  ...
  if(!v.ok){
    var why=v.reason==='no_goal_distance'
      ?'' + T('ui.ohne_zieldistanz_gibt_es_nichts') + ''
      :v.missing.length
        ?('' + T('ui.es_fehlt') + ''+v.missing.map(function(m){return gmEsc(GM_MISSING_TEXT[m]||m);}).join(', ')+'.')
        :'' + T('ui.es_fehlt_ein_belastbarer_leistungswert') + '';
  ...
  var tgtTxt='';
  if(v.target>0){
    var tw={likely:'' + T('ui.deine_zielzeit') + ''+gmGoalForecastMin(v.target)+'' + T('ui.liegt_im_korridor') + '',
      edge:'' + T('ui.deine_zielzeit') + ''+gmGoalForecastMin(v.target)+'' + T('ui.liegt_an_der_optimistischen_kante') + '',
      beyond:'' + T('ui.deine_zielzeit') + ''+gmGoalForecastMin(v.target)+'' + T('ui.liegt_unter_dem_was_der') + ''}[v.reachable];
  ...
  var weeksTxt=(v.weeks&&v.weeks.min!=null)
    ?('' + T('ui.geschaetzter_zeitraum_etwa') + ''+gmEsc(String(v.weeks.min))+(v.weeks.max!=null?' bis '+gmEsc(String(v.weeks.max))+'' + T('ui.wochen') + '':'' + T('ui.wochen_oder_deutlich_mehr') + '')+'' + T('ui.spanne_keine_terminzusage') + ''):'';
  var basisTxt=(lvl==='p'&&v.confidence)
    ?('' + T('ui.grundlage_beleglage') + ''+gmEsc(String(v.confidence))+'' + T('ui.bandbreite') + ''+gmEsc(String(v.bandPct))+' %.'):'';
...
var GM_PQ_LABELS=[['goalCoverage','Zielabdeckung'],['recoveryDistribution','Erholungsverteilung'],
  ...
  ['sportBalance','Sportbalance'],['dataQuality','' + T('ui.datenqualitaet') + '']];
var GM_PQ_NA_TEXT={no_goal:'' + T('ui.kein_ziel_hinterlegt') + '',goal_without_distance_model:'' + T('ui.fuer_diese_zielart_noch_kein') + '',
  no_availability_config:'' + T('ui.verfuegbarkeit_nicht_gepflegt') + '',single_sport:'' + T('ui.nur_eine_sportart_aktiv') + '',
  no_sessions:'' + T('ui.keine_einheiten_geplant') + '',too_few_active_days:'' + T('ui.zu_wenige_trainingstage') + '',
  no_sport_normalizer:'' + T('ui.sportart_zuordnung_nicht_verfuegbar') + '',too_few_known_sports:'' + T('ui.zu_wenige_bekannte_sportarten') + '',
  no_plan:'' + T('ui.kein_plan') + ''};
...
function gmAdaptiveSection(){
  ...
  if(!body)return '';
  return '<div class="sectlabel" data-gm-slot="plan-adaptive">' + T('ui.adaptive_einschaetzung') + '</div>'+
...
function renderGMPlan(){
  ...
  h+='<div class="hdr"><div><div class="greet">'+(meta.wk!=null?'' + T('ui.trainingswoche') + ''+meta.wk:'Wochenplan')+(meta.phase?' · '+gmEsc(meta.phase)+'phase':'')+'</div><h1>' + T('ui.dein_plan') + '</h1><div class="date">'+gmEsc(meta.range)+(meta.phase?' · '+gmEsc(meta.phase):'')+(lvl==='p'?'' + T('ui.struktur_varianten_prognose') + '':'')+'</div></div><div class="hdr-actions">'+
    '<button class="iconbtn" id="gmPlanConfBadge" style="color:var(--attention);'+(_confN>0?'':'display:none')+'" aria-label="' + T('ui.plan_konflikte') + '" onclick="gmOpenPlanConflictsSheet()">'+icon('alert','sm')+'</button>'+
  ...
  h+='<div class="sectlabel" data-gm-slot="plan-variant">' + T('ui.planvariante') + '<span class="edit" onclick="gmOpenVariantSheet()">'+gmEsc(pMeta.name)+' · ' + T('ui.details_') + '</span></div>';
  ...
  h+='<div class="card"><div class="ctitle"><div class="l">' + T('ui.variante') + ''+pSel+' · '+gmEsc(pMeta.name)+'</div><span class="more" onclick="gmOpenVariantSheet()">Wechseln '+icon('chev','xs')+'</span></div>'+
    ...
    '<div class="mini-note" style="margin:10px 0 0">'+icon('info','xs')+'<div><b>' + T('ui.auswirkung') + '</b> '+((pd&&pd.count!=null&&pvm)?(pSel==='A'?'' + T('ui.alle') + ''+pvm.total+'' + T('ui.geplanten_einheiten_aktiv') + '':pd.count+' von '+pvm.total+'' + T('ui.einheiten_aktiv') + ''+(pd.core||0)+'' + T('ui.kernreize_bleiben_entfallende_sind_unten') + ''):GM_NA+'' + T('ui.ohne_variantenmodell_keine_aussage') + '')+((pvm&&pvm.note)?' '+gmEsc(pvm.note):'')+'</div></div></div>';
  ...
  var _wLbl='' + T('ui.diese_woche') + '',_wRange='';
  ...
  h+='<div class="sectlabel" data-gm-slot="plan-week">'+gmEsc(_wLbl)+
     '<span class="edit">'+
       ...
       (_wOff!==0?'<button class="iconbtn" aria-label="' + T('ui.zur_aktuellen_woche_') + '" onclick="gmPlanWeekToday()" style="margin-left:6px">Heute</button>':'')+
     '</span></div>';
  ...
  if(_wOff!==0){
    var _provNote=(_wOff<0)
      ?'' + T('ui.vergangene_woche_die_einheiten_zeigen') + ''
      :(GM_PROV_NOTE[_wSel&&_wSel.provenance]||'' + T('ui.kommende_woche_fuer_diese_woche') + '');
  ...
  for(var di=0;di<7;di++){
    ...
    if(!items.length){
      ...
      var _dsL=_ds==='rest'?['moon','' + T('ui.ruhetag_') + '','—']
        :_ds==='unavailable'?['info','' + T('ui.nicht_verfuegbar_') + '','' + T('ui.laut_verfuegbarkeit_gesperrt') + '']
        :['info','Frei','' + T('ui.verfuegbar_keine_einheit_geplant') + ''];
    ...
    for(var ii=0;ii<items.length;ii++){var it=items[ii];
      ...
      cards+='<div class="session-card'+(done?' done':'')+(pSkip?' pvar-skip':'')+'" data-sid="'+gmEsc(it.id||'')+'" role="button" tabindex="0" onclick="planEntryClick('+di+','+ii+',\''+gmEsc(k)+'\')" onkeydown="if(event.key===\'Enter\')planEntryClick('+di+','+ii+',\''+gmEsc(k)+'\')">'+
        ...
        '<span class="session-state'+(done?' done':'')+'">'+(done?'' + T('ui.erledigt_') + '':(pSkip?'' + T('ui.entfaellt') + ''+pSel+')':'—'))+'</span></div>';
        ...
        if(_res&&_res.actual){
          ...
          if(_bits.length){
            ...
            var _dbTxt=_dbRec&&_dbRec.rpe!=null
              ?'' + T('ui.rpe') + ''+_dbRec.rpe+(_dbRec.pain?'' + T('ui.schmerz_gemeldet') + '':'')+(_dbRec.deltaRpe!=null?' (erwartet '+_dbRec.expectedRpe+')':'')
              :'' + T('ui.rueckmeldung_offen') + '';
            cards+='<div class="mini-note" style="margin:-4px 0 8px 44px">'+icon('check','xs')+
              '<div><b>' + T('ui.absolviert') + '</b> '+gmEsc(_bits.join(' · '))+
  ...
  var pqNote;
  if(!pq){pqNote=GM_NA+'.';}
  else if(gmLevel()==='a'){pqNote='<b>Planqualität: '+gmEsc(pq.rating.l)+'.</b> '+(pq.warns.length?pq.warns.length+'' + T('ui.hinweis') + ''+(pq.warns.length>1?'e':'')+'' + T('ui.im_sheet') + '':'' + T('ui.keine_auffaelligkeiten') + '');}
  else{pqNote='<b>Planqualität: '+gmEsc(pq.rating.l)+'.</b> '+(pq.warns.length?gmEsc(pq.warns[0][0])+(pq.warns.length>1?' (+'+(pq.warns.length-1)+' weitere)':'')+'' + T('ui.details_im_planqualitaets_sheet') + '':'' + T('ui.keine_auffaelligkeiten_in_den_planpruefungen') + '')+(gmLevel()==='p'?'' + T('ui.subscores_erscheinen_mit_der_externen') + '':'');}
  h+='<div class="sectlabel" data-gm-slot="plan-quality">' + T('ui.planqualitaet') + '<span class="edit" onclick="openPlanQualitySheet()">' + T('ui.details_') + '</span></div>';
  ...
  if(lvl!=='a'){
    ...
    h+='<div class="sectlabel" data-gm-slot="plan-phases">' + T('ui.phasen_bis_zum_ziel') + '<span class="edit">'+(_dtsOk?'noch '+Math.max(0,Math.ceil(_dts/7))+'' + T('ui.wochen') + '':'—')+'</span></div>';
    if(!phases.length){
      h+='<div class="card"><div class="ph-now"><b>—</b><span>'+GM_NA+' — ohne Zieldatum gibt es kein Phasenmodell.</span></div>'+
        '<div class="mini-note" style="margin-top:10px">'+icon('info','xs')+'<div>' + T('ui.sobald_ein_wettkampfdatum_im_ziel') + '</div></div></div>';
    }else{
      ...
      var _curTxt=_cur?(_wkLbl(_cur)+(_cur.d?' · '+_cur.d:'')):(_dtsOk&&_dts<0?'' + T('ui.wettkampf_liegt_hinter_dir') + '':GM_NA);
      h+='<div class="card">'+
        ...
        '<div class="ph-row '+((_dtsOk&&_dts<0)?'done':'todo')+'"><span class="ph-mk">'+((_dtsOk&&_dts<0)?'✓':'')+'</span><span class="ph-n">' + T('ui.ziel__') + '· '+gmEsc(_gShort)+'</span><span class="ph-w">'+(_dtsOk?(_dts<0?'vorbei':'in '+Math.max(0,Math.ceil(_dts/7))+'' + T('ui.wo') + ''):'—')+'</span></div>'+
        '</div>'+
        '<div class="mini-note" style="margin-top:10px">'+icon('info','xs')+'<div>' + T('ui.phasen_aus_dem_kanonischen_phasenmodell') + '</div></div></div>';
    }
    ...
    var volCols=vals.map(function(v){
      return '<div class="vol-col"><div class="vol-bars">'+
        ...
        '<b>'+(v.act!=null?gmEsc(fmtDe(v.act)):(v.plan!=null?gmEsc(fmtDe(v.plan)):'—'))+'</b><small>'+v.lbl+(v.act!=null&&v.plan!=null?'' + T('ui.ziel') + ''+gmEsc(fmtDe(v.plan)):'')+'</small></div>';}).join('');
    h+='<div class="sectlabel" data-gm-slot="plan-week-km">' + T('ui.wochenkilometer_lauf') + '<span class="edit">' + T('ui.zielkorridor') + '</span></div>';
    h+='<div class="card"><div class="vol-row">'+volCols+'</div>'+
      '<div class="dist-leg" style="margin-top:6px"><span><i style="background:rgba(255,255,255,.4)"></i>' + T('ui.geplant_ziel') + '</span><span><i style="background:var(--ready)"></i>' + T('ui.absolviert_ist') + '</span></div>'+
      '<div class="mini-note" style="margin-top:8px">'+icon('info','xs')+'<div>' + T('ui.ist_aus_der_kanonischen_wochenaggregation') + '</div></div></div>';
  }
  /* 10–12. Tagesziele + Abschluss */
  h+='<div class="sectlabel" data-gm-slot="plan-daily-goals">' + T('ui.tagesziele') + '<span class="edit" onclick="gmOpenDailyGoalsSheet()">'+icon('pen','xs')+' Bearbeiten</span></div>';
  h+=gmDailyGoalsBlock();
...
function gmOpenDailyGoalsSheet(){
  var sh=document.getElementById('detailSheet');if(!sh)return;
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('target')+'</div><div><h3>' + T('ui.tagesziele_') + '</h3><div class="sh-sub" style="margin:2px 0 0">'+GM_NA+'</div></div></div>'+
    '<div class="sh-block"><p>' + T('ui.tagesziele_schritte_kalorien_wasser_schlaf') + '</p></div>';
  gmOpenSheet('detailSheet');
}
...
function gmOpenSessionPage(di,ii,dateIso){
  ...
  var cta=isToday
    ?'<button class="cta prim" style="margin:0 18px;width:calc(100% - 36px)" onclick="gmCloseSessionPage();startPlannedUnit('+di+','+ii+',\''+dIso+'\')">'+icon('play','sm')+' ' + T('ui.training_starten') + '</button>'
    :'<div class="mini-note" style="margin:0 18px">'+icon('info','xs')+'<div>' + T('ui.nur_lesbar_starten_ist_nur') + '</div></div>'+
     '<button class="cta wide-ghost" style="margin:10px 18px 0;width:calc(100% - 36px)" onclick="gmCloseSessionPage();openPlanEditor()">' + T('ui.plan_bearbeiten_verschieben') + '</button>';
  var _dLbl2='';try{var _d4=new Date(dIso+'T12:00');_dLbl2=' · '+_d4.getDate()+'.'+(_d4.getMonth()+1);}catch(_){ }
  var _undoPd=_pd2?'<button class="cta wide-ghost" style="margin:10px 18px 0;width:calc(100% - 36px)" onclick="confirmUndoPlanDone(\''+gmEsc(it.t)+'\',\''+gmEsc(dIso)+'\',\''+gmEsc(_occ2)+'\')">' + T('ui.erledigt_') + '-Markierung zurücknehmen</button>':'';
  pg.innerHTML='<div class="page-head"><div class="page-head-row"><button class="backbtn" onclick="gmCloseSessionPage()" aria-label="' + T('ui.zurueck') + '">'+icon('chev')+'</button><div><h2>'+gmEsc(DAYNAMES[di])+_dLbl2+' · '+gmEsc(it.l)+'</h2><p>Planvorgabe</p></div></div></div>'+   /* siehe GM7.9h-Notiz unter dieser Funktion */
    '<div class="plan-hero"><div class="plan-kicker">'+gmEsc(it.t)+'</div><h2>'+gmEsc(it.l)+'</h2><p>' + T('ui.geplante_einheit_aus_deinem_wochenplan') + '</p>'+
    '<div class="week-progress"><div class="wp"><b>'+(it.d&&!/^(iv|ez|lr|tempo)$/.test(it.d)?gmEsc(it.d):'—')+'</b><span>' + T('ui.umfang') + '</span></div><div class="wp"><b>—</b><span>' + T('ui.intensitaet_') + '</span></div><div class="wp"><b>—</b><span>' + T('ui.ziel___') + '</span></div><div class="wp"><b>—</b><span>' + T('ui.konfidenz___') + '</span></div></div></div>'+
    '<div class="coach-card"><h3>'+icon('sparkle','sm')+' Warum diese ' + T('ui.einheit') + '?</h3><p>'+(note?gmEsc(note):'' + T('ui.eine_kanonische_begruendung_ist_noch') + '')+'</p></div>'+
...
function gmActSrcLabel(src){
  ...
  return {import:'Import',manual:'' + T('ui.manuell_erfasst') + '',orvia_workout:'' + T('ui.orvia_workout') + '',live:'Live-Workout',legacy_local:'' + T('ui.lokal_erfasst') + '',
    garmin:'Garmin',garmin_unofficial:'Garmin',garmin_official:'Garmin',strava:'Strava',apple_health:'' + T('ui.apple_health') + '',health_connect:'' + T('ui.health_connect') + ''}[src]||(src?String(src):'—');
}
...
function gmActGlyph(gmSport){
  ...
  if(gmSport==='Schwimmen'){var lanes=[24,42,60].map(function(y,i){return '<path d="M0 '+y+'' + T('ui.q_37') + ''+(y-9)+' 75 '+y+'' + T('ui.t_150') + ''+y+'' + T('ui.t_225') + ''+y+'' + T('ui.t_300') + ''+y+'" fill="none" stroke="'+c+'" stroke-width="2.6" opacity="'+(0.75-i*0.18).toFixed(2)+'"/>';}).join('');return '<svg class="act-glyph" viewBox="0 0 300 82" preserveAspectRatio="none">'+lanes+'</svg>';}
...
function renderGMActivity(){
  ...
  var dateTxt=lvl==='a'?'' + T('ui.starten_ansehen_verstehen') + '':lvl==='p'?'' + T('ui.start_flow_volumen_belastung_debrief') + '':'' + T('ui.starten_planen_und_auswerten') + '';
  h+='<div class="hdr"><div><div class="greet">' + T('ui.trainingszentrale') + '</div><h1>' + T('ui.aktivitaeten_') + '</h1><div class="date">'+dateTxt+'</div></div><button class="iconbtn" aria-label="' + T('ui.verbindungen_amp_import') + '" onclick="openImportSheet()">'+icon('link','sm')+'</button></div>';
  ...
  h+='<div class="hub-actions">'+
    '<button class="hub-act primary" onclick="gmOpenStartSheet()"><span class="ha-ic">'+icon('play','sm')+'</span><div><b>' + T('ui.training_starten') + '</b><span>' + T('ui.sport_waehlen_geplant_oder_frei') + '</span></div></button>'+
    '<button class="hub-act" onclick="gmOpenStartSheet(\'planned\')"><span class="ha-ic">'+icon('calendar','sm')+'</span><div><b>' + T('ui.geplante_einheit') + '</b><span>'+(tItem?'' + T('ui.heute__') + ''+gmEsc(tItem.l):'' + T('ui.heute_') + '')+'</span></div></button>'+
    '<button class="hub-act" onclick="gmOpenStartSheet(\'free\')"><span class="ha-ic">'+icon('bolt','sm')+'</span><div><b>' + T('ui.freies_training') + '</b><span>' + T('ui.ohne_plan') + '</span></div></button>'+
    '<button class="hub-act" onclick="openManualActivity()"><span class="ha-ic">'+icon('pen','sm')+'</span><div><b>' + T('ui.manuell_hinzufuegen') + '</b><span>' + T('ui.nachtragen') + '</span></div></button>'+
    (function(){var la=null;try{var _l=listActivitiesUnified(1);la=(_l&&_l[0])||null;}catch(_){ }
      if(la){var vm2=null;try{vm2=activityDetailViewModel(la);}catch(_){ }var lbl=(vm2&&(vm2.sportLabel||vm2.title))||'' + T('ui.letzte_einheit') + '';var um=(vm2&&vm2.distanceLabel)?' '+vm2.distanceLabel:'';
        return '<button class="hub-act" onclick="gmOpenStartSheet(\'repeat\')"><span class="ha-ic">'+icon('run','sm')+'</span><div><b>' + T('ui.letzte_wiederholen') + '</b><span>'+gmEsc(lbl+um)+'</span></div></button></div>';}
      return '<button class="hub-act" disabled aria-disabled="true"><span class="ha-ic">'+icon('run','sm')+'</span><div><b>' + T('ui.letzte_wiederholen') + '</b><span>' + T('ui.noch_keine_aktivitaet') + '</span></div></button></div>';})();
  ...
  var kpis=[
    ...
    [(wk&&wk.totals&&wk.totals.sessionCount!=null)?String(wk.totals.sessionCount):'—','' + T('ui.einheiten_') + ''],
  ...
  if(lvl!=='a'){
    ...
    if(wk&&wk.totals&&wk.totals.completeness&&wk.totals.completeness.duration&&wk.totals.knownDurationMin>0){
      ...
      segs=[['Laufen',pct(vRun),'var(--ready)'],['Kraft',pct(vKraft),'var(--gold)'],['Rad',pct(vRad),'var(--activity)'],['' + T('ui.sonstiges') + '',pct(vRest),'var(--cyan)']];
    }
    h+='<div class="card"><div class="ctitle"><div class="l">' + T('ui.sportartenverteilung') + '</div><span class="more">'+(gmActScope==='week'?'Woche':'Monat')+'</span></div>'+
      ...
      '<div class="dist-leg">'+(segs?segs.map(function(s){return '<span><i style="background:'+s[2]+'"></i>'+s[0]+' '+s[1]+'%</span>';}).join(''):['Laufen','Kraft','Rad','' + T('ui.sonstiges') + ''].map(function(n,i){var c=['var(--ready)','var(--gold)','var(--activity)','var(--cyan)'][i];return '<span><i style="background:'+c+'"></i>'+n+' —</span>';}).join(''))+'</div></div>';
  }
  ...
  var _btSub='—';if(_bt){var _fs=function(sec){var m=Math.floor(sec/60),ss=Math.round(sec%60);return m+':'+String(ss).padStart(2,'0');};
    if(_bt.t5!=null)_btSub='5 km '+_fs(_bt.t5)+(_bt.real.k5?'':'' + T('ui.prognose') + '');else if(_bt.t10!=null)_btSub='10 km '+_fs(_bt.t10)+(_bt.real.k10?'':'' + T('ui.prognose') + '');}
  ...
  h+='<div class="hub-actions"><button class="hub-act" onclick="gmOpenBestTimesEntry()"><span class="ha-ic">'+icon('bolt','sm')+'</span><div><b>Bestleistung</b><span>'+gmEsc(_btSub)+'</span></div></button><button class="hub-act" onclick="gmOpenMilestonesEntry()"><span class="ha-ic">'+icon('target','sm')+'</span><div><b>' + T('ui.meilenstein') + '</b><span>'+gmEsc(_msSub)+'</span></div></button></div>';
  ...
  var cards=list.map(function(a){
    ...
    var um=vm.distanceLabel||((a.sportId==='gym'&&a.summary&&a.summary.exerciseCount!=null)?a.summary.exerciseCount+'' + T('ui.uebungen_') + '':null)||'—';
    var tempo=vm.paceLabel||((a.sportId==='gym'&&a.summary&&a.summary.rpe!=null)?'' + T('ui.rpe') + ''+a.summary.rpe:null)||'—';
    return '<article class="activity-card" role="button" tabindex="0" data-aid="'+gmEsc(aid)+'" onclick="gmOpenActivityPage(\''+gmEsc(aid)+'\')" onkeydown="if(event.key===\'Enter\')gmOpenActivityPage(\''+gmEsc(aid)+'\')">'+
      ...
      (a.status==='completed'?'<span class="session-state done">' + T('ui.abgeschlossen') + '</span>':'<span class="session-state">—</span>')+'</div>'+
      '<div class="activity-metrics"><div><b>'+gmEsc(um)+'</b><span>' + T('ui.umfang') + '</span></div><div><b>'+gmEsc(vm.durationLabel||'—')+'</b><span>DAUER</span></div><div><b>'+gmEsc(tempo)+'</b><span>TEMPO</span></div><div><b>'+(vm.avgHr!=null?gmEsc(vm.avgHr)+' bpm':'—')+'</b><span>' + T('ui.hf_') + '</span></div></div></div></article>';
  }).join('');
  h+='<div class="activity-list">'+(list.length?cards:'<div class="empty"><div class="e-ic">'+icon('activity')+'</div><div class="et">' + T('ui.keine_aktivitaet_in_diesem_filter') + '</div></div>')+'</div>';
  /* 9. Abschluss */
  h+='<div class="tabspacer"></div>';
  host.innerHTML=h;
}
...
function gmActStreamDefs(sportId){
  ...
  if(paceSports[sportId])speedDef={key:'speed',label:'' + T('ui.tempo_min_km') + '',unit:'/km',color:'sleep',hb:false,dec:2,fmt:paceFmt,conv:function(v){return v>0.3?(1000/v/60):null;}};
  else if(sportId==='cycling')speedDef={key:'speed',label:'' + T('ui.geschwindigkeit_km_h') + '',unit:' km/h',color:'sleep',hb:true,dec:1,conv:function(v){return v*3.6;}};
  else if(sportId==='swimming')speedDef={key:'speed',label:'' + T('ui.tempo_min_100_m') + '',unit:'/100 m',color:'sleep',hb:false,dec:2,fmt:paceFmt,conv:function(v){return v>0.1?(100/v/60):null;}};
  else speedDef={key:'speed',label:'' + T('ui.geschwindigkeit_m_s') + '',unit:' m/s',color:'sleep',hb:true,dec:1,conv:null};
  return [
    {key:'heart_rate',label:'' + T('ui.herzfrequenz_bpm') + '',unit:' bpm',color:'ready',hb:false,dec:0,conv:null},
    speedDef,
    {key:'cadence',label:'' + T('ui.kadenz_spm') + '',unit:' spm',color:'cyan',hb:true,dec:0,conv:null},
    {key:'elevation',label:'' + T('ui.hoehe_m') + '',unit:' m',color:'activity',hb:null,dec:0,conv:null}
  ];
}
...
function gmOpenActivityPage(aid){
  ...
  var dl=(vm.date?((typeof fmtDate==='function')?fmtDate(vm.date):vm.date):'—')+((vm.time&&!(vm.source==='legacy_local'&&vm.time==='00:00'))?' · '+vm.time:'');
  var h='<div class="page-head"><div class="page-head-row"><button class="backbtn" onclick="gmCloseActivityPage()" aria-label="' + T('ui.zurueck') + '">'+icon('chev')+'</button><div><h2>'+gmEsc(vm.title||vm.sportLabel||'Aktivität')+'</h2><p>'+gmEsc(dl)+'</p></div></div></div>';
  /* Route-Slot nur bei Laufaktivitäten; ohne GPS ehrlicher Empty-State im selben Slot. */
  if(run||route){
    h+='<div class="route-map">'+(route?((typeof routeSVG==='function')?routeSVG(route):''):'<div class="route-empty">'+icon('activity')+'<div>' + T('ui.keine_gps_route_fuer_diese') + '</div></div>')+'</div>';
  }
  h+='<div class="detail-title"><div class="plan-kicker">'+gmEsc(vm.sportLabel||'Aktivität')+(vm.planLink?'' + T('ui.plan_ist_verknuepft') + '':'')+'</div><h1>'+gmEsc(vm.title||vm.sportLabel||'—')+'</h1><p>'+gmEsc(gmActSrcLabel(vm.source))+(vm.planLink?'' + T('ui.dem_wochenplan_zugeordnet') + '':(vm.source==='orvia_workout'?'' + T('ui.in_orvia_aufgezeichnet_keine_nachbearbeitung') + '':'' + T('ui.quelle_unveraendert_uebernommen_keine_nachbearbeitung') + ''))+'</p></div>';
  ...
  var _hrC=[vm.avgHr!=null?vm.avgHr+' bpm':'—','' + T('ui.herzfrequenz') + ''];
  var kcells;
  if(_fam==='gym'){
    kcells=[
      ...
      _hrC,
      [_rpe!=null?'' + T('ui.rpe') + ''+_rpe:'—','BELASTUNG']
    ];
  }else if(_fam==='cycling'){
    kcells=[
      [vm.distanceLabel||'—','DISTANZ'],
      [vm.durationLabel||'—','DAUER'],
      [_spd!=null?fmtDe(_spd)+' km/h':'—','' + T('ui.geschwindigkeit') + ''],
      _hrC,
    ...
    kcells=[
      ...
      [vm.distanceLabel||'—','DISTANZ'],
      _hrC,
      [vm.maxHr!=null?vm.maxHr+' bpm':'—','' + T('ui.max_herzfrequenz') + ''],
      [vm.caloriesKcal!=null?fmtDe(vm.caloriesKcal)+' kcal':'—','KALORIEN'],
      [_rpe!=null?'' + T('ui.rpe') + ''+_rpe:'—','BELASTUNG']
    ];
  }
  ...
  if(_actTl!=null)kcells.push([String(_actTl),'' + T('ui.belastung_garmin') + '']);
  ...
  if((vm.source==='orvia_workout'||vm.source==='live')&&vm.status!=='active'&&a.durationSeconds!=null){
    ...
    h+='<div class="mini-note" style="margin:2px 18px 10px">'+icon('pen','xs')+'<div>'+
      (_dc?('' + T('ui.dauer_manuell_korrigiert') + ''+(_dc.fromMin!=null?_dc.fromMin+' min':'—')+' → <b>'+_dc.toMin+' min</b>. '):'')+
      '<a href="#" onclick="event.preventDefault();gmOpenDurationCorrectSheet(\''+gmEsc(a.clientRecordId||a.id)+'\','+Math.round(a.durationSeconds/60)+')" style="font-weight:700">' + T('ui.dauer_korrigieren') + '</a>'+
      (_dc?'':'' + T('ui.z_b_wenn_die_app') + '')+'</div></div>';
  }
  ...
  h+='<div class="coach-card"><h3>'+icon('sparkle','sm')+' ' + T('ui.orvia_debrief') + '</h3><p>'+(rate?gmEsc(rate.txt):'' + T('ui.ein_kanonisches_debrief_ist_fuer') + '')+'</p>'+
    '<div class="coach-tags"><span>'+(rate?'' + T('ui.beibehalten') + ''+gmEsc(rate.badge):'' + T('ui.das_beibehalten') + '')+'</span><span>'+(rate&&rate.next?gmEsc(rate.next):'' + T('ui.naechstes_mal') + '')+'</span><span>'+(rate?'' + T('ui.planwirkung') + '':'' + T('ui.auswirkung_auf_die_planung') + '')+'</span></div></div>';
  ...
  if(_fam==='gym'){
    ...
    if(_gx&&_gx.length){
        ...
        if(e.sets&&e.sets.length){
          sub=e.sets.map(function(st,i){
            var mid=(st.weight!=null&&st.reps!=null)?'<strong>'+fmtDe(st.weight)+' kg</strong><span class="gs-x">×</span><strong>'+st.reps+'</strong>'
              :(st.reps!=null)?'<strong>'+st.reps+'' + T('ui.wdh') + '</strong>'
              :(st.weight!=null)?'<strong>'+fmtDe(st.weight)+' kg</strong>':'<strong>—</strong>';
            return '<div class="gymset-row"><span>Satz '+(i+1)+'</span>'+mid+'</div>';}).join('');
        }else{
          sub='<div class="gymset-row"><span>'+(e.setsN!=null?e.setsN+'' + T('ui.saetze_') + '':'—')+'</span><strong>'+(e.kg!=null?fmtDe(e.kg)+' kg':'—')+'</strong><span class="gs-x">×</span><strong>'+(e.reps!=null?e.reps:'—')+'</strong></div>';
        }
        return '<div class="gymex"><div class="gymex-head"><b>'+gmEsc(e.name)+'</b><span>'+(e.volumeKg?gmKg(e.volumeKg)+'' + T('ui.kg_volumen') + '':'')+'</span></div>'+sub+'</div>';
      }).join('');
    }else{
      ...
      if(_st==='loading')_grows='<div class="gm-split-empty">' + T('ui.satzdetails_werden_aus_der_cloud') + '</div>';
      else if(_off&&_canServer)_grows='<div class="gm-split-empty">' + T('ui.offline_die_satzdetails_liegen_nicht') + '</div>';
      else if(_st==='server_empty')_grows='<div class="gm-split-empty">' + T('ui.fuer_diese_einheit_sind_weder') + '</div>';
      else if(_st==='error')_grows='<div class="gm-split-empty">Die Satzdetails konnten nicht geladen werden (Verbindungsfehler). <a href="#" onclick="event.preventDefault();gmActRetryGym(\''+gmEsc(String(aid))+'\')" style="font-weight:700">' + T('ui.erneut_versuchen') + '</a></div>';
      else _grows='<div class="gm-split-empty">'+GM_NA+' — für diese ' + T('ui.einheit') + ' sind keine Übungs- und Satzdetails gespeichert. ORVIA erfindet keine ' + T('ui.saetze__') + '.</div>';
      if(_canServer)_grows+='<div class="mini-note" style="margin-top:8px">'+icon('info','xs')+'<div><a href="#" onclick="event.preventDefault();gmOpenSetsDiagnose(\''+gmEsc(String(aid))+'\')" style="font-weight:700">Warum fehlen die ' + T('ui.saetze__') + '?</a> — zeigt, wo die Daten tatsächlich liegen.</div></div>';
    }
    h+='<div class="card"><div class="ctitle"><div class="l">'+icon('dumbbell')+'' + T('ui.uebungen_') + ' &amp;' + T('ui.saetze_') + '</div>'+(_gym&&_gym.setCount?'<span class="more">'+_gym.setCount+'' + T('ui.saetze_') + '</span>':'')+'</div>'+_grows+'</div>';
  }
  ...
  if(run||_hasStream||splits){
    ...
    if(run||_slots)h+='<div class="card"><div class="ctitle"><div class="l">'+icon('chart')+' Aktivitäts-Messreihen (Garmin)</div><span class="more">'+(_gmActCharts.length?_gmActCharts.length+'' + T('ui.serien') + '':'')+'</span></div>'+
      (_slots?_slots:'<div class="gm-chart-empty">'+GM_NA+' — für diese ' + T('ui.einheit') + ' liegt keine kanonische Messreihe vor. Keine nachgebaute Kurve.</div>')+'</div>';
    ...
    if(splits){
      ...
      _splitNote='<div style="margin-top:8px;font-size:10.5px;color:var(--muted)">'+(splitsCanon?'' + T('ui.echte_runden_aus_der_aktivitaetsquelle') + '':'' + T('ui.runden_aus_der_verknuepften_trainingssession') + '')+'</div>';
    }else{
      srows='<div class="gm-split-empty">'+GM_NA+' — für diese ' + T('ui.einheit') + ' liegen keine echten ' + T('ui.runden_') + ' vor. Aus den gespeicherten Messreihen lassen sich keine ' + T('ui.splits') + ' ableiten (die Serien tragen keinen Zeitstempel) — ORVIA rechnet hier nichts hoch.</div>';
    }
    if(run||splits)h+='<div class="card"><div class="ctitle"><div class="l">'+(splitsCanon?'' + T('ui.runden_') + '':'Kilometer-' + T('ui.splits') + '')+'</div>'+(splits?'<span class="more">'+splits.length+'</span>':'')+'</div><div class="split-list">'+srows+'</div>'+_splitNote+'</div>';
  }
  h+='<div class="tabspacer"></div>';
  pg.innerHTML=h;
  pg.classList.add('on');
...
function gmOpenSetsDiagnose(aid){
  ...
  var body='<div class="md-pro">'+
    row('' + T('ui.aktivitaet_gefunden') + '',a?'ja':'nein',!!a)+
    row('Quelle',(a&&a.source)||'—')+
    row('' + T('ui.uebungen_auf_diesem_geraet') + '',String(snapN),snapN>0)+
    row('' + T('ui.saetze_auf_diesem_geraet') + '',String(setsN),setsN>0)+
    ...
    '</div>';
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('info')+'</div><div><h3>' + T('ui.wo_liegen_die_saetze') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.gemessener_ist_zustand_keine_vermutung') + '</div></div></div>'+
    '<div class="sh-block">'+body+'<p class="note" style="text-align:left;margin-top:10px" id="gmSetsDiagOut">' + T('ui.tippe_in_der_cloud_nachsehen') + '</p></div>'+
    '<div class="sheet-cta"><button class="sec" onclick="gmCloseSheets()">' + T('ui.schliessen') + '</button>'+
...
function gmSetsDiagProbe(aid,sid){
  var out=document.getElementById('gmSetsDiagOut');if(out)out.textContent='' + T('ui.frage_die_cloud_ab') + '';
  var repos=window.ORVIA&&ORVIA.repos&&ORVIA.repos.workout;
  if(!repos||!repos.loadWorkoutTree){if(out)out.textContent='' + T('ui.cloud_modul_nicht_geladen') + '';return;}
  repos.loadWorkoutTree(sid).then(function(r){
    if(!out)return;
    if(!(r&&r.success)){out.textContent='' + T('ui.cloud_abfrage_fehlgeschlagen_') + ''+((r&&r.error&&r.error.message)?': '+r.error.message:'.')+'' + T('ui.die_saetze_sind_dadurch_nicht') + '';return;}
    var exs=(r.data&&r.data.exercises)||[];var n=0;exs.forEach(function(e){n+=((e&&e.sets)||[]).length;});
    if(n>0){out.textContent='' + T('ui.in_der_cloud_liegen') + ''+exs.length+'' + T('ui.uebungen_mit') + ''+n+'' + T('ui.saetzen_sie_werden_jetzt_auf') + '';
    ...
    }else{out.textContent='' + T('ui.auch_in_der_cloud_sind') + '';}
  }).catch(function(e){if(out)out.textContent='' + T('ui.cloud_abfrage_fehlgeschlagen') + ''+String(e&&e.message||e);});
}
...
function gmOpenDurationCorrectSheet(aid,curMin){
  var sh=document.getElementById('detailSheet');if(!sh)return;
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('pen')+'</div><div><h3>' + T('ui.dauer_korrigieren') + '</h3><div class="sh-sub" style="margin:2px 0 0">Aktuell '+curMin+' min</div></div></div>'+
    '<div class="sh-block"><p>' + T('ui.trainierte_zeit_in_minuten_z') + '</p>'+
    '<div class="calc-field" style="margin-top:8px"><label>' + T('ui.dauer_min') + '</label><input type="number" id="gmDurCorrIn" inputmode="numeric" min="1" max="1440" value="'+curMin+'" style="width:110px;text-align:right"></div></div>'+
    '<div class="sheet-cta"><button class="sec" onclick="gmCloseSheets()">' + T('ui.abbrechen') + '</button><button class="prim" onclick="gmApplyDurationCorrect(\''+gmEsc(aid)+'\')">' + T('ui.speichern') + '</button></div>';
  gmOpenSheet('detailSheet');
...
function gmApplyDurationCorrect(aid){
  ...
  if(!(v>0&&v<=1440)){if(typeof toast==='function')toast('' + T('ui.bitte_eine_dauer_zwischen_1') + '');return;}
  var ws=window.ORVIA&&ORVIA.workoutStore;
  if(!ws||!ws.correctFinishedDuration){if(typeof toast==='function')toast('' + T('ui.korrektur_nicht_verfuegbar') + '');return;}
  ws.correctFinishedDuration(aid,v).then(function(r){
    gmCloseSheets();
    if(r&&r.success){if(typeof toast==='function')toast('' + T('ui.dauer_korrigiert') + ''+v+' min ✓');
      try{gmOpenActivityPage(aid);}catch(_){ }
      try{if(typeof renderAkt==='function')renderAkt();}catch(_){ }}
    else{if(typeof toast==='function')toast('' + T('ui.korrektur_fehlgeschlagen') + ''+(r&&r.error&&r.error.message?': '+r.error.message:'.'));}
  });
}
...
function gmOpenActTeaserSheet(kind){
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon(kind==='best'?'bolt':'target')+'</div><div><h3>'+t+'</h3><div class="sh-sub" style="margin:2px 0 0">'+GM_NA+'</div></div></div>'+
    '<div class="sh-block"><p>'+t+' erscheinen mit deinen ersten abgeschlossenen ' + T('ui.aktivitaeten_') + ' — gemessen, nicht erfunden. ORVIA zeigt keine erfundenen Werte.</p></div>';
  gmOpenSheet('detailSheet');
}
...
function gmOpenStartSheet(mode){
  ...
  var sh=document.getElementById('detailSheet');if(!sh)return;
  var lvl=(typeof gmLevel==='function')?gmLevel():'f';
  var title=mode==='planned'?'' + T('ui.geplante_einheit_starten') + '':mode==='repeat'?'' + T('ui.letztes_training_wiederholen') + '':mode==='free'?'' + T('ui.freies_training') + '':'' + T('ui.training_starten') + '';
  var sub=lvl==='a'?'' + T('ui.waehle_deine_sportart') + '':lvl==='p'?'' + T('ui.sportart_geplant_frei_pre_start') + '':'' + T('ui.sportart_waehlen_dann_geplant_oder') + '';
...
function gmStartSport(sport){
  ...
  if(!hint)hint='' + T('ui.keine_kanonische_readiness_bewertung_verfuegbar') + '';
  if(plannedMode&&!planned)hint=(_sel.status==='ambiguous')
    ?'' + T('ui.mehrere_passende_planeinheiten_heute_oeffne') + ''
    :'' + T('ui.fuer_diese_sportart_ist_heute') + '';
  var canStart=plannedMode?planned:!!(window.ORVIA&&ORVIA.workoutUI&&ORVIA.workoutUI.startSport);
  var rows=[
    ['' + T('ui.ziel_der_einheit') + '',planned?gmEsc(tItem.l):'—'],
    ['Dauer','—'],
    [sport==='Krafttraining'?'Volumen':'' + T('ui.distanz_') + '','—'],
    ['' + T('ui.intensitaet') + '','—'],
    ['Ausrüstung','—'],
    ['Wearable','—']
  ];
  ...
  if(typeof gmFeatureFlag!=='function'||gmFeatureFlag('preWorkoutGarmin')){
    ...
    var pr=[
      ...
      ['' + T('ui.stress_heute') + '',(_st5&&_st5.value!=null)?fmtDe(_st5.value):'—'],
      ['Readiness',(_os5&&_os5.score!=null)?_os5.score+' · '+gmEsc(_os5.statusText||''):'—']
    ];
    preRows='<div class="sh-block" style="margin:0 0 6px"><div class="bh">' + T('ui.vor_start_werte_gemessen') + '</div>'+
      ...
      '<p style="margin:6px 0 0;font-size:11px;color:var(--muted)">' + T('ui.aus_deinen_garmin_check_in') + '</p></div>';
  }
  sh.innerHTML='<div class="grab"></div><h3>'+gmEsc(sport)+'</h3><div class="sh-sub">' + T('ui.vor_dem_start') + '</div>'+
...
function gmStartFromPreStart(){
  ...
  if(_gmStartCtx.mode==='planned'){
    if(!sel||sel.status!=='unique'){
      if(typeof toast==='function')toast(sel&&sel.status==='ambiguous'?'' + T('ui.mehrere_passende_planeinheiten_bitte_im') + '':'' + T('ui.keine_passende_planeinheit_fuer_diese') + '');
...
function gmStoryPages(a){
  ...
  var dl=(vm.date?((typeof fmtDate==='function')?fmtDate(vm.date):vm.date):'')+((vm.time&&!(vm.source==='legacy_local'&&vm.time==='00:00'))?' · '+vm.time:'');
  var title=vm.title||vm.sportLabel||'' + T('ui.training') + '';
  ...
  var VERB={running:'gelaufen',trail_running:'gelaufen',walking:'gegangen',hiking:'gewandert',cycling:'' + T('ui.rad_gefahren') + '',swimming:'geschwommen',rowing:'gerudert'};
  ...
  if(vm.avgHr!=null)subParts.push('' + T('ui.hf') + ''+vm.avgHr+' bpm');
  if(vm.caloriesKcal!=null)subParts.push(fmtDe(vm.caloriesKcal)+' kcal');
  var sub=subParts.join(' · ');
  var hl;
  if(fam==='gym'&&gym&&gym.exCount){
    hl='' + T('ui.du_hast') + ''+em(gym.exCount+(gym.exCount===1?'' + T('ui.uebung') + '':'' + T('ui.uebungen_') + ''))+(gym.setCount?' mit '+em(gym.setCount+'' + T('ui.saetzen') + ''):'')+' absolviert.';
    sub=[gym.volumeKg?gmKg(gym.volumeKg)+'' + T('ui.kg_bewegtes_volumen') + '':null,durTxt,vm.avgHr!=null?'' + T('ui.hf') + ''+vm.avgHr+' bpm':null].filter(Boolean).join(' · ');
  }else if(VERB[sNorm]&&vm.distanceLabel){
    hl='' + T('ui.du_bist') + ''+em(vm.distanceLabel)+' '+VERB[sNorm]+'.';
  }else if(VERB[sNorm]&&durTxt){
    hl='' + T('ui.du_bist') + ''+em(durTxt)+' '+VERB[sNorm]+'.';
  }else if(PLAY[sNorm]&&durTxt){
    hl='' + T('ui.du_hast') + ''+em(durTxt)+' '+gmEsc(vm.sportLabel||'')+' gespielt.';
  }else if(durTxt){
    hl='' + T('ui.du_hast') + ''+em(durTxt)+' '+gmEsc(vm.sportLabel||'' + T('ui.training') + '')+' absolviert.';
  }else{
    hl=gmEsc(vm.sportLabel||'' + T('ui.einheit') + '')+' abgeschlossen.';
  }
  ...
  if(!cover){
    var bigV=(fam==='gym'&&gym&&gym.setCount)?[String(gym.setCount),'' + T('ui.saetze__') + '']
      :(vm.distanceLabel?[vm.distanceLabel,'' + T('ui.distanz_') + '']:(durTxt?[durTxt,'Dauer']:null));
  ...
  pages.push(page('<div class="wst-kick'+(route?'':' ctr')+'">' + T('ui.einheit_abgeschlossen') + '</div>'+cover,foot(hl,sub)));
  ...
  try{
    ...
    if(distPB){
      ...
      pages.push('<div class="wst-bg pr" style="--acc:'+GOLD2+';--accsoft:'+GOLDSOFT2+'"></div><div class="wst-in" style="--acc:'+GOLD2+';--accsoft:'+GOLDSOFT2+'">'+top+
        '<div class="wst-mid"><div class="wst-kick pr">'+icon('bolt','sm')+'<span>' + T('ui.neue_bestzeit_gemessen') + '</span></div><div class="wst-prval">'+gmEsc(fmtDe(distPB.targetKm))+' km in '+gmEsc(_fs3(distPB.sec))+'</div></div>'+
        foot('' + T('ui.deine_schnellste_gemessene') + ''+em(fmtDe(distPB.targetKm)+' km')+'' + T('ui.strecke') + ''+(distPB.method==='stream_window'?'' + T('ui.aus_den_messreihen_deiner_uhr') + '':distPB.method==='lap_window'?'' + T('ui.aus_den_runden_deiner_uhr') + '':'' + T('ui.aus_dieser_aktivitaet') + '')+'.','' + T('ui.gemessen_ueber') + ''+fmtDe(distPB.km)+'' + T('ui.km_keine_schaetzung') + '')+'</div>');
    }
    var pb=distPB?null:gmActPersonalBest(a,vm,fam);
    if(pb){
      var GOLD='#DCC79A',GOLDSOFT='rgba(220,199,154,.35)';
      var prTitle=(pb.cur.kind==='speed')?'' + T('ui.neue_bestleistung') + '':'' + T('ui.neue_bestzeit') + '';
      ...
      if(pb.cur.kind==='speed'){
        ...
        prSub=fmtDe(dKmh)+'' + T('ui.km_h_schneller_als_deine') + ''+prevTxt+')';
      }else{
        var dSec=Math.max(0,Math.round(pb.bestOther-pb.cur.metric));
        prSub=fmtPace(dSec)+' schneller pro '+(pb.cur.kind==='pace100'?'100 m':'km')+'' + T('ui.als_deine_bisherige_bestzeit') + ''+prevTxt+')';
      }
  ...
  try{
    ...
    if(laps&&laps.length>=2){
      ...
      pages.push(page('<div class="wst-kick">' + T('ui.runden_') + '</div><div class="wst-laps">'+rows+'</div>'+(laps.length>maxRows?'<div class="wst-note">+ '+(laps.length-maxRows)+' weitere im Aktivitätsdetail.</div>':''),
        foot('' + T('ui.schnellste_runde') + ''+em(fmtL(fast)),'' + T('ui.runde') + ''+(iFast+1)+' von '+laps.length+' · langsamste '+fmtL(slow))));
    }
  }catch(_){ }
  ...
  if(fam==='gym'&&gym&&gym.list&&gym.list.length){
    var exRows=gym.list.slice(0,8).map(function(e,i){
      ...
      if(e.sets&&e.sets.length){
        det=e.sets.map(function(s4){
          ...
          if(s4.reps!=null)return s4.reps+'' + T('ui.wdh') + '';
          return s4.weight!=null?fmtDe(s4.weight)+' kg':'—';
        }).join(' · ');
      }else{
        det=[(e.setsN?e.setsN+'' + T('ui.saetze_') + '':null),(e.kg!=null?fmtDe(e.kg)+' kg':null),(e.reps!=null?'× '+e.reps:null)].filter(Boolean).join(' · ')||'—';
      }
    ...
    pages.push(page('<div class="wst-kick">' + T('ui.uebungen__') + '</div><div class="wst-laps">'+exRows+'</div>'+(gym.list.length>8?'<div class="wst-note">+ '+(gym.list.length-8)+' weitere im Aktivitätsdetail.</div>':''),
      foot(gym.setCount?em(gym.setCount+'' + T('ui.saetze_') + '')+'' + T('ui.im_log') + '':'' + T('ui.dein_krafttraining') + '',gym.volumeKg?gmKg(gym.volumeKg)+'' + T('ui.kg_gesamtvolumen_summe_aus_gewicht') + '':null)));
  }
  ...
  if(hr){
    ...
    pages.push(page('<div class="wst-kick">' + T('ui.herzfrequenz__') + '</div>'+gmStoryDotChart(hr,' bpm',0),
      foot('Ø '+em(hrAvg+' bpm')+'' + T('ui.ueber_die_einheit') + '','' + T('ui.maximal') + ''+hrMax+'' + T('ui.bpm_gemessene_werte_nichts_nachgerechnet') + '')));
  }
  ...
  if(fam==='cycling'){
    ...
    if(spdC){
      ...
      pages.push(page('<div class="wst-kick">' + T('ui.geschwindigkeit__') + '</div>'+gmStoryDotChart(spdC,' km/h',1),
        foot('Ø '+em(fmtDe(spAvg)+' km/h')+'.','' + T('ui.gemessene_geschwindigkeit_reine_einheitenumrechnung_aus') + '')));
    }
  }
  /* ---------- 6) Kennzahlen-Raster: nur belegte Zellen, gestaffelt ---------- */
  var cells=[];
  if(vm.distanceLabel)cells.push([vm.distanceLabel,'' + T('ui.distanz_') + '']);
  if(durTxt)cells.push([durTxt,'Dauer']);
  if(fam==='cycling'){if(spdAvg!=null)cells.push([fmtDe(spdAvg)+' km/h','' + T('ui.geschwindigkeit_') + '']);}
  else if(vm.paceLabel)cells.push([vm.paceLabel,'' + T('ui.tempo_') + '']);
  if(vm.avgHr!=null)cells.push([vm.avgHr+' bpm','' + T('ui.herzfrequenz_') + '']);
  if(vm.maxHr!=null)cells.push([vm.maxHr+' bpm','' + T('ui.max_herzfrequenz_') + '']);
  if(gym){
    if(gym.exCount!=null)cells.push([String(gym.exCount),'' + T('ui.uebungen__') + '']);
    if(gym.setCount!=null)cells.push([String(gym.setCount),'' + T('ui.saetze__') + '']);
  ...
  if(cad!=null&&fam==='pace')cells.push([cad+' spm','' + T('ui.schrittfrequenz') + '']);
  ...
  if(rpe0!=null)cells.push(['' + T('ui.rpe') + ''+rpe0,'Belastung']);
  if(cells.length>=2){
    pages.push(page('<div class="wst-kick">' + T('ui.deine_zahlen') + '</div><div class="wst-grid">'+cells.slice(0,8).map(function(c,i){
      return '<div class="wst-cell" style="animation-delay:'+(120+i*80)+'ms"><b>'+gmEsc(String(c[0]))+'</b><span>'+gmEsc(c[1])+'</span></div>';}).join('')+'</div>',
      foot('' + T('ui.alles_aus_deiner_einheit') + '','' + T('ui.werte_unveraendert_aus_der_aktivitaetsquelle') + '')));
  }
  ...
  if(rate2){
    pages.push(page('<div class="wst-kick">' + T('ui.orvia_debrief') + '</div><div class="wst-debrief">'+gmEsc(rate2.txt)+'</div>'+
      '<div class="wst-tags"><span>' + T('ui.beibehalten') + ''+gmEsc(rate2.badge||'—')+'</span>'+(rate2.next?'<span>'+gmEsc(rate2.next)+'</span>':'')+'</div>',
      foot('' + T('ui.eingeordnet') + ''+em(rate2.badge||'—'),null)));
  }
  ...
  if(lvl!=='a'){
    try{
      ...
      if(S2&&S2.ctl&&S2.ctl.length&&!sup3){
        ...
        pages.push(page('<div class="wst-kick">' + T('ui.wirkung') + '</div><div class="wst-grid">'+[[String(ctl2),'' + T('ui.fitness_ctl') + ''],[String(atl2),'' + T('ui.ermuedung_atl') + ''],[(tsb2>=0?'+':'')+tsb2,'' + T('ui.form_tsb') + '']].map(function(c,i){
          return '<div class="wst-cell" style="animation-delay:'+(120+i*80)+'ms"><b>'+gmEsc(String(c[0]))+'</b><span>'+gmEsc(c[1])+'</span></div>';}).join('')+'</div>',
          foot('' + T('ui.deine_belastung_heute') + '','' + T('ui.tageswerte_des_kanonischen_lastmodells_srpe') + '')));
      }
    }catch(_){ }
  }
  return pages;
...
function gmStoryRender(){
  ...
  host.innerHTML=bars+
    ...
    '<button class="wst-nav prev" aria-label="' + T('ui.zurueck') + '" onclick="gmStoryPrev()"></button>'+
    '<button class="wst-nav next" aria-label="' + T('ui.weiter') + '" onclick="gmStoryNext()"></button>'+
...
function gmAnaOverview(ctx){
  ...
  if(d&&lvl==='p'){heroP='' + T('ui.readiness_') + ''+(sc&&sc.score!=null?sc.score:'—')+'' + T('ui.ctl') + ''+(ctl!=null?ctl:'—')+'' + T('ui.acwr') + ''+(acwrShow!=null?fmtDe(acwrShow):'—')+'' + T('ui.alle_werte_read_only_aus') + '';}
  if(!heroP)heroP=GM_NA+'' + T('ui.die_entscheidung_erscheint_nach_dem') + '';
  var h='<div class="decision-hero"><div class="eyebrow">' + T('ui.wichtigste_erkenntnis_heute') + '</div><h2>'+gmEsc(heroT)+'</h2><p>'+gmEsc(heroP)+'</p>'+
  ...
  var kpis=[
    ...
    [ctl!=null?String(ctl):'—','' + T('ui.fitness_ctl_') + '',lvl==='p'?('' + T('ui.srpe_skala_atl') + ''+(atl!=null?atl:'—')):(ctl!=null?'' + T('ui.srpe_skala_42_t') + '':'—')],
    [acwrShow!=null?fmtDe(acwrShow):'—','' + T('ui.belastung_acwr') + '',acwrShow!=null?'Lastmodell':'—'],
    (function(){/* GM7: Planerfuellung aus dem kanonischen Plan-Ist-Abgleich (7 Tage) */
      try{if(typeof planActualResolveForDates==='function'&&Calc.resolvePlanActual){
        ...
        if(tot>0)return [Math.round(donec/tot*100)+'%','Planerfüllung','' + T('ui.7_tage') + ''+donec+'/'+tot];
      }}catch(_){ }
      return ['—','Planerfüllung','' + T('ui.kein_plan_ist_abgleich') + ''];})()
  ];
  ...
  h+='<div class="card"><div class="ctitle"><div class="l">'+icon('chart')+' ' + T('ui.form') + '&amp; Belastbarkeit</div><span class="more">' + T('ui.14_tage') + '</span></div>'+
    (ctx.ok?'<div class="oc2" id="gmAnaChart"></div>':gmAnaChartEmpty((ctx.lcc&&ctx.lcc.ctlAtlNote)?ctx.lcc.ctlAtlNote:GM_NA+'' + T('ui.die_form_kurve_erscheint_ab') + ''))+'</div>';
  /* 3 Insight-Slots: Safety-Warnung zuerst (in allen Modi), dann weekInsights, Rest Missing */
  h+='<div class="sectlabel" data-gm-slot="analysis-insights">' + T('ui.was_orvia_daraus_macht') + '</div>';
  var slots=[];
    ...
    try{
      ...
      tips.forEach(function(t){if(slots.length>=3)return;
        slots.push({ic:t.sev>=4?'alert':t.sev>=3?'gauge':'info',b:t.title,
          p:t.reason+'' + T('ui.konfidenz') + ''+t.conf+')',il:'Tip-Engine',ir:t.rec||'—'});});
    }catch(_){ }
  }
  while(slots.length<3)slots.push({ic:'info',b:'—',p:GM_NA+'' + T('ui.orvia_zeigt_hier_nur_echte') + '',il:'Insight',ir:'—'});
  ...
  h+='<div class="sectlabel" data-gm-slot="analysis-progress">' + T('ui.fortschritt_amp_naechster_schritt') + '</div>';
  h+=(function(){/* Naechster Meilenstein aus dem gemessenen Achievements-Modell (2026-08-04). */
    ...
    if(!nm)return '<div class="mile" role="button" tabindex="0" onclick="gmOpenAnaTeaserSheet(\'ms\')" onkeydown="if(event.key===\'Enter\')gmOpenAnaTeaserSheet(\'ms\')"><div class="mi-ic">'+icon('target','sm')+'</div><div class="mile-b"><div class="mile-t">' + T('ui.naechster_meilenstein') + '</div><div class="mile-d">'+GM_NA+' — folgt mit deinen ersten ' + T('ui.aktivitaeten_') + '.</div><div class="mile-track"><i style="width:0%"></i></div></div>'+icon('chev','sm')+'</div>';
    return '<div class="mile" role="button" tabindex="0" onclick="gmOpenMilestonesEntry()" onkeydown="if(event.key===\'Enter\')gmOpenMilestonesEntry()"><div class="mi-ic">'+icon(nm.icon||'target','sm')+'</div><div class="mile-b"><div class="mile-t">Nächster ' + T('ui.meilenstein') + ': '+gmEsc(nm.label)+' '+gmEsc(gmAchFmtVal(nm.next,nm.unit))+'</div><div class="mile-d">Ist '+gmEsc(gmAchFmtVal(nm.current,nm.unit))+' — gemessen aus deinen ' + T('ui.aktivitaeten_') + '.</div><div class="mile-track"><i style="width:'+(nm.progress||0)+'%"></i></div></div>'+icon('chev','sm')+'</div>';})();
  h+=(function(){/* GM7: Bestzeiten aus bestTimes() (kanonisch, gleicher Renderer wie Profil) */
    ...
    var t2=(bt&&bt.t5!=null)?('5 km '+fs0(bt.t5)+(bt.real.k5?'':'' + T('ui.prognose') + '')):(bt&&bt.t10!=null)?('10 km '+fs0(bt.t10)+(bt.real.k10?'':'' + T('ui.prognose') + '')):null;
    /* KF-021: die Unterzeile nennt die tatsaechliche Quelle des angezeigten Werts. */
    var d2=t2?(bt.n+'' + T('ui.laeufe_ausgewertet') + ''+gmBtSrcLabel(bt,(bt.t5!=null?'k5':'k10'))):(GM_NA+'' + T('ui.keine_erfundene_bestzeit') + '');
    return '<div class="mile" role="button" tabindex="0" onclick="gmOpenBestTimesEntry()" onkeydown="if(event.key===\'Enter\')gmOpenBestTimesEntry()"><div class="mi-ic">'+icon('bolt','sm')+'</div><div class="mile-b"><div class="mile-t">'+(t2?'' + T('ui.beste_zeit') + ''+gmEsc(t2):'' + T('ui.letzte_bestzeit') + '')+'</div><div class="mile-d">'+gmEsc(d2)+'</div></div>'+icon('chev','sm')+'</div>';})();
  return h;
}
...
function gmAnaEndurance(ctx){
  var lvl=(typeof gmLevel==='function')?gmLevel():'f';
  var sub=lvl==='a'?'' + T('ui.deine_fitness_und_form_auf') + '':lvl==='p'?'' + T('ui.form_fitness_modell_schwellen_und') + '':'' + T('ui.form_belastung_und_entwicklung_je') + '';
  var h='<div class="body-head"><div class="ana-kick">' + T('ui.ausdauer') + '</div><div class="ana-sub">'+sub+'</div></div>';
  /* Form & Fitness — D1-Anbindung (CTL/ATL/TSB read-only) */
  var note=lvl==='a'?'' + T('ui.gold_steigend_du_wirst_fitter') + '':lvl==='p'?'' + T('ui.ctl_42_tage_ewma_der_') + '':'' + T('ui.fitness_baut_sich_ueber_wochen_') + '';
  var ffLegend='<div class="dist-leg" style="margin-top:6px"><span><i style="background:var(--gold-soft,#c9ae7c)"></i>' + T('ui.ctl_fitness') + '</span><span><i style="background:var(--crit)"></i>' + T('ui.atl_ermuedung') + '</span><span><i style="background:var(--ready)"></i>' + T('ui.form_tsb') + '</span></div>';
  h+='<div class="card"><div class="ctitle"><div class="l">'+icon('chart')+' ' + T('ui.form') + '&amp; Fitness</div><span class="more">' + T('ui.42_tage') + '</span></div>'+
    (ctx.ok?'<div class="oc2" id="gmFFChart"></div>'+ffLegend:gmAnaChartEmpty((ctx.lcc&&ctx.lcc.ctlAtlNote)?ctx.lcc.ctlAtlNote:GM_NA+'' + T('ui.ctl_atl_erscheinen_ab_14') + ''))+
  ...
  var kv=[
    ...
    [thr?(Calc&&Calc.fmtPace?Calc.fmtPace(thr.value):fmtDe(thr.value)):'—','' + T('ui.schwelle_km') + '',lvl==='a'?'':(thr?'LT-Pace':'—')],
    [rkm!=null?fmtDe(rkm):'—','Wochen-km','Laufen'],
    [endMin!=null?gmActFmtMin(endMin):'—','' + T('ui.ausdauer_h') + '','Woche']
  ];
  ...
  if(lvl!=='a'){
    ...
    h+='<div class="card"><div class="ctitle"><div class="l">'+icon('activity')+' Wochenvolumen Laufen</div><span class="more">' + T('ui.6_wochen') + '</span></div>'+
      (wkOk?'<div class="oc2" id="gmVolChart"></div>':gmAnaChartEmpty(GM_NA+'' + T('ui.fuer_wochen_ohne_belastbare_kanonische') + ''+wkm.map(function(v){return v==null?'—':fmtDe(v);}).join(' · ')))+
      '<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.ist_kilometer_aus_der_kanonischen') + '</div></div></div>';
  }
  ...
  h+=(function(){
    ...
    var body=rows.map(function(r2){var v=r2[1];
      return '<div class="calc-field" style="margin-bottom:8px"><label>'+r2[0]+'</label><div style="text-align:right"><b style="font-size:16px">'+(v!=null?gmEsc(fs(v)):'—')+'</b><div style="font-size:10px;color:var(--muted)">'+(v!=null?(r2[2]?'' + T('ui.echte_bestzeit') + '':'' + T('ui.prognose_riegel') + ''):'—')+'</div></div></div>';}).join('');
    var noteP=bt?('' + T('ui.prognose_aus_deinem_schnellsten_lauf') + ''+bt.n+'' + T('ui.laeufe_riegel_exponent_1_06') + ''):(GM_NA+'' + T('ui.noch_keine_auswertbaren_laeufe_fuer') + '');
    return '<div class="sectlabel" data-gm-slot="analysis-race-forecast">' + T('ui.wettkampfprognose') + '</div><div class="card"><div class="link-row">'+body+'</div><div class="mini-note">'+icon('info','xs')+'<div>'+noteP+'</div></div></div>';})();
  ...
  h+=(function(){
    ...
    return '<div class="sectlabel">' + T('ui.garmin_wettkampfprognose') + '</div><div class="card"><div class="link-row">'+body2+'</div><div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.von_der_uhr_gemessene_prognose') + '</div></div></div>';})();
  ...
  h+=(function(){
    var perf=[['vo2max_running','' + T('ui.vo_max_lauf') + '','pulse'],['vo2max_cycling','' + T('ui.vo_max_rad') + '','pulse'],['endurance_score','Endurance','activity'],['running_tolerance','Run-Toleranz','activity'],['fitness_age','Fitnessalter','pulse'],['respiration_avg','Atemfrequenz','wind']];
    var tiles='',any=false;
    ...
    return any?('<div class="sectlabel">' + T('ui.leistungswerte') + '<span class="ana-count">' + T('ui.tippen_fuer_details') + '</span></div><div class="kgrid">'+tiles+'</div>'):'';
  })();
  ...
  var _btSub2=(function(){var b2=null;try{b2=(typeof bestTimes==='function')?bestTimes():null;}catch(_){return '—';}
    ...
    if(b2.t5!=null)return '5 km '+fs2(b2.t5)+(b2.real&&b2.real.k5?'':'' + T('ui.prognose') + '');
    if(b2.t10!=null)return '10 km '+fs2(b2.t10)+(b2.real&&b2.real.k10?'':'' + T('ui.prognose') + '');
    return '—';})();
  h+='<div class="sectlabel" data-gm-slot="analysis-quick-access">' + T('ui.schnellzugriff') + '</div><div class="hub-actions">'+
    ...
    '<button class="hub-act" onclick="gmOpenPaceCalcSheet()"><span class="ha-ic">'+icon('gauge','sm')+'</span><div><b>' + T('ui.pace_rechner') + '</b><span>' + T('ui.zielzeit_pace_und_prognose') + '</span></div></button>'+
...
/* --- Erholung --- */
var GM_RCV_TILES=[
  {id:'sleep_duration_min',label:'' + T('ui.schlaf') + '',ic:'moon'},
  ...
  {id:'recovery_time_h',label:'' + T('ui.recovery_time') + '',ic:'heart'},
  /* GM7.4-2: Group-1 load_recovery (Garmin-Provider-Werte, keine ORVIA-Scores) */
  {id:'training_readiness',label:'Readiness',ic:'bolt'},
  {id:'acute_load',label:'' + T('ui.acute_load') + '',ic:'gauge'},
  {id:'load_ratio',label:'' + T('ui.load_ratio') + '',ic:'gauge'}
];
function gmAnaRecovery(){
  var lvl=(typeof gmLevel==='function')?gmLevel():'f';
  var sub=lvl==='a'?'' + T('ui.wie_gut_du_dich_erholst') + '':lvl==='p'?'' + T('ui.resolver_snapshot_werte_quelle_freshness') + '':'' + T('ui.schlaf_hrv_stress_und_energie') + '';
  var h='<div class="body-head"><div class="ana-kick">' + T('ui.erholung_') + '</div><div class="ana-sub">'+sub+'</div></div>';
  ...
  h+=(function(){
    var defs=[['sleep_duration_min','' + T('ui.schlaf') + '','sleep',function(v){return fmtDe(v/60)+' h';}],
    ...
    defs.forEach(function(df){
      ...
      if(ser&&ser.values.length>=3){any=true;
        ...
        rows2+='<div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--hair)"><div style="width:92px;font-size:11.5px;font-weight:700;color:var(--muted)">'+df[1]+'</div><div style="flex:1;font-size:10.5px;color:var(--faint)">'+(ser?'erst '+ser.values.length+'' + T('ui.messwert_e') + '':'' + T('ui.keine_serie_gespeichert') + '')+'</div><div style="width:70px;text-align:right;font-size:13px;font-weight:800;color:var(--muted)">—</div></div>';
      }});
    var note2=lvl==='p'?'' + T('ui.einzelserien_aus_dem_kanonischen_metrik') + '':'' + T('ui.deine_gemessenen_erholungswerte_der_letzten') + '';
    return '<div class="card"><div class="ctitle"><div class="l">'+icon('moon')+' Erholungstrend</div><span class="more">' + T('ui.14_tage') + '</span></div>'+
      (any?rows2:gmAnaChartEmpty(GM_NA+'' + T('ui.noch_keine_gespeicherten_erholungsserien_min') + ''))+
  ...
  var res=gmAnaResolved();
  h+='<div class="sectlabel" data-gm-slot="analysis-recovery-all">' + T('ui.alle_erholungswerte') + '<span class="ana-count">'+(_gmAnaState==='loading'?'wird geladen':_gmAnaState==='error'?'' + T('ui.nicht_verfuegbar__') + '':'' + T('ui.tippen_fuer_details') + '')+'</span></div>';
  if(_gmAnaState==='error'){
    h+=gmStateError({icon:'wifi',title:'' + T('ui.erholungswerte_konnten_nicht_geladen_werden') + '',
      desc:'' + T('ui.die_werte_sind_unbekannt_orvia') + '',
      retry:'gmAnaRetry()'});
  ...
  if(lvl!=='a'){
    h+='<div class="insight-card"><div class="insight-head"><span>'+icon('info','sm')+'</span><div><b>Zusammenhang</b></div></div><p>'+(lvl==='p'?GM_NA+'' + T('ui.keine_kanonische_zusammenhangsanalyse_orvia_erfindet') + '':GM_NA+'' + T('ui.keine_kanonische_analyse') + '')+'</p><div class="impact"><span>' + T('ui.empfehlung_') + '</span><strong>—</strong></div></div>';
  }
  ...
  if((typeof gmFeatureFlag!=='function'||gmFeatureFlag('recoveryIntel'))
    ...
    h+='<div class="sectlabel" data-gm-slot="recovery-risk">' + T('ui.belastungsrisiko_amp_regeneration') + '</div>';
    var _dd3=0;try{_dd3=(typeof dataDays==='function')?dataDays():0;}catch(_){ }
    if(_dd3<4){
      h+='<div class="card"><p class="muted" style="margin:0">'+GM_NA+'' + T('ui.belastbar_ab_7_tagen_check') + ''+_dd3+').</p></div>';
    }else{
      try{
        ...
        var _mk3=function(title,d){
          return '<div class="card"><div class="ctitle"><div class="l">'+icon(title==='Belastungsrisiko'?'shield':'heart')+' '+title+'</div>'+
            '<span class="pill-badge" style="background:transparent;color:'+(_cmap[d.state.c]||'var(--muted)')+'' + T('ui.border_1px_solid_currentcolor') + ''+gmEsc(d.state.l)+'</span></div>'+
            ...
            '<div class="mini-note">'+icon('info','xs')+'<div><b>' + T('ui.empfehlung') + '</b> '+gmEsc(d.rec)+'</div></div></div>';};
        h+=_mk3('Belastungsrisiko',_rk)+_mk3('Regenerationsdefizit',_rd3);
        h+='<div class="mini-note" style="margin:2px 18px 0">'+icon('db','xs')+'<div>Regelbasierte Heuristik aus Check-ins, HRV/Ruhepuls-Baselines und Wochenvolumen · Datenbasis '+_dd3+'' + T('ui.tage_') + ' · kein Ersatz für die Tagesentscheidung.</div></div>';
      }catch(_){ }
    }
  }
  return h;
...
var GM_MV_META={
  below:{l:'' + T('ui.unter_ziel_') + '',c:'var(--attention)',t:'rgba(237,180,78,.16)',sym:'▽'},
  in:{l:'' + T('ui.im_ziel_') + '',c:'var(--ready)',t:'rgba(67,214,147,.16)',sym:'✓'},
  above:{l:'' + T('ui.ueber_ziel_') + '',c:'var(--activity)',t:'rgba(90,160,240,.16)',sym:'▲'},
  low_history:{l:'' + T('ui.wenig_historie_') + '',c:'var(--sleep)',t:'rgba(149,133,237,.16)',sym:'~'},
  no_data:{l:'' + T('ui.keine_daten') + '',c:'var(--faint)',t:'rgba(138,147,161,.14)',sym:'–'}
};
...
function gmMvSt(m){
  var st=(typeof mvStatusModel==='function')?mvStatusModel(m):{key:'no_data',label:'' + T('ui.keine_daten') + '',sym:'–'};
...
function gmBodySVG(model,side){
  ...
  Object.keys(data).forEach(function(slug){
    var id=map[slug];
    if(!id){data[slug].forEach(function(pts){neutral+='<polygon points="'+pts+'" fill="var(--surface-2)" stroke="rgba(0,0,0,.' + T('ui.45') + ')" stroke-width="0.5"/>';});return;}
  ...
  return '<svg viewBox="0 0 100 200" class="bodysvg anat" role="img" aria-label="Anatomische Muskelkarte '+(side==='front'?'' + T('ui.vorderseite') + '':'' + T('ui.rueckseite') + '')+'">'+neutral+plates+'</svg>';
}
function gmMuscleTile(model,id){
  ...
  var sub=(eq!=null?fmtDe(eq)+'' + T('ui.effektive_saetze') + '':'—')+'' + T('ui.richtwert') + ''+(lo!=null&&hi!=null?lo+'–'+hi+'/Woche':'—')+(lvl==='p'?'' + T('ui.konfidenz_') + ''+conf:'');
  var bar='';
...
function gmAnaBody(){
  var lvl=(typeof gmLevel==='function')?gmLevel():'f';
  var sub=lvl==='a'?'' + T('ui.so_gut_deckst_du_deine') + '':lvl==='p'?'' + T('ui.effektive_satzaequivalente_direkt_indirekt_vs') + '':'' + T('ui.woechentliches_volumen_je_muskelgruppe_vs') + '';
  var h='<div class="body-head"><div class="ana-kick">' + T('ui.koerper_muskelvolumen') + '</div><div class="ana-sub">'+sub+'</div></div>';
  ...
  h+='<div class="body-toggle"><button class="'+(gmBodySide==='front'?'on':'')+'" onclick="gmSetBodySide(\'front\')">' + T('ui.vorderseite') + '</button><button class="'+(gmBodySide==='back'?'on':'')+'" onclick="gmSetBodySide(\'back\')">' + T('ui.rueckseite') + '</button></div>';
  ...
  if(model&&(model.status==='load_error'||model.status==='data_unavailable')){
    h+=gmStateError({icon:'wifi',title:'' + T('ui.muskelvolumen_konnte_nicht_geladen_werden') + '',
      desc:(model.status==='data_unavailable'?'' + T('ui.der_lokale_speicher_war_nicht') + '':'' + T('ui.mindestens_eine_datenquelle_ist_fehlgeschlagen') + ''),
      retry:'gmAnaRetry()'});
    return h;
  }
  if(_gmMvState==='error'){
    h+=gmStateError({icon:'wifi',title:'' + T('ui.muskelvolumen_konnte_nicht_geladen_werden') + '',
      desc:'' + T('ui.das_volumen_ist_unbekannt_fehlende') + '',
      retry:'gmAnaRetry()'});
    return h;
  }
  if(_gmMvState==='loading'){
    ...
    h+='<div class="sectlabel" data-gm-slot="analysis-body-map">'+(gmBodySide==='front'?'' + T('ui.vorderseite') + '':'' + T('ui.rueckseite') + '')+' <span class="ana-count">wird geladen</span></div>';
  ...
  if(_mvEmpty&&gmBodyRange<90){
    ...
    h+='<div class="mini-note" style="margin-bottom:10px">'+icon('info','xs')+'<div><b>Im gewählten ' + T('ui.zeitraum_') + ' ('+gmBodyRange+' T.) liegen keine Krafttrainingsdaten.</b> '+
      (ago!=null?('' + T('ui.letztes_krafttraining_vor') + ''+ago+'' + T('ui.tagen') + ''):'')+
      '<span class="deeplink" role="button" tabindex="0" onclick="gmSetBodyRange(90)">' + T('ui.zeitraum_auf_90_t_stellen') + '</span></div></div>';
  }
  h+='<div class="body-wrap">'+gmBodySVG(model,gmBodySide)+'</div>';
  /* Legende: vollständige GM-Struktur; „Warnung" neutral als nicht verfügbar */
  var legs=[GM_MV_META.below,GM_MV_META.in,GM_MV_META.above,{l:'' + T('ui.warnung') + '',c:'var(--neutral)',sym:'!'},GM_MV_META.low_history,GM_MV_META.no_data];
  h+='<div class="mlegend">'+legs.map(function(v){var na=(v.c==='var(--neutral)')?' aria-label="' + T('ui.warnung') + ' — Statusart mit der kanonischen Engine noch ' + T('ui.nicht_verfuegbar__') + '" title="' + T('ui.noch_nicht_verfuegbar') + '"':'';return '<span class="mleg"'+na+'><i style="background:'+v.c+'"></i>'+v.sym+' '+v.l+'</span>';}).join('')+'</div>';
  h+='<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.farbe') + '<b>und</b>' + T('ui.symbol_zeigen_den_status_fehlende') + '<b>nicht</b> zu wenig ' + T('ui.training') + '.'+(gmBodySide==='front'?' <b>' + T('ui.seitliche_schulter_') + '</b> hat keine eigene anatomische Region und wird unten in der Liste geführt.':'')+'</div></div>';
  h+='<div class="sectlabel" data-gm-slot="analysis-body-map">'+(gmBodySide==='front'?'' + T('ui.vorderseite') + '':'' + T('ui.rueckseite') + '')+' <span class="ana-count">'+ids.length+' Gruppen</span></div>';
...
function gmOpenMuscleSheet(id){
  ...
  var eff=(eq==null||lo==null||hi==null)?(GM_NA+'' + T('ui.ohne_zielkorridor_keine_einordnung') + '')
    :(eq<lo?'Aktuell <b>unter</b> dem wirksamen Bereich für spürbaren Aufbau.':eq>hi?'Aktuell <b>' + T('ui.ueber') + '</b> dem nötigen Bereich – mehr bringt kaum Zusatznutzen, erhöht aber Ermüdung.':'Aktuell im <b>wirksamen</b> Bereich für ' + T('ui.dein_ziel') + '.')+'' + T('ui.direkt_indirekt_zusammengefasst_unveraendert_aus') + '';
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:'+st.t+';color:'+st.c+'">'+icon('dumbbell')+'</div><div><h3>'+gmEsc(name)+'</h3><div class="sh-sub" style="margin:2px 0 0">'+st.sym+' '+st.l+' · letzte '+gmBodyRange+'' + T('ui.tage_') + '</div></div></div>'+
    '<div class="statgrid3"><div><div class="n">'+(m&&m.realWorkingSets!=null?m.realWorkingSets:'—')+'</div><div class="l">' + T('ui.arbeitssaetze') + '</div></div><div><div class="n">'+(eq!=null?fmtDe(eq):'—')+'</div><div class="l">effektiv</div></div><div><div class="n">'+(lo!=null&&hi!=null?lo+'–'+hi:'—')+'</div><div class="l">' + T('ui.ziel_woche') + '</div></div></div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.verlauf_saetze_woche') + '</div><div class="oc2"><div class="gm-chart-empty">'+GM_NA+' — eine kanonische Wochenhistorie je Muskel liegt noch nicht vor.</div></div></div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.wirksamkeit_fuers_ziel') + '</div><p>'+eff+'</p></div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.zuletzt_beteiligte_uebungen') + '</div><div class="msheet-ex">'+(exNames.length?exNames.slice(0,6).map(function(e){return '<span>'+gmEsc(e)+'</span>';}).join(''):'<span>—</span>')+'</div></div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.empfehlung_naechste_woche') + '</div><p>'+gmEsc((typeof mvNextStep==='function')?mvNextStep(st.key):'—')+'</p></div>'+
    (lvl==='p'?'<div class="sh-block"><div class="bh">' + T('ui.datenqualitaet') + '</div><div class="confidence"><span class="confchip">'+icon('check','xs')+' ' + T('ui.konfidenz__') + ' <b>'+(m&&m.confidence!=null?Math.round(m.confidence*100)+'%':'—')+'</b></span><span class="confchip">'+icon('db','xs')+' Trend <b>—</b></span></div></div>':'')+
...
function gmOpenMetricsLibrary(){
  ...
  var h='<div class="page-head"><div class="page-head-row"><button class="backbtn" onclick="gmCloseMetricsLibrary()" aria-label="' + T('ui.zurueck') + '">'+icon('chev')+'</button><div><h2>' + T('ui.kennzahlenbibliothek') + '</h2><p>' + T('ui.alle_rohdaten_quellen_und_trends') + '</p></div></div></div>';
  if(!reg||!reg.METRICS){
    h+='<div class="card"><p>'+GM_NA+' — Metrik-Katalog nicht geladen.</p></div>';
  }else{
    var res=gmAnaResolved();
    if(res==null&&_gmAnaState==='loading'){
      h+='<div class="sectlabel">' + T('ui.kennzahlen') + '<span class="ana-count">wird geladen</span></div><div class="ana-grid">'+gmStateLoading({kind:'kcard',bare:false,blocks:6})+'</div>';
    }else if(res==null&&_gmAnaState==='error'){
      h+=gmStateError({icon:'wifi',title:'' + T('ui.kennzahlen_konnten_nicht_geladen_werden') + '',desc:'' + T('ui.die_werte_sind_unbekannt_orvia_') + '',retry:'gmAnaRetry();gmOpenMetricsLibrary()'});
    }else{
      ...
      h+='<div class="mini-note" style="margin:4px 18px 12px">'+icon('info','xs')+'<div>' + T('ui.werte_aus_dem_kanonischen_metrik') + '</div></div>';
    }
...
function gmLibCheckinSections(){
  ...
  var SEC=[
    {t:'' + T('ui.subjektiv_check_in') + '',rows:[
      {label:'Tagesenergie',where:'eve',key:'energy',unit:rng('EVENING','energy'),src:'' + T('ui.abend_check_in') + ''},
      {label:'Stimmung',where:'eve',key:'mood',unit:rng('EVENING','mood'),src:'' + T('ui.abend_check_in') + ''},
      {label:lbl('MORNING','doms','' + T('ui.muskelschmerz_doms') + ''),where:'morning',key:'doms',unit:rng('MORNING','doms'),src:'' + T('ui.morgen_check_in') + ''},
      {label:'Schmerz',where:'morning',key:'knee',unit:rng('MORNING','knee')||' / 10',src:'' + T('ui.morgen_check_in') + ''},
      {label:'' + T('ui.stress_subj') + '',where:'morning',key:'stress',unit:'',src:'' + T('ui.morgen_check_in') + ''}
    ]},
    {t:'Ernährung',rows:[
      {label:'Tagesumsatz',where:null,key:null,unit:'',src:'' + T('ui.kein_ernaehrungsvertrag') + ''},
      {label:'Protein',where:'eve',key:'prot',unit:' g',src:'' + T('ui.abend_check_in') + ''},
      {label:'Kohlenhydrate',where:'eve',key:'carbs',unit:'',src:'' + T('ui.abend_check_in') + ''},
      {label:'Flüssigkeit',where:'eve',key:'hydL',unit:' l',src:'' + T('ui.abend_check_in') + ''}
    ]}
  ];
  var out='';
  SEC.forEach(function(s){
    ...
    var tiles=s.rows.map(function(r){
      ...
      if(got){
        ...
        if(_age==null)_age=(got.ageDays===0?'heute':(got.ageDays===1?'gestern':('vor '+got.ageDays+'' + T('ui.tagen_') + '')));
        foot=r.src+' · '+_age;
...
function gmOpenPaceCalcSheet(){
  ...
  var sh=document.getElementById('detailSheet');if(!sh)return;
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('gauge')+'</div><div><h3>' + T('ui.pace_rechner') + '</h3><div class="sh-sub" style="margin:2px 0 0">'+GM_NA+'</div></div></div>'+
    '<div class="sh-block"><p>' + T('ui.der_pace_rechner_ist_gerade') + '</p></div>';
  gmOpenSheet('detailSheet');
}
/* --- Hauptrenderer --- */
function renderGMAnalysis(){
  ...
  var lvl=(typeof gmLevel==='function')?gmLevel():'f';
  var dateTxt=lvl==='a'?'' + T('ui.einfach_erklaert') + '':lvl==='p'?'' + T('ui.datenlage_baselines_amp_konfidenz') + '':'' + T('ui.trends_amp_zusammenhaenge') + '';
  var segs=[['overview','Überblick'],['endurance','' + T('ui.ausdauer') + ''],['recovery','' + T('ui.erholung_') + ''],['body','Körper']];
  var h='<div class="hdr"><div><div class="greet">' + T('ui.orvia_intelligence') + '</div><h1>' + T('ui.analyse') + '</h1><div class="date">'+dateTxt+'</div></div><button class="iconbtn" role="button" aria-label="' + T('ui.kennzahlenbibliothek') + '" onclick="gmOpenMetricsLibrary()">'+icon('chart','sm')+'</button></div>';
  ...
  try{
    if(window.ORVIA&&ORVIA.charts&&ORVIA.charts.richChart&&ctx&&ctx.ok&&ctx.S){
      ...
      if(el2){var k=Math.min(28,ctx.S.ctl.length);
        ...
        ORVIA.charts.richChart(el2,{label:'' + T('ui.fitness_ctl_srpe') + '',series:_win.map(function(v){return Math.round(v);}),times:(ctx.ld.labels||[]).slice(-k),unit:'',color:'gold',baseline:_avg,higherBetter:true,dec:0,
...
function gmPPageHead(title,sub,action){
  return '<div class="page-head"><div class="page-head-row"><button class="backbtn" onclick="gmCloseProfPage()" aria-label="' + T('ui.zurueck') + '">'+icon('chev')+'</button><div><h2>'+gmEsc(title)+'</h2>'+(sub?'<p>'+gmEsc(sub)+'</p>':'')+'</div>'+(action?'<span class="page-action" role="button" tabindex="0" onclick="'+action.fn+'">'+action.label+'</span>':'')+'</div></div>';
}
...
function gmProfGoalCard(){
  var g=null;try{g=(typeof goalOf==='function')?goalOf():null;}catch(_){ }
  if(!g||!g.type)return '<div class="goal-card"><div class="goal-top"><div><h4>—</h4><p>'+GM_NA+' — lege dein Hauptziel ' + T('ui.im_ziel') + '-Editor fest.</p></div><span class="goal-badge">' + T('ui.hauptziel') + '</span></div><div class="goal-line"><i style="width:0%"></i></div></div>';
  ...
  try{if(g.raceDate){var d=new Date(g.raceDate+'T12:00');sub=d.toLocaleDateString('de-DE',{day:'2-digit',month:'2-digit',year:'numeric'});
    if(typeof daysTo==='function'){var w=Math.max(0,Math.ceil(daysTo(g.raceDate)/7));sub+=' · noch '+w+'' + T('ui.wochen') + '';}}}catch(_){ }
  /* Kein Zielprozent im UI — die Fortschrittsspur bleibt als ehrlich leerer Slot. */
  return '<div class="goal-card"><div class="goal-top"><div><h4>'+gmEsc(t)+'</h4><p>'+gmEsc(sub)+'</p></div><span class="goal-badge">' + T('ui.hauptziel') + '</span></div><div class="goal-line"><i style="width:0%"></i></div></div>';
}
function gmProfEdit(){
  if(window.ORVIA&&ORVIA.profileCenter&&typeof ORVIA.profileCenter.open==='function'){ORVIA.profileCenter.open();return;}
  var sh=document.getElementById('detailSheet');if(!sh)return;
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('pen')+'</div><div><h3>' + T('ui.profil_bearbeiten') + '</h3><div class="sh-sub" style="margin:2px 0 0">'+GM_NA+'</div></div></div><div class="sh-block"><p>' + T('ui.der_profil_editor_ist_gerade') + '</p></div>';
  gmOpenSheet('detailSheet');
}
...
function gmProfSyncLabel(){
  ...
  try{if(typeof window.orviaSyncState==='function'){var st=window.orviaSyncState();
    return ({local:'' + T('ui.cloud_lokaler_modus') + '',synced:'' + T('ui.cloud_synchronisiert') + '',pending:'' + T('ui.cloud_sync_laeuft') + '',error:'Cloud-Sync-Fehler',offline:'' + T('ui.offline_lokal') + ''})[st]||'—';}}catch(_){ }
  return '—';
}
/* ---------- Profilhauptseite ---------- */
function renderGMProfile(){
  ...
  var h='<div class="profile-cover"></div><div class="ig-profile"><div class="ig-top"><div class="ig-avatar">'+avatar+'</div><div class="ig-actions"><button class="mini-btn primary" onclick="gmProfEdit()">' + T('ui.profil_bearbeiten') + '</button><button class="mini-btn" aria-label="Einstellungen" onclick="gmOpenProfPage(\'settings\')">'+icon('gear','sm')+'</button></div></div>'+
    ...
    '<div class="ig-bio">'+((typeof PROFILE!=='undefined'&&PROFILE&&PROFILE.bio)?gmEsc(PROFILE.bio):(GM_NA+'' + T('ui.bio_im_profil_editor_persoenliche') + ''))+'</div>'+
    (function(){/* GM7: Einheiten = Gesamtzahl der kanonisch zusammengefuehrten Aktivitaeten (Server+lokal+Legacy, dedupliziert) */
      ...
      return '<div class="ig-stats"><div class="ig-stat"><b>'+(units!=null?fmtDe(units):'—')+'</b><span>' + T('ui.einheiten_') + '</span></div><div class="ig-stat"><b>'+(sports.length?sports.length:'—')+'</b><span>' + T('ui.sportarten') + '</span></div><div class="ig-stat"><b>'+(ctl!=null?ctl:'—')+'</b><span>' + T('ui.fitness_srpe') + '</span></div><div class="ig-stat"><b>—</b><span>' + T('ui.zielaufbau') + '</span></div></div></div>';})();
  h+='<div class="sectlabel" data-gm-slot="profile-sports">Deine ' + T('ui.sportarten') + ' <span class="edit" role="button" tabindex="0" onclick="gmOpenProfPage(\'goals\')">Bearbeiten</span></div>';
  ...
  h+='<div class="sectlabel" data-gm-slot="profile-control">' + T('ui.profil_amp_kontrolle') + '</div><div class="setting-group">'+
    gmPRow('target','' + T('ui.ziele_amp_sportarten') + '','' + T('ui.prioritaeten_rollen_und_langfristziele') + '','',"gmOpenProfPage('goals')")+
    gmPRow('link','' + T('ui.geraete_amp_daten') + '',gmEsc(gmProfSyncLabel()),'',"gmOpenProfPage('connections')")+
    gmPRow('gear','Einstellungen','' + T('ui.ansicht_training_datenschutz_und_konto') + '','',"gmOpenProfPage('settings')")+'</div>';
  h+='<div class="sectlabel" data-gm-slot="profile-performance">' + T('ui.leistung_amp_fortschritt') + '</div><div class="setting-group">'+
    gmPRow('gauge','Leistungsdaten',(typeof gmPerfRowSub==='function'?gmPerfRowSub():'' + T('ui.wettkampf_test_und_schwellenwerte') + ''),'',"gmOpenProfPage('performance')")+
    gmPRow('bolt','Bestzeiten','' + T('ui.persoenliche_rekorde_je_distanz') + '','',"gmOpenProfPage('bestTimes')")+
    gmPRow('shield','Medaillen','' + T('ui.erreichte_und_offene_auszeichnungen') + '','',"gmOpenProfPage('medals')")+
    gmPRow('target','Meilensteine','' + T('ui.fortschritt_richtung_ziel') + '','',"gmOpenProfPage('milestones')")+
    gmPRow('gauge','' + T('ui.pace_rechner') + '','' + T('ui.lauf_rad_und_schwimmen') + '','',"gmOpenProfPage('paceCalc')")+'</div>';
  h+='<div class="tabspacer"></div>';
  host.innerHTML=h;
}
/* ---------- Subpages ---------- */
...
function gmProfModeLabel(){var m=(typeof uiDetailMode==='function')?uiDetailMode():'fortgeschritten';return ({anfaenger:'Einfach',fortgeschritten:'' + T('ui.fortgeschritten_') + '',profi:'Profi'})[m]||'—';}
function gmProfSettings(){
  return gmPPageHead('Einstellungen','' + T('ui.dein_orvia_deine_regeln') + '')+
    '<div class="page-intro">' + T('ui.alle_anzeige_trainings_datenschutz_und') + '</div>'+
    '<div class="setting-title">' + T('ui.darstellung') + '</div><div class="setting-group">'+
    gmPRow('sun','Erscheinungsbild','' + T('ui.dunkel_hell_oder_automatisch') + '',gmEsc(({dark:'Dunkel',light:'Hell',auto:'Automatisch'})[(typeof orviaThemePref==='function')?orviaThemePref():'dark']||'Dunkel'),"gmOpenProfPage('appearance')")+
    gmPRow('activity','' + T('ui.ansicht_amp_detailtiefe') + '','' + T('ui.aendert_erklaerungen_und_informationsdichte') + '',gmEsc(gmProfModeLabel()),"gmOpenProfPage('appearance')")+
    gmPToggleNA('activity','' + T('ui.bewegung_reduzieren') + '',GM_NA)+'</div>'+
    '<div class="setting-title">' + T('ui.training_amp_gesundheit') + '</div><div class="setting-group">'+
    gmPRow('calendar','' + T('ui.plan_amp_wochenstruktur') + '','' + T('ui.ruhetage_zeitfenster_struktur_und_doppel') + '',(function(){
      try{var M2=window.ORVIA&&ORVIA.profileModel;if(M2&&M2.availabilitySummary&&typeof PROFILE!=='undefined'&&PROFILE){
        ...
        return gmEsc(av.availableDays+'' + T('ui.tage_') + ''+(av.preferredRestDays&&av.preferredRestDays.length?'' + T('ui.ruhe') + ''+av.preferredRestDays.join(', '):''));}}catch(_){ }
      return '—';})(),"gmOpenProfPage('planSettings')")+
    gmPRow('target','' + T('ui.ziele_amp_sportarten') + '','' + T('ui.hauptziel_nebenziele_rollen') + '',(gmProfSports().length||'—')+'',"gmOpenProfPage('goals')")+
    gmPRow('heart','' + T('ui.gesundheit_amp_check_in') + '','' + T('ui.warnzeichen_schmerz_schlaf_hrv') + '','—',"gmOpenProfPage('health')")+
    gmPRow('link','' + T('ui.geraete_amp_datenquellen') + '','' + T('ui.cloud_sync_import_manuelle_daten') + '',gmEsc(gmProfSyncLabel()),"gmOpenProfPage('connections')")+
    gmPRow('gauge','' + T('ui.einheiten_amp_berechnungen') + '','' + T('ui.metrisch_wochenbeginn_herzfrequenzzonen') + '','km · kg',"gmOpenProfPage('units')")+'</div>'+
    '<div class="setting-title">' + T('ui.kommunikation') + '</div><div class="setting-group">'+
    gmPRow('bell','Benachrichtigungen','' + T('ui.training_erholung_planaenderungen') + '','—',"gmOpenProfPage('notifications')")+'</div>'+
    '<div class="setting-title">' + T('ui.konto_amp_kontrolle') + '</div><div class="setting-group">'+
    gmPRow('lock','' + T('ui.datenschutz_amp_ki') + '','' + T('ui.profil_einwilligungen_datenverwendung_und_consent') + '','—',"gmOpenProfPage('privacy')")+
    gmPRow('db','' + T('ui.daten_verwalten') + '','' + T('ui.export_sicherung_loeschen') + '','',"gmOpenProfPage('data')")+
    gmPRow('shield','' + T('ui.konto_amp_sicherheit') + '','' + T('ui.e_mail_passwort_geraete') + '','',"gmOpenProfPage('account')")+
    gmPRow('info','' + T('ui.hilfe_amp_ueber_orvia') + '','' + T('ui.support_dokumentation_version_build_und') + '','',"gmOpenProfPage('about')")+'</div>'+
...
function gmProfAppearance(){
  ...
  return gmPPageHead('' + T('ui.ansicht_detailtiefe') + '','' + T('ui.persoenlich_ohne_die_engine_zu') + '')+
    '<div class="setting-title">' + T('ui.farbmodus') + '</div><div class="choice-grid">'+
    ...
    '<div class="setting-title">' + T('ui.informationsdichte') + '</div><div class="choice-grid">'+
    '<button class="choice '+(m==='anfaenger'?'on':'')+'" onclick="gmProfSetMode(\'anfaenger\')">Einfach</button>'+
    '<button class="choice '+(m==='fortgeschritten'?'on':'')+'" onclick="gmProfSetMode(\'fortgeschritten\')">' + T('ui.fortgeschritten_') + '</button>'+
    '<button class="choice '+(m==='profi'?'on':'')+'" onclick="gmProfSetMode(\'profi\')">Profi</button></div>'+
    '<div class="card"><div class="ctitle"><div class="l">'+icon('info')+' Was ändert sich?</div></div><p class="prescription"><b>' + T('ui.einfach') + '</b>' + T('ui.klare_handlung_und_wenig_kennzahlen') + '<br><br><b>' + T('ui.fortgeschritten') + '</b>' + T('ui.zusammenhaenge_trends_und_planwirkung') + '<br><br><b>' + T('ui.profi') + '</b>' + T('ui.datenqualitaet_lastmodelle_quellen_und_entscheidungsgruende') + '<br><br>' + T('ui.die_trainingsentscheidung_bleibt_in_allen') + '</p></div>';
}
function gmProfNotifications(){
  return gmPPageHead('Benachrichtigungen','' + T('ui.nur_das_was_dir_wirklich') + '')+
    '<div class="setting-title">' + T('ui.grundsaetzlich') + '</div><div class="setting-group">'+gmPToggleNA('bell','' + T('ui.benachrichtigungen_erlauben') + '','' + T('ui.master_schalter') + ''+GM_NA)+'</div>'+
    '<div class="setting-title">' + T('ui.coaching') + '</div><div class="setting-group">'+
    gmPToggleNA('run','Trainingserinnerungen',GM_NA)+
    gmPToggleNA('calendar','Planänderungen','' + T('ui.noch_nicht_verfuegbar_kein_mitteilungsvertrag') + '')+
    '<div class="setting-item gm-dis"><span class="setting-icon">'+icon('heart','sm')+'</span><span class="setting-copy"><b>' + T('ui.erholung_amp_warnzeichen') + '</b><span>' + T('ui.nur_relevante_veraenderungen_und_safety') + '</span></span><span class="setting-value" title="' + T('ui.safety_hinweise_sind_nicht_abschaltbar') + '">' + T('ui.immer_aktiv') + '</span></div>'+
    gmPToggleNA('chart','' + T('ui.wochenreview') + '','' + T('ui.noch_nicht_verfuegbar_kein_wochenreview') + '')+'</div>'+
    '<div class="setting-title">' + T('ui.sonstiges') + '</div><div class="setting-group">'+gmPToggleNA('sparkle','Produktneuigkeiten',GM_NA)+'</div>';
}
function gmProfPrivacy(){
  return gmPPageHead('' + T('ui.datenschutz_ki') + '','' + T('ui.du_entscheidest_ueber_deine_daten') + '')+
    '<div class="setting-title">' + T('ui.sichtbarkeit') + '</div><div class="setting-group">'+
    gmPToggleNA('lock','' + T('ui.privates_profil') + '','' + T('ui.noch_nicht_verfuegbar_kein_sichtbarkeitsvertrag') + '')+
    gmPToggleNA('heart','Gesundheitsfreigabe','' + T('ui.kein_consent_vertrag') + ''+GM_NA.toLowerCase())+'</div>'+
    '<div class="setting-title">' + T('ui.intelligente_auswertung') + '</div><div class="setting-group">'+
    gmPToggleNA('sparkle','' + T('ui.personalisierte_ki_analyse') + '','' + T('ui.noch_nicht_verfuegbar_erfordert_consent') + '')+
    gmPRow('shield','Safety-Regeln','' + T('ui.medizinische_warnzeichen_ueberstimmen_leistungsziele') + '','' + T('ui.immer_aktiv') + '','',false)+'</div>'+
    '<div class="card"><p class="prescription">' + T('ui.orvia_zeigt_nachvollziehbar_welche_daten') + '</p></div>';
}
...
function gmProfHorizonCard(horizon,badge,adj){
  ...
  if(!mine.length){
    head='—';
    sub=GM_NA+' — kein aktives '+adj+'' + T('ui.ziel_hinterlegt') + '';
  }else{
    var g=mine[0],parts=[];
    ...
    if(g.targetDate){
      try{var d=new Date(g.targetDate+'T12:00');
        ...
        if(typeof daysTo==='function'){var w=Math.ceil(daysTo(g.targetDate)/7);
          if(isFinite(w)&&w>0)parts.push('noch '+w+'' + T('ui.wochen') + '');}
      }catch(_){ }
    }
...
function gmProfGoals(){
  ...
  return gmPPageHead('' + T('ui.ziele_sportarten') + '','' + T('ui.hierarchie_statt_zielchaos') + '',{label:'' + T('ui.hinzufuegen') + '',fn:'gmProfAddGoal()'})+
    '<div class="page-intro">' + T('ui.dein_hauptziel_steuert_die_aktuelle') + '</div>'+
    ...
    '<div class="setting-title">' + T('ui.aktive_sportprofile') + '</div><div class="sport-chips">'+(chips.length?chips.map(function(s){return '<span class="sport-chip on">'+s+'</span>';}).join(''):'<span class="sport-chip">—</span>')+'</div>';
}
function gmProfDailyGoals(){
  var rows=[['Schritte','' + T('ui.dein_taegliches_bewegungsziel') + ''],['' + T('ui.aktive_kalorien') + '','' + T('ui.aktivitaet_ohne_grundumsatz') + ''],['Wasser','' + T('ui.tagesziel_in_millilitern') + ''],['' + T('ui.schlaf') + '','' + T('ui.persoenliche_zielstunden') + '']];
  return gmPPageHead('' + T('ui.tagesziele_') + '','' + T('ui.individuell_statt_starrer_standardwerte') + '',{label:'' + T('ui.fertig_') + '',fn:'gmCloseProfPage()'})+
    '<div class="page-intro">' + T('ui.diese_ziele_sind_leitplanken_ein') + '</div>'+
...
function gmProfPlanSettings(){
  ...
  try{var M2=window.ORVIA&&ORVIA.profileModel;
    if(M2&&M2.availabilitySummary&&typeof PROFILE!=='undefined'&&PROFILE){
      var av=M2.availabilitySummary(PROFILE.availability);
      rest=(av.preferredRestDays&&av.preferredRestDays.length)?av.preferredRestDays.join(', '):'' + T('ui.keiner_festgelegt') + '';
      days=av.availableDays+'' + T('ui.tage_') + '';
      dbl=(av.doubleDays>0)?(av.doubleDays+'' + T('ui.tage_erlaubt') + ''):'Aus';}}catch(_){ }
  return gmPPageHead('' + T('ui.plan_wochenstruktur') + '','' + T('ui.wie_orvia_deine_woche_bauen') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('moon','' + T('ui.fester_ruhetag') + '','' + T('ui.wird_vom_planer_niemals_automatisch') + '',gmEsc(rest),"openAvailabilityEditor&&openAvailabilityEditor()")+
    gmPRow('calendar','' + T('ui.bevorzugte_trainingstage') + '','' + T('ui.fuer_plan_und_wochenreview') + '',gmEsc(days),"openAvailabilityEditor&&openAvailabilityEditor()")+
    gmPRow('activity','Doppel-' + T('ui.einheiten_') + '','' + T('ui.nur_nach_ausdruecklicher_freigabe') + '',gmEsc(dbl),"openAvailabilityEditor&&openAvailabilityEditor()")+
    gmPRow('gauge','' + T('ui.maximale_tagesbelastung') + '','' + T('ui.noch_nicht_verfuegbar_wird_spaeter') + '','—','',true)+'</div>';
}
function gmProfHealth(){
  var h=gmPPageHead('' + T('ui.gesundheit_check_in') + '','' + T('ui.safety_zuerst') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('alert','Warnzeichen','' + T('ui.nur_sichtbar_wenn_symptome_ja') + '','' + T('ui.aktiv') + '','',false)+
    gmPRow('knee','Verletzungshistorie','' + T('ui.aus_deinem_check_in') + '','—','',true)+
    gmPRow('moon','Schlafziel','' + T('ui.kein_produktiver_zielkorridor_vertrag') + '','—','',true);
  ...
  try{
    ...
    if((typeof gmFeatureFlag!=='function'||gmFeatureFlag('cycle'))&&(_sexF||_cyc)){
      ...
      h+=gmPRow('calendar','Zyklus',_cp?('' + T('ui.tag') + ''+_cp.day+' · '+_cp.phase):'' + T('ui.optional_unterstuetzende_hinweise_keine_diagnose') + '',_cyc?'' + T('ui.aktiv') + '':'—','gmCloseSheets&&gmCloseSheets();openCycleEditor&&openCycleEditor()',false);
    }
  }catch(_){ }
  h+='</div>';
  ...
  if(typeof gmFeatureFlag!=='function'||gmFeatureFlag('baselines')){
    ...
    try{
      ...
      if(_rows&&_rows.length){
        ...
        _bl=gmPRow('pulse','Baselines',GM_NA+'' + T('ui.belastbar_ab_7_tagen_check_') + '','—','',true);
      }
    }catch(_){_bl=gmPRow('pulse','Baselines',GM_NA,'—','',true);}
    h+='<div class="setting-title">' + T('ui.baselines_7_28_tage_read') + '</div><div class="setting-group">'+_bl+'</div>';
  }
  return h;
}
...
function gmDeviceSyncPoll(base,token,baseline){
  ...
  function tick(){
    if(Date.now()>deadline){
      _gmDevSyncNow={state:'error',error:'' + T('ui.laeuft_im_hintergrund_weiter_zeitueberschreitung') + ''};
    ...
    gmDeviceSyncStatus(base,token).then(function(st){
      ...
      if(errNow&&errNow!==baseline.err){
        _gmDevSyncNow={state:'error',error:'' + T('ui.sync_fehlgeschlagen') + ''+errNow+').'};
        gmDeviceSyncPollFinish();
        return;
      }
...
async function gmDeviceSyncNowTrigger(){
  ...
  if(!token){_gmDevSyncNow={state:'error',error:'' + T('ui.keine_aktive_sitzung') + ''};gmRerenderConnections();return;}
  if(!base){_gmDevSyncNow={state:'error',error:'' + T('ui.worker_nicht_konfiguriert') + ''};gmRerenderConnections();return;}
  var baseline=null;
  ...
  if(!baseline){_gmDevSyncNow={state:'error',error:'' + T('ui.status_nicht_abrufbar_offline_oder') + ''};gmRerenderConnections();return;}
  try{
    ...
    else if(resp.status===409){_gmDevSyncNow={state:'error',error:'' + T('ui.geraet_ist_nicht_verbunden') + ''};}
    else{_gmDevSyncNow={state:'error',error:'' + T('ui.synchronisierung_fehlgeschlagen') + ''+resp.status+').'};}
  }catch(e){_gmDevSyncNow={state:'error',error:'' + T('ui.worker_nicht_erreichbar_offline_oder') + ''};}
  try{_gmDevSync.fetchedAt=0;gmDeviceSyncRefresh();}catch(_){ }
  gmRerenderConnections();
}
function gmProfConnections(){
  ...
  var syncValue=reauth?'Gesperrt':(running?'' + T('ui.laeuft') + '':(_gmDevSyncNow.state==='success'?'' + T('ui.abgeschlossen') + '':(_gmDevSyncNow.state==='error'?'Fehler':'')));
  var syncNote=!connected?'' + T('ui.kein_geraet_verbunden') + '':(reauth?'' + T('ui.anmeldung_abgelaufen_erst_neu_anmelden') + '':(running?'' + T('ui.synchronisiert_gerade') + '':(_gmDevSyncNow.state==='error'?_gmDevSyncNow.error:(_gmDevSyncNow.state==='success'?'' + T('ui.neue_daten_sind_eingetroffen') + '':'' + T('ui.ruft_die_neuesten_daten_vom') + ''))));
  var reauthRow=reauth?gmPRow('alert','' + T('ui.neuanmeldung_erforderlich_') + '','' + T('ui.die_garmin_anmeldung_ist_abgelaufen') + ''+(_gmDevSync.errCode?'' + T('ui.code') + ''+gmEsc(_gmDevSync.errCode):''),'','',false):'';
  return gmPPageHead('' + T('ui.geraete_datenquellen') + '','' + T('ui.eine_wahrheit_klare_herkunft') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('check','Cloud-Sync',gmEsc(gmProfSyncLabel()),'',"",false)+
    gmPRow('link','Garmin',connected?gmEsc(devLbl):'—',reauth?'' + T('ui.neuanmeldung_noetig') + '':'','',false)+
    reauthRow+
    gmPRow('activity','' + T('ui.jetzt_synchronisieren') + '',gmEsc(syncNote),gmEsc(syncValue),(connected&&!running&&!reauth)?'gmDeviceSyncNowTrigger()':'',!connected||running||reauth)+
    gmPRow('link','' + T('ui.apple_health') + '','' + T('ui.zusaetzliche_gesundheitsdaten') + '','—','',true)+
    gmPRow('db','' + T('ui.manuelle_daten') + '','' + T('ui.check_in_training_koerperwerte') + '','' + T('ui.aktiv') + '','',false)+
    /* Phase 3 · Block 2: Equipment-Verschleiss — bestehender km-Zaehler je Schuh/Rad. */
    ((typeof gmFeatureFlag!=='function'||gmFeatureFlag('equipment'))?gmPRow('gauge','' + T('ui.equipment_amp_verschleiss') + '','' + T('ui.schuhe_und_rad_km_zaehler') + '','',"gmOpenEquipmentSheet()",false):'')+'</div>'+
...
function gmGateBuild(){
  ...
  if(!EXP||!SP||!MAP){_gmGate.state='error';_gmGate.msg='' + T('ui.module_nicht_geladen_seite_neu') + '';return null;}
  var r=EXP.buildGarminStrengthWorkout({
    occurrence:{occurrenceId:gmGateOcc(),l:'' + T('ui.geraetetest_g1_g3') + '',t:'Gym'},
  ...
  if(!r.ok){_gmGate.state='error';_gmGate.msg='' + T('ui.exporter') + ''+r.reason;return null;}
...
async function gmGateSend(){
  ...
  _gmGate.state='sending';_gmGate.msg='' + T('ui.sende') + '';_gmGate.result=null;gmRerenderConnections();
  var base=(window.ORVIA_CFG&&ORVIA_CFG.GARMIN_WORKER_URL)||'';
  ...
  if(!base){_gmGate.state='error';_gmGate.msg='' + T('ui.worker_nicht_konfiguriert') + '';return gmRerenderConnections();}
  if(!token){_gmGate.state='error';_gmGate.msg='' + T('ui.keine_aktive_sitzung_bitte_neu') + '';return gmRerenderConnections();}
  ...
  try{
    ...
    else if(resp.status===422)_gmGate.msg='' + T('ui.abgelehnt_sehr_wahrscheinlich_steht_strength') + '';
    else if(resp.status===409)_gmGate.msg='' + T('ui.dieser_clientref_wurde_heute_schon') + '';
    else if(resp.status===401)_gmGate.msg='' + T('ui.anmeldung_abgelaufen_oder_garmin_token') + '';
    else _gmGate.msg='' + T('ui.worker_antwortet_mit') + ''+resp.status+'.';
  }catch(e){
    _gmGate.state='error';_gmGate.msg='' + T('ui.worker_nicht_erreichbar_offline_oder') + '';
  }
  gmRerenderConnections();
}
function gmGateTestSection(){
  ...
  rows+=gmPRow('activity','' + T('ui.payload_pruefen') + '','' + T('ui.rechnet_mit_den_echten_modulen') + '','',
  ...
  if(b){
    ...
    for(var i=0;i<b.stepBindings.length;i++){
      ...
      if(s.kind==='set')lines.push('  '+s.reps+'' + T('ui.wdh') + '');
      if(s.kind==='rest')lines.push('  '+s.seconds+'' + T('ui.s_pause') + '');
    }
    rows+=gmPRow('check','Kontrollwerte',gmEsc('' + T('ui.clientref') + ''+gmGateRef()),'',"",false);
    rows+=gmPRow('check','payloadHash',gmEsc(b.payloadHash),'',"",false);
    rows+=gmPRow('dumbbell','Schritte',gmEsc(lines.join(' · ')),'',"",false);
    rows+=gmPRow('gauge','Gewichte','' + T('ui.20_kg_20000_30_kg') + '','',"",false);
  }
  rows+=gmPRow('link','' + T('ui.an_garmin_senden') + '',
    b?'' + T('ui.sendet_mit_devicetest_der_worker') + '':'' + T('ui.erst_pruefen') + '',
    (_gmGate.state==='sending')?'' + T('ui.laeuft') + '':(_gmGate.state==='sent'?'Gesendet':''),
    (b&&_gmGate.state!=='sending')?'gmGateSend()':'',!b||_gmGate.state==='sending');
  if(_gmGate.msg)rows+=gmPRow(_gmGate.state==='error'?'alert':'check','' + T('ui.ergebnis') + '',gmEsc(_gmGate.msg),'',"",false);
  var r=_gmGate.result;
  if(r){
    rows+=gmPRow('info','Antwort','' + T('ui.http') + ''+r.status+' · '+gmEsc(JSON.stringify(r.body).slice(0,180)),'',"",false);
    if(r.status===200&&r.body&&r.body.workoutId){
      rows+=gmPRow('check','' + T('ui.workoutid_notieren') + '',gmEsc(String(r.body.workoutId)),'',"",false);
    }
  }
  return '<div class="setting-title">' + T('ui.geraetetest_g1_g3_nur_mit') + '</div><div class="setting-group">'+rows+'</div>'+
...
function gmRxPreviewBuild(){
  ...
  if(!sh||typeof sh.buildWeekNow!=='function'||!WP){
    _gmRxPrev.state='error';_gmRxPrev.msg='' + T('ui.engine_module_nicht_geladen_seite') + '';
    gmRerenderConnections();return null;
  }
  var wk=null;
  try{wk=sh.buildWeekNow();}catch(e){
    _gmRxPrev.state='error';_gmRxPrev.msg='' + T('ui.die_engine_konnte_nicht_rechnen') + ''+((e&&e.message)||'' + T('ui.unbekannter_fehler') + '');
    gmRerenderConnections();return null;
  }
  if(!wk||!wk.result){
    _gmRxPrev.state='error';
    _gmRxPrev.msg='' + T('ui.die_engine_hat_keine_woche') + '';
    gmRerenderConnections();return null;
  }
  var proj=null;
  try{proj=WP.projectWeek(wk.result);}catch(e2){
    _gmRxPrev.state='error';_gmRxPrev.msg='' + T('ui.die_woche_liess_sich_nicht') + ''+((e2&&e2.message)||'' + T('ui.unbekannter_fehler') + '');
    gmRerenderConnections();return null;
  }
  if(!proj||proj.ok!==true){
    _gmRxPrev.state='error';_gmRxPrev.msg='' + T('ui.die_woche_liess_sich_nicht_') + ''+((proj&&proj.error)?(' ('+proj.error+')'):'')+'.';
    gmRerenderConnections();return null;
  }
...
function gmRxPreviewUnitHTML(it){
  ...
  if(!F||!it.rx){
    return kopf+'<div class="muted" style="font-size:12px;margin-top:2px">' + T('ui.keine_vorgabe_hinterlegt') + '</div>';
  }
  ...
  var warn=(r.warnings&&r.warnings.length)
    ? '<div class="muted" style="font-size:11px;margin-top:3px">'+gmEsc(r.warnings.length+'' + T('ui.block_bloecke_nicht_darstellbar') + '')+'</div>' : '';
...
function gmRxPreviewSection(){
  var WD=['' + T('ui.mo_') + '','' + T('ui.di') + '','' + T('ui.mi') + '','' + T('ui.do') + '','' + T('ui.fr') + '','' + T('ui.sa') + '','' + T('ui.so') + ''];
  var rows='';
  rows+=gmPRow('activity','' + T('ui.engine_woche_berechnen') + '',
    '' + T('ui.rechnet_die_woche_und_zeigt') + '',
    (_gmRxPrev.state==='built')?'Berechnet':'','gmRxPreviewBuild()',false);
  if(_gmRxPrev.msg)rows+=gmPRow('alert','' + T('ui.kein_ergebnis') + '',gmEsc(_gmRxPrev.msg),'',"",false);
  var p=_gmRxPrev.week;
  if(p){
    var gesamt=0;
    for(var d=0;d<7;d++)gesamt+=((p.days&&p.days[d])||[]).length;
    rows+=gmPRow('check','' + T('ui.einheiten_in_der_woche') + '',
...
function gmOpenEquipmentSheet(){
  ...
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('gauge')+'</div><div><h3>' + T('ui.equipment_amp_verschleiss') + '</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.km_je_schuh_rad_automatisch') + '</div></div></div>'+
    '<div class="sh-block" id="gmEquipSheetBody">'+body+'</div>'+
    '<div class="source">'+icon('info','xs')+' ' + T('ui.warnung') + ' ab 90 % des Wechsel-Limits — kein Limit gesetzt = reiner Zähler, keine erfundene Lebensdauer.</div>';
  gmOpenSheet('detailSheet');
}
...
function gmProfUnits(){
  return gmPPageHead('' + T('ui.einheiten_berechnungen') + '','' + T('ui.einheitlich_ueber_alle_bereiche') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('activity','' + T('ui.distanz_amp_gewicht') + '','' + T('ui.kilometer_kilogramm') + '','Metrisch','',false)+
    gmPRow('calendar','Wochenbeginn','' + T('ui.fuer_plan_und_wochenreview') + '','Montag','',false)+
    gmPRow('heart','Herzfrequenzzonen','' + T('ui.keine_neue_zonenregel_im_ui') + '','—','',true)+'</div>';
}
function gmProfData(){
  ...
  return gmPPageHead('' + T('ui.daten_verwalten') + '','' + T('ui.volle_kontrolle') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('db','' + T('ui.daten_exportieren') + '','' + T('ui.json_export_ueber_die_sicherung') + '','Export','exportData&&exportData()')+
    ...
    gmPRow('db','' + T('ui.cloud_export_vollstaendig') + '','' + T('ui.alle_cloud_tabellen_als_json') + '','Export','exportCloudData&&exportCloudData()')+
    gmPRow('copy','' + T('ui.lokale_sicherung') + '','' + T('ui.letzte_sicherung') + ''+gmEsc(lb),'','',false)+
    gmPRow('x','' + T('ui.alle_daten_loeschen') + '','' + T('ui.nur_mit_erneuter_bestaetigung') + '','','orviaDeleteAccount&&orviaDeleteAccount()')+'</div>';
}
function gmProfAccount(){
  var email='—';try{email=(window.ORVIA&&ORVIA.user&&ORVIA.user.email)?ORVIA.user.email:'—';}catch(_){ }
  return gmPPageHead('' + T('ui.konto_sicherheit') + '','' + T('ui.persoenliche_zugangsdaten') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('heart',gmEsc(gmProfName()||'—'),gmEsc(email),'','',false)+
    gmPRow('lock','' + T('ui.passwort_amp_anmeldung') + '','' + T('ui.ueber_den_bestehenden_auth_flow') + '','—','orviaChangePassword&&orviaChangePassword()')+
    gmPRow('shield','' + T('ui.angemeldete_geraete') + '','' + T('ui.keine_kanonische_geraeteliste') + '','—','',true)+'</div>';
}
function gmOrviaBuildLabel(){
  try{
    var m=document.querySelector('meta[name="orvia-build"]');
    var v=m&&m.content;
    return v?('' + T('ui.build') + ''+v.replace(/^orvia-/,'')):'—';
  }catch(_){return '—';}
}
function gmProfAbout(){
  return gmPPageHead('' + T('ui.ueber_orvia') + '','' + T('ui.das_system_hinter_deinem_training') + '')+
    '<div class="setting-title">' + T('ui.konfiguration') + '</div><div class="setting-group">'+
    gmPRow('info','ORVIA','' + T('ui.progressive_web_app') + '',gmOrviaBuildLabel(),'',false)+
    gmPRow('shield','' + T('ui.medizinischer_hinweis') + '','' + T('ui.kein_ersatz_fuer_medizinische_diagnose') + '','','',false)+
    gmPRow('book','' + T('ui.hilfe_amp_support') + '','' + T('ui.dokumentation_folgt') + ''+GM_NA,'—','',true)+'</div>';
}
...
var GM_MS_STATUS={planned:'geplant',in_progress:'' + T('ui.in_arbeit') + '',achieved:'erreicht',skipped:'übersprungen'};
function gmProfBestTimes(){
  ...
  var rows=[
    ...
    {d:'21,1',u:'km',t:null},{d:'400',u:'' + T('ui.m_schwimm') + '',t:null},{d:'20',u:'' + T('ui.km_rad') + '',t:null}
  ];
  var lvl=(typeof gmLevel==='function')?gmLevel():'f';
  var h=gmPPageHead('Bestzeiten','' + T('ui.persoenliche_rekorde_je_distanz') + '')+'<div style="padding:0 18px">';
  _gmBtSlots=[];
  rows.forEach(function(r,i){
    ...
    var sub=r.t==null?GM_NA:(r.k?gmBtSrcLabel(b,r.k):(r.real?'' + T('ui.eingetragene_bestleistung') + '':'' + T('ui.geschaetzt_riegel_modell_keine_messung') + ''));
    var subFull=sub+(lvl==='p'&&r.t!=null&&m&&m.laps?' · '+m.laps+(m.laps===1?'' + T('ui.runde_') + '':'' + T('ui.runden') + ''):'');
  ...
  h+='<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.gemessene_bestzeiten_stammen_aus_zusammenhaengenden') + '</div></div></div><div class="tabspacer"></div>';
  return h;
}
...
function gmOpenBtRowSheet(i){
  ...
  var sh=document.getElementById('detailSheet');if(!sh)return;
  var kv=[['' + T('ui.disziplin_distanz') + '',s.dist],['Leistung',s.has?s.time:GM_NA],['' + T('ui.datum_') + '',s.date!=null?s.date:GM_NA]];
  ...
  if(s.meas&&s.meas.km!=null)kv.push(['' + T('ui.gemessene_strecke') + '',(typeof fmtDe==='function'?fmtDe(s.meas.km):s.meas.km)+' km']);
  if(s.meas&&s.meas.laps)kv.push(['' + T('ui.runden_im_messfenster') + '',String(s.meas.laps)]);
  if(s.imp!=null)kv.push(['Verbesserung',s.imp]);
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon('bolt')+'</div><div><h3>'+gmEsc(s.dist)+'</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.bestzeit') + '</div></div></div>'+
    ...
    '<div class="sh-block"><div class="bh">' + T('ui.herkunft_amp_einordnung') + '</div><p>'+gmEsc(s.sub)+'</p></div>'+
    '<div class="source">'+icon('info','xs')+' Gleiche Werte wie in der Zeile — keine zusätzliche ' + T('ui.berechnung') + '.</div>';
  gmOpenSheet('detailSheet');
}
...
function gmProfMedals(){
  ...
  var h=gmPPageHead('Medaillen','' + T('ui.fuer_nachhaltigen_fortschritt_nicht_fuer') + '')+'<div class="medal-grid" style="padding:0 18px">';
  if(!ach){
    for(var i=0;i<6;i++){
    ...
    h+='</div><div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.medaillen_erscheinen_mit_deinen_ersten') + '</div></div><div class="tabspacer"></div>';
    return h;
  }
  ...
  ach.milestones.forEach(function(m){
    ...
    slots.push('<div class="medal locked"><div class="m-badge">'+icon(m.icon||'shield')+'</div><b>'+gmEsc(gmAchFmtVal(m.next,m.unit))+'</b><span>'+gmEsc(m.label)+'' + T('ui.ist') + ''+gmEsc(gmAchFmtVal(m.current,m.unit))+'</span><div class="m-prog"><i style="width:'+m.progress+'%"></i></div></div>');
  });
...
function gmProfMilestones(){
  ...
  var ach=gmAchievements();
  var h=gmPPageHead('Meilensteine','' + T('ui.fortschritt_mit_realistischen_naechsten_schritten') + '')+'<div style="padding:0 18px">';
  _gmMileSlots=[];
  if(!ach){
    ...
    for(var i=0;i<6;i++){
      ...
      var d=m?'' + T('ui.aus_deinem_zielportfolio') + '':GM_NA;
      _gmMileSlots.push(gmMileSlot(m,t,d));
      h+='<button type="button" class="mile" onclick="gmOpenMileRowSheet('+i+')"><div class="mi-ic">'+icon(m?'target':'info','sm')+'</div><div class="mile-b"><div class="mile-t">'+gmEsc(t)+'</div><div class="mile-d">'+gmEsc(d)+'</div><div class="mile-track"><i style="width:0%"></i></div><div class="mile-meta"><span>' + T('ui.start') + '</span><span>—</span><span>' + T('ui.ziel_') + '</span></div></div></button>';
    }
    h+='<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.kein_fortschritt_wird_im_ui') + '</div></div></div><div class="tabspacer"></div>';
    return h;
  }
  ...
  ach.milestones.forEach(function(mm){
    ...
    rows.push({
      kind:'measured',
      t:mm.label+(mm.next!=null?'' + T('ui.naechste_stufe') + ''+gmAchFmtVal(mm.next,mm.unit):'' + T('ui.alle_stufen_erreicht') + ''),
      d:'' + T('ui.ist_') + ''+gmAchFmtVal(mm.current,mm.unit)+' — gemessen',
      prog:mm.progress,icon:mm.icon||'target',
      slot:{title:mm.label,desc:'' + T('ui.gemessen_aus_deinen_abgeschlossenen_aktivitaeten') + '',present:true,
        start:last?gmAchFmtVal(last.step,mm.unit):null,
        target:mm.next!=null?gmAchFmtVal(mm.next,mm.unit):'' + T('ui.alle_stufen_erreicht_') + '',
      ...
      meta:['' + T('ui.stufe') + ''+(last?gmAchFmtVal(last.step,mm.unit):'—'),gmAchFmtVal(mm.current,mm.unit),(mm.next!=null?'' + T('ui.ziel__') + ''+gmAchFmtVal(mm.next,mm.unit):'Erreicht')]
    });
  });
  list.forEach(function(m){
    rows.push({kind:'goal',t:m.label||m.title||m.name||'—',d:'' + T('ui.aus_deinem_zielportfolio') + '',prog:0,icon:'target',
      slot:gmMileSlot(m,m.label||m.title||m.name||'—','' + T('ui.aus_deinem_zielportfolio') + ''),meta:['' + T('ui.start') + '','—','' + T('ui.ziel_') + '']});
  });
  while(rows.length<6)rows.push({kind:'na',t:'—',d:GM_NA,prog:0,icon:'info',
    slot:gmMileSlot(null,'—',GM_NA),meta:['' + T('ui.start') + '','—','' + T('ui.ziel_') + '']});
...
function gmOpenMileRowSheet(i){
  ...
  var kv=[['Startwert',s.start],['' + T('ui.zielwert') + '',s.target],['' + T('ui.aktueller_stand') + '',s.current],['Status',st]];
  if(s.date!=null)kv.push(['Zieldatum',s.date]);
  sh.innerHTML='<div class="grab"></div><div class="sh-head"><div class="sh-hic" style="background:var(--surface-2);color:var(--muted)">'+icon(s.present?'target':'info')+'</div><div><h3>'+gmEsc(s.title)+'</h3><div class="sh-sub" style="margin:2px 0 0">' + T('ui.meilenstein') + '</div></div></div>'+
    '<div class="sh-block"><div class="bh">' + T('ui.beschreibung') + '</div><p>'+gmEsc(s.desc)+'</p></div>'+
...
function gmProfPaceCalc(){
  ...
  var targets=sp==='bike'?[['speed','' + T('ui.geschwindigkeit__') + ''],['time','' + T('ui.zeit') + ''],['dist','' + T('ui.distanz_') + '']]:[['pace','Pace'],['time','' + T('ui.zeit') + ''],['dist','' + T('ui.distanz_') + '']];
  ...
  var distF=fld('pcDist','' + T('ui.distanz') + ''+(sp==='swim'?'m':'km')+')','decimal',sp==='swim'?'1500':'10');
  var timeF=fld('pcTime','' + T('ui.zeit_h_mm_ss') + '','numeric','52:30');
  var paceF=fld('pcPace','' + T('ui.pace') + ''+(sp==='swim'?'mm:ss/100m':'mm:ss/km')+')','numeric',sp==='swim'?'1:52':'5:15');
  var speedF=fld('pcSpeed','' + T('ui.geschw_km_h') + '','decimal','33');
  var fields;
  ...
  var h=gmPPageHead('' + T('ui.pace_geschwindigkeitsrechner') + '','' + T('ui.zwei_werte_eingeben_der_dritte') + '')+'<div style="padding:0 18px">';
  ...
  h+=fields;
  h+='<div class="calc-field result"><label>' + T('ui.ergebnis') + '</label><b id="pcResult">—</b></div>';
  if(sp==='run'){
    h+='<div class="sectlabel">' + T('ui.wettkampfprognosen') + '</div><div class="card"><div class="link-row">'+
      ...
      '</div><div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.riegel_schaetzung_aus_deiner_eingabe') + '</div></div></div>';
  }
...
var GM_PERF_REASONS={
  out_of_range:'' + T('ui.der_wert_liegt_ausserhalb_des') + '',
  implausible_pace:'' + T('ui.daraus_ergibt_sich_keine_realistische_') + '',
  implausible_css:'' + T('ui.daraus_ergibt_sich_keine_realistische') + '',
  implausible_swim_pace:'' + T('ui.daraus_ergibt_sich_keine_realistische') + '',
  css_400_not_slower_than_200:'' + T('ui.die_400_m_zeit_muss') + '',
  date_in_future:'' + T('ui.das_datum_liegt_in_der') + '',
  date_too_old:'' + T('ui.das_datum_liegt_mehr_als') + '',
  date_unreadable:'' + T('ui.das_datum_ist_nicht_lesbar') + '',
  not_a_time:'' + T('ui.das_ist_keine_lesbare_zeitangabe') + '',
  not_positive:'' + T('ui.der_wert_muss_groesser_als') + '',
  unknown_protocol:'' + T('ui.unbekanntes_testprotokoll') + '',
  unknown_field:'' + T('ui.unbekanntes_feld') + ''
};
function gmPerfReason(e){
  var t=GM_PERF_REASONS[e&&e.reason]||('' + T('ui.ungueltige_eingabe') + ''+gmEsc(String(e&&e.reason||'?'))+')');
...
function gmPerfShow(res){
  ...
  if(res.status==='needs_input'){
    var n=(res.needs||[])[0];
    _gmPerfMsg={k:'need',t:(n&&n.hint)||'' + T('ui.es_fehlt_noch_eine_angabe') + '',
      alts:(n&&n.alternatives)||null};
    return;
  }
  _gmPerfMsg={k:'bad',t:(res.errors||[]).map(gmPerfReason).join(' · ')||'' + T('ui.eingabe_abgelehnt') + ''};
}
...
function gmPerfDeleteBest(idx){
  ...
  _gmPerfMsg={k:'ok',t:'' + T('ui.eintrag_entfernt') + ''};
  gmOpenProfPage('performance');
}
function gmPerfDeleteTest(idx){
  ...
  _gmPerfMsg={k:'ok',t:'' + T('ui.test_entfernt') + ''};
  gmOpenProfPage('performance');
}
...
function gmProfPerformance(){
  ...
  var h=gmPPageHead('Leistungsdaten','' + T('ui.wettkampf_test_und_schwellenwerte_je') + '');
  if(!PI){
    return h+'<div style="padding:0 18px"><div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.das_leistungsmodul_ist_nicht_geladen') + '</div></div></div><div class="tabspacer"></div>';
  }
  ...
  if(_gmPerfMsg){
    ...
    if(_gmPerfMsg.alts){
      h+='<br><span style="color:var(--muted)">Moegliche Lesarten: '+
        ...
        '. Trag die ' + T('ui.zeit') + ' eindeutig ein, z. B. „' + T('ui.1_50') + ':00".</span>';
    }
    h+='</div></div>';
    _gmPerfMsg=null;
  }
  ...
  h+='<div class="sectlabel">' + T('ui.stand___') + '</div>';
  if(mine&&mine.ok){
    h+='<div class="bt-row" style="cursor:default"><div class="bt-b"><div class="bt-time">Zonen vorhanden '+gmEsc(EV?EV.marker(mine.evidence):'')+'</div>'+
      ...
      (mine.ageDays!=null?' · '+mine.ageDays+(mine.ageDays===1?'' + T('ui.tag_') + '':'' + T('ui.tage_') + '')+' alt':'')+
      '' + T('ui.status') + ''+gmEsc((EV&&EV.FRESH_LABEL[mine.freshness])||'—')+'</div></div></div>';
    if(mine.staleHint){
      h+='<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.dieser_wert_ist_ueber_seiner') + '</div></div>';
    }
  }else{
    h+='<div class="mini-note">'+icon('info','xs')+'<div><b>' + T('ui.noch_keine_zonen_fuer_diese') + '</b> Solange hier nichts steht, bleiben Intensitaet, Zielprognose und ' + T('ui.tagesziele_') + ' im Plan bei „—". '+
      gmEsc((mine&&mine.suggestion)||'' + T('ui.trag_ein_ergebnis_ein_oder') + '')+'</div></div>';
  }

  /* WETTKAMPF / BESTZEIT */
  h+='<div class="sectlabel">' + T('ui.ergebnis_eintragen') + '</div>';
  h+='<div class="perf-form">'+
    '<label>' + T('ui.distanz_') + '<input id="gmPerfDist" type="text" inputmode="decimal" placeholder="' + T('ui.10_km_hm_marathon') + '"></label>'+
    '<label>' + T('ui.zeit') + '<input id="gmPerfTime" type="text" inputmode="numeric" placeholder="' + T('ui.48_30_oder_1_50') + '"></label>'+
    '<label>' + T('ui.kontext') + '<select id="gmPerfCtx"><option value="Wettkampf">Wettkampf</option><option value="Test">' + T('ui.test_trainingsbestzeit') + '</option></select></label>'+
    '<label>' + T('ui.datum_') + '<input id="gmPerfDate" type="date"></label>'+
    '<button class="mini-btn primary" onclick="gmPerfSaveRace()">' + T('ui.eintragen') + '</button>'+
    '</div>';
  h+='<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.ein_wettkampf_ist_der_staerkste') + '</div></div>';

  /* TESTS — mit Anleitung direkt hier */
  var protos=PI.protocolsFor(_gmPerfSport)||[];
  if(protos.length){
    h+='<div class="sectlabel">' + T('ui.test_machen') + '</div>';
    protos.forEach(function(p){
      ...
      h+='<button type="button" class="bt-row" onclick="gmPerfPickTest(\''+p.id+'\')"><div class="bt-b">'+
        ...
        '<div class="bt-sub">'+gmEsc(p.level==='anfaenger'?'Einsteiger':'' + T('ui.fortgeschritten_') + '')+'</div></div>'+
      ...
      if(open){
        ...
        h+='<label>' + T('ui.datum_') + '<input id="gmPerfTestDate" type="date"></label>';
        h+='<button class="mini-btn primary" onclick="gmPerfSaveTest()">' + T('ui.ergebnis_speichern') + '</button></div>';
      }
    });
  }
  ...
  if(vals.length){
    h+='<div class="sectlabel">' + T('ui.bekannte_werte') + '</div><div class="perf-form">';
    vals.forEach(function(f){
      ...
      h+='<label>'+gmEsc(spec.label)+' ('+gmEsc(spec.unit)+')<input id="gmPerfV_'+f+'" type="text" inputmode="decimal"></label>'+
        '<label>' + T('ui.datum_') + '<input id="gmPerfVDate_'+f+'" type="date"></label>'+
        '<button class="mini-btn" onclick="gmPerfSaveValue(\''+f+'\')">'+gmEsc(spec.label)+' speichern</button>';
    });
    h+='</div>';
    h+='<div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.ein_selbst_eingetragener_wert_zaehlt') + '</div></div>';
  }
  ...
  if(mineBests.length||mineTests.length){
    h+='<div class="sectlabel">' + T('ui.erfasst') + '</div>';
    mineBests.forEach(function(x){
      ...
      h+='<div class="bt-row" style="cursor:default"><div class="bt-b"><div class="bt-time">'+gmEsc(String(b.distance))+' km · '+gmEsc(tt)+'</div>'+
        ...
        '<span class="edit" role="button" tabindex="0" onclick="gmPerfDeleteBest('+x.i+')">' + T('ui.entfernen') + '</span></div>';
    });
    mineTests.forEach(function(x){
      ...
      h+='<div class="bt-row" style="cursor:default"><div class="bt-b"><div class="bt-time" style="font-size:15px">'+gmEsc((proto&&proto.label)||t.id)+'</div>'+
        (t.evidence?gmPerfEvLine(t.evidence):'<div class="bt-sub">'+gmEsc(t.date||'' + T('ui.ohne_datum') + '')+'</div>')+'</div>'+
        '<span class="edit" role="button" tabindex="0" onclick="gmPerfDeleteTest('+x.i+')">' + T('ui.entfernen') + '</span></div>';
    });
  }
...
var GM_PERF_FIELD_LABEL={
  distanceKm:'' + T('ui.distanz_in_km') + '',durationMin:'' + T('ui.dauer_in_minuten') + '',avgWatts:'' + T('ui.durchschnittsleistung_in_watt') + '',
  avgHr:'' + T('ui.durchschnittliche_herzfrequenz') + '',distanceM:'' + T('ui.distanz_in_metern') + '',
  t400Sec:'' + T('ui.400_m_zeit_in_sekunden') + '',t200Sec:'' + T('ui.200_m_zeit_in_sekunden') + ''
};
var GM_PERF_FIELD_HINT={
  distanceKm:'' + T('ui.z_b_2_8') + '',durationMin:'' + T('ui.z_b_22') + '',avgWatts:'' + T('ui.z_b_263') + '',avgHr:'' + T('ui.z_b_172') + '',
  distanceM:'' + T('ui.z_b_520') + '',t400Sec:'' + T('ui.z_b_372') + '',t200Sec:'' + T('ui.z_b_178') + ''
};
...
function gmPerfRowSub(){
  try{
    var PI=gmPerfMod();if(!PI)return '' + T('ui.wettkampf_test_und_schwellenwerte') + '';
    var cov=PI.coverage((typeof PROFILE!=='undefined'&&PROFILE)||null,{today:gmPerfToday()});
    if(!cov)return '' + T('ui.wettkampf_test_und_schwellenwerte') + '';
    var miss=cov.missing||[];
    if(!miss.length)return '' + T('ui.zonen_fuer_alle_drei_sportarten') + '';
    if(!cov.anyOk)return '' + T('ui.noch_keine_zonen_deshalb_steht') + '';
    var L={running:'Laufen',cycling:'Rad',swimming:'Schwimmen'};
    return '' + T('ui.fehlt') + ''+miss.map(function(m){return L[m]||m;}).join(', ');
  }catch(_){return '' + T('ui.wettkampf_test_und_schwellenwerte') + '';}
}
...
function gmRenderDebriefSheet(){
  if(!_gmDbCtx)return;
  var c=_gmDbCtx;
  var h='<div class="sheet-head"><h3>' + T('ui.wie_war_die_einheit') + '</h3><p>'+gmEsc((c.unit&&c.unit.l)||'')+'</p></div><div class="sheet-body">';
  h+='<div class="sectlabel" style="padding-left:0">' + T('ui.anstrengung') + '</div>';
  h+='<div class="db-rpe">';
  for(var i=1;i<=10;i++){
    h+='<button type="button" class="db-rpe-b'+(_gmDbRpe===i?' on':'')+'" onclick="gmDbSetRpe('+i+')">'+i+'</button>';
  }
  h+='</div><div class="mini-note">'+icon('info','xs')+'<div>' + T('ui.1_sehr_locker_10_maximal') + '</div></div>';
  h+='<div class="sectlabel" style="padding-left:0">' + T('ui.schmerzen') + '</div>';
  h+='<div class="seg"><button type="button" class="seg-b'+(!_gmDbPain?' on':'')+'" onclick="if('+(_gmDbPain?'true':'false')+')gmDbTogglePain()">' + T('ui.nein') + '</button>'+
  ...
  if(pre&&pre.judged&&(pre.adherence==='abgebrochen'||pre.adherence==='zu langsam'||pre.adherence==='zu schnell')){
    h+='<div class="sectlabel" style="padding-left:0">' + T('ui.woran_lag_es') + '</div><div class="db-reasons">';
  ...
  h+='<button class="mini-btn primary" style="width:100%;margin-top:14px" onclick="gmDbSave()">' + T('ui.speichern') + '</button>';
  h+='</div>';
...
var GM_DB_REASONS=[
  ['fatigue','' + T('ui.muede_schwere_beine') + ''],['time','' + T('ui.zeit_gefehlt') + ''],['pain','Beschwerden'],
  ['weather','' + T('ui.wetter_strecke') + ''],['illness','' + T('ui.krank_angeschlagen') + ''],['felt_good','' + T('ui.ging_leicht') + ''],
  ['other','Anderes']
];
...
function gmDbSave(){
  ...
  try{renderGMPlan();}catch(_){ }
  try{toast('' + T('ui.rueckmeldung_gespeichert') + '');}catch(_){ }
}
...
function gmOpenDebriefAt(di,ii,clickDateIso){
  try{
    ...
    if(!dateIso){
      if(typeof toast==='function')toast('' + T('ui.keine_rueckmeldung_moeglich_fehlender_datumskontext') + '');
      return;
    }
    if(dateIso>todayStr()){
      if(typeof toast==='function')toast('' + T('ui.keine_rueckmeldung_moeglich_diese_einheit') + '');
      return;
    }
