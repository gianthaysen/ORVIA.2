...
(function (root) {
  ...
  function goalProgress(g, d) {
    ...
    if (typeof g.targetValue === 'number' && typeof g.currentValue === 'number' && g.metricType !== 'time') {
  ...
  function goalCard(g, d, opts) {
    ...
    var bar = '<div class="goal-line"><i style="width:' + (p.pct != null ? p.pct : 0) + '%' + (role !== 'main' ? ';background:linear-gradient(90deg,var(--activity),var(--ready))' : '') + '"></i></div>';
    ...
    return '<div class="goal-card tap' + (role !== 'main' ? ' sec' : '') + '" onclick="openGoalDetail(\'' + esc(g.id) + '\')">' +
      '<div class="goal-top"><div style="min-width:0"><h4>' + esc(g.title || catLabel(g.category)) + '</h4><p>' + esc(sub) + '</p></div>' + badge + '</div>' +
  ...
  function goalsHTML(d) {
    ...
    if (paused.length) h += sectlabel(T('pv.pausiert')) + '<div class="setting-group">' + paused.map(function (g) { return '<div class="prow" onclick="openGoalDetail(\'' + esc(g.id) + '\')"><div class="p-b"><div class="p-t">' + esc(g.title || catLabel(g.category)) + '</div><div class="p-d">' + esc(goalSub(g, d)) + '</div></div><button type="button" class="gmc-b" onclick="event.stopPropagation();goalSetStatus(\'' + esc(g.id) + '\',\'active\')">' + esc(T('pv.fortsetzen')) + '</button></div>'; }).join('') + '</div>';
    if (d.conflicts.length) {
      h += sectlabel(T('pv.zielkonflikte'));
      var titleOf = function (id) { var g = gs.filter(function (x) { return x.id === id; })[0]; return g ? (g.title || catLabel(g.category)) : id; };
    ...
    if (done.length) {
      h += sectlabel(T('pv.erreicht_verfehlt')) + '<div class="setting-group">' + done.map(function (g) {
        ...
        return '<div class="prow" onclick="openGoalDetail(\'' + esc(g.id) + '\')"><div class="p-ic" style="background:var(--' + (ok ? 'ready' : 'attention') + '-t);color:var(--' + (ok ? 'ready' : 'attention') + ')">' + ic(ok ? 'shield' : 'target', 'sm') + '</div><div class="p-b"><div class="p-t">' + esc(g.title || catLabel(g.category)) + '</div><div class="p-d">' + esc(sub) + '</div></div><span class="pill-badge ' + (ok ? 'ready' : 'att') + '">' + esc(ok ? T('pv.erreicht') : T('pv.verfehlt')) + '</span></div>';
      }).join('') + '</div>';
    }
  ...
  function bestLifts(records) {
    ...
    (records || []).forEach(function (r) { if (!r) return; var name = String(r.exerciseName || r.exerciseId || '');
      out.forEach(function (o) { if (!o.re.test(name)) return; var v = num(r.estimatedOneRepMax) != null ? r.estimatedOneRepMax : num(r.weightKg);
        if (o.key === 'pv.lift_pullup' && num(r.repetitions) != null) { if (o.reps == null || r.repetitions > o.reps) { o.reps = r.repetitions; o.date = r.date || null; } return; }
        if (v != null && (o.value == null || v > o.value)) { o.value = v; o.date = r.date || null; } }); });
    return out;
  }
  function srcLabel(s) { var map = { manual: T('pv.src_manual'), garmin: 'Garmin', strava: 'Strava', apple_health: 'Apple Health', import: T('pv.src_import'), calculated: T('pv.src_calculated'), activity: T('pv.src_activity'), derived_estimate: T('pv.src_estimate'), device: T('pv.src_device'), device_sync: T('pv.src_device'), provider_sync: T('pv.src_device') }; return map[s] || (s ? String(s) : ''); }
  function performanceHTML(d) {
    ...
    h += '<div class="card tight"><div class="datarow" style="margin-top:0">' + lifts.map(function (l) { var v = l.reps != null ? l.reps + ' ' + T('pv.wdh') : (l.value != null ? fmtDe(l.value) + ' kg' : null); return '<div class="datacell"><div class="dl">' + esc(l.label) + '</div><div class="dn' + (v ? '' : ' muted') + '">' + esc(v || '—') + '</div></div>'; }).join('') + '</div>' +
