...
(function (root) {
  ...
  // Vollständige Pipeline: liefert {snapshots, diagnostics}. Lokale + (optional) Server-Activities.
  // Gelöschte (Tombstone), Mobility/Nicht-Gym, Zeitfenster, fehlende Snapshots werden begründet abgelehnt.
  function gymPipeline(opts) {
    ...
    var diag = {
      rawLocalActivityCount: local.length, rawServerActivityCount: server.length, rawLegacySessionCount: (opts.legacyActivities || []).length,
    ...
    schema('activityStore', local); schema('serverActivities', server);
  ...
  // Rückwärtskompatibel: nur die Snapshots.
  ...
  function buildShadowReport(opts) {
    ...
    var pipe = gymPipeline({ days: days, serverActivities: opts.serverActivities });
    ...
    return {
      ...
      diagnostics: pipe.diagnostics   // nur Feldnamen + Zählwerte, KEINE IDs/E-Mail/Auth
    };
  }
