...
(function (root) {
  ...
  function visibleActivityPipeline() {
    var info = { functionName: null, resultCount: null, gymResultCount: null, gymRealWorkoutCount: 0, gymEmptyCount: 0, gymProjectionCount: 0, gymLegacyCount: 0, gymCanonicalCount: 0, gymDuplicateGroups: 0 };
    try {
      if (typeof window !== 'undefined' && typeof window.listActivitiesUnified === 'function') {
        ...
        var gym = list.filter(function (a) { return isGymSport(a); });
        info.gymResultCount = gym.length;
        var dayGroups = {};
        gym.forEach(function (a) {
          if (a.source === 'orvia_workout') info.gymCanonicalCount++; else if (a.source === 'legacy_local' || a.source === 'legacy_db') info.gymLegacyCount++;
          var hasReal = (a.workoutSnapshot && a.workoutSnapshot.length) || (a.summary && a.summary.workingSetCount > 0) || (a.durationSeconds && a.durationSeconds > 0);
          if (hasReal) info.gymRealWorkoutCount++; else info.gymEmptyCount++;
          var day = (a.startedAt || '').slice(0, 10) || (a._legacy && a._legacy.date) || '?';
  ...
  async function buildShadowReport(opts) {
    ...
    var muscles = MUSCLES.map(function (mk) {
      ...
      return {
        ...
        targetRange: target, confidence: conf
      };
    ...
    if (vap.gymResultCount != null && vap.gymRealWorkoutCount != null && vap.gymResultCount >= vap.gymRealWorkoutCount + 5 && vap.gymResultCount >= 2 * Math.max(1, vap.gymRealWorkoutCount)) warnings.push('VISIBLE_GYM_DUPLICATION_SUSPECTED');
    ...
    return sanitizeReport(report);   // entfernt jegliche UUID-/E-Mail-/Token-Werte aus dem gesamten Report
  }
