...
(function (root) {
  ...
  var SPORTS_REQUIRED = ['normalizeSportsSelection', 'validateSportsSelection', 'sportsComplete', 'getPrimarySport', 'getPlannedSports', 'getVisibleSports', 'getOccasionalSports', 'setPrimarySport', 'toggleSport', 'setSportRole', 'setPlanningEnabled', 'setVisible', 'reorderPlannedSports', 'seedFromExistingProfile', 'buildUserSportConfiguration', 'validateSportCatalog', 'plannable'];
  function slFull() {
    var sl = SL();
  ...
  function renderSportsStep() {
    ...
    var aChips = cat.map(function (s) { var on = chosen.indexOf(s.id) >= 0; return '<button type="button" class="ob2-chip' + (on ? ' on' : '') + '" id="sp-' + s.id + '" aria-pressed="' + (on ? 'true' : 'false') + '">' + esc(s.label) + '</button>'; }).join('');
    var bSection = '', cSection = '';
    if (chosen.length) {
      // Hauptsportart NUR aus planbaren Sportarten wählbar (nicht planbare wie „Andere" erscheinen hier nicht).
      var pChips = sel.sports.filter(function (e) { return sl.plannable(e.sportId); }).map(function (e) { var on = e.role === 'primary'; return '<button type="button" class="ob2-chip' + (on ? ' on' : '') + '" id="pr-' + e.sportId + '" aria-pressed="' + (on ? 'true' : 'false') + '">' + esc(byId[e.sportId].label) + '</button>'; }).join('');
      bSection = '<div class="ob2-field"><label id="sports-b-label">Was ist deine Hauptsportart?</label>' + (priErr ? '<span class="ob2-err" id="err-primary" role="alert">' + esc(errors._primary) + '</span>' : '') + '<div class="ob2-chips" role="group" aria-labelledby="sports-b-label"' + (priErr ? ' aria-describedby="err-primary" aria-invalid="true"' : '') + '>' + pChips + '</div></div>';
      // Reihenfolge der aktiv geplanten Sekundär-Sportarten (für ↑/↓-Grenzen).
      var plannedOrder = sl.getPlannedSports(sel);
      var plannedSec = plannedOrder.filter(function (id) { var e = null; sel.sports.forEach(function (x) { if (x.sportId === id) e = x; }); return e && e.role !== 'primary'; });
      var rows = sel.sports.map(function (e) {
        var lbl = byId[e.sportId].label;
        if (e.role === 'primary') return '<div class="ob2-sportrow"><span class="ob2-sportname">' + esc(lbl) + ' <span class="ob2-badge">Hauptsportart</span></span><span class="ob2-sportsub">aktiv geplant · sichtbar · Priorität 1</span></div>';
        if (!sl.plannable(e.sportId)) {   // nicht planbar: nur sichtbar/ausblenden, keine Planung, keine Hauptsportart
          return '<div class="ob2-sportrow"><span class="ob2-sportname">' + esc(lbl) + ' <span class="ob2-badge sub">zusätzliche Aktivität</span></span><div class="ob2-sportopts">' +
            '<span class="ob2-sportsub">nicht planbar</span>' +
            '<label class="ob2-mini"><input type="checkbox" id="vis-' + e.sportId + '"' + (e.visible ? ' checked' : '') + '> sichtbar</label></div></div>';
        }
        var prio = '';
        if (e.planningEnabled) {
          var si = plannedSec.indexOf(e.sportId);
          prio = '<span class="ob2-prio">Priorität ' + (e.priority || (si + 2)) + '</span>' +
            '<button type="button" class="ob2-mvbtn" id="up-' + e.sportId + '" aria-label="Priorität erhöhen: ' + esc(lbl) + '"' + (si <= 0 ? ' disabled' : '') + '>↑</button>' +
            '<button type="button" class="ob2-mvbtn" id="dn-' + e.sportId + '" aria-label="Priorität senken: ' + esc(lbl) + '"' + (si >= plannedSec.length - 1 ? ' disabled' : '') + '>↓</button>';
        }
        return '<div class="ob2-sportrow"><span class="ob2-sportname">' + esc(lbl) + '</span><div class="ob2-sportopts">' +
          '<label class="ob2-mini"><input type="checkbox" id="occ-' + e.sportId + '"' + (e.role === 'occasional' ? ' checked' : '') + '> gelegentlich</label>' +
          '<label class="ob2-mini"><input type="checkbox" id="plan-' + e.sportId + '"' + (e.planningEnabled ? ' checked' : '') + (e.role === 'occasional' ? ' disabled' : '') + '> geplant' + (e.role === 'occasional' ? ' <span class="ob2-sub">(gelegentlich: nicht planbar)</span>' : '') + '</label>' +
          '<label class="ob2-mini"><input type="checkbox" id="vis-' + e.sportId + '"' + (e.visible ? ' checked' : '') + '> sichtbar</label>' + prio + '</div></div>';
      }).join('');
      cSection = '<div class="ob2-field"><label>Wie soll ORVIA sie behandeln?</label>' + rows + '<p class="ob2-hint">Ausgeblendete Sportarten bleiben weiterhin unter „Weitere Aktivität" verfügbar.</p></div>';
    }
    var card = S.el.querySelector('.ob2-card');
    card.innerHTML = progressHTML() +
      '<h2 id="ob2-title" class="ob2-title" tabindex="-1">Deine Sportarten</h2>' +
      '<p class="ob2-desc">Wähle die Sportarten, die ORVIA für dich berücksichtigen soll. Andere Aktivitäten kannst du später trotzdem jederzeit absolvieren.</p>' +
    ...
    sel.sports.forEach(function (e) {
      ...
      var occ = doc.getElementById('occ-' + e.sportId); if (occ) occ.addEventListener('change', function () { setSports(sl.setSportRole(S.draft.draftData.sports, e.sportId, this.checked ? 'occasional' : 'secondary')); });
      var pl = doc.getElementById('plan-' + e.sportId); if (pl) pl.addEventListener('change', function () { setSports(sl.setPlanningEnabled(S.draft.draftData.sports, e.sportId, this.checked)); });
      var vi = doc.getElementById('vis-' + e.sportId); if (vi) vi.addEventListener('change', function () { setSports(sl.setVisible(S.draft.draftData.sports, e.sportId, this.checked)); });
  ...
  root.openOrviaOnboarding = function (options) {
    ...
    if (ob && typeof ob.open === 'function') {
      return ob.open({ fresh: options.fresh === true, source: options.source || 'unknown' }) === true;
    }
