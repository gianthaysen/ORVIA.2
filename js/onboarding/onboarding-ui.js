...
(function (root) {
  ...
  function renderSummaryStep() {
    ...
    card.innerHTML = progressHTML() + '<h2 id="ob2-title" class="ob2-title" tabindex="-1">Dein ORVIA-Profil</h2>' +
      ...
      sec('Hauptziel', '<p>' + esc(primary ? primary.title : '—') + '</p>', 'goals') +
      sec('Weitere Ziele', '<p>' + esc(goals.filter(function (g) { return g !== primary; }).map(function (g) { return g.title; }).join(', ') || '—') + '</p>', 'goals') +
      sec('Trainingsalltag', '<p>' + esc(availDays + ' verfügbare Tage' + (av.maxSessionsPerWeek != null ? ' · bis zu ' + av.maxSessionsPerWeek + ' Einheiten' : '')) + '</p>', 'availability') +
