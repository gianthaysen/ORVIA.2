...
(function (root) {
  ...
  var S = { draft: null, userId: null, el: null, busy: false, lastNav: 0, corruptNote: false, bound: false, previousFocus: null, profileSubmitted: false, heightDirty: false, weightDirty: false };
  var NAV_LOCK_MS = 300;
  ...
  // Profil-Logik gilt nur als funktionsfähig, wenn ALLE benötigten Funktionen vorhanden sind.
  function plFull() { var pl = PL(); return !!(pl && typeof pl.normalizeProfile === 'function' && typeof pl.validateProfile === 'function' && typeof pl.profileComplete === 'function' && typeof pl.profileSeedFromExisting === 'function'); }
  ...
  function render(showCorrupt) {
    ...
    card.innerHTML =
      ...
      (isLast ? '<p class="ob2-note">Dein Basisprofil ist gespeichert. Sportarten, Ziele und Trainingsalltag folgen in den nächsten Schritten.</p>' : '') +
      '<div class="ob2-nav">' +
  ...
  function goReviewReady() {
    ...
    if (!r || !r.ok) { try { console.warn('[ORVIA onboarding] Review nicht vormerkbar:', r && r.error); } catch (e) {} render(); S.busy = false; return; }
  ...
  function renderReviewDone() {
    ...
    card.innerHTML =
      ...
      '<p class="ob2-desc">Dein Basisprofil ist gespeichert (lokal auf diesem Gerät). Die weiteren Bereiche für Sportarten, Ziele und Trainingsalltag folgen in den nächsten Entwicklungsschritten.</p>' +
  ...
  function captureCurrentStep() {
    if (S.draft && S.draft.currentStep === 'profile' && PL() && S.draft.draftData && S.draft.draftData.profile) {
  ...
  function goNext() { if (S.busy || navLocked()) return; S.busy = true; L().advance(S.draft, now()); render(); S.busy = false; }
  function goBack() { if (S.busy || navLocked()) return; S.busy = true; captureCurrentStep(); L().back(S.draft, now()); render(); S.busy = false; }
  ...
  O.onboardingV2 = {
    ...
    _reset: function () { S.draft = null; S.el = null; S.busy = false; S.lastNav = 0; S.corruptNote = false; S.previousFocus = null; S.profileSubmitted = false; S.heightDirty = false; S.weightDirty = false; }
  };

  // Zentraler Dispatcher (von auth.js bei orvia_onboard_pending genutzt). Gibt true bei Öffnung.
  root.openPendingOnboarding = function () {
    try {
      if (O.onboardingV2 && O.onboardingV2.open) { return O.onboardingV2.open({ fresh: true, source: 'registration' }); }
    } catch (e) {}
    try { if (typeof root.openOnboarding === 'function') { root.openOnboarding(true); return true; } } catch (e) {} // kontrollierter Legacy-Fallback
    return false;
  };
