...
(function (root) {
  ...
  var STEP_IDS = ['welcome', 'profile', 'sports_placeholder', 'goals_placeholder', 'schedule_placeholder', 'review_placeholder'];
  ...
  var STEP_ALIASES = { profile_placeholder: 'profile' };
  ...
  function advance(d, now) {
    ...
    if (nxt) { d.currentStep = nxt; } else { d.status = 'ready_for_review'; }
    if (now != null) d.updatedAt = now;
    return d;
  }
  // Fachfunktion Profil-Schritt: nur am 'profile'-Schritt; validiert; schließt nur bei Gültigkeit ab.
  function advanceProfile(d, now) {
    ...
    var pl = profileLogic();
    var v = pl ? pl.validateProfile(d.draftData && d.draftData.profile) : { valid: false, errors: {} };
  ...
  function normalizeCompletedSteps(rawSteps, currentStep, draft) {
    ...
    (Array.isArray(rawSteps) ? rawSteps : []).forEach(function (s) {
      ...
      if (a === 'profile' && !profileOk) return;
      present[a] = true;
    });
    var reviewFinal = (currentStep === 'review_placeholder' && draft && draft.status === 'ready_for_review' && profileOk);
  ...
  function reconcileDraftStatus(d) {
    if (!d) return d;
    if (!Array.isArray(d.completedSteps)) d.completedSteps = [];
    var ok = profileValid(d);
    if ((d.status === 'ready_for_review' || d.status === 'completed') && !ok) { d.status = 'in_progress'; d.currentStep = 'profile'; }
    if (d.status === 'completed') d.status = 'ready_for_review';   // in diesem Batch kein 'completed'
  ...
  // Review-Vormerkung: strukturiertes Resultat. Nur bei currentStep=review, validem Profil und
  // lückenlos abgeschlossenen Vorschritten. Kein 'completed'.
  function markReadyForReview(d, now) {
    ...
    if (!profileValid(d)) return { ok: false, draft: d, error: 'Basisprofil unvollständig.' };
    var need = stepsBeforeReview();
    for (var i = 0; i < need.length; i++) { if (d.completedSteps.indexOf(need[i]) < 0) return { ok: false, draft: d, error: 'Vorherige Schritte unvollständig.' }; }
  ...
  function readyForReview(d) { return !!(d && d.status === 'ready_for_review'); }
  ...
  var api = {
    ...
    progress: progress, startDraft: startDraft, advance: advance, advanceProfile: advanceProfile, back: back,
