...
function renderAkt() {
  ...
  var head = '<div class="act-actions">' +
    ...
    '<button class="btn' + (demoEnabled ? ' sec' : '') + '" style="' + (demoEnabled ? 'margin-top:10px' : '') + '" onclick="openManualActivity()"><svg class="ic"><use href="#i-plus"/></svg>Aktivität manuell hinzufügen</button>' +
    '<p class="note" style="text-align:left;margin-top:10px">Strava/Garmin-Autosync folgt serverseitig. Bis dahin bleiben Aktivitäten manuell oder per Import in deinem eigenen Konto.</p></div>';
  if (!acts.length) {
