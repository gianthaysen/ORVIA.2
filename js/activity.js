...
function listActivitiesUnified(limit) {
  ...
  var merged = (window.ORVIA && ORVIA.activityConfig && ORVIA.activityConfig.mergeAllActivities)
    ? ORVIA.activityConfig.mergeAllActivities(_serverActivities, local, legacy)
    : local.concat(legacy);
...
function openActivityDetails(idOrEl) {
  ...
  // Cross-device: nur serverseitig bekannte Activity (nicht im lokalen Store).
  ...
  if (a && a.source === 'orvia_workout') {              // kanonisches Workout → Snapshot-first-Renderer (Server-Tree als Fallback)
