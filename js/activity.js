...
function workoutCardHTML(date, typ, s) {
  ...
  return '<div class="wcard">' +
    ...
    ((s.exercises && s.exercises.length) ? '<div class="wc-ex"><div class="wc-ex-h">Übungen (' + s.exercises.length + ')</div><div class="wc-ex-chips">' + s.exercises.map(function (n) { return '<span class="wc-exc">' + escH(n) + '</span>'; }).join('') + '</div></div>' : '') +
...
function renderAkt() {
  ...
  var head = '<div class="act-actions">' +
    '<button class="btn" onclick="importRealRun()"><svg class="ic"><use href="#i-pulse"/></svg>Strava-Lauf importieren</button>' +
    ...
    '<p class="note" style="text-align:left;margin-top:10px">„Strava-Lauf importieren" lädt deinen echten handewitter Sommerlauf — mit GPS-Strecke, echten Splits, Pace-Kurve und Bestzeiten. Voll-Autosync folgt.</p></div>';
  if (!acts.length) {
