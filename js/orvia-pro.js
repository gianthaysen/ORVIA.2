...
function openLegal(){
  oModal('Rechtliches & Daten',
    ...
    '</div>'+
    '<p class="note" style="text-align:left;margin-top:12px">Sprache: Deutsch · Region: EU. <span id="ov-ver" onclick="orviaVerTap()" style="cursor:default">App-Version v8 · Datenmodell v4</span>. Rechtstexte sind Platzhalter und müssen vor Veröffentlichung juristisch geprüft werden.</p>'+
    '<div id="ov-diag"></div>');
}
/* Versteckter Diagnosezugang: 7 Taps auf die Versionszeile innerhalb von 5 s. NICHT persistent
   (nach App-Neustart wieder inaktiv). Kein Element in der normalen Oberfläche. */
var _verTaps=0,_verT0=0;
function orviaVerTap(){
  var now=Date.now();
  if(now-_verT0>5000){_verTaps=0;_verT0=now;}
  _verTaps++;
  if(_verTaps>=7){
    _verTaps=0;
    var d=document.getElementById('ov-diag'); if(!d||d.innerHTML)return;
