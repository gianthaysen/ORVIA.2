...
function nutProfile() {
  ...
  var n = p.nutrition || {};
  return {
    sex: n.sex || p.sex || 'm', age: p.age || 30, heightCm: p.heightCm || 175, weightKg: p.weightKg || 75,
...
function trainingBurnToday() {
  var w = (typeof PROFILE !== 'undefined' && PROFILE && PROFILE.weightKg) || 75;
...
function nutToday() {
  var np = nutProfile();
  if (!np.weightKg || !np.heightCm) return null;
...
function nutWeekly() {
  var t = nutToday(); var pTarget = t ? t.protein : 140;
...
function renderNutritionToday() {
  ...
  el.innerHTML = '<div class="card nutcard"><h2><svg class="ic"><use href="#i-nutrition"/></svg>Energie &amp; Ernährung<span class="nut-day">' + escH(dt) + '</span></h2>' +
    ...
    '<div class="nut-week">Protein-Ziel diese Woche: <b>' + wk.proteinDays + '/7</b> Tage' + (wk.weightTrend ? ' · Gewicht 7T: <b>' + escH(wk.weightTrend) + '</b>' : '') + '</div></div>';
}
