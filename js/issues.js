...
function moduleStatus(key){
  ...
  if(!s.length)return{key:key,label:(ORVIA_MODULES[key]||{}).label||key,status:'kein',score:null,streak:0};
  var latest=s[s.length-1].v;
  // Null-Streak (Tage am Stück bei 0, von hinten)
  var zero=0;for(var i=s.length-1;i>=0;i--){if(s[i].v===0)zero++;else break;}
  var st;
  if(latest>=5)st='warn';
  else if(latest>=3)st='aktiv';
  else if(latest>=1)st='beobachten';
  else st=(zero>=14)?'praevention':(zero>=7)?'stabil':'ruhig';
  return{key:key,label:(ORVIA_MODULES[key]||{}).label||key,status:st,score:latest,streak:zero,series:s};
}
var STATUS_META={
...
function activeModuleKeys(){
  var set={};(((typeof PROFILE!=='undefined'&&PROFILE)?PROFILE.issues:[])||[]).forEach(function(k){if(ORVIA_MODULES[k])set[k]=1;});
  // alles, was in den letzten 21 Tagen > 0 geloggt wurde, ebenfalls einblenden
  Object.keys(ORVIA_MODULES).forEach(function(k){var s=issueSeries(k,21);if(s.some(function(x){return x.v>0;}))set[k]=1;});
...
function logIssue(key,val){
  ...
  if(typeof PROFILE!=='undefined'&&PROFILE){
    ...
    if(val>0&&PROFILE.issues.indexOf(key)<0){PROFILE.issues.push(key);saveProfile();}
  }
  save();
  renderModules();
...
function removeModule(key){
  if(typeof PROFILE!=='undefined'&&PROFILE&&PROFILE.issues){PROFILE.issues=PROFILE.issues.filter(function(k){return k!==key;});saveProfile();}
  renderModules();closeSupp();if(typeof toast==='function')toast('Modul pausiert');
}
