  ...
  async function start() {
    ...
    if (countLocalDays() > 0) {
      ...
      if (owner && owner !== u.id) {
        ...
        try { localStorage.setItem('orvia_data_owner', u.id); } catch (e) {}
        /* H3 (2026-07-11): NICHT mehr blind pushen. Wenn die Cloud einen Stand traegt,
           den DIESES Geraet nicht kennt (savedAt ≠ zuletzt gesehene Rev), stammt er von
           einem anderen Geraet → migratePrompt (war vorher toter Code; stiller
           Zweitgeraete-Clobber der Blob-Daten: weekPlan, Termine, Recovery, Prefs …). */
        let remote0 = null;
        try {
        ...
        const remoteRev = remoteHas0 ? String(remote0.data.savedAt || '') : '';
        if (remoteHas0 && remoteRev && remoteRev !== knownRev) {
