    ...
    try {
      if (mode === 'register') {
        ...
        if (reg.needsConfirmation) {
          // E-Mail-Bestätigung nötig: Bestätigungsmail anstoßen, dann zum Login.
          try { await sb.auth.resend({ type: 'signup', email: email }); } catch (e) {}
          setMode('login');
          err('Fast geschafft! Bitte bestätige deine E-Mail (Postfach prüfen) und melde dich dann an.');
          busy(false);
          return;
        }
