...
(function () {
  ...
  async function onAuthed(session) {
    O.user = session.user;
    // Kontowechsel auf diesem Gerät: fremde lokale Daten SOFORT verwerfen (vor Render/Sync)
    ...
    // Neuer Account → individuelles Profil einrichten (auch nach E-Mail-Bestätigung beim 1. Login)
    try {
  ...
  async function onSubmit(ev) {
    ...
    try {
      if (mode === 'register') {
        ...
        localStorage.setItem('orvia_onboard_pending', '1');   // neuer Account → Profil-Onboarding
        const { data, error } = await sb.auth.signUp({ email, password: pw });
        if (error) { err(germanAuthError(error)); localStorage.removeItem('orvia_pending_invite'); localStorage.removeItem('orvia_onboard_pending'); busy(false); return; }
  ...
  window.orviaLogout = async function () {
    ...
    try { ['orvia_active_user', 'orvia_pending_invite', 'orvia_onboard_pending'].forEach(function (k) { localStorage.removeItem(k); }); } catch (e) {}
    O.user = null;
    location.reload();
  };
