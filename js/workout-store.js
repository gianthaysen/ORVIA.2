...
(function () {
  ...
  async function finishWorkout(summary) {
    ...
    const patch = { status: 'completed', finished_at: new Date().toISOString(), duration_min: durationMin,
      total_paused_seconds: Math.round(pausedSec), paused_at: null,
      session_rpe: summary.sessionRpe != null ? summary.sessionRpe : null, perceived_effort: summary.perceivedEffort != null ? summary.perceivedEffort : null, notes: summary.notes != null ? summary.notes : s.notes };
    let r;
    if (online() && s.id) r = await O.repos.workout.updateSession(s.id, patch);
    else r = await offlineUpsert('workout_sessions', Object.assign(buildSessionRow({ localDate: s.local_date, clientSessionId: s.client_session_id, status: 'completed' }), patch), 'user_id,client_session_id', { clientId: s.client_session_id });
    if (!r.success) return r;
    O.workout.session = Object.assign({}, s, patch);
    // Trainingslast GENAU EINMAL je Session (Dedupe über client_session_id 'workout_session:<id|clientId>').
    // Kein erfundener RPE: ohne session_rpe KEINE Last (incomplete_no_rpe). Load-Fehler ≠ Scheinerfolg.
    const loadKey = 'workout_session:' + (s.id || s.client_session_id);
    let loadStatus = 'incomplete_no_rpe';
    if (patch.session_rpe != null) {
      if (online() && O.repos && O.repos.trainingLoad && s.id) {
        try {
          const lr = await O.repos.trainingLoad.save(s.local_date, s.sport || 'Gym', { dur: durationMin, rpe: patch.session_rpe, source: 'workout', client_session_id: loadKey });
          loadStatus = (lr && lr.success) ? 'written' : 'load_error';
        } catch (e) { loadStatus = 'load_error'; }
      } else if (O.repos && O.repos.trainingLoad && O.repos.trainingLoad.toRow && O.offlineQueue) {
        // Offline-Abschluss mit RPE: Lastzeile dedup-sicher in die Queue (kein erfundener Wert).
        const lrow = O.repos.trainingLoad.toRow(s.local_date, s.sport || 'Gym', { dur: durationMin, rpe: patch.session_rpe, source: 'workout', client_session_id: loadKey });
    ...
    clearLocal();
    return res(true, { session: O.workout.session, loadStatus: loadStatus }, null, r.source, r.sync_status);
  }

  // mode: 'later' (aktiv lassen) | 'aborted' (als abgebrochen) | 'delete' (löschen)
  async function cancelWorkout(mode, reason) {
    const s = O.workout.session; if (!s) return res(true, { skipped: true }, null, 'empty', 'synced');
    if (mode === 'later') { saveLocal(); return res(true, { kept: true }, null, 'indexeddb', 'synced'); }
    if (mode === 'delete') {
      let r = (online() && s.id) ? await O.repos.workout.deleteSession(s.id) : await offlineDelete('workout_sessions', { clientId: s.client_session_id, recordId: s.id });
      O.workout.session = null; O.workout.exercises = []; clearLocal(); return res(r.success, { deleted: true }, r.error, r.source, r.sync_status);
    }
    // aborted
    const patch = { status: 'aborted', finished_at: new Date().toISOString(), cancel_reason: reason || null };
    let r = (online() && s.id) ? await O.repos.workout.updateSession(s.id, patch) : await offlineUpsert('workout_sessions', Object.assign(buildSessionRow({ localDate: s.local_date, clientSessionId: s.client_session_id, status: 'aborted' }), patch), 'user_id,client_session_id', { clientId: s.client_session_id });
    O.workout.session = null; O.workout.exercises = []; clearLocal();
    return res(r.success, { aborted: true }, r.error, r.source, r.sync_status);
  }
