...
(function () {
  ...
  async function finishWorkout(summary) {
    ...
    if (online() && s.id) {
      ...
      durationMin = startedAt ? Math.max(0, Math.round((Date.now() - startedAt.getTime() - pausedSec * 1000) / 60000)) : (summary.durationMin != null ? summary.durationMin : null);
      const q = await offlineTerminal(s, 'completed', { finished_at: new Date().toISOString(), duration_min: durationMin, total_paused_seconds: Math.round(pausedSec), paused_at: null, session_rpe: rpe });
    ...
    clearSessionState();
    return res(true, { completed: true, loadStatus: loadStatus, durationMin: durationMin, sessionId: s.id, sport: sport, sessionRpe: rpe, clientSessionId: s.client_session_id }, null, (online() && s.id) ? 'supabase' : 'indexeddb', (online() && s.id) ? 'synced' : 'pending');
  }
  ...
  async function cancelWorkout(mode, reason) {
    ...
    if (mode === 'delete') {
      let r;
      if (online() && s.id) r = await O.repos.workout.deleteSession(s.id);   // Repo verifiziert: no_row_deleted
    ...
    if (online() && s.id) {
      ...
      const q = await offlineTerminal(s, 'aborted', { finished_at: new Date().toISOString(), cancel_reason: reason || null });
      if (!q.success) return q;
    }
    clearSessionState();
