...
function _perfSeedFromCanonical(perf){try{var b=perf.body;
  function seed(metric,val,unit){if(metric.value==null&&val!=null){metric.value=val;metric.unit=unit;metric.source=metric.source||'manual';}}
...
function _perfMirrorCanonical(perf){var M=_perfM();var p={};
  p.weightKg=M.currentWeightKg(perf);            // aktuellstes Gewicht (Verlauf > Körperdaten)
  p.heightCm=perf.body.height.value;
...
function _perfSave(){var M=_perfM();PROFILE.performance=M.normalizePerformance(window._perfEd.perf,PROFILE.body);
  Object.assign(PROFILE,_perfMirrorCanonical(PROFILE.performance));   // P2: kanonische flache Felder folgen dem Editor
