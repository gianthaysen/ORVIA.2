...
function openSportProfileEditor(sportId){var M=pmModel();var sc=M.sportProfileSchema(sportId);if(!sc)return;
  ...
  _modal('_sppEdM','<div id="sppBody"></div>');renderSportProfileEditor();}
function renderSportProfileEditor(){var M=pmModel();var E=window._sppEd;var sc=M.sportProfileSchema(E.sportId);var sp=E.sp;var box=document.getElementById('sppBody');if(!box)return;
  var html='<h3>'+escH(sc.label+' — Profil')+'</h3>';
  // Rolle & Niveau
  ...
  html+='<button class="btn" onclick="saveSportProfileEditor()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="cancelSportProfileEditor()">Abbrechen</button>';
  box.innerHTML=html;}
...
function cancelSportProfileEditor(){var E=window._sppEd;_sppCollectInto(E.sp);
  if(pmModel().diffState(JSON.parse(E.orig),{role:E.role,sp:E.sp})){
    _modal('_sppDiscard','<h3>Änderungen verwerfen?</h3><p class="modtext" style="margin:0 0 14px">Deine letzten Anpassungen wurden noch nicht gespeichert.</p>'+
      ...
      '<button class="btn danger-btn" style="margin-top:10px" onclick="_closeM(\'_sppDiscard\');_closeM(\'_sppEd\')">Änderungen verwerfen</button>');
...
function openAvailabilityEditor(){var M=pmModel();var av=M.normalizeAvailability(PROFILE.availability);
  ...
  _modal('_secEdM','<h3>Trainingsverfügbarkeit</h3><div id="avBody"></div>');renderAvailabilityEditor();}
...
function renderAvailabilityEditor(){var M=pmModel();var E=window._avEd;var av=E.av;var box=document.getElementById('avBody');if(!box)return;var sports=_avSports();
  ...
  box.innerHTML=cards+
    ...
    '<div class="gm-modal-actions"><button class="btn" onclick="saveAvailabilityEditor()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="cancelAvailabilityEditor()">Abbrechen</button></div>';}
...
function openPerformanceManager(){var M=_perfM();window._perfEd={orig:JSON.stringify(M.normalizePerformance(PROFILE.performance,PROFILE.body)),perf:M.normalizePerformance(PROFILE.performance,PROFILE.body)};
  _modal('_perfMgr','<div id="perfBody"></div>');renderPerformanceManager();}
...
function renderPerformanceManager(){var M=_perfM();var perf=window._perfEd.perf;var box=document.getElementById('perfBody');if(!box)return;
  ...
  box.innerHTML='<h3>Körper und Leistung</h3>'+
    ...
    legacy+
    '<div class="gm-modal-actions"><button class="btn sec" onclick="_closeM(\'_perfMgr\')">Schließen</button></div>';}
...
function openBodyEditor(){var perf=window._perfEd.perf;var b=perf.body;
  ...
  _modal('_perfSub','<h3>Körperdaten</h3>'+f('height','Größe','cm')+f('weight','Gewicht','kg')+f('bodyFat','Körperfett','%')+f('leanMass','Fettfreie Masse','kg')+f('waist','Taillenumfang','cm')+f('restingHr','Ruhepuls','bpm')+f('maxHr','Maximale Herzfrequenz','bpm')+
    '<span class="ob2-err" id="bd_err" role="alert"></span><div class="gm-modal-actions"><button class="btn" onclick="saveBodyEditor()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="_closeM(\'_perfSub\')">Abbrechen</button></div>');}
...
function openWeightAdd(){_modal('_perfSub','<h3>Gewichtsmessung</h3><div class="gm-field"><label>Gewicht (kg)</label><input type="number" id="wa_val"></div><div class="gm-field"><label>Datum</label><input type="date" id="wa_date" value="'+_today()+'"></div><span class="ob2-err" id="wa_err" role="alert"></span><div class="gm-modal-actions"><button class="btn" onclick="saveWeightAdd()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="_closeM(\'_perfSub\')">Abbrechen</button></div>');}
...
function openEnduranceEditor(){var M=_perfM();var perf=window._perfEd.perf;var sports=(window.ORVIA&&ORVIA.profile&&ORVIA.profile.activeSports)?ORVIA.profile.activeSports():[];
  _modal('_perfSub','<h3>Ausdauerwerte</h3>'+
    '<div class="gm-field"><label>VO₂max</label><input type="number" id="en_vo2" value="'+(perf.vo2max.value!=null?perf.vo2max.value:'')+'"></div>'+
    ...
    '<span class="ob2-err" id="en_err" role="alert"></span><div class="gm-modal-actions"><button class="btn" onclick="saveEnduranceEditor()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="_closeM(\'_perfSub\')">Abbrechen</button></div>');}
...
function openPbEditor(id){var M=_perfM();var perf=window._perfEd.perf;var b=id?perf.personalBests.filter(function(x){return x.id===id;})[0]:null;
  ...
  _modal('_perfSub','<h3>'+(b?'Bestzeit bearbeiten':'Bestzeit hinzufügen')+'</h3>'+
    '<div class="gm-field"><label>Sportart</label>'+segHTML('pb_sport',sports.map(function(s){return [s.sportId,s.customName||_sportLabel(s)];}),(b&&b.sportId)||(sports[0]&&sports[0].sportId)||'')+'</div>'+
    ...
    '<div class="gm-modal-actions"><button class="btn" onclick="savePbEditor(\''+(id||'')+'\')">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="_closeM(\'_perfSub\')">Abbrechen</button></div>');}
...
function openSrEditor(id){var M=_perfM();var perf=window._perfEd.perf;var r=id?perf.strengthRecords.filter(function(x){return x.id===id;})[0]:null;
  _modal('_perfSub','<h3>'+(r?'Kraftwert bearbeiten':'Kraftwert hinzufügen')+'</h3>'+
    '<div class="gm-field"><label>Übung</label><input id="sr_ex" value="'+escH(r?r.exerciseName:'')+'"><span class="ob2-err" id="sr_err" role="alert"></span></div>'+
    ...
    '<div class="gm-modal-actions"><button class="btn" onclick="saveSrEditor(\''+(id||'')+'\')">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="_closeM(\'_perfSub\')">Abbrechen</button></div>');}
...
function openRecoveryEditor(){var M=pmModel();var r=M.normalizeRecovery(PROFILE.recovery,PROFILE.recovery);
  ...
  _modal('_recEdM','<h3>Regeneration und Alltag</h3>'+
    '<p class="note" style="text-align:left">ORVIA nutzt Schlaf- und Stressdaten später zur Einschätzung von Erholung und Belastbarkeit.</p>'+
    ...
    '<div class="gm-modal-actions"><button class="btn" onclick="saveRecoveryEditor()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="cancelRecoveryEditor()">Abbrechen</button></div>');}
...
function openPreferencesEditor(){var M=pmModel();var p=M.normalizePreferences(PROFILE.preferences||PROFILE.trainingPrefs,PROFILE.trainingPrefs);
  ...
  _modal('_prefEdM','<h3>Trainingspräferenzen</h3>'+
    ...
    '<div class="gm-modal-actions"><button class="btn" onclick="savePreferencesEditor()">Speichern</button><button class="btn sec" style="margin-top:10px" onclick="cancelPreferencesEditor()">Abbrechen</button></div>');}
...
function cancelPreferencesEditor(){_prefCollect();if(pmModel().diffState(JSON.parse(window._prefEd.orig),window._prefEd.p)){
  _modal('_secDiscard','<h3>Änderungen verwerfen?</h3><p class="modtext" style="margin:0 0 14px">Deine letzten Anpassungen wurden noch nicht gespeichert.</p><button class="btn sec" onclick="_closeM(\'_secDiscard\')">Weiter bearbeiten</button><button class="btn danger-btn" style="margin-top:10px" onclick="_closeM(\'_secDiscard\');_closeM(\'_prefEd\')">Änderungen verwerfen</button>');
