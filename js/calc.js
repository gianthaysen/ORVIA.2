...
/* Differenzierter Status. NIEMALS „überlastet" allein aus einer Satzgrenze.
   opts: {days, priority, historyWeeks, baselineWeekly, hasComplaint, color je Zustand}.
...
function muscleVolumeStatus(weekly, opts){
  ...
  if(opts.historyWeeks!=null&&opts.historyWeeks<2)return R('insufficient','Zu wenig Historie');
...
const Calc={HM_KM,RACE_DATE,avg,median,sd,clampC,fmtPace,fmtTime,fmtDuration,paceZones,bmr,nutritionTargets,ewma,sessionLoad,acwr,
  weekKmTarget,effectiveKmTarget,runnaWeek,planStatus,activityDuplicate,racePhases,buildIntervals,swimPace100,aggregateMuscleVolume,muscleVolumeStatus,muscleWeeklyEquivalent,muscleTargetRange,racePhase,trendDir,readiness,ampel,hrvScoreOf,riegelHM,goalEngine,
