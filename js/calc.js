...
function loadModel(loads){
  ...
  const a=ewma(loads,7),c=ewma(loads,42);
  const atl=a[a.length-1],ctl=c[c.length-1];
  ...
  return {atl:atlR,ctl:ctlR,tsb:ctlR-atlR,
    ...
    acwr:ac.ratio,acwrReliable:!!ac.enough,dataDays:loads.filter(x=>x>0).length};
}
