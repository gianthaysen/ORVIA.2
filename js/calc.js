...
function safetyCheck(c){
  c=c||{};var flags=[];
  // Feld-Aliasse: akzeptiert sowohl Kurz- als auch ausführliche Namen (UI-kompatibel).
  ...
  if(c.fever)flags.push('Fieber');
  if(c.severePain||(c.pain!=null&&c.pain>6))flags.push('starke Schmerzen');
  if(c.swelling)flags.push('akute Schwellung');
  if(c.instability)flags.push('Instabilitätsgefühl');
  if(c.chestPain)flags.push('Brustschmerz');
  if(breath)flags.push('Atemnot');
  if(c.dizziness)flags.push('Schwindel/Ohnmacht');
  if(neuro)flags.push('neurologische Symptome');
  if(trauma)flags.push('Schmerz nach Unfall');
  var critical=!!(c.chestPain||breath||c.dizziness||neuro||trauma);
  var advice=flags.length?('ORVIA kann keine Diagnose stellen. Aufgrund deiner Angaben wird heute keine intensive Einheit empfohlen.'+((critical||c.severePain)?' Bei starken, plötzlichen oder anhaltenden Beschwerden bitte fachlich abklären lassen.':'')):'';
  return {triggered:flags.length>0,critical:critical,flags:flags,advice:advice};
}
...
function buildTrainingDecision(input){
  ...
  var ds=dayStateEngine({pain:c.pain,region:c.region,illness:c.illness,doms:c.doms,
    sleepH:c.sleepH,sleepQ:c.sleepQ,feel:c.feel,motivation:c.motivation,stress:c.stress,
    hrv:c.hrv,readiness:c.readiness,load3:L.load3,load7:L.load7,load14:L.load14});
  var state=safety.triggered?'RED':ds.state;
  // Fester Termin (Spiel/Wettkampf) in 0–2 Tagen?
  var matchConflict=null,ti=(i.todayIndex!=null?i.todayIndex:0);
  ...
  var sess=adaptSessionPlan(Object.assign({},planned||{},{kind:tt.type}),{state:state,flags:ds.flags},{region:c.region,doms:c.doms});
  // Spiel/Wettkampf-Schutz: harte/Beinlast nicht 24–48 h davor
  ...
  // Safety hat Vorrang
  if(safety.triggered){sess={action:(safety.critical||c.severePain)?'REST':'REPLACE_WITH_RECOVERY',
    label:(safety.critical||c.severePain)?'Trainingspause':'Sehr leichte Bewegung / Pause',detail:safety.advice,reason:safety.flags.join(', ')};}
  var riskFlags={
    kneeRisk:(c.pain!=null&&c.pain>=4&&/knie|knee|schienbein|shin|achill/i.test(c.region||'')),
    illnessRisk:!!c.illness,
    overloadRisk:!!ds.flags.loadSpike,
    domsRisk:!!ds.flags.domsHigh,
    matchConflictRisk:!!matchConflict,
    safetyRisk:safety.triggered
  };
  ...
  if(i.weekPlan&&i.todayIndex!=null){
    var wk=adaptWeekPlan(i.weekPlan.map(function(day){return (day||[]).map(function(it){return Object.assign({},it,{kind:classifyTrainingType(it).type});});}),
      {todayIndex:ti,state:state,illness:ds.flags.illness,fixedEvents:i.fixedEvents});
    weekAdjustments=wk.changes;weekPlanAdjusted=wk.plan;
    // Ersatz-Einheit SICHTBAR im heutigen Plan-Slot (statt leer) — markiert als adaptiveReplacement.
    if(weekPlanAdjusted&&sess.action!=='KEEP'&&planned){
      ...
      weekPlanAdjusted[ti]=[{
        t:repType,l:'Ersatz für '+(planned.l||'Einheit')+': '+sess.label,d:sess.detail||'',
        adaptiveReplacement:true,actionType:sess.action,reason:sess.reason,
        originalSession:origItem,source:'adaptive_engine'
      }];
    }
  }
  var reasons=ds.reasons.slice();
  if(matchConflict)reasons.unshift('Fester Termin in '+matchConflict.days+' Tag(en)');
  if(safety.triggered)reasons=safety.flags.concat(reasons);
  ...
  var coachSummary='state='+state+'|action='+sess.action+'|risks='+(Object.keys(riskFlags).filter(function(k){return riskFlags[k];}).join(',')||'-')+'|reasons='+reasons.slice(0,4).join(',');
  var dq=i.dataQuality||{};
  return {
    dayState:state,readinessReasons:reasons,riskFlags:riskFlags,
    todayAction:sess.action,
...
const Calc={HM_KM,HR_MAX,RHR_FALLBACK,RACE_DATE,avg,median,sd,clampC,fmtPace,fmtTime,fmtDuration,paceZones,bmr,nutritionTargets,ewma,sessionLoad,acwr,
