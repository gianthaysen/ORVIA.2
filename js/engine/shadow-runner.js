...
(function (root) {
  ...
  /* v1-Welt → v2-Input: delegiert an den TrainingInputResolver (Phase 8,
     EINE Input-Logik). Ohne Resolver (Ladefehler) ehrlich leerer Input mit
     missing-Flag — es wird nichts dupliziert und nichts erfunden. */
  function buildInput() {
    if (O.trainingInputResolver && typeof O.trainingInputResolver.collect === 'function') {
      return O.trainingInputResolver.collect();
    }
    return {
      readiness: { score: null, confidence: 'low', warnings: [], missingData: [] },
      safetyFlags: {},
      illness: false,
      constraints: [],
      plannedSession: null,
      recentLoad: { acute7: null, chronic28PerWeek: null, dataDays: 0, hardYesterday: false, hardStreak: 0 },
      goalContext: { daysToEvent: null },
      availabilityToday: null,
      _shadowMissing: ['training_input_resolver_missing']
    };
  }
