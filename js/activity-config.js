...
(function (root) {
  ...
  function canonSportId(sportId) {
    var c = SL() && SL().CATALOG_BY_ID; if (!c) return null;
    var low = String(sportId == null ? '' : sportId).trim().toLowerCase();
    if (c[low]) return low;                 // direkte kanonische ID (rowing, hiking, walking, basketball, ...)
    var n = normSport(sportId);             // deutsche Legacy-Labels → kanonisch
    // normSport rät bei Unbekanntem auf 'athletics' — diesen Rate-Fallback NICHT übernehmen,
    // außer die Eingabe meint wirklich Leichtathletik.
    if (n && c[n] && (n !== 'athletics' || /athlet|leichtathletik/.test(low))) return n;
    return null;
  }
  function sportLabel(sportId) { var c = SL() && SL().CATALOG_BY_ID; var k = canonSportId(sportId); if (c && k && c[k]) return c[k].label; try { console.warn('[ORVIA activity] kein Label für Sport-ID', sportId); } catch (e) {} return 'Aktivität'; }
  function sportIcon(sportId) { var c = SL() && SL().CATALOG_BY_ID; var k = canonSportId(sportId); return (c && k && c[k] && c[k].icon) || 'pulse'; }
