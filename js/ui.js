...
function generateWeekPlan(){
  ...
  var w=[[],[],[],[],[],[],[]];
  if(triGoal||(swim&&bike&&run)){
    w[0]=[gpS('Technik','~900 m')];w[1]=[gpR('Intervalle','iv')];w[2]=[gpB('Easy Z2','75 min')];
    w[3]=[gpS('Ausdauer','~1200 m')].concat(gym?[gpG('Ganzkörper')]:[]);w[4]=[gpR('Z2 Dauerlauf','ez')];
    w[5]=[gpB('Long Ride','2–3 h'),gpR('Koppellauf','ez')];w[6]=[gpR('Long Run','lr')];
  }else if(runGoal||(run&&!bike&&!swim&&!(gym&&!run))){
    if(lvl==='anfaenger'){w[1]=[gpR('Z2 Dauerlauf','ez')];w[3]=[gym?gpG('Ganzkörper'):gpM()];w[5]=[gpR('Z2 Dauerlauf','ez')];w[6]=[gpR('Long Run','lr')];}
    else{w[0]=[gym?gpG('Oberkörper'):gpM()];w[2]=[gpR('Intervalle','iv')].concat(gym?[gpG('Ganzkörper')]:[]);w[3]=[gym?gpG('Oberkörper'):gpM()];w[4]=[gpR('Z2 Dauerlauf','ez')];w[5]=bike?[gpB('Easy Z2','60 min')]:[];w[6]=[gpR('Long Run','lr')];if(lvl==='profi')w[1]=[gpR('Tempo','tempo')];}
...
function renderConfidence(){
  ...
  el.innerHTML='<div class="conf conf-'+c.level.c+'"><span class="conf-dot"></span><span class="conf-t"><b>Empfehlungssicherheit: '+c.level.l+'</b> · '+esc(c.msg)+'</span></div>';
}
...
function renderAdaptCard(){
  ...
  if(mode==='automatic'&&!ch){var ea=(typeof entry==='function')?entry(todayStr()):e;if(ea){ea.adaptChoice={tier:a.tier,choice:'accepted'};if(typeof save==='function')save();}ch='accepted';}
  var headTxt=mode==='manual'?'ORVIA-Hinweis · du entscheidest':mode==='automatic'?'Automatisch angepasst':'ORVIA passt heute an';
  var head='<div class="adp-head">'+ic('pulse')+'<span>'+esc(headTxt)+'</span></div>';
  ...
  var actions;
  if(accepted){actions='<div class="adp-accepted">✓ '+(mode==='automatic'?'Automatisch angepasst':'Übernommen')+' — logge die angepasste Einheit. <button class="linklike" onclick="adaptReopen()">'+(mode==='automatic'?'Rückgängig':'ändern')+'</button></div>';}
  else{
...
function autoMorning(){if(!document.getElementById('m_sleep')&&!document.getElementById('m_qfeel'))return;entry(cur).morning=gatherMorning();if(typeof gatherCheckinIssues==='function')gatherCheckinIssues();save();renderDecision();}
...
function renderWeekPlan(){
  ...
  let html='';for(let i=0;i<7;i++){const d=new Date(mon);d.setDate(mon.getDate()+i);const k=todayStr(d);const e=DB[k];const isToday=k===todayStr();
    const items=activeWeekPlan()[i].map((it,idx)=>{const done=e&&e.sessions&&e.sessions[it.t];const det=pd[it.d]||it.d;
      ...
      return `<div class="pitem clk" onclick="openUnit(${i},${idx})"><span class="pic">${TYPES[it.t].ic}</span><span class="pitem-main"><span class="pl">${lbl}</span> <span class="pdt">· ${det}</span></span>${pri?'<span class="ppri ppri-'+pri+'">'+pri+'</span>':''}${done?'<span class="pchk">✓</span>':'<span class="pchev">›</span>'}</div>`;}).join('');
