...
function renderCommand(){
  ...
  el.innerHTML=`<div class="occ ${a.c}">
    ...
    <div class="occ-subs">${subs}</div>
    <div class="occ-why"><span class="occ-whyh">Warum?</span><ul>${why.map(w=>'<li>'+esc(w)+'</li>').join('')}</ul></div>
  </div>`;
}
...
function gatherMorning(){return{sleepMin:+v('m_sleep'),sleepQ:+v('m_sleepQ'),
  ...
  knee:(document.getElementById('m_knee')?+v('m_knee'):null),feel:+v('m_feel'),legs:+v('m_legs'),ankle:+v('m_ankle'),doms:+v('m_doms'),ts:Date.now()};}
...
function savePost(silent){const e=entry(cur);e.sessions=e.sessions||{};
  ...
  if(!silent){var praise=(activeTypes.has('Laufen')&&typeof trainingPraise==='function')?trainingPraise():null;
    toast(praise||(activeTypes.size?activeTypes.size+' Einheit(en) gespeichert ✓':'Keine Einheit gewählt'));}}
function autoPost(){savePost(true);}
/* Motivierendes Feedback beim Logging — vergleicht den Lauf mit dem letzten ähnlichen */
function trainingPraise(){
  var e=DB[cur];if(!e||!e.sessions||!e.sessions.Laufen)return null;var L=e.sessions.Laufen;
  if(L.sub==='Wettkampf')return 'Wettkampf gespeichert — stark, dass du dich gestellt hast!';
  if(!L.dist||!L.dur)return null;var pace=L.dur*60/L.dist;
  ...
  for(var i=0;i<days.length;i++){var pe=DB[days[i]];if(pe&&pe.sessions&&pe.sessions.Laufen){var P=pe.sessions.Laufen;
    if(P.dist&&P.dur&&Math.abs(P.dist-L.dist)/L.dist<=0.3){var diff=Math.round(P.dur*60/P.dist-pace);
      if(diff>=8)return 'Stark — '+diff+' s/km schneller als beim letzten ähnlichen Lauf!';
      if(diff>=3)return 'Sauber — etwas schneller als zuletzt bei ähnlicher Distanz.';
      if(diff<=-8)return 'Bewusst lockerer als letztes Mal — gut für die Erholung.';
      return 'Solide Einheit — dranbleiben.';}}}
  return 'Gespeichert — ab jetzt vergleicht ORVIA deine Läufe.';
}
...
function renderWeekPlan(){
  ...
  let html='';for(let i=0;i<7;i++){const d=new Date(mon);d.setDate(mon.getDate()+i);const k=todayStr(d);const e=DB[k];const isToday=k===todayStr();
    const items=WEEKPLAN[i].map((it,idx)=>{const done=e&&e.sessions&&e.sessions[it.t];const det=pd[it.d]||it.d;
      return `<div class="pitem clk" onclick="openUnit(${i},${idx})"><span class="pic">${TYPES[it.t].ic}</span><span class="pitem-main"><span class="pl">${it.l}</span> <span class="pdt">· ${det}</span></span>${done?'<span class="pchk">✓</span>':'<span class="pchev">›</span>'}</div>`;}).join('');
...
function unitStruct(item,kind){
  var S={
    interval:{goal:'VO2max & Schnelligkeit — Reize über der Schwelle.',warmup:'10–15 min locker + 3–4 Strides',main:'Harte Intervalle (z. B. 5–6 × 800 m oder 4–5 × 3 min) im Intervall-Tempo, dazwischen 2 min Trab-Pause',cooldown:'10 min auslaufen',rpe:'RPE 8–9',alt:'Bei Müdigkeit/Knie: Easy Z2 oder Bike Z2'},
    ...
    long:{goal:'Ausdauer & Long-Run-Ökonomie.',warmup:'locker starten',main:'Langer Dauerlauf im Long-Run-Tempo; in der Peak-Phase optional 2 × 10 min Race-Pace',cooldown:'letzte Minuten locker',rpe:'RPE 4–5',alt:'Bei Knie: kürzen oder durch Bike ersetzen'},
    tempo:{goal:'Laktatschwelle — kontrolliert hart.',warmup:'10 min locker',main:'20–30 min im Tempo/Schwellen-Bereich, gleichmäßig',cooldown:'10 min locker',rpe:'RPE 7',alt:'Bei Müdigkeit: in Easy umwandeln'},
...
function openUnit(di,ii){
  var row=WEEKPLAN[di];if(!row)return;var item=row[ii];if(!item)return;
  var kind=unitKind(item),s=unitStruct(item,kind);
  var stats=[];if(s.pace)stats.push(['Ziel-Pace',s.pace]);if(s.hf)stats.push(['HF-Zone',s.hf]);if(s.rpe)stats.push(['Intensität',s.rpe]);
  var statHTML=stats.length?'<div class="unit-stats">'+stats.map(function(x){return '<div class="unit-stat"><span class="us-k">'+escH(x[0])+'</span><span class="us-v">'+escH(x[1])+'</span></div>';}).join('')+'</div>':'';
  var steps='';
  ...
  var body='<div class="unit-goal">'+escH(s.goal)+'</div>'+statHTML+
    (steps?'<div class="unit-steps">'+steps+'</div>':'')+
    '<div class="unit-rec"><b>Heute für dich</b><br>'+escH(unitGuidance(item,kind))+'</div>'+
    (s.alt?'<div class="unit-alt"><b>Alternative:</b> '+escH(s.alt)+'</div>':'');
...
function setTopTitle(name){const el=document.getElementById('topTitle');if(!el)return;
  if(name==='heute'){try{el.textContent=relDayTitle(cur)+' · '+new Date(cur+'T12:00').toLocaleDateString('de-DE',{day:'numeric',month:'long'});}catch(e){el.textContent=relDayTitle(cur);}}
  else el.textContent=TAB_TITLES[name]||'';}
