...
function renderDay(){const hr=new Date().getHours();const today=cur===todayStr();
  ...
  renderMotivation();renderBanners();renderCommand();renderMorning();
  if(today){var _ro=document.getElementById('readyOut');if(_ro)_ro.innerHTML='';var _ao=document.getElementById('ampelOut');if(_ao)_ao.innerHTML='';}
  else{renderReadiness();renderAmpel();}
...
function renderPlan(){flushAuto();renderRaceHeader();if(typeof renderRaceMode==='function')renderRaceMode();renderPaceZones();
  ...
  if(g.type==='halfmarathon'){
    ...
    document.getElementById('cdPlan').innerHTML=cdHTML();renderPhases();renderRamp();renderWeekPlan();renderGoals();renderPace();
  }else{
...
function setTopTitle(name){const el=document.getElementById('topTitle');if(!el)return;
  if(name==='heute'){try{el.textContent='Heute · '+new Date(cur+'T12:00').toLocaleDateString('de-DE',{day:'numeric',month:'long'});}catch(e){el.textContent='Heute';}}
