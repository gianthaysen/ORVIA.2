...
function _mvCtx(d,group){
  ...
  return {days:d.days,priority:priority,baselineWeekly:bl,historyWeeks:(bl!=null&&bl>0)?2:1,targetLow:t.low,targetHigh:t.high};
}
...
function showMuscleDetail(group){
  ...
  var interp=s.status.key==='in_target'?'Im Orientierungsbereich.':s.status.key==='below_target'?'Unter dem Orientierungsbereich — ggf. 1–2 Sätze/Woche ergänzen.':s.status.key==='above_target'?'Über dem Orientierungsbereich — beobachten, kein zwingender Handlungsbedarf.':s.status.key==='large_increase'?'Deutlicher Anstieg gegenüber der Vorperiode — Steigerung moderat halten.':s.status.key==='insufficient'?'Noch zu wenig Historie für eine Einordnung.':s.status.key==='not_prioritized'?'Im aktuellen Plan kein Schwerpunkt.':'Noch keine belastbare Beurteilung.';
...
function renderMuscleVolume(){
  ...
  loadMuscleVolume(days).then(function(d){
    ...
    var src='';
    if(d.source==='legacy'){var why=d.fallbackReason==='rpc_missing'?'Live-Auswertung ist noch nicht verfügbar. Vorübergehend werden lokale Daten verwendet.':'Quelle: bisheriges lokales Trainingsprotokoll (7 Tage).';src='<div class="mv-note">'+escH(why)+'</div>';}
