...
var EXERCISES=[
 ...
 {n:'Rudern',g:'Pull (Oberkörper)',m:'oberer Rücken'},
...
function gymExSummary(arr){if(!arr||!arr.length)return '';return '<div class="gx-sum">'+arr.map(function(n){return '<span class="gx-chip">'+esc(n)+'</span>';}).join('')+'</div>';}
function openGymPicker(){
  var cur=((document.getElementById('g_ex')||{}).value||'').split('|').filter(Boolean);
  var groups={};EXERCISES.forEach(function(x){(groups[x.g]=groups[x.g]||[]).push(x);});
  var body=Object.keys(groups).map(function(g){
    return '<div class="gxg"><div class="gxg-h">'+esc(g)+'</div>'+groups[g].map(function(x){
      var on=cur.indexOf(x.n)>=0;
      return '<button type="button" class="gx'+(on?' on':'')+'" data-n="'+esc(x.n)+'" onclick="this.classList.toggle(\'on\')"><span class="gx-n">'+esc(x.n)+'</span><span class="gx-m">'+esc(x.m)+'</span></button>';
    }).join('')+'</div>';
  }).join('');
  var wrap=document.createElement('div');wrap.className='orvia-modal-bg';
  wrap.innerHTML='<div class="orvia-modal goal-modal gym-pick"><h3>Übungen wählen</h3><div class="gx-scroll">'+body+'</div>'+
...
function applyGymPicker(){
  var sel=[];if(window._gymPick){var ns=window._gymPick.querySelectorAll('.gx.on');for(var i=0;i<ns.length;i++)sel.push(ns[i].dataset.n);}
  var inp=document.getElementById('g_ex');if(inp)inp.value=sel.join('|');
  var btn=document.getElementById('g_ex_btn');if(btn)btn.textContent=sel.length?sel.length+' Übungen gewählt':'Übungen auswählen';
  var list=document.getElementById('g_ex_list');if(list)list.innerHTML=gymExSummary(sel);
...
function blockGym(d){return `<div class="sescard"><div class="seshead">${ic('dumbbell')}Gym</div>
  ...
  <div class="field"><label>Übungen</label>
    <input type="hidden" id="g_ex" value="${esc((d.exercises||[]).join('|'))}">
    <button type="button" class="btn sec gym-ex-btn" id="g_ex_btn" onclick="openGymPicker()">${(d.exercises&&d.exercises.length)?(d.exercises.length+' Übungen gewählt'):'Übungen auswählen'}</button>
    <div class="gym-ex-list" id="g_ex_list">${gymExSummary(d.exercises||[])}</div></div>
...
function savePost(silent){const e=entry(cur);e.sessions=e.sessions||{};
  ...
  if(activeTypes.has('Laufen'))e.sessions.Laufen={sub:chipGet('l_sub')[0]||'',dist:numIn('l_dist',...LIM.runKm),dur:durMin('l_dur',LIM.runMin[0],LIM.runMin[1]),cad:numIn('l_cad',100,260),hr:numIn('l_hr',...LIM.hr),elev:numIn('l_elev',...LIM.elev),hrmin:numIn('l_hrmin',...LIM.hr),hrmax:numIn('l_hrmax',...LIM.hr),rpe:+v('l_rpe'),perf:+v('l_perf'),knee:+v('l_knee'),note:v('l_note')};
  if(activeTypes.has('Gym'))e.sessions.Gym={sub:chipGet('g_sub'),dur:numIn('g_dur',...LIM.gymMin),sets:numIn('g_sets',...LIM.sets),exercises:(v('g_ex')||'').split('|').filter(Boolean),rpe:+v('g_rpe'),perf:+v('g_perf'),knee:+v('g_knee'),note:v('g_note')};
  ...
  if(!silent){
    var fb=(activeTypes.has('Laufen')&&typeof trainingFeedback==='function')?trainingFeedback():null;
