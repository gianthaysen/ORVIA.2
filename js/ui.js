...
function _storeRunsByDay(){
  try{
    ...
    st.listActivities({sportId:'running'}).forEach(function(a){
      ...
      var dt=new Date(a.startedAt);if(isNaN(dt.getTime()))return;
      var k=todayStr(dt);                                  // lokaler Kalendertag
      if(map[k])return;                                    // neueste zuerst; 1-Lauf/Tag-Modell
      var s=a.summary||{};
      var distKm=s.distance_m!=null?s.distance_m/1000:(s.distanceKm!=null?s.distanceKm:null);
      var durMin=a.durationSeconds!=null?a.durationSeconds/60:null;
      if(!(distKm>0&&durMin>0))return;
      map[k]={sub:'',dist:Math.round(distKm*100)/100,dur:Math.round(durMin),
        hr:s.avg_hr!=null?Math.round(s.avg_hr):null,elev:s.elevation_gain_m!=null?Math.round(s.elevation_gain_m):null,source:'sync'};
    });
    return map;
  }catch(e){return {};}
}
...
function weekRunKm(off){const now=new Date();const day=(now.getDay()+6)%7;const mon=new Date(now);mon.setDate(now.getDate()-day-7*(off||0));
  const ext=_storeRunsByDay();
  let km=0;for(let i=0;i<7;i++){const d=new Date(mon);d.setDate(mon.getDate()+i);const k=todayStr(d);const e=DB[k];const r=e&&e.sessions&&e.sessions.Laufen;if(r&&_validRun(r))km+=r.dist||0;else if(ext[k])km+=ext[k].dist||0;}return km;}
function allLoads(){
...
function _ciAutoRow(f,a){return '<div class="field ci-auto"><label>'+esc(f.label)+'</label>'+
  '<div class="ci-auto-val"><strong>'+esc(a.text)+'</strong><span class="ci-auto-src">Automatisch von Garmin</span>'+
  '<button type="button" class="linklike" onclick="ciEditManually(\''+f.key+'\')">Bearbeiten</button></div></div>';}
...
function renderMorning(){
  ...
  var body=REG?_ciFormHTML(REG,m,mode):'<p class="note">Check-in-Modul nicht geladen.</p>';
...
function renderRoutines(){const e=entry(cur);const r=e.routines||{};
  ...
  if(chips)chips.innerHTML=activeRoutines().map(([k,lab])=>`<button type="button" class="chip gn${r[k]?' on':''}" onclick="toggleRoutine('${k}',this)">${lab}</button>`).join('');
...
function renderEve(){const e=(entry(cur).eve)||{};
  ...
  document.getElementById('eveForm').innerHTML=fields?_ciFormHTML(fields,e,'full'):'<p class="note">Check-in-Modul nicht geladen.</p>';
  initRanges();}
...
function renderBestTimes(){
  ...
  el.innerHTML='<div class="bt-grid">'+cell('1 km',b.t1,b.real.k1)+cell('5 km',b.t5,b.real.k5)+cell('10 km',b.t10,b.real.k10)+'</div>'+
    '<p class="note" style="text-align:left;margin-top:10px">'+(anyReal?'„Strava" = echte Bestleistung aus importierten Läufen. ':'')+(b.estPace?'„Geschätzt" = aus deinem schnellsten Lauf ('+fmtDe(b.estDist)+' km @ '+fmtPace(b.estPace)+'/km, Riegel-Modell).':'')+'</p>';
}
function renderDash(){
  flushAuto();
