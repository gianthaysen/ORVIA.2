...
function userLevel(){
  try{
    ...
    var prim=null;for(var i=0;i<sports.length;i++){var x=sports[i];if(x&&x.role==='primary'){prim=x;break;}}
    var kit=prim&&prim.level;
    if(kit==='beginner')return 'anfaenger';
    if(kit==='competitive')return 'profi';
    if(kit==='intermediate'||kit==='advanced')return 'fortgeschritten';
  }catch(e){}
  var l=(typeof PROFILE!=='undefined'&&PROFILE&&PROFILE.level)||'fortgeschritten';
...
function renderMuscleVolume(){
  ...
  }).catch(function(e){ if(reqId!==_mvReq)return; try{console.error('[muscleVolume]',e);}catch(_){ } out('<p class="muted" style="margin:0">Muskelvolumen konnte momentan nicht berechnet werden.</p><button class="btn sec" style="margin-top:10px" onclick="renderMuscleVolume()">Erneut versuchen</button>'); });
}
...
function goalOf(){
  ...
  try{
    ...
    var cand=gs.filter(function(g){return g&&g.status==='active'&&RACE_DIST[g.category];})
      .sort(function(a,b){return (a.priority||9)-(b.priority||9);})[0];
    if(cand){
      var tm=null;
      if(cand.metricType==='time'&&typeof cand.targetValue==='number')tm=cand.targetValue/60;
      else if(typeof cand.targetValue==='number'&&(!cand.unit||cand.unit==='min'))tm=cand.targetValue;
      return {type:cand.category,distanceKm:RACE_DIST[cand.category],raceDate:cand.targetDate||'',
...
function showTab(name){
  ...
  function _safe(fn){try{fn();}catch(e){try{console.error('[ORVIA showTab] Renderer-Fehler:',name,e&&e.message);}catch(_){}}}
