...
function orviaScore(){
  ...
  // --- Kopplung an die zentrale Engine: dayState + Safety bestimmen die Caps ---
  var pain=(m.knee!=null?m.knee:0),region='';
  if(e.issues){Object.keys(e.issues).forEach(function(k){var vv=e.issues[k];if(typeof vv==='number'&&vv>pain){pain=vv;region=(typeof ISSUE_LABELS!=='undefined'&&ISSUE_LABELS[k])||k;}});}
  if(!region&&pain>0)region='Knie';
  ...
  var sfIn={pain:pain,region:region,illness:!!m.ill,fever:m.fever,swelling:m.swelling,instability:m.instability,chestPain:m.chestPain,shortnessOfBreath:m.shortnessOfBreath,dizziness:m.dizziness,neurologicalSymptoms:m.neurologicalSymptoms,accidentPain:m.accidentPain};
  var safety=Calc.safetyCheck(sfIn);
  var sleepH=(m.sleepMin!=null?m.sleepMin/60:null);
  var ds=Calc.dayStateEngine({pain:pain,region:region,illness:!!m.ill,doms:m.doms,sleepH:sleepH,sleepQ:m.sleepQ,feel:m.feel,stress:m.stress,hrv:m.hrv,readiness:recovery,load3:load3,load7:load7});
  var dayState=safety.triggered?'RED':ds.state;
  // geplante Einheit für sport-spezifische Caps
  var u=(typeof todayPrimaryUnit==='function')?todayPrimaryUnit():null;
  var runPlanned=!!(u&&u.t==='Laufen');
  var legPlanned=!!(u&&((u.t==='Gym'&&/bein|ganzk/i.test(u.l||''))||u.t==='Laufen'));
  var impactRegion=/knie|knee|schienbein|shin|achill|fuß|fuss|sprung|ankle/i.test(region);
  var cap=100;
  if(m.ill)cap=Math.min(cap,60);
  if(safety.triggered)cap=Math.min(cap,40);
  if(m.sleepQ!=null&&m.sleepQ<=3)cap=Math.min(cap,72);
  if(m.feel!=null&&m.feel<=3)cap=Math.min(cap,70);
  if(pain>=5&&impactRegion&&runPlanned)cap=Math.min(cap,60);
  if((m.doms||0)>=7&&legPlanned)cap=Math.min(cap,65);
  if(m.hrv==='Low'&&sleepH!=null&&sleepH<6)cap=Math.min(cap,68);
  if(m.stress==='High'&&m.feel!=null&&m.feel<=5)cap=Math.min(cap,70);
  cap=Math.min(cap,{GREEN:100,YELLOW:79,ORANGE:64,RED:44}[dayState]);
  score=Math.min(score,cap);
  if((m.knee??0)>=6)score=Math.min(score,40);else if((m.knee??0)>=4)score=Math.min(score,62);
  score=Math.round(Calc.clampC(score,0,100));
  // Status-Text streng an dayState + Score gekoppelt — kein „Peak" bei Einschränkungen
  var peakOK=(score>=85&&dayState==='GREEN'&&!m.ill&&pain<3&&(m.doms||0)<5&&!safety.triggered);
  const status=peakOK?{l:'Peak',c:'g'}
    :dayState==='GREEN'?{l:'Bereit',c:'g'}
    :dayState==='YELLOW'?{l:'Reduzieren',c:'y'}
    :dayState==='ORANGE'?{l:'Anpassen',c:'o'}
    :{l:'Regeneration',c:'r'};
  const subs=[['Erholung',recovery],['Risiko',riskSub],['Last',loadSub],['Umsetzung',execSub],['Fortschritt',progSub]];
  return{score:score,status:status,subs:subs,r:r,ctx:ctx,m:m,recovery:recovery,dayState:dayState,safety:safety};
}
...
function renderConfidence(){
  ...
  el.innerHTML='<div class="cfd cfd-'+c.level.c+'"><span class="cfd-dot"></span><span class="cfd-t"><b>Empfehlungssicherheit: '+c.level.l+'</b> · '+esc(c.msg)+'</span></div>';
}
...
function applyWeekAdjustments(confirmed){
  ...
  PROFILE.weekPlan=d.weekPlanAdjusted.map(function(day){return (day||[]).map(function(it){var c=Object.assign({},it);delete c.kind;return c;});});
...
function revertWeekAdjustments(){
  ...
  try{var e=entry(todayStr());if(e._weekAdjBatch)delete e._weekAdjBatch;}catch(_){}
  if(typeof saveProfile==='function')saveProfile();if(typeof save==='function')save();
  if(typeof renderPlan==='function')renderPlan();if(typeof renderAdaptCard==='function')renderAdaptCard();
...
/* ---- ZENTRALE Tagesentscheidung für die UI — EINE Quelle: Calc.buildTrainingDecision ---- */
function currentDecision(){
  ...
  var os=(typeof orviaScore==='function')?orviaScore():null;var m=(os&&os.m)||e.morning||{};
  var region='',pain=(m.knee!=null?m.knee:0);
  if(e.issues){Object.keys(e.issues).forEach(function(k){var vv=e.issues[k];if(typeof vv==='number'&&vv>pain){pain=vv;region=(typeof ISSUE_LABELS!=='undefined'&&ISSUE_LABELS[k])||k;}});}
  if(!region&&pain>0)region='Knie';
  var load3=null,load7=null;try{var ld=allLoads().loads;if(ld&&ld.length>=7){load3=Calc.avg(ld.slice(-3));load7=Calc.avg(ld.slice(-7));}}catch(_){}
  ...
  return Calc.buildTrainingDecision({
    checkin:{pain:pain,region:region,illness:!!m.ill,doms:m.doms,sleepH:(m.sleepMin!=null?m.sleepMin/60:null),sleepQ:m.sleepQ,feel:m.feel,stress:m.stress,hrv:m.hrv,readiness:(os?os.recovery:null),
      // Safety-Felder (von der Engine akzeptiert; UI fragt sie noch nicht aktiv ab):
...
function renderAdaptCard(){
  ...
  var why=d.readinessReasons.length?'<div class="adp-why"><span class="adp-wh">Warum?</span> '+d.readinessReasons.slice(0,4).map(esc).join(' · ')+'</div>':'';
  var DN=['Mo','Di','Mi','Do','Fr','Sa','So'];
...
function autoMorning(){if(!document.getElementById('m_sleep')&&!document.getElementById('m_qfeel'))return;if(!canEditCur(true))return;entry(cur).morning=gatherMorning();if(typeof gatherCheckinIssues==='function')gatherCheckinIssues();save();renderDecision();}
...
function renderDecision(){renderCommand();if(typeof renderAdaptCard==='function')renderAdaptCard();var t=(cur===todayStr());var ro=document.getElementById('readyOut'),ao=document.getElementById('ampelOut');if(t){if(ro)ro.innerHTML='';if(ao)ao.innerHTML='';}else{renderReadiness();renderAmpel();}}
function saveMorning(){if(!canEditCur())return;entry(cur).morning=gatherMorning();if(typeof gatherCheckinIssues==='function')gatherCheckinIssues();if(window._correctionMode&&cur!==todayStr())logCorrection('morning');save();renderDecision();toast(window._correctionMode&&cur!==todayStr()?'Korrektur gespeichert ✓':'Gespeichert ✓');window.scrollTo({top:0,behavior:'smooth'});}
...
function volRate(s){if(s<=0)return null;if(s<6)return {l:'zu niedrig',c:'y'};if(s<=18)return {l:'passend',c:'g'};if(s<=22)return {l:'hoch',c:'o'};return {l:'zu hoch',c:'r'};}
function renderMuscleVolume(){
  var el=document.getElementById('muscleVolBox');if(!el)return;
  var card=el.parentElement;
  if(!hasGymData()){el.innerHTML='';if(card)card.style.display='none';return;}
  if(card)card.style.display='';
  var vol=muscleVolume();var any=MUSCLE_ORDER.some(function(m){return vol[m]>0;});
  if(!any){el.innerHTML='<p class="muted" style="margin:0">Logge Gym-Übungen mit Sätzen — dann zeigt ORVIA dein Wochenvolumen je Muskelgruppe.</p>';return;}
  var max=Math.max(22,Math.max.apply(null,MUSCLE_ORDER.map(function(m){return vol[m];})));
  var rows=MUSCLE_ORDER.map(function(m){var s=vol[m];var r=volRate(s);var pct=s>0?Math.min(100,Math.round(s/max*100)):0;
    return '<div class="mv-row"><span class="mv-m">'+m+'</span><div class="mv-bar-wrap"><i class="mv-bar mv-'+(r?r.c:'g')+'" style="width:'+pct+'%"></i></div><span class="mv-s">'+s+(r?' · '+r.l:'')+'</span></div>';
  }).join('');
  var over=MUSCLE_ORDER.filter(function(m){return vol[m]>22;});
  var low=MUSCLE_ORDER.filter(function(m){var r=volRate(vol[m]);return r&&r.l==='zu niedrig';});
  var note='';
  if(over.length)note='<div class="mv-note mv-note-r">Sehr hohes Volumen: '+esc(over.join(', '))+'. Deload erwägen — 1 Woche Sätze reduzieren, höhere RIR, keine Maximalversuche.</div>';
  else if(low.length)note='<div class="mv-note">Untertrainiert: '+esc(low.join(', '))+'. 1–2 Sätze/Woche ergänzen.</div>';
  el.innerHTML='<div class="mv">'+rows+'</div>'+note+'<p class="note" style="text-align:left;margin-top:8px">Sätze/Woche je Muskelgruppe (Richtwerte: &lt;6 niedrig · 10–18 passend · &gt;22 zu hoch).</p>';
}
...
function renderWeekPlan(){
  ...
  let html='';for(let i=0;i<7;i++){const d=new Date(mon);d.setDate(mon.getDate()+i);const k=todayStr(d);const e=DB[k];const isToday=k===todayStr();
    const items=activeWeekPlan()[i].map((it,idx)=>{const done=e&&e.sessions&&e.sessions[it.t];const det=pd[it.d]||it.d;
      ...
      const adaptBadge=it.adaptiveReplacement?'<span class="pl-adapt">angepasst</span> ':'';
      return `<div class="pitem clk${it.adaptiveReplacement?' pitem-adapt':''}" onclick="openUnit(${i},${idx})"><span class="pic">${(TYPES[it.t]||TYPES.Mobilität).ic}</span><span class="pitem-main"><span class="pl">${adaptBadge}${lbl}</span> <span class="pdt">· ${det}</span></span>${pri?'<span class="ppri ppri-'+pri+'">'+pri+'</span>':''}${done?'<span class="pchk">✓</span>':'<span class="pchev">›</span>'}</div>`;}).join('');
