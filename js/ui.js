...
// Muskel-Slug → ORVIA-Körperkartengruppe (deutsch). Nicht gelistete Slugs = neutrale Silhouette.
var SLUG_DE={chest:'Brust','front-deltoids':'Schultern','back-deltoids':'Schultern',biceps:'Bizeps',triceps:'Trizeps',
  ...
  quadriceps:'Quads',hamstring:'Hamstrings',gluteal:'Glutes',calves:'Waden','left-soleus':'Waden','right-soleus':'Waden'};
...
function renderBodyMap(statusByGroup){
  function fig(data,tx){
    ...
    Object.keys(data).forEach(function(slug){
      ...
      if(de){
        ...
        var fill=bmFill(st.color);
        out+='<g class="bm-region" tabindex="0" role="button" aria-label="'+escH(de+': '+st.label)+'" data-g="'+escH(de)+'" onclick="showMuscleDetail(\''+escH(de)+'\')" onkeydown="muscleKeyActivate(event,\''+escH(de)+'\')"><title>'+escH(de+': '+st.label)+'</title>'+
          polys.map(function(p){return '<polygon points="'+p+'" fill="'+fill+'" stroke="rgba(0,0,0,.45)" stroke-width="0.4"/>';}).join('')+'</g>';
      }else{
  ...
  return '<div class="bodymap"><svg viewBox="0 0 232 230" width="100%" role="img" aria-label="Körperkarte Muskelvolumen">'+
    fig(BODY_ANT,8)+fig(BODY_POST,128)+
    '<text x="58" y="228" text-anchor="middle" font-size="8" fill="var(--mut,#94a3b8)">Vorderseite</text>'+
    '<text x="178" y="228" text-anchor="middle" font-size="8" fill="var(--mut,#94a3b8)">Rückseite</text>'+
    '</svg>'+legend+'</div>';
}
