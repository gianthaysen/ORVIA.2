...
function showTab(name){
  ...
  if(name==='training'){try{if(typeof renderTypeGrid==='function')renderTypeGrid();if(typeof renderPostBlocks==='function')renderPostBlocks();if(typeof bindReps==='function')bindReps();}catch(e){}if(window.ORVIA&&window.ORVIA.workoutUI&&window.ORVIA.workoutUI.renderHub)window.ORVIA.workoutUI.renderHub();}
  // Zuletzt aktive Hauptseite merken (Profil/Overlays NICHT). Reload kehrt hierher zurück.
  try{if(['heute','plan','training','dash'].indexOf(name)>=0)localStorage.setItem('orvia_tab',name);}catch(e){}
  window.scrollTo(0,0);}
...
renderDay();
// Reload kehrt zur zuletzt aktiven Hauptseite zurück (Profil NIE automatisch öffnen).
try{var _lt=localStorage.getItem('orvia_tab');if(_lt&&['plan','training','dash'].indexOf(_lt)>=0)showTab(_lt);}catch(e){}
