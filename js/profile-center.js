...
(function (root) {
  ...
  function cardHTML(p, id, now) {
    ...
    return '<button type="button" class="pc-card" id="pc-card-' + esc(id) + '" aria-label="' + esc(SECTION_LABELS[id] || id) + ' öffnen">' +
      ...
      '<span class="pc-card-sub">' + esc(sectionSummary(p, id)) + '</span></span>' +
  ...
  function bindHandlers(now) {
    ...
    all.forEach(function (sid) {
      ...
      el.onclick = handler;
      try { el.addEventListener('click', function (ev) { if (ev && ev.preventDefault) ev.preventDefault(); handler(); }); } catch (e) {}
    });
    var p = P();
    buildSmartPrompts(p, now).forEach(function (x) {
      var el = doc.getElementById('pc-prompt-' + x.id);
      if (!el) return;
      var handler = function () { try { if (typeof root.openProfileSection === 'function') root.openProfileSection(x.sectionId); } catch (e) {} };
      el.onclick = handler;
      try { el.addEventListener('click', function (ev) { if (ev && ev.preventDefault) ev.preventDefault(); handler(); }); } catch (e) {}
    });
  }
  ...
  function open() {
    ...
    root.openSheet({
      ...
      onClose: function () { if (_unsub) { try { _unsub(); } catch (e) {} _unsub = null; } }
    });
    bindHandlers(now);
