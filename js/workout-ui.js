...
(function () {
  ...
  function statusRow(main, sub, kind) {
    return '<div class="wo-status' + (kind ? ' ' + kind : '') + '" role="button" tabindex="0" aria-label="Training öffnen" onclick="ORVIA.workoutUI.openTrainingTab()">' +
  ...
  O.workoutUI.renderHub = function () {
    ...
    // 1) Dynamische Hauptaktion
    let main;
    if (active) {
      const prog = WS().progress();
      main = '<div class="hub-hero"><div class="hub-hero-tag">Workout läuft</div><div class="hub-hero-title">' + esc(s.sport || 'Training') + '</div>' +
        '<div class="hub-hero-sub">' + (prog.kind === 'sets' ? (prog.completed + '/' + prog.planned + ' Sätze') : (exs.length + ' Übungen')) + '</div>' +
        '<button class="btn cta" onclick="ORVIA.workoutUI.open()"><span class="cta-txt"><span class="cta-main">Fortsetzen</span></span></button></div>';
    } else {
      main = '<div class="hub-hero"><div class="hub-hero-tag">Training starten</div><div class="hub-hero-sub">Freie Einheit oder Schnellstart wählen</div>' +
        '<div class="hub-quick">' +
        '<button class="hub-q" onclick="ORVIA.workoutUI.startSport(\'Gym\')"><svg class="ic"><use href="#i-dumbbell"/></svg>Gym</button>' +
        '<button class="hub-q" onclick="ORVIA.workoutUI.startSport(\'Laufen\')"><svg class="ic"><use href="#i-run"/></svg>Laufen</button>' +
        '<button class="hub-q" onclick="ORVIA.workoutUI.startSport(\'Rad\')"><svg class="ic"><use href="#i-bike"/></svg>Rad</button>' +
        '<button class="hub-q" onclick="ORVIA.workoutUI.startSport(\'Schwimmen\')"><svg class="ic"><use href="#i-swim"/></svg>Schwimmen</button>' +
        '<button class="hub-q" onclick="ORVIA.workoutUI.startSport(\'Mobilität\')"><svg class="ic"><use href="#i-stretch"/></svg>Mobility</button>' +
        '</div></div>';
    }
    // 2) Navigationsbereiche (progressive Offenlegung; Pläne/Verlauf nutzen bestehende Views)
    const nav =
      '<div class="hub-row" onclick="ORVIA.workoutUI._openPlans()"><span class="pic"><svg class="ic"><use href="#i-layers"/></svg></span><span class="hub-row-txt"><b>Meine Pläne</b><small>Wochenstruktur & Trainingsplan</small></span><span class="pchev">›</span></div>' +
    ...
    host.innerHTML = main + '<div class="hub-nav">' + nav + '</div><div class="card"><h2><svg class="ic"><use href="#i-clock"/></svg>Verlauf</h2><div id="workoutHistory"></div></div>';
    renderHistory();
  };
  O.workoutUI._openPlans = function () { try { const b = document.querySelector('.tabbar button[data-tab="plan"]'); if (b) b.click(); } catch (e) {} };
  ...
  O.workoutUI.close = function () { const ov = document.getElementById('workoutOverlay'); if (ov) ov.classList.add('hide'); stopElapsed(); if (typeof renderDay === 'function') renderDay(); const th = document.getElementById('tab-training'); if (th && !th.classList.contains('hide') && O.workoutUI.renderHub) O.workoutUI.renderHub(); };
  ...
  O.workoutUI._filter = function () {
    ...
    }).join('') : '<p class="muted" style="padding:16px">Keine Übung gefunden.</p>';
  };
