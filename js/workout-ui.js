...
(function () {
  ...
  O.workoutUI.discardActive = async function () {
    const okc = await O.workoutUI._confirmSheet('Workout verwerfen?', 'Es wird als „abgebrochen" beendet, damit du ein neues starten kannst.', 'Verwerfen', 'Zurück', true);
    if (!okc) return;
    await ensureActiveWorkoutLoaded();              // sicherstellen, dass der Store die Session kennt
    const r = await WS().cancelWorkout('aborted', 'verworfen');
    if (!r.success) { try { console.error('[discard]', r.error); } catch (e) {} toastIt(humanErr(r.error)); return; }
    toastIt('Workout verworfen — du kannst neu starten.');
  ...
  O.workoutUI._doFinish = async function (noRpe) {
    ...
    _finishing = false;
    if (!r.success) { all.forEach(b => b.disabled = false); if (btn) btn.textContent = 'Workout abschließen'; try { console.error('[finish]', r.error); } catch (e) {} toastIt('Workout konnte nicht abgeschlossen werden. Bitte erneut versuchen.'); return; }
    closeSheet();
  ...
  O.workoutUI._optAbort = async function () {
    ...
    if (!r.success) { try { console.error('[abort]', r.error); } catch (e) {} toastIt(humanErr(r.error)); return; }
  ...
  O.workoutUI._optDelete = async function () {
    ...
    if (!r.success) { try { console.error('[delete]', r.error); } catch (e) {} toastIt(humanErr(r.error)); return; }
