...
(function () {
  ...
  O.workoutUI.renderEntry = function () {
    ...
    if (logged.length) inner += '<div class="ts-list">' + logged.map(a => '<div class="ts-row"><span class="ts-name">' + esc(a.type) + '</span><span class="ts-sub">' + esc(a.sub || '') + '</span></div>').join('') + '</div>';
    if (!active && !logged.length) inner += '<p class="muted" style="margin:2px 0 12px">Noch keine Aktivität erfasst.</p>';
    inner += '<div class="ts-actions"><button class="btn" onclick="ORVIA.workoutUI.openTrainingTab()">Training starten</button>' +
