...
(function () {
  ...
  function _quickTilesHTML() {
    ...
    return tiles.map(t => '<button class="hub-q q3" onclick="ORVIA.workoutUI.startSport(\'' + esc(t.label).replace(/'/g, '') + '\')"><svg class="ic"><use href="#i-' + hubIcon(t.sportId) + '"/></svg><span>' + esc(t.label) + '</span></button>').join('');
  }
  ...
  function detSheet(headHTML, bodyHTML) {
    openSheet('<h3 class="wo-sheet-t">Workout-Details</h3>' + headHTML + '<div class="wo-det-body">' + bodyHTML + '</div><button class="wo-sheet-btn ghost" onclick="ORVIA.workoutUI.closeSheet()">Schließen</button>');
  }
  ...
  O.workoutUI.openDetails = async function (sessionId) {
    ...
    try {
      ...
      if (act) {
        const head = detHead(act.startedAt ? act.startedAt.slice(0, 10) : '', act.sportId, durationLabel({ duration_seconds: act.durationSeconds }), null) + detSummary(act.summary);
        detSheet(head, '<p class="muted">Für diese Einheit sind keine Satzdetails gespeichert.</p>'); return;
      }
    } catch (e) {}
  ...
  function recordLiveToActivity(d, distKm) {
    ...
    try {
      ...
      const TYPES_OK = (typeof TYPES !== 'undefined' && TYPES) ? TYPES : null;
      let type = d.sport || 'Gym';
      if (TYPES_OK && !(type in TYPES_OK)) type = 'Gym';
      ...
      const merged = Object.assign({}, existing, {
        ...
        source: 'live', workoutSessionId: d.sessionId || existing.workoutSessionId || null,
