...
(function () {
  ...
  function _renderHubActive(host) {
    ...
    const main = '<div class="hub-hero live"><div class="hub-hero-tag">Workout läuft</div><div class="hub-hero-title">' + esc(s.sport || 'Training') + '</div>' +
      ...
      '<button class="hub-discard" onclick="ORVIA.workoutUI.discardActive()">Workout verwerfen</button></div>';
  ...
  O.workoutUI.discardActive = async function () {
    if (typeof confirm === 'function' && !confirm('Aktives Workout verwerfen? Es wird beendet, damit du ein neues starten kannst.')) return;
    ...
    toastIt(r && r.success ? 'Workout verworfen — du kannst neu starten.' : humanErr(r && r.error));
