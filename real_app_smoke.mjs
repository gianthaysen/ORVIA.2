...
/* Reproduzierbarkeit (Phase 0, KF-016): das aufgeloeste playwright-Paket haengt vom
   Aufrufort ab und zeigt je nach Aufloesung auf einen NICHT installierten Browser
   (chromium_headless_shell-1234 statt chromium-1194). Der Lauf schlug dadurch aus
   einem git-worktree fehl, obwohl der Code identisch war. Deshalb wird der Browser
   explizit gepinnt, mit Rueckfall auf die Paketaufloesung. */
const PW_EXEC = process.env.ORVIA_CHROME
  || ['/opt/pw-browsers/chromium-1194/chrome-linux/chrome']
     .find(p => { try { return existsSync(p); } catch { return false; } })
  || null;
const launchOpts = PW_EXEC ? { executablePath: PW_EXEC } : {};
