...
begin
  ...
  -- AUDIT 2B: Nur ABGESCHLOSSENE Sessions werden zu Aktivitäten (aborted/cancelled nicht).
  ...
  do update set
     ...
     -- AUDIT 2B: leeres summary darf ein bereits berechnetes NICHT überschreiben.
     summary = coalesce(nullif(excluded.summary, '{}'::jsonb), public.activities.summary),
     updated_at = now()
...
-- AUDIT 2B: security invoker braucht trotzdem ein EXECUTE-Grant für authenticated.
grant execute on function public.orvia_upsert_activity_from_session(uuid, jsonb) to authenticated;

-- ------------------------------------------------------------
--  OPTIONALER, IDEMPOTENTER BACKFILL (live, NUR manuell ausführen nach Verifikation):
--  erzeugt fehlende Aktivitäten für bereits abgeschlossene eigene Workout-Sessions.
--  Dank Unique-Index (user_id, source, source_record_id) entstehen keine Dubletten.
-- ------------------------------------------------------------
-- insert into public.activities
--   (user_id, sport_id, source, source_record_id, workout_session_id, local_date,
--    started_at, ended_at, duration_seconds, status, summary)
-- select s.user_id, coalesce(s.sport_key, s.sport), 'orvia_workout', s.id::text, s.id, s.local_date,
--        s.started_at, s.finished_at,
--        case when s.finished_at is not null and s.started_at is not null
--             then greatest(0, round(extract(epoch from (s.finished_at - s.started_at)) - coalesce(s.total_paused_seconds,0)))::integer
--             when s.duration_min is not null then (s.duration_min*60)::integer else null end,
--        'completed', '{}'::jsonb
--   from public.workout_sessions s
--  where s.status = 'completed'
-- on conflict (user_id, source, source_record_id) where source_record_id is not null do nothing;

-- ------------------------------------------------------------
--  OUTBOX-/SYNC-HINWEIS (Frontend, siehe js/activity-sync.js):
--   - Offline erzeugte lokale Activity nutzt sourceRecordId = client_session_id.
--   - Erst NACH Server-Sync der Session (echte workout_sessions.id) die RPC mit der id aufrufen;
--     der lokale Datensatz wird dann via activityStore.markSynced(clientRecordId, serverActivityId)
--     auf die Server-Identität (source_record_id = session-uuid) gehoben. Kein Doppel-Insert.
-- ------------------------------------------------------------
