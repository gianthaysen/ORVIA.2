...
load('js/onboarding/onboarding-profile-logic.js');   // Issue #6: computeAge delegiert an das kanonische calculateAge
const O = global.window.ORVIA;
// Harness-Brücke: onboarding-profile-logic hängt an globalThis.ORVIA, profile-store an
// window.ORVIA — im Browser dasselbe Objekt, hier (window={}) explizit verbinden.
O.onboardingProfileLogic = globalThis.ORVIA.onboardingProfileLogic;
...
const run = async () => {
  ...
  global.PROFILE = {
    ...
    // Legacy:
    goals: ['x'], sports: ['Gym'], weekPlan: [[1]], level: 'profi', gear: ['shoe'], issues: ['knee'], location: 'Flensburg'   // location ist seit 0016 MAPPED (s. unten)
  };
  ...
  ok('Legacy ERHALTEN (goals/sports/weekPlan/level/gear/issues)',
     JSON.stringify(P.goals) === '["x"]' && P.sports[0] === 'Gym' && P.weekPlan && P.level === 'profi' && P.gear[0] === 'shoe' && P.issues[0] === 'knee');
  /* 0016 (Geräte-Divergenz-Incident): location ist jetzt server-gemapptes Feld (user_profiles.location)
     und wird bei source=empty wie alle MAPPED-Felder kontrolliert geleert — NICHT mehr Legacy. */
  ok('0016: location ist MAPPED und wird bei leerer Tabelle neutralisiert', P.location === null);
