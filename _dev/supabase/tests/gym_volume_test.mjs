...
const G = (await import(new URL('../../../app/js/gym-volume.js', import.meta.url))).default;
...
ok('27 unbekannte Übung unclassified, keine Muskelzuordnung', Object.keys(un.byMuscle).length === 0 && un.unclassified['Hyrox Sled Push'].workingSetCount === 2 && un.unclassified['Hyrox Sled Push'].occurrenceCount === 1);
...
// Neue Mappings: Step-up (Quads+Glutes direkt) + Hyperextensions (Lower Back direkt)
ok('Step-up → Quads+Glutes direkt, Hamstrings indirekt (0.25)', (function () { let v = vol([{ exercises: [ex('Step-up', [set(), set()])] }]); return v.quads.directSets === 2 && v.glutes.directSets === 2 && v.hamstrings.effectiveSets === 0.5; })());
ok('Step Up / Step-ups erkannt', (function () { let v = vol([{ exercises: [ex('Step Up', [set()]), ex('Step-ups', [set()])] }]); return v.quads.directSets === 2; })());
ok('Hyperextensions → Lower Back direkt, Glutes/Hamstrings indirekt', (function () { let v = vol([{ exercises: [ex('Hyperextensions', [set(), set()])] }]); return v.lower_back.directSets === 2 && v.glutes.effectiveSets === 1 && v.hamstrings.effectiveSets === 1; })());
ok('Tippvariante Hyperextentions erkannt', vol([{ exercises: [ex('Hyperextentions', [set()])] }]).lower_back.directSets === 1);
ok('Back Extension erkannt', vol([{ exercises: [ex('Back Extension', [set()])] }]).lower_back.directSets === 1);
ok('lower_back ist eigene Muskelgruppe', G.MUSCLES.indexOf('lower_back') >= 0 && G.MUSCLE_LABEL.lower_back === 'Unterer Rücken');
