...
const rout = await page.evaluate(() => {
  ...
  const custom = { visible: card.style.display !== 'none' && card.classList.contains('p3-live'),
...
ok('LIVE E-22 · konfigurierte Routinen erscheinen als Karte inkl. eigener Routine + Editor',
...
const nutr = await page.evaluate(() => {
  ...
  return { live: box.classList.contains('p3-live'), html: box.innerHTML };
});
ok('LIVE E-23 · Karte aktiv; ohne Konfiguration ehrlicher Einstieg statt Sackgasse',
   nutr.live && (/Jetzt einrichten/.test(nutr.html) || /nut-kcal/.test(nutr.html)), nutr.live + '');
