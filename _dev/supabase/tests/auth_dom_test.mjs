...
const AUTHLOGIC = (await import(new URL('../../../app/js/auth-logic.js', import.meta.url))).default;
const AUTH_SRC = fs.readFileSync(new URL('../../../app/js/auth.js', import.meta.url), 'utf8');
...
function harness(opts) {
  ...
  const sb = {
    auth: {
      ...
      // ROLLEN-FIX 2026-07-15: echter Supabase-Vertrag — getUser() liefert den serververifizierten
      // Nutzer der aktiven Session (loadAccessProfile verifiziert die Identität jetzt darüber).
      getUser() {
        const s = (opts.exchange && opts.exchange.data && opts.exchange.data.session)
          || (opts.session && opts.session.data && opts.session.data.session) || null;
        return Promise.resolve({ data: { user: s ? s.user : null }, error: s ? null : { message: 'no session' } });
      },
