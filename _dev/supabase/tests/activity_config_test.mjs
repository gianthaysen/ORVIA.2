...
globalThis.ORVIA.activityNormalize = (await import(new URL('../../../app/js/activity-normalize.js', import.meta.url))).default;
globalThis.ORVIA.onboardingSportsLogic = (await import(new URL('../../../app/js/onboarding/onboarding-sports-logic.js', import.meta.url))).default;
...
const C = (await import(new URL('../../../app/js/activity-config.js', import.meta.url))).default;
...
ok('Padel: Aktivitätstyp + gespielte Seite', C.allowedFieldKeys('padel').indexOf('padelType') >= 0 && C.allowedFieldKeys('padel').indexOf('side') >= 0);
ok('Fußball: 4 Typen + Position', C.allowedFieldKeys('football').indexOf('footballType') >= 0 && C.allowedFieldKeys('football').indexOf('position') >= 0);
...
ok('Leichtathletik: Trainingsart statt pauschalem Lauf', C.allowedFieldKeys('athletics').indexOf('athleticsType') >= 0 && C.allowedFieldKeys('athletics').indexOf('distanceKm') < 0);
...
/* ===== 2B-Aktivierung: Server-Normalisierung + 3-Quellen-Merge ===== */
let srv = C.normalizeServerActivity({ id: 's1', client_record_id: 'crA', user_id: 'u1', sport_id: 'gym', source: 'orvia_workout', source_record_id: 'w1', workout_session_id: 'w1', started_at: '2026-06-27T10:00:00Z', duration_seconds: 6060, summary: { exerciseCount: 7 }, metrics: {} });
ok('normalizeServerActivity: snake→camel', srv.id === 's1' && srv.clientRecordId === 'crA' && srv.sportId === 'gym' && srv.workoutSessionId === 'w1' && srv.syncStatus === 'synced');
// Dedup: gleiche Activity über Server (id/crid) und lokal (clientRecordId) → einmal, Server gewinnt
let localPend = { id: null, clientRecordId: 'crA', source: 'orvia_workout', sourceRecordId: 'w1', workoutSessionId: 'w1', startedAt: '2026-06-27T10:00:00Z', syncStatus: 'pending' };
let m = C.mergeAllActivities([srv], [localPend], []);
ok('Merge: Server+lokal selbe Activity → 1 (Server gewinnt)', m.length === 1 && m[0]._server === true);
// zwei echte getrennte Workouts → zwei
let srv2 = C.normalizeServerActivity({ id: 's2', client_record_id: 'crB', source: 'orvia_workout', source_record_id: 'w2', workout_session_id: 'w2', started_at: '2026-06-27T18:00:00Z' });
ok('Merge: zwei getrennte Workouts → 2', C.mergeAllActivities([srv, srv2], [], []).length === 2);
// Legacy mit kanonischem Gegenstück (gleiche workoutSessionId) → nicht doppelt
let legMirror = C.legacySessionToActivity('2026-06-27', 'Gym', { dur: 101, workoutSessionId: 'w1' });
ok('Merge: Legacy-Spiegel von Server-Workout → keine Dublette', C.mergeAllActivities([srv], [], [legMirror]).length === 1);
// Legacy ohne Gegenstück bleibt
ok('Merge: Legacy ohne Gegenstück bleibt', C.mergeAllActivities([srv], [], [C.legacySessionToActivity('2026-06-10', 'Laufen', { dur: 30, dist: 5 })]).length === 2);
ok('Merge: nach Datum sortiert', (function () { var x = C.mergeAllActivities([srv, srv2], [], []); return x[0].startedAt > x[1].startedAt; })());

/* ===== Live-Fix: leere Gym-Dubletten + Tombstone-Filter ===== */
let realGym = C.normalizeServerActivity({ id: 'g9', source: 'orvia_workout', source_record_id: 'wG', workout_session_id: 'wG', sport_id: 'gym', started_at: '2026-06-27T10:00:00Z', duration_seconds: 3600, summary: { exerciseCount: 5, workingSetCount: 15 } });
let emptyLegacyGym = C.legacySessionToActivity('2026-06-27', 'Gym', {});   // 0 min, keine Inhalte
ok('leerer Gym-Legacy ist isEmpty', emptyLegacyGym.isEmpty === true);
let mg = C.mergeAllActivities([realGym], [], [emptyLegacyGym]);
ok('Leer-Gym + echtes Gym selber Tag → nur echtes', mg.length === 1 && mg[0].id === 'g9');
// einzelnes Leer-Workout ohne Gegenstück bleibt (damit löschbar)
ok('einzelnes Leer-Workout ohne Gegenstück bleibt sichtbar', C.mergeAllActivities([], [], [C.legacySessionToActivity('2026-05-01', 'Gym', {})]).length === 1);
// zwei nicht-leere getrennte Gym am selben Tag → beide
let g2 = C.normalizeServerActivity({ id: 'g10', source: 'orvia_workout', source_record_id: 'wG2', workout_session_id: 'wG2', sport_id: 'gym', started_at: '2026-06-27T18:00:00Z', duration_seconds: 3600, summary: { workingSetCount: 10 } });
ok('zwei echte Gym am selben Tag bleiben zwei', C.mergeAllActivities([realGym, g2], [], []).length === 2);
// Tombstone-Filter: gelöschte Server-Activity erscheint nicht
let tombSet = { 'g9': 1 };
ok('Tombstone blendet gelöschte Server-Activity aus', C.mergeAllActivities([realGym, g2], [], [], { isTombstoned: function (a) { return !!tombSet[a.id]; } }).every(x => x.id !== 'g9'));
