...
const fn = R('supabase/functions/delete-account/index.ts');
...
const sqlAll = readdirSync(join(APP, 'supabase/migrations')).filter(f => f.endsWith('.sql'))
  .map(f => R('supabase/migrations/' + f)).join('\n') + '\n' + R('supabase/schema.sql');
function tableDef(t) {
